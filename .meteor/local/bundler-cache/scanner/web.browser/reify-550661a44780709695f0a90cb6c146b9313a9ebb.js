module.export({Pie:()=>Pie,PieCanvas:()=>PieCanvas,ResponsivePie:()=>ResponsivePie,ResponsivePieCanvas:()=>ResponsivePieCanvas,defaultProps:()=>defaultProps,useNormalizedData:()=>useNormalizedData,usePie:()=>usePie,usePieArcs:()=>usePieArcs,usePieFromBox:()=>usePieFromBox,usePieLayerContext:()=>usePieLayerContext});let useMemo,useState,useCallback,createElement,Fragment$1,useRef,useEffect;module.link('react',{useMemo(v){useMemo=v},useState(v){useState=v},useCallback(v){useCallback=v},createElement(v){createElement=v},Fragment(v){Fragment$1=v},useRef(v){useRef=v},useEffect(v){useEffect=v}},0);let usePropertyAccessor,useValueFormatter,degreesToRadians,radiansToDegrees,Container,useDimensions,bindDefs,SvgWrapper,ResponsiveWrapper,useTheme,getRelativeCursor;module.link('@nivo/core',{usePropertyAccessor(v){usePropertyAccessor=v},useValueFormatter(v){useValueFormatter=v},degreesToRadians(v){degreesToRadians=v},radiansToDegrees(v){radiansToDegrees=v},Container(v){Container=v},useDimensions(v){useDimensions=v},bindDefs(v){bindDefs=v},SvgWrapper(v){SvgWrapper=v},ResponsiveWrapper(v){ResponsiveWrapper=v},useTheme(v){useTheme=v},getRelativeCursor(v){getRelativeCursor=v}},1);let useArcGenerator,computeArcBoundingBox,ArcsLayer,ArcLinkLabelsLayer,ArcLabelsLayer,useArcLabels,useArcLinkLabels,drawCanvasArcLinkLabels,drawCanvasArcLabels,findArcUnderCursor;module.link('@nivo/arcs',{useArcGenerator(v){useArcGenerator=v},computeArcBoundingBox(v){computeArcBoundingBox=v},ArcsLayer(v){ArcsLayer=v},ArcLinkLabelsLayer(v){ArcLinkLabelsLayer=v},ArcLabelsLayer(v){ArcLabelsLayer=v},useArcLabels(v){useArcLabels=v},useArcLinkLabels(v){useArcLinkLabels=v},drawCanvasArcLinkLabels(v){drawCanvasArcLinkLabels=v},drawCanvasArcLabels(v){drawCanvasArcLabels=v},findArcUnderCursor(v){findArcUnderCursor=v}},2);let BoxLegendSvg,renderLegendToCanvas;module.link('@nivo/legends',{BoxLegendSvg(v){BoxLegendSvg=v},renderLegendToCanvas(v){renderLegendToCanvas=v}},3);let jsx,Fragment;module.link('react/jsx-runtime',{jsx(v){jsx=v},Fragment(v){Fragment=v}},4);let pie;module.link('d3-shape',{pie(v){pie=v}},5);let useOrdinalColorScale,useInheritedColor;module.link('@nivo/colors',{useOrdinalColorScale(v){useOrdinalColorScale=v},useInheritedColor(v){useInheritedColor=v}},6);let BasicTooltip,useTooltip;module.link('@nivo/tooltip',{BasicTooltip(v){BasicTooltip=v},useTooltip(v){useTooltip=v}},7);
