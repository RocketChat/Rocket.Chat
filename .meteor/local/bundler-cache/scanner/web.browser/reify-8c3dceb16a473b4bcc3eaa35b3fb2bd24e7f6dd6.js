module.export({ArcLabelsLayer:()=>ArcLabelsLayer,ArcLinkLabelsLayer:()=>ArcLinkLabelsLayer,ArcShape:()=>ArcShape,ArcsLayer:()=>ArcsLayer,arcTransitionModeById:()=>arcTransitionModeById,arcTransitionModes:()=>arcTransitionModes,computeArcBoundingBox:()=>computeArcBoundingBox,computeArcCenter:()=>computeArcCenter,computeArcLink:()=>computeArcLink,computeArcLinkTextAnchor:()=>computeArcLinkTextAnchor,drawCanvasArcLabels:()=>drawCanvasArcLabels,drawCanvasArcLinkLabels:()=>drawCanvasArcLinkLabels,findArcUnderCursor:()=>findArcUnderCursor,interpolateArc:()=>interpolateArc,interpolateArcCenter:()=>interpolateArcCenter,isCursorInRing:()=>isCursorInRing,useAnimatedArc:()=>useAnimatedArc,useArcCenters:()=>useArcCenters,useArcCentersTransition:()=>useArcCentersTransition,useArcGenerator:()=>useArcGenerator,useArcLabels:()=>useArcLabels,useArcLinkLabels:()=>useArcLinkLabels,useArcLinkLabelsTransition:()=>useArcLinkLabelsTransition,useArcLinks:()=>useArcLinks,useArcTransitionMode:()=>useArcTransitionMode,useArcsTransition:()=>useArcsTransition});let useMemo,createElement,useCallback;module.link('react',{useMemo(v){useMemo=v},createElement(v){createElement=v},useCallback(v){useCallback=v}},0);let radiansToDegrees,midAngle,positionFromAngle,useMotionConfig,useTheme,usePropertyAccessor,textPropsByEngine,degreesToRadians,getDistance,getAngle;module.link('@nivo/core',{radiansToDegrees(v){radiansToDegrees=v},midAngle(v){midAngle=v},positionFromAngle(v){positionFromAngle=v},useMotionConfig(v){useMotionConfig=v},useTheme(v){useTheme=v},usePropertyAccessor(v){usePropertyAccessor=v},textPropsByEngine(v){textPropsByEngine=v},degreesToRadians(v){degreesToRadians=v},getDistance(v){getDistance=v},getAngle(v){getAngle=v}},1);let useInheritedColor;module.link('@nivo/colors',{useInheritedColor(v){useInheritedColor=v}},2);let to,useTransition,animated,useSpring;module.link('@react-spring/web',{to(v){to=v},useTransition(v){useTransition=v},animated(v){animated=v},useSpring(v){useSpring=v}},3);let jsx,jsxs;module.link('react/jsx-runtime',{jsx(v){jsx=v},jsxs(v){jsxs=v}},4);let line,arc;module.link('d3-shape',{line(v){line=v},arc(v){arc=v}},5);
