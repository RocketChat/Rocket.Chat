module.export({UserAgent:()=>UserAgent});let DigestAuthentication,Grammar,IncomingRequestMessage,IncomingResponseMessage,Levels,LoggerFactory,Parser,URI,UserAgentCore;module.link("../core",{DigestAuthentication(v){DigestAuthentication=v},Grammar(v){Grammar=v},IncomingRequestMessage(v){IncomingRequestMessage=v},IncomingResponseMessage(v){IncomingResponseMessage=v},Levels(v){Levels=v},LoggerFactory(v){LoggerFactory=v},Parser(v){Parser=v},URI(v){URI=v},UserAgentCore(v){UserAgentCore=v}},0);let createRandomToken,utf8Length;module.link("../core/messages/utils",{createRandomToken(v){createRandomToken=v},utf8Length(v){utf8Length=v}},1);let defaultSessionDescriptionHandlerFactory;module.link("../platform/web/session-description-handler",{defaultSessionDescriptionHandlerFactory(v){defaultSessionDescriptionHandlerFactory=v}},2);let WebTransport;module.link("../platform/web/transport",{Transport(v){WebTransport=v}},3);let LIBRARY_VERSION;module.link("../version",{LIBRARY_VERSION(v){LIBRARY_VERSION=v}},4);let EmitterImpl;module.link("./emitter",{EmitterImpl(v){EmitterImpl=v}},5);let Invitation;module.link("./invitation",{Invitation(v){Invitation=v}},6);let Inviter;module.link("./inviter",{Inviter(v){Inviter=v}},7);let Message;module.link("./message",{Message(v){Message=v}},8);let Notification;module.link("./notification",{Notification(v){Notification=v}},9);let SIPExtension,UserAgentRegisteredOptionTags;module.link("./user-agent-options",{SIPExtension(v){SIPExtension=v},UserAgentRegisteredOptionTags(v){UserAgentRegisteredOptionTags=v}},10);let UserAgentState;module.link("./user-agent-state",{UserAgentState(v){UserAgentState=v}},11);
