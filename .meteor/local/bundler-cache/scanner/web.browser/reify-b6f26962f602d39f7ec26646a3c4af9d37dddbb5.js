module.export({GroupedVirtuoso:()=>GroupedVirtuoso,Virtuoso:()=>Virtuoso,VirtuosoGrid:()=>VirtuosoGrid});let systemToComponent;module.link('@virtuoso.dev/react-urx',{systemToComponent(v){systemToComponent=v}},0);let system,stream,statefulStream,connect,pipe,scan,map,statefulStreamFromEmitter,withLatestFrom,distinctUntilChanged,filter,streamFromEmitter,subscribe,handleNext,tup,publish,merge,duc,skip,mapTo,debounceTime,combineLatest,prop,getValue,tap,throttleTime,compose;module.link('@virtuoso.dev/urx',{system(v){system=v},stream(v){stream=v},statefulStream(v){statefulStream=v},connect(v){connect=v},pipe(v){pipe=v},scan(v){scan=v},map(v){map=v},statefulStreamFromEmitter(v){statefulStreamFromEmitter=v},withLatestFrom(v){withLatestFrom=v},distinctUntilChanged(v){distinctUntilChanged=v},filter(v){filter=v},streamFromEmitter(v){streamFromEmitter=v},subscribe(v){subscribe=v},handleNext(v){handleNext=v},tup(v){tup=v},publish(v){publish=v},merge(v){merge=v},duc(v){duc=v},skip(v){skip=v},mapTo(v){mapTo=v},debounceTime(v){debounceTime=v},combineLatest(v){combineLatest=v},prop(v){prop=v},getValue(v){getValue=v},tap(v){tap=v},throttleTime(v){throttleTime=v},compose(v){compose=v}},1);let useRef,useCallback,useEffect,memo,createElement;module.link('react',{useRef(v){useRef=v},useCallback(v){useCallback=v},useEffect(v){useEffect=v},memo(v){memo=v},createElement(v){createElement=v}},2);let ResizeObserver;module.link('resize-observer-polyfill',{default(v){ResizeObserver=v}},3);
...
var UP = 'up';
var DOWN = 'down';
var domIOSystem = /*#__PURE__*/system(function () {
  var scrollTop = stream();
  var deviation = statefulStream(0);
  var smoothScrollTargetReached = stream();
  var statefulScrollTop = statefulStream(0);
  var viewportHeight = stream();
  var headerHeight = statefulStream(0);
  var footerHeight = statefulStream(0);
  var scrollTo = stream();
  var scrollBy = stream();
  connect(scrollTop, statefulScrollTop);
  var scrollDirection = statefulStream(DOWN);
  connect(pipe(scrollTop, scan(function (acc, scrollTop) {
    return {
...
var sizeSystem = /*#__PURE__*/system(function () {
  var sizeRanges = stream();
  var totalCount = stream();
  var unshiftWith = stream();
  var firstItemIndex = statefulStream(0);
  var groupIndices = statefulStream([]);
  var fixedItemSize = statefulStream(undefined);
  var defaultItemSize = statefulStream(undefined);
  var data = statefulStream(undefined);
  var initial = initialSizeState();
  var sizes = statefulStreamFromEmitter(pipe(sizeRanges, withLatestFrom(groupIndices), scan(sizeStateReducer, initial), distinctUntilChanged()), initial);
  connect(pipe(groupIndices, filter(function (indexes) {
    return indexes.length > 0;
  }), withLatestFrom(sizes), map(function (_ref2) {
  ...
  var trackItemSizes = statefulStreamFromEmitter(pipe(fixedItemSize, map(function (size) {
    return size === undefined;
  })), true);
  connect(pipe(defaultItemSize, filter(function (value) {
  ...
  var listRefresh = streamFromEmitter(pipe(sizeRanges, withLatestFrom(sizes), scan(function (_ref3, _ref4) {
  ...
  connect(pipe(firstItemIndex, scan(function (prev, next) {
    return {
      diff: prev.prev - next,
      prev: next
    };
  ...
  var beforeUnshiftWith = streamFromEmitter(unshiftWith);
  connect(pipe(unshiftWith, withLatestFrom(sizes), map(function (_ref5) {
...
var scrollToIndexSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      sizes = _ref$.sizes,
  ...
  var scrollToIndex = stream();
  var topListHeight = statefulStream(0);
  ...
  connect(pipe(scrollToIndex, withLatestFrom(sizes, viewportHeight, totalCount, topListHeight, headerHeight), map(function (_ref2) {
    ...
    if (behavior === 'smooth') {
      ...
      unsubscribeListRefresh = subscribe(listRefresh, function (changed) {
        listChanged = listChanged || changed;
      });
      unsubscribeNextListRefresh = handleNext(smoothScrollTargetReached, function () {
        retry(listChanged);
      });
    } else {
...
}, /*#__PURE__*/tup(sizeSystem, domIOSystem), {
  singleton: true
});

var stateFlagsSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      scrollTop = _ref$.scrollTop,
      viewportHeight = _ref$.viewportHeight;
  var isAtBottom = statefulStream(false);
  var isAtTop = statefulStream(true);
  var atBottomStateChange = stream();
  var atTopStateChange = stream();
  var listStateListener = stream(); // skip 1 to avoid an initial on/off flick

  var isScrolling = streamFromEmitter(pipe(merge(pipe(duc(scrollTop), skip(1), mapTo(true)), pipe(duc(scrollTop), skip(1), mapTo(false), debounceTime(100))), distinctUntilChanged()));
  connect(pipe(duc(scrollTop), map(function (top) {
    return top === 0;
  }), distinctUntilChanged()), isAtTop);
  connect(isAtTop, atTopStateChange);
  connect(pipe(combineLatest(listStateListener, duc(scrollTop), duc(viewportHeight)), map(function (_ref2) {
    var _ref2$ = _ref2[0],
        bottom = _ref2$.bottom,
  ...
  }), distinctUntilChanged()), isAtBottom);
  subscribe(isAtBottom, function (value) {
    setTimeout(function () {
      return publish(atBottomStateChange, value);
    });
...
}, /*#__PURE__*/tup(domIOSystem));

var propsReadySystem = /*#__PURE__*/system(function () {
  var propsReady = statefulStream(false);
  var didMount = streamFromEmitter(pipe(propsReady, filter(function (ready) {
    return ready;
...
var initialTopMostItemIndexSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      sizes = _ref$.sizes,
      listRefresh = _ref$.listRefresh,
  ...
  var scrolledToInitialItem = statefulStream(true);
  var initialTopMostItemIndex = statefulStream(0);
  connect(pipe(didMount, withLatestFrom(initialTopMostItemIndex), filter(function (_ref2) {
    var index = _ref2[1];
    return index !== 0;
  }), mapTo(false)), scrolledToInitialItem);
  subscribe(pipe(listRefresh, withLatestFrom(scrolledToInitialItem, sizes), filter(function (_ref3) {
    var scrolledToInitialItem = _ref3[1],
        sizeTree = _ref3[2].sizeTree;
    return !empty(sizeTree) && !scrolledToInitialItem;
  }), withLatestFrom(initialTopMostItemIndex)), function (_ref4) {
    var initialTopMostItemIndex = _ref4[1];
    handleNext(scrollTop, function () {
...
}, /*#__PURE__*/tup(sizeSystem, domIOSystem, scrollToIndexSystem, propsReadySystem), {
  singleton: true
});
...
var followOutputSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      totalCount = _ref$.totalCount,
      listRefresh = _ref$.listRefresh,
  ...
  var followOutput = statefulStream(false);
  subscribe(pipe(duc(totalCount), withLatestFrom(followOutput, isAtBottom, scrolledToInitialItem), filter(function (_ref2) {
    ...
    handleNext(listRefresh, function () {
...
}, /*#__PURE__*/tup(sizeSystem, stateFlagsSystem, scrollToIndexSystem, initialTopMostItemIndexSystem));
...
var groupedListSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      totalCount = _ref$.totalCount,
  ...
  connect(pipe(combineLatest(scrollTop, sizes, headerHeight), filter(function (_ref2) {
    var sizes = _ref2[1];
  ...
  }), distinctUntilChanged(), map(function (index) {
    return [index];
  })), topItemsIndexes);
  return {
...
var sizeRangeSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      scrollTop = _ref$.scrollTop,
      viewportHeight = _ref$.viewportHeight,
      deviation = _ref$.deviation,
      headerHeight = _ref$.headerHeight;
  var listBoundary = stream();
  var topListHeight = statefulStream(0);
  var overscan = statefulStream(0);
  var visibleRange = statefulStreamFromEmitter(pipe(combineLatest(duc(scrollTop), duc(viewportHeight), duc(headerHeight), duc(listBoundary, tupleComparator), duc(overscan), duc(topListHeight), duc(deviation)), map(function (_ref2) {
    ...
    return value != null;
  }), distinctUntilChanged(tupleComparator)), [0, 0]);
  return {
    // input
...
var listStateSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
  ...
  var topItemsIndexes = statefulStream([]);
  var itemsRendered = stream();
  connect(groupedListSystem.topItemsIndexes, topItemsIndexes);
  var listState = statefulStreamFromEmitter(pipe(combineLatest(didMount, duc(visibleRange), duc(totalCount), duc(sizes), duc(initialTopMostItemIndex), scrolledToInitialItem, duc(topItemsIndexes), duc(firstItemIndex), data), filter(function (_ref2) {
    var mount = _ref2[0];
    return mount;
  }), map(function (_ref3) {
    var _ref3$ = _ref3[1],
    ...
    if (hasGroups(sizesValue)) {
      var scrollTop = Math.max(getValue(statefulScrollTop) - getValue(headerHeight), 0);
  ...
  connect(pipe(data, filter(function (data) {
  ...
  connect(pipe(listState, map(function (state) {
    return [state.top, state.bottom];
  })), listBoundary);
  connect(pipe(listState, map(function (state) {
    return state.items;
  })), itemsRendered);
  var endReached = streamFromEmitter(pipe(listState, filter(function (_ref4) {
    var items = _ref4.items;
    return items.length > 0;
  }), withLatestFrom(totalCount, data), filter(function (_ref5) {
  ...
  }), distinctUntilChanged(tupleComparator), map(function (_ref7) {
    var count = _ref7[0];
    return count;
  })));
  var startReached = streamFromEmitter(pipe(listState, throttleTime(100), filter(function (_ref8) {
  ...
  var rangeChanged = streamFromEmitter(pipe(listState, filter(function (_ref10) {
...
}, /*#__PURE__*/tup(domIOSystem, sizeSystem, groupedListSystem, sizeRangeSystem, initialTopMostItemIndexSystem, scrollToIndexSystem, stateFlagsSystem, propsReadySystem), {
  singleton: true
});

var initialItemCountSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      sizes = _ref$.sizes,
      firstItemIndex = _ref$.firstItemIndex,
      listState = _ref[1].listState,
      didMount = _ref[2].didMount;
  var initialItemCount = statefulStream(0);
  connect(pipe(didMount, withLatestFrom(initialItemCount), filter(function (_ref2) {
    var count = _ref2[1];
    return count !== 0;
  }), withLatestFrom(sizes, firstItemIndex), map(function (_ref3) {
    var _ref3$ = _ref3[0],
        count = _ref3$[1],
        sizes = _ref3[1],
...
}, /*#__PURE__*/tup(sizeSystem, listStateSystem, propsReadySystem), {
  singleton: true
});

var scrollSeekSystem = /*#__PURE__*/system(function (_ref) {
  ...
  connect(pipe(isScrolling, filter(function (value) {
    return !value;
  }), mapTo(0)), scrollVelocity);
  connect(pipe(scrollTop, throttleTime(100), scan(function (_ref2, next) {
    var prev = _ref2[1];
    return [prev, next];
  ...
  connect(pipe(scrollVelocity, withLatestFrom(scrollSeekConfiguration, isSeeking, rangeChanged), filter(function (_ref4) {
    var config = _ref4[1];
    return !!config;
  }), map(function (_ref5) {
    var speed = _ref5[0],
  ...
  subscribe(pipe(combineLatest(isSeeking, scrollVelocity, rangeChanged), withLatestFrom(scrollSeekConfiguration)), function (_ref6) {
    var _ref6$ = _ref6[0],
        isSeeking = _ref6$[0],
        velocity = _ref6$[1],
        range = _ref6$[2],
...
}, /*#__PURE__*/tup(domIOSystem, stateFlagsSystem), {
  singleton: true
});

var topItemCountSystem = /*#__PURE__*/system(function (_ref) {
  var topItemsIndexes = _ref[0].topItemsIndexes;
  var topItemCount = statefulStream(0);
  connect(pipe(topItemCount, filter(function (length) {
    return length > 0;
  }), map(function (length) {
    return Array.from({
      length: length
...
}, /*#__PURE__*/tup(listStateSystem));

var totalListHeightSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      footerHeight = _ref$.footerHeight,
      headerHeight = _ref$.headerHeight,
      listState = _ref[1].listState;
  var totalListHeightChanged = stream();
  var totalListHeight = statefulStreamFromEmitter(pipe(combineLatest(footerHeight, headerHeight, listState), map(function (_ref2) {
...
}, /*#__PURE__*/tup(domIOSystem, listStateSystem), {
  singleton: true
});
...
var upwardScrollFixSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      ...
      sizes = _ref$2.sizes;
  var deviationOffset = pipe(listState, withLatestFrom(scrollTop, scrollDirection), filter(function (_ref2) {
  ...
  if (GLITCHY_SCROLL_BY) {
    connect(pipe(deviationOffset, withLatestFrom(deviation), map(function (_ref8) {
      var amount = _ref8[0],
          deviation = _ref8[1];
    ...
    subscribe(pipe(isScrolling, filter(function (is) {
      return !is;
    }), withLatestFrom(deviation), filter(function (_ref9) {
    ...
    connect(pipe(deviationOffset, map(function (offset) {
      return {
        top: offset,
        behavior: 'auto'
      };
  ...
  var unshiftPayload = stream();
  connect(pipe(beforeUnshiftWith, map(function (indexOffset) {
    var currentTopIndex = getValue(listState).items[0].originalIndex;
    return {
      index: currentTopIndex + indexOffset,
      offset: offsetOf(currentTopIndex, getValue(sizes))
    };
  })), unshiftPayload);
  connect(pipe(unshiftWith, withLatestFrom(unshiftPayload), map(function (_ref11) {
    var _ref11$ = _ref11[1],
        index = _ref11$.index,
        offset = _ref11$.offset;
    var newOffset = offsetOf(index, getValue(sizes));
    return {
...
}, /*#__PURE__*/tup(domIOSystem, stateFlagsSystem, listStateSystem, sizeSystem));

var initialScrollTopSystem = /*#__PURE__*/system(function (_ref) {
  var totalListHeight = _ref[0].totalListHeight,
      didMount = _ref[1].didMount,
      scrollTo = _ref[2].scrollTo;
  var initialScrollTop = statefulStream(0);
  subscribe(pipe(didMount, withLatestFrom(initialScrollTop), filter(function (_ref2) {
    var offset = _ref2[1];
    return offset !== 0;
  }), map(function (_ref3) {
    var offset = _ref3[1];
    ...
    handleNext(pipe(totalListHeight, filter(function (val) {
      return val !== 0;
    })), function () {
      setTimeout(function () {
...
}, /*#__PURE__*/tup(totalListHeightSystem, propsReadySystem, domIOSystem), {
  singleton: true
});

var alignToBottomSystem = /*#__PURE__*/system(function (_ref) {
  var viewportHeight = _ref[0].viewportHeight,
      totalListHeight = _ref[1].totalListHeight;
  var alignToBottom = statefulStream(false);
  var paddingTopAddition = statefulStreamFromEmitter(pipe(combineLatest(alignToBottom, viewportHeight, totalListHeight), filter(function (_ref2) {
    var enabled = _ref2[0];
    return enabled;
  }), map(function (_ref3) {
...
}, /*#__PURE__*/tup(domIOSystem, totalListHeightSystem), {
  singleton: true
});

// fix this with 4.1 recursive conditional types

var featureGroup1System = /*#__PURE__*/system(function (_ref) {
...
}, /*#__PURE__*/tup(sizeRangeSystem, initialItemCountSystem, propsReadySystem, scrollSeekSystem, totalListHeightSystem, initialScrollTopSystem, alignToBottomSystem));
var listSystem = /*#__PURE__*/system(function (_ref2) {
  var _ref2$ = _ref2[0],
...
}, /*#__PURE__*/tup(sizeSystem, initialTopMostItemIndexSystem, domIOSystem, followOutputSystem, listStateSystem, scrollToIndexSystem, upwardScrollFixSystem, topItemCountSystem, groupedListSystem, featureGroup1System));
...
var listComponentPropsSystem = /*#__PURE__*/system(function () {
  var itemContent = statefulStream(function (index) {
    return "Item " + index;
  });
  var groupContent = statefulStream(function (index) {
    return "Group " + index;
  });
  ...
  var distinctProp = function distinctProp(propName, defaultValue) {
    ...
    return statefulStreamFromEmitter(pipe(components, map(function (components) {
      return components[propName];
    }), distinctUntilChanged()), defaultValue);
  };
...
var combinedSystem = /*#__PURE__*/system(function (_ref) {
  ...
  function deprecateComponentProp(stream, componentName, propName) {
    connect(pipe(stream, withLatestFrom(propsSystem.components), map(function (_ref2) {
      var _extends2;
  ...
  subscribe(deprecatedProps.scrollSeek, function (_ref3) {
    ...
    publish(propsSystem.components, _extends({}, getValue(propsSystem.components), {
...
}, /*#__PURE__*/tup(listSystem, listComponentPropsSystem));
...
var Items = /*#__PURE__*/memo(function VirtuosoItems(_ref5) {
...
var Header = /*#__PURE__*/memo(function VirtuosoHeader() {
  ...
  return Header ? createElement(headerFooterTag, {
    ref: ref
  }, createElement(Header)) : null;
});
var Footer = /*#__PURE__*/memo(function VirtuosoFooter() {
  ...
  return Footer ? createElement(headerFooterTag, {
    ref: ref
...
function buildScroller(_ref6) {
  ...
  var Scroller = memo(function VirtuosoScroller(_ref7) {
    ...
    return createElement(ScrollerComponent, _extends({
      ref: scrollerRef,
...
var ListRoot = /*#__PURE__*/memo(function VirtuosoRoot(_ref8) {
  ...
  return createElement(Scroller, Object.assign({}, props), createElement("div", {
    style: viewportStyle,
    ref: viewportRef
  }, createElement(Header, null), createElement(Items, null), createElement(Footer, null)), createElement("div", {
...
var _systemToComponent = /*#__PURE__*/systemToComponent(combinedSystem, {
  required: {},
  optional: {
    firstItemIndex: 'firstItemIndex',
    itemContent: 'itemContent',
...
var gridSystem = /*#__PURE__*/system(function (_ref) {
  var _ref$ = _ref[0],
      ...
      didMount = _ref$3.didMount;
  var totalCount = statefulStream(0);
  var initialItemCount = statefulStream(0);
  var gridState = statefulStream(INITIAL_GRID_STATE);
  var viewportDimensions = statefulStream({
    height: 0,
    width: 0
  });
  var itemDimensions = statefulStream({
    height: 0,
    width: 0
  });
  var scrollToIndex = stream();
  ...
  connect(pipe(combineLatest(duc(totalCount), visibleRange, duc(itemDimensions, function (prev, next) {
    return prev && prev.width === next.width && prev.height === next.height;
  })), withLatestFrom(viewportDimensions), map(function (_ref4) {
    var _ref4$ = _ref4[0],
        totalCount = _ref4$[0],
        _ref4$$ = _ref4$[1],
        startOffset = _ref4$$[0],
  ...
  connect(pipe(viewportDimensions, map(function (_ref5) {
    var height = _ref5.height;
    return height;
  })), viewportHeight);
  connect(pipe(combineLatest(viewportDimensions, itemDimensions, gridState), map(function (_ref6) {
  ...
  connect(pipe(listBoundary, withLatestFrom(gridState), map(function (_ref7) {
    var _ref7$ = _ref7[0],
        bottom = _ref7$[1],
  ...
  var endReached = streamFromEmitter(pipe(duc(gridState), filter(function (_ref8) {
    var items = _ref8.items;
    return items.length > 0;
  }), withLatestFrom(totalCount), filter(function (_ref9) {
  ...
  var startReached = streamFromEmitter(pipe(duc(gridState), filter(function (_ref11) {
    var items = _ref11.items;
    return items.length > 0 && items[0].index === 0;
  }), mapTo(0), distinctUntilChanged()));
  var rangeChanged = streamFromEmitter(pipe(duc(gridState), filter(function (_ref12) {
  ...
  connect(pipe(scrollToIndex, withLatestFrom(viewportDimensions, itemDimensions, totalCount), map(function (_ref14) {
...
}, /*#__PURE__*/tup(sizeRangeSystem, domIOSystem, stateFlagsSystem, scrollSeekSystem, propsReadySystem));
...
var gridComponentPropsSystem = /*#__PURE__*/system(function () {
  var itemContent = statefulStream(function (index) {
    return "Item " + index;
  });
  var components = statefulStream({});
  var itemClassName = statefulStream('virtuoso-grid-item');
  var listClassName = statefulStream('virtuoso-grid-list');
  var computeItemKey = statefulStream(identity);
...
var combinedSystem$1 = /*#__PURE__*/system(function (_ref) {
  ...
  function deprecateComponentProp(stream, componentName, propName) {
    connect(pipe(stream, withLatestFrom(gridComponentPropsSystem.components), map(function (_ref2) {
      var _extends2;
  ...
  subscribe(deprecatedProps.scrollSeek, function (_ref3) {
    ...
    console.warn("react-virtuoso: scrollSeek property is deprecated. Pass scrollSeekConfiguration and specify the placeholder in components.ScrollSeekPlaceholder instead.");
    publish(gridComponentPropsSystem.components, _extends({}, getValue(gridComponentPropsSystem.components), {
      ScrollSeekPlaceholder: placeholder
    }));
...
}, /*#__PURE__*/tup(gridSystem, gridComponentPropsSystem));
var GridItems = /*#__PURE__*/memo(function GridItems() {
...
var GridRoot = /*#__PURE__*/memo(function GridRoot(_ref4) {
  ...
  return createElement(Scroller$1, Object.assign({}, props), createElement("div", {
    style: viewportStyle,
    ref: viewportRef
  }, createElement(GridItems, null)));
});

var _systemToComponent$1 = /*#__PURE__*/systemToComponent(combinedSystem$1, {
  optional: {
    totalCount: 'totalCount',
    overscan: 'overscan',
    itemContent: 'itemContent',
