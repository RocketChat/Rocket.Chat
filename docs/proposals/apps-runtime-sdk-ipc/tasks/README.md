...
| Getters that expose no `do*` | `getListenerBridge`, `getInternalFederationBridge` |
...
| Pairs that an accessor emits today | **114** |
| Pairs among those that send the `'APP_ID'` sentinel | **108** |
| Pairs that no accessor emits | **35** |
| Getters with no traffic at all | 6 |
| `bridgeCall` call sites in `base-runtime` | 151 over 39 files |
| Listener methods (`AppMethod` `check*`/`execute*`) | 74, over 51 `AppInterface` members |
| `_zPING` / `_zPONG` literal definitions | 4, over 4 files |
| Modules that import the JSON-RPC surface | 23 in `packages/apps`, plus the two `jsonrpc.ts` modules |
...
The plan splits PR 5 and PR 6 at "the ~30 methods accessors actually emit" against "the remaining
~120". The measurement inverts that ratio: accessors emit **114** of the 149 pairs, and only **35**
pairs have no traffic. The split criterion still holds — PR 5 takes the set with traffic, because
sentinel removal must ride with each entry — but it now puts 114 entries in PR 5 and 35 in PR 6.

The ADR treats "the emitted set" and "the sentinel-sending set" as one set. They differ by 6: 114
pairs have traffic, and 108 of those send `'APP_ID'`. Task 16 takes all 114, so that the 6
