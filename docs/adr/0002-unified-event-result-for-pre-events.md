...
- The restricted return type also gives `EventResult.patch(...)` correct `Partial<T>` checking via
  contextual typing. `patch` takes `Partial<NoInfer<T>>`, so the argument contributes no inference
  candidate and `T` comes from the declared return type alone; an unpatchable or misspelled property
