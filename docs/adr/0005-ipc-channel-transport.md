...
- stdin is closed. stdout and stderr carry logs again, which the host forwards. The subprocess
  metrics ride the IPC channel as a notification instead of stderr JSON.
...
4. **The subprocess applies secure fields by walking the message.** `applySecureFieldsDeep` runs on
   ...
   clone carries it.
5. **Metrics ride the IPC channel.** The subprocess answers `_zPING` with `_zPONG` and a `metrics`
   notification carrying `{ pid }`. `queueSize` is deleted together with the queue that reported it.
...
- **The metrics are no longer delayed by the backlog they report.** They rode stderr rather than the
  outbound queue for exactly that reason; with no queue, the argument and the metric both go.
...
2. **Consume the metrics.** The host still only `debug()`s the `metrics` notification. `queueSize`
   is gone with the queue; `{ pid }` is what remains to consume.
3. **`protocol/` still has to be built.** [ADR 0006](./0006-apps-subprocess-protocol.md) decides
...
- Metrics: `packages/apps/base-runtime/src/lib/metricsCollector.ts`
...
- Metrics onto the channel: `8c4cdc07df`
