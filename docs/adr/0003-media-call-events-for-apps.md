...
`CallPreventionRecord` is one non-union type. `text` is always there; `i18n` is there only when the
app named a key, and it holds `key`, `ns` and `args` together:

| What an app said | What is stored |
|---|---|
| `EventResult.prevent({ reason })` — the words | `text`, capped at 1000 characters |
| `EventResult.prevent({ i18n })` — a key | `i18n.key`, `i18n.ns`, `i18n.args`, **and** `text`: the app's own wording for that key in the workspace's default language, snapshotted while the app was still installed |

A record built either way reads through the same three fields, so `if (record.i18n)` is the whole of
telling them apart. A union of the two would overlap — the text-only member is a structural subset
of the key member — which nothing that validates a stored record can resolve.
...
The 1000-character cap protects the database, not a layout: what an app says is written to
`media_calls` and nothing else bounds it. A cut is marked with an ellipsis, so a reader sees the
sentence was cut rather than that it stopped dead. `i18n.args` is stored as the app sent it.
