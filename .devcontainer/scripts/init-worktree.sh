...
#
# Since compose files are static, the mount is injected by generating an
# override file that devcontainer.json always includes. In the normal
# (non-worktree) checkout there is nothing to mount and this writes a stub.
set -euo pipefail

root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
out="$root/.devcontainer/docker-compose.worktree.yml"
...
# workspace, so the bind mount already covers it) — emit a no-op override.
if [ -z "$common" ] || [ "$common" = "$root/.git" ]; then
	cat >"$out" <<-'YAML'
		# Generated by .devcontainer/scripts/init-worktree.sh — do not edit.
		# Not a linked git worktree: nothing to mount.
		services:
		  app: {}
	YAML
	exit 0
...
{
	echo "# Generated by .devcontainer/scripts/init-worktree.sh — do not edit."
