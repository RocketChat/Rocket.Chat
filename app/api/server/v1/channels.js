...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			if (bodyParams.teams) {
				const canSeeAllTeams = hasPermission(
					this.userId,
					'view-all-teams',
				);
				const teams = Promise.await(
					Team.listByNames(bodyParams.teams, {
						projection: { _id: 1 },
					}),
				);
				const teamMembers = [];
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			if (!users.length) {
				return API.v1.failure(
					'invalid-user-invite-list',
					'Cannot invite if no users are provided',
				);
			}
	...
	{ authRequired: true },
	{
		post() {
			...
			if (!channelId && !channelName) {
				return API.v1.failure(
					'The parameter "channelId" or "channelName" is required',
				);
			}
