...
// 		const team = teamId ? Promise.await(TaskRoom.getOneById(teamId)) : Promise.await(TaskRoom.getOneByName(teamName));
// 		if (!team) {
...
// 		const validRooms = Promise.await(TaskRoom.addRooms(this.userId, rooms, team._id));
...
// 		const room = Promise.await(TaskRoom.removeRoom(this.userId, roomId, team._id, canRemoveAny));
...
// 		const team = Promise.await(TaskRoom.getOneByRoomId(roomId));
...
// 		const room = Promise.await(TaskRoom.updateRoom(this.userId, roomId, isDefault, canUpdateAny));
...
// 		const { records, total } = Promise.await(TaskRoom.listRooms(this.userId, team._id, listFilter, { offset, count }));
...
// 		const { records, total } = Promise.await(TaskRoom.listRoomsOfUser(this.userId, team._id, userId, allowPrivateTeam, { offset, count }));
...
// 		const { records, total } = Promise.await(TaskRoom.members(this.userId, team._id, canSeeAllMembers, { offset, count }, query));
...
// 		Promise.await(TaskRoom.addMembers(this.userId, team._id, members));
...
// 		Promise.await(TaskRoom.updateMember(team._id, member));
...
// 		if (!Promise.await(TaskRoom.removeMembers(this.userId, team._id, [{ userId }]))) {
// 			return API.v1.failure();
// 		}

// 		if (rooms?.length) {
// 			const roomsFromTeam: string[] = Promise.await(TaskRoom.getMatchingTeamRooms(team._id, rooms));
...
// 		Promise.await(TaskRoom.removeMembers(this.userId, team._id, [{
// 			userId: this.userId,
// 		}]));
...
// 			? Promise.await(TaskRoom.getInfoById(teamId))
// 			: Promise.await(TaskRoom.getInfoByName(teamName));

// 		if (!teamInfo) {
// 			return API.v1.failure('TaskRoom not found');
// 		}
...
// 		const rooms: string[] = Promise.await(TaskRoom.getMatchingTeamRooms(team._id, roomsToRemove));
...
// 		Promise.await(TaskRoom.unsetTeamIdOfRooms(team._id));

// 		// And finally delete the team itself
// 		Promise.await(TaskRoom.deleteById(team._id));
...
// 		const teams = Promise.await(TaskRoom.autocomplete(this.userId, name));
...
// 		const team = teamId && Promise.await(TaskRoom.getOneById(teamId));
// 		if (!team) {
// 			return API.v1.failure('team-does-not-exist');
// 		}
...
// 		Promise.await(TaskRoom.update(this.userId, teamId, { name: data.name, type: data.type }));
