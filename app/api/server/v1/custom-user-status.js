...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			return API.v1.success({
				customUserStatus: CustomUserStatus.findOneByName(
					userStatusData.name,
				),
			});
		},
		...
		post() {
			...
			if (!customUserStatusId) {
				return API.v1.failure(
					'The "customUserStatusId" params is required!',
				);
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			const customUserStatus = CustomUserStatus.findOneById(
				userStatusData._id,
			);
			...
			return API.v1.success({
				customUserStatus: CustomUserStatus.findOneById(
					userStatusData._id,
				),
			});
		},
