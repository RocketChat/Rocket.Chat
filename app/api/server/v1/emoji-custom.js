				...
				return API.v1.success({
					emojis: {
						...
						remove: Promise.await(
							EmojiCustom.trashFindDeletedAfter(
								updatedSinceDate,
							).toArray(),
						),
					},
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			const emojiToUpdate = Promise.await(
				EmojiCustom.findOneById(fields._id),
			);
			if (!emojiToUpdate) {
			...
			if (fields.newFile) {
				const isUploadable = Promise.await(
					Media.isImage(emoji.fileBuffer),
				);
				if (!isUploadable) {
					throw new Meteor.Error(
						'emoji-is-not-image',
