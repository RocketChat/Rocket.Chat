		...
		post: {
			...
			async action(): Promise<ResultFor<'POST', 'settings/:_id'>> {
				...
				if (
					...
					&& isSettingsUpdatePropsColor(this.bodyParams)
				) {
					Settings.updateOptionsById<ISettingColor>(
						this.urlParams._id,
						{
				...
				if (
					...
					&& await Settings.updateValueNotHiddenById(
						this.urlParams._id,
						this.bodyParams.value,
					)
				) {
					const s = await Settings.findOneNotHiddenById(
						this.urlParams._id,
					);
					if (!s) {
						return API.v1.failure();
