		...
		post() {
			...
			if (!msg) {
				return API.v1.failure(
					`No message found with the id of "${ this.bodyParams.msgId }".`,
				);
			}
			...
			Meteor.runAsUser(
				this.bodyParams.asUser ? msg.u._id : this.userId,
				() => {
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			return API.v1.success({
				result: {
					updated: normalizeMessagesForUser(
						result.updated,
						this.userId,
					),
	...
	{ authRequired: true },
	{
		get() {
			if (!this.queryParams.msgId) {
				return API.v1.failure(
					'The "msgId" query parameter must be provided.',
				);
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			if (
				!this.bodyParams.messageId
				|| !this.bodyParams.messageId.trim()
			) {
				throw new Meteor.Error(
			...
			const [message] = normalizeMessagesForUser(
				[pinnedMessage],
				this.userId,
			);
		...
		post() {
			if (!this.bodyParams.message) {
				throw new Meteor.Error(
					...
					'The "message" parameter must be provided.',
				);
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			if (!description) {
				return API.v1.failure(
					'The required "description" param is missing.',
				);
			}
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			Meteor.runAsUser(this.userId, () =>
				Meteor.call('ignoreUser', { rid, userId, ignore }),
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			if (!since) {
				throw new Meteor.Error(
					'The required "since" query param is missing.',
				);
	...
	{ authRequired: true },
	{
		get() {
			...
			const typeThread = {
				_hidden: { $ne: true },
				...type === 'following' && {
					replies: { $in: [this.userId] },
				},
				...type === 'unread' && {
					_id: {
						$in: Subscriptions.findOneByRoomIdAndUserId(
							room._id,
							user._id,
						).tunread,
					},
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			messages.messages = normalizeMessagesForUser(
				messages.messages,
				this.userId,
			);
