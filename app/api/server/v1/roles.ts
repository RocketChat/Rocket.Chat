	...
	{ authRequired: true },
	{
		async get() {
			...
			return API.v1.success({
				roles: {
					update: await Roles.findByUpdatedDate(
						new Date(updatedSince),
					).toArray(),
					remove: await Roles.trashFindDeletedAfter(
						new Date(updatedSince),
					).toArray(),
				},
			});
		...
		async post() {
			...
			const roleId = (
				await Roles.createWithRandomId(
					name,
					scope && ['Users', 'Subscriptions'].includes(scope)
						? scope
						: 'Users',
					description,
					false,
	...
	{ authRequired: true },
	{
		async post() {
			if (!isRoleAddUserToRoleProps(this.bodyParams)) {
				throw new Meteor.Error(
					'error-invalid-role-properties',
					isRoleAddUserToRoleProps.errors
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async get() {
			...
			if (
				!await hasPermissionAsync(this.userId, 'access-permissions')
			) {
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async post() {
			...
			const existingUsers = await Roles.findUsersInRole(
				role.name,
				role.scope,
			);
		...
		async post() {
			...
			if (role._id === 'admin') {
				const adminCount = await (
					await Roles.findUsersInRole('admin')
				).count();
				if (adminCount === 1) {
					throw new Meteor.Error(
