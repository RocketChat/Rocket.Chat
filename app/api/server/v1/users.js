...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const actualSort =				sort && sort.name
	...
	{ authRequired: true },
	{
		post() {
			...
			if (
				...
				Meteor.runAsUser(this.userId, () =>
					Meteor.call('resetAvatar', user._id),
				);
			} else {
				throw new Meteor.Error(
					'error-not-allowed',
					'Reset avatar is not allowed',
					{
		...
		post() {
			...
			if (sentTheUserByFormData) {
				if (fields.userId) {
					...
					user = Users.findOneByUsernameIgnoringCase(
						fields.username,
						{
							fields: { username: 1 },
						},
				...
				if (
					isAnotherUser
					&& !hasPermission(this.userId, 'edit-other-user-avatar')
				) {
					throw new Meteor.Error('error-not-allowed', 'Not allowed');
				}
			}
			...
			Meteor.runAsUser(user._id, () => {
				...
				if (this.bodyParams.status) {
					...
					if (validStatus.includes(this.bodyParams.status)) {
						...
						if (
							status === 'offline'
							&& !settings.get('Accounts_AllowInvisibleStatusOption')
						) {
							throw new Meteor.Error(
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const adminId = hasPermission(this.userId, 'view-all-teams')
				? undefined
				: this.userId;

			const teams = Promise.await(
				Team.findBySubscribedUserIds(userId, adminId),
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		post() {
			...
			if (
				userId !== this.userId
				&& !hasPermission(this.userId, 'logout-other-user')
			) {
				return API.v1.unauthorized();
			}
