...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async get() {
			...
			const { records, total } = await Team.list(
				this.userId,
				{ offset, count },
				{ sort, query },
			);
		...
		async post() {
			if (!isTeamsConvertToChannelProps(this.bodyParams)) {
				return API.v1.failure(
					'invalid-body-params',
					isTeamsConvertToChannelProps.errors
						?.map((e) => e.message)
						.join('\n '),
				);
			}
			...
			const rooms = await Team.getMatchingTeamRooms(
				team._id,
				roomsToRemove,
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async post() {
			...
			const validRooms = await Team.addRooms(
				this.userId,
				rooms,
				team._id,
			);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async post() {
			...
			if (
				!hasPermission(this.userId, 'remove-team-channel', team.roomId)
			) {
				return API.v1.unauthorized();
			}

			const canRemoveAny = !!hasPermission(
				this.userId,
			...
			const room = await Team.removeRoom(
				this.userId,
				roomId,
				team._id,
				canRemoveAny,
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async post() {
			...
			const canUpdateAny = !!hasPermission(
				this.userId,
				'view-all-team-channels',
				team.roomId,
			);

			const room = await Team.updateRoom(
				this.userId,
				roomId,
				isDefault,
				canUpdateAny,
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async get() {
			...
			const allowPrivateTeam: boolean = hasPermission(
				this.userId,
				'view-all-teams',
				team.roomId,
			);
			...
			const { records, total } = await Team.listRooms(
				this.userId,
				team._id,
				listFilter,
				{ offset, count },
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async get() {
			...
			const allowPrivateTeam = hasPermission(
				this.userId,
				'view-all-teams',
				team.roomId,
			);
			...
			const { records, total } = await Team.listRoomsOfUser(
				this.userId,
				team._id,
				userId,
				allowPrivateTeam,
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		async get() {
			...
			const canSeeAllMembers = hasPermission(
				this.userId,
				'view-all-teams',
				team.roomId,
			);
			...
			const { records, total } = await Team.members(
				this.userId,
				team._id,
				canSeeAllMembers,
				{ offset, count },
	...
	{ authRequired: true },
	{
		async post() {
			...
			if (
				!hasAtLeastOnePermission(
					this.userId,
					['add-team-member', 'edit-team-member'],
					team.roomId,
				)
			) {
				return API.v1.unauthorized();
	...
	{ authRequired: true },
	{
		async post() {
			if (!isTeamsUpdateMemberProps(this.bodyParams)) {
				return API.v1.failure(
					'invalid-params',
					isTeamsUpdateMemberProps.errors
						?.map((e) => e.message)
						.join('\n '),
				);
			}
	...
	{ authRequired: true },
	{
		async post() {
			if (!isTeamsRemoveMemberProps(this.bodyParams)) {
				return API.v1.failure(
					'invalid-params',
					isTeamsRemoveMemberProps.errors
						?.map((e) => e.message)
						.join('\n '),
				);
			}
			...
			if (
				!await Team.removeMembers(this.userId, team._id, [{ userId }])
			) {
				return API.v1.failure();
			}

			if (rooms?.length) {
				const roomsFromTeam: string[] = await Team.getMatchingTeamRooms(
					team._id,
					rooms,
				);
	...
	{ authRequired: true },
	{
		async post() {
			if (!isTeamsLeaveProps(this.bodyParams)) {
				return API.v1.failure(
					'invalid-params',
					isTeamsLeaveProps.errors?.map((e) => e.message).join('\n '),
				);
			}
	...
	{ authRequired: true },
	{
		async post() {
			...
			if (!isTeamsDeleteProps(this.bodyParams)) {
				return API.v1.failure(
					'invalid-params',
					isTeamsDeleteProps.errors?.map((e) => e.message).join('\n '),
				);
			}
			...
			const rooms: string[] = await Team.getMatchingTeamRooms(
				team._id,
				roomsToRemove,
			);
	...
	{ authRequired: true },
	{
		async post() {
			if (!isTeamsUpdateProps(this.bodyParams)) {
				return API.v1.failure(
					'invalid-params',
					isTeamsUpdateProps.errors?.map((e) => e.message).join('\n '),
				);
			}
