...
export async function findAdminRooms({
	...
	const showTypes = Array.isArray(types)
		? types.filter((type) => !typesToRemove.includes(type))
		: [];
	const options = {
		fields,
	...
	if (name && showTypes.length) {
		...
		cursor = Rooms.findByNameContaining(
			name,
			discussion,
			includeTeams,
			showOnlyTeams,
...
export async function findChannelAndPrivateAutocomplete({ uid, selector }) {
	...
	const rooms = await Rooms.findRoomsWithoutDiscussionsByRoomIds(
		selector.name,
		userRoomsIds,
		options,
	).toArray();
...
export async function findAdminRoomsAutocomplete({ uid, selector }) {
	if (
		!await hasAtLeastOnePermissionAsync(uid, [
	...
	const rooms = await Rooms.findRoomsByNameOrFnameStarting(
		selector.name,
		options,
	).toArray();
...
export async function findChannelAndPrivateAutocompleteWithPagination({
	...
	const cursor = await Rooms.findRoomsWithoutDiscussionsByRoomIds(
		selector.name,
		userRoomsIds,
		options,
	);
...
export async function findRoomsAvailableForTeams({ uid, name }) {
	...
	const rooms =		await Rooms.findChannelAndGroupListWithoutTeamsByNameStartingByOwner(
		uid,
		name,
		userRooms,
		options,
