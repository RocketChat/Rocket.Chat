...
export async function findMentionedMessages({
	...
	const user = (await Users.findOneById(uid, { fields: { username: 1 } })) as IUser;
	if (!user) {
...
export async function findSnippetedMessageById({ uid, messageId }: { uid: string; messageId: string }): Promise<unknown> {
