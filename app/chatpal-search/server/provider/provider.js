...
class ChatpalProvider extends SearchProvider {
		...
		return (
			...
			|| (this._indexConfig.backendtype === 'onsite'
				&& this._indexConfig.baseurl
					!== (this._settings.get('Base_URL').endsWith('/')
						? this._settings.get('Base_URL').slice(0, -1)
						: this._settings.get('Base_URL')))
			|| (this._indexConfig.backendtype === 'cloud'
				&& this._indexConfig.httpOptions.headers['X-Api-Key']
					!== this._settings.get('API_Key'))
	...
	start(reason, resolve, reject) {
		...
		this._getIndexConfig().then((server) => {
			...
			this.index = new Index(
				this._indexConfig,
				this.indexFail || clear,
				this._stats.message.oldest || new Date().valueOf(),
			);
	...
	search(text, context, payload, callback) {
		...
		const type =			payload.resultType === 'All'
			? ['message', 'user', 'room']
			: ['message'];
