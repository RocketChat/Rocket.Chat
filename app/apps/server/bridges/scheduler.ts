...
export class AppSchedulerBridge extends SchedulerBridge {
	...
	protected async scheduleOnce(
		...
		this.orch.debugLog(
			`The App ${ appId } is scheduling an onetime job (processor ${ id })`,
		);
		try {
			...
			const job = await this.scheduler.schedule(
				when,
				id,
				this.decorateJobData(data, appId),
			);
	...
	protected async scheduleRecurring(
		...
		this.orch.debugLog(
			`The App ${ appId } is scheduling a recurring job (processor ${ id })`,
		);
		try {
			await this.startScheduler();
			const job = await this.scheduler.every(
				interval,
				id,
				this.decorateJobData(data, appId),
				{ skipImmediate },
