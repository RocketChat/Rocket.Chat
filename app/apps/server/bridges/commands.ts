...
export class AppCommandsBridge extends CommandBridge {
	...
	protected doesCommandExist(command: string, appId: string): boolean {
		this.orch.debugLog(
			`The App ${ appId } is checking if "${ command }" command exists.`,
		);
		...
		return (
			typeof slashCommands.commands[cmd] === 'object'
			|| this.disabledCommands.has(cmd)
		);
	}

	protected enableCommand(command: string, appId: string): void {
		this.orch.debugLog(
			`The App ${ appId } is attempting to enable the command: "${ command }"`,
		);
	...
	protected disableCommand(command: string, appId: string): void {
		this.orch.debugLog(
			`The App ${ appId } is attempting to disable the command: "${ command }"`,
		);
	...
	protected modifyCommand(command: ISlashCommand, appId: string): void {
		this.orch.debugLog(
			`The App ${ appId } is attempting to modify the command: "${ command }"`,
		);
		...
		item.params = command.i18nParamsExample
			? command.i18nParamsExample
			: item.params;
		item.description = command.i18nDescription
			? command.i18nDescription
			: item.params;
		item.callback = this._appCommandExecutor.bind(this);
		item.providesPreview = command.providesPreview;
		item.previewer = command.previewer
			? this._appCommandPreviewer.bind(this)
			: item.previewer;
	...
	protected registerCommand(command: ISlashCommand, appId: string): void {
		this.orch.debugLog(
			`The App ${ appId } is registering the command: "${ command.command }"`,
		);
		...
		const item = {
			...
			params: Utilities.getI18nKeyForApp(
				command.i18nParamsExample,
				appId,
			),
			description: Utilities.getI18nKeyForApp(
				command.i18nDescription,
				appId,
			),
	...
	protected unregisterCommand(command: string, appId: string): void {
		this.orch.debugLog(
			`The App ${ appId } is unregistering the command: "${ command }"`,
		);
	...
	private _verifyCommand(command: ISlashCommand): void {
		...
		if (
			command.i18nParamsExample
			&& typeof command.i18nParamsExample !== 'string'
		) {
			throw new Error(
				'Invalid Slash Command parameter provided, it must be a valid ISlashCommand object.',
			);
		...
		if (
			command.i18nDescription
			&& typeof command.i18nDescription !== 'string'
		) {
			throw new Error(
				'Invalid Slash Command parameter provided, it must be a valid ISlashCommand object.',
			);
	...
	private async _appCommandPreviewExecutor(
		...
		Promise.await(
			this.orch
				.getManager()
				?.getCommandManager()
				.executePreview(command, preview, context),
		);
	}
}
