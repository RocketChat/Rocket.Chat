...
export class AppPersistenceBridge extends PersistenceBridge {
	...
	protected async purge(appId: string): Promise<void> {
		this.orch.debugLog(
			`The App's persistent storage is being purged: ${ appId }`,
		);
	...
	protected async create(data: object, appId: string): Promise<string> {
		this.orch.debugLog(
			`The App ${ appId } is storing a new object in their persistence.`,
			data,
		);
	...
	protected async createWithAssociations(
		...
		return this.orch
			.getPersistenceModel()
			.insert({ appId, associations, data });
	}
	...
	protected async remove(
		...
		this.orch.debugLog(
			`The App ${ appId } is removing one of their records by the id: "${ id }"`,
		);

		const record = this.orch
			.getPersistenceModel()
			.findOne({ _id: id, appId });
	...
	protected async update(
		...
		this.orch.debugLog(
			`The App ${ appId } is updating the record "${ id }" to:`,
			data,
		);
