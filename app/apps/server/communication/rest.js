...
const appsEngineVersionForMarketplace = Info.marketplaceApiVersion.replace(
	/-.*/g,
	'',
);
	...
	addManagementRoutes() {
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				get() {
					...
					if (this.queryParams.marketplace) {
						...
						if (!result || result.statusCode !== 200) {
							orchestrator
								.getRocketChatLogger()
								.error('Error getting the Apps:', result.data);
							return API.v1.failure();
						}
					...
					if (this.queryParams.categories) {
						...
						try {
							...
							orchestrator
								.getRocketChatLogger()
								.error(
									'Error getting the categories from the Marketplace:',
									e.response.data,
								);
							return API.v1.internalError();
						}
					...
					if (
						...
						const subscribeRoute =							this.queryParams.details === 'true'
							? 'subscribe/details'
							: 'subscribe';
				...
				post() {
					...
					if (this.bodyParams.url) {
						if (
							settings.get('Apps_Framework_Development_Mode')
							!== true
						) {
							return API.v1.failure({
						...
						try {
							...
							orchestrator
								.getRocketChatLogger()
								.error(
									'Error getting the app from url:',
									e.response.data,
								);
							return API.v1.internalError();
						}
						...
						this.bodyParams.appId
						&& this.bodyParams.marketplace
						&& this.bodyParams.version
					) {
						const baseUrl = orchestrator.getMarketplaceUrl();
						...
						const marketplacePromise = new Promise(
							(resolve, reject) => {
								const token = Promise.await(
									getWorkspaceAccessToken(),
								);
						...
						try {
							const [downloadResult, marketplaceResult] =								Promise.await(
								Promise.all([
									downloadPromise,
									marketplacePromise,
								]),
					...
					const user = orchestrator
						.getConverters()
						.get('users')
						.convertToApp(Meteor.user());

					const aff = Promise.await(
						manager.add(buff, {
							marketplaceInfo,
							permissionsGranted,
							enable: true,
			...
			{ authRequired: true },
			{
				post() {
					...
					try {
						...
						const result = Apps.getBridges()
							.getListenerBridge()
							.externalComponentEvent(event, externalComponent);
						...
						orchestrator
							.getRocketChatLogger()
							.error(
								`Error triggering external components' events ${ e.response.data }`,
							);
						return API.v1.internalError();
					}
				},
		...
		this.api.addRoute(
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				get() {
					...
					try {
						result = HTTP.get(
							`${ baseUrl }/v1/bundles/${ this.urlParams.id }/apps`,
							{
								headers,
							},
						);
						...
						orchestrator
							.getRocketChatLogger()
							.error(
								"Error getting the Bundle's Apps from the Marketplace:",
								e.response.data,
							);
						return API.v1.internalError();
					}
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				get() {
					if (
						...
						try {
							result = HTTP.get(
								`${ baseUrl }/v1/apps/${ this.urlParams.id }?appVersion=${ this.queryParams.version }`,
								{
									headers,
								},
							);
					...
					if (
						...
						try {
							result = HTTP.get(
								`${ baseUrl }/v1/apps/${ this.urlParams.id }/latest?frameworkVersion=${ appsEngineVersionForMarketplace }`,
								{
									headers,
								},
							);
					...
					if (!app) {
						return API.v1.notFound(
							`No App found by the id of: ${ this.urlParams.id }`,
						);
					}
				...
				post() {
					...
					if (this.bodyParams.url) {
						...
						try {
							...
							orchestrator
								.getRocketChatLogger()
								.error(
									'Error getting the App from the Marketplace:',
									e.response.data,
								);
							return API.v1.internalError();
						}
					...
					const aff = Promise.await(
						manager.update(buff, permissionsGranted),
					);
					const info = aff.getAppInfo();
		...
		this.api.addRoute(
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				post() {
					...
					const workspaceIdSetting = Promise.await(
						Settings.findOneById('Cloud_Workspace_Id'),
					);

					let result;
					try {
						result = HTTP.get(
							`${ baseUrl }/v1/workspaces/${ workspaceIdSetting.value }/apps/${ this.urlParams.id }`,
							{
								headers,
							},
						);
						...
						orchestrator
							.getRocketChatLogger()
							.error(
								'Error syncing the App from the Marketplace:',
								e.response.data,
							);
						return API.v1.internalError();
					}
					...
					Promise.await(
						Apps.updateAppsMarketplaceInfo([result.data]),
					);
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				get() {
					...
					if (prl) {
						...
						const logs = Promise.await(
							orchestrator.getLogStorage().find(ourQuery, options),
						);
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				...
				post() {
					if (!this.bodyParams || !this.bodyParams.settings) {
						return API.v1.failure(
							'The settings to update must be present.',
						);
					}
					...
					this.bodyParams.settings.forEach((s) => {
						if (settings[s.id]) {
							Promise.await(
								manager
									.getSettingsManager()
									.updateAppSetting(this.urlParams.id, s),
							);
							// Updating?
							updated.push(s);
						}
		...
		this.api.addRoute(
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				get() {
					try {
						const setting = manager
							.getSettingsManager()
							.getAppSetting(
								this.urlParams.id,
								this.urlParams.settingId,
							);
						...
						if (e.message.includes('No setting found')) {
							return API.v1.notFound(
								`No Setting found on the App by the id of: "${ this.urlParams.settingId }"`,
							);
						}
				...
				post() {
					if (!this.bodyParams.setting) {
						return API.v1.failure(
							'Setting to update to must be present on the posted body.',
						);
					}
		...
		this.api.addRoute(
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				get() {
					...
					if (prl) {
						return API.v1.success({
							apis: manager.apiManager.listApis(
								this.urlParams.id,
							),
						});
					}
		...
		this.api.addRoute(
			...
			{ authRequired: true, permissionsRequired: ['manage-apps'] },
			{
				...
				post() {
					if (
						!this.bodyParams.status
						|| typeof this.bodyParams.status !== 'string'
					) {
						return API.v1.failure(
							'Invalid status provided, it must be "status" field and a string.',
						);
					}
