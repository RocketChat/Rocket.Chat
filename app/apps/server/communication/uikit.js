...
router.use((req, res, next) => {
	...
	if (visitorToken) {
		req.visitor = Apps.getConverters()
			.get('visitors')
			.convertByToken(visitorToken);
	}
...
const appsRoutes = (orch) => (req, res) => {
	...
	switch (type) {
		case UIKitIncomingInteractionType.BLOCK: {
			...
			try {
				...
				const result = Promise.await(
					orch.triggerEvent(eventInterface, action),
				);
		...
		case UIKitIncomingInteractionType.VIEW_CLOSED: {
			...
			try {
				Promise.await(
					orch.triggerEvent('IUIKitInteractionHandler', action),
				);
