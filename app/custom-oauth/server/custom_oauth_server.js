...
export class CustomOAuth {
	...
	_normalizeId(identity) {
		...
		if (identity.user && identity.user.userid) {
			if (identity.user.userid_sec && identity.user.userid_sec[0]) {
				return identity.user.userid_sec[0];
			}
		...
		if (identity.metadata && identity.metadata.uid) {
	...
	_normalizeForServices(originalIdentity) {
		const normalized = {};

		// Fix for Reddit
		const identity = originalIdentity.result || originalIdentity;

		normalized.id = this._normalizeId(identity);
		...
		if (identity.user) {
			...
			// Fix for Xenforo [BD]API plugin for 'user.user_id; instead of 'id'
			} else if (identity.user.user_email) {
				identity.email = identity.user.user_email;
			}
		}

		// Fix OpenShift identities where id is in 'metadata' object
		if (identity.metadata && identity.metadata.fullName) {
			identity.name = identity.fullName;
		}
	...
	normalizeIdentity(identity) {
		if (identity) {
			identity = this._normalizeForServices(identity);
		}
