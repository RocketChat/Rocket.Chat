...
export class MentionsParser {
	...
	replaceChannels = (msg, { temp, channels }) => msg
		...
		.replace(this.channelMentionRegex, (match, prefix, mention) => {
			if (!temp && !(channels && channels.find((c) => { return c.dname ? c.dname === mention : c.name === mention; }))) {
				return match;
			}

			const channel = channels && channels.find(({ name, dname }) => { return dname ? dname === mention : name === mention; });
