...
const resolveJitsiCallUrl = (room) => {
	...
	return `${ settings.get('Jitsi_SSL') ? 'https://' : 'http://' }${ settings.get(
		'Jitsi_Domain',
	) }/${ settings.get('Jitsi_URL_Room_Prefix') }${ rname }${ settings.get(
				...
				const message = Messages.createWithTypeRoomIdMessageAndUser(
					...
					Meteor.user(),
					{
						...
						customFields: {
							...
							}, // Note: this is just a temporary solution for the jitsi calls to work in Livechat. In future we wish to create specific events for specific to livechat calls (eg: start, accept, decline, end, etc) and this url info will be passed via there
						},
					},
				);
