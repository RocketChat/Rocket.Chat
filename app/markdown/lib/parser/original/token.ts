...
export const isToken = (msg: string): boolean =>
	/=!=[.a-z0-9]{17}=!=/gim.test(msg.trim());
...
export const validateAllowedTokens = (
	...
	const tokensFound = message.tokens.filter(({ token }) =>
		tokens.includes(token),
	);
	return (
		tokensFound.length === 0
		|| tokensFound.every((token) => desiredTokens.includes(token.type))
	);
};
...
export const validateForbiddenTokens = (
	...
	return (
		tokensFound.length === 0
		|| !tokensFound.some((token) => desiredTokens.includes(token.type))
	);
};
