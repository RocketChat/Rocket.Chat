...
const endsWithWhitespace = (text) =>
	text.substring(text.length - 1).match(/\s/);
...
const getRegexReplacer = (replaceFunction, getRegex) => (marker, tagName) => {
	...
	return (msg) =>
		msg.replace(getRegex(marker), (...args) =>
			replaceFunction(wrapper, ...args),
		);
};
...
const getParserWithCustomMarker = getRegexReplacer(
	(wrapper, match, p1, p2, p3) => {
		...
		return wrapper(
			p1.substring(finalMarkerCount),
			p2,
			p3.substring(finalMarkerCount),
		);
	},
	(marker) =>
		new RegExp(
...
const parseItalic = getRegexReplacer(
	(wrapper, match, p1, p2, p3, p4, p5) => {
		...
		return wrapper(
			p2.substring(finalMarkerCount),
			p3,
			p4.substring(finalMarkerCount),
		);
	},
	() =>
		new RegExp(
...
const parseNotEscaped = (
	...
	msg = msg.replace(
		...
		(match, title, url) => {
			...
			if (
				isToken(title)
				&& !validateAllowedTokens(message, title, [
					'bold',
					'italic',
					'strike',
				])
