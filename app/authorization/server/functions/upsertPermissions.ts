...
export const upsertPermissions = async (): Promise<void> => {
	...
	const permissions = [
		...
		},
		{
			...
			roles: ['livechat-manager', 'livechat-monitor'],
		},
		{
	...
	const getPreviousPermissions = async function(
		...
		await Permissions.find(selector).forEach(function(
	...
	const createPermissionsForExistingSettings =		async function(): Promise<void> {
		...
		for await (const obsoletePermission of Object.keys(
			previousSettingPermissions,
		)) {
			if (
				previousSettingPermissions.hasOwnProperty(
					obsoletePermission,
				)
			) {
