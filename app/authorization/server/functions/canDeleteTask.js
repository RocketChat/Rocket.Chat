...
export const canDeleteTaskAsync = async (uid, { u, rid, ts }) => {
	...
	const deleteAllowed = await getValue('Task_AllowDeleting');

	if (!deleteAllowed) {
		return false;
	}

	const allowedToDeleteAny = await hasPermissionAsync(uid, 'delete-task', rid);

	const allowed = allowedToDeleteAny || (uid === u._id && await hasPermissionAsync(uid, 'delete-own-task'));
	if (!allowed) {
		return false;
	}
	...
	if (room.ro === true && !await hasPermissionAsync(uid, 'post-readonly', rid)) {
		...
		if (!(room.unmuted || []).includes(u.username)) {
			throw new Error('You can\'t delete tasks because the room is readonly.');
		}
	}
...
export const canDeleteTask = (uid, { u, rid, ts }) => Promise.await(canDeleteTaskAsync(uid, { u, rid, ts }));
