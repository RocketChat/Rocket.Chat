...
export function EditRoom({ roomId }) {
	...
	const readOnly = newData.readOnly ?? data.ro ?? false;
	const archived = newData.archived ?? data.archived ?? false;
	const isDefault = newData.default ?? data.default ?? false;
				...
				<Field.Row>
					<Box display='flex' flexDirection='row' justifyContent='space-between' w='full'>
						<Margins inlineEnd='x4'>
							...
							<Button flexGrow={1} disabled={!hasUnsavedChanges} onClick={() => setNewData({})}>{t('Reset')}</Button>
							<Button mie='none' flexGrow={1} disabled={!hasUnsavedChanges}>{t('Save')}</Button>
						</Margins>
					</Box>
				</Field.Row>
			</Field>
