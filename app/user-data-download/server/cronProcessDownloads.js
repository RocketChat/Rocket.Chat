...
export const getRoomData = (roomId, ownUserId) => {
	...
	const [userId] =		roomData.t === 'd'
		? roomData.uids.filter((uid) => uid !== ownUserId)
		: [null];
...
export const loadUserSubscriptions = function(
	exportOperation,
	fileType,
	userId,
) {
...
const hideUserName = function(username, userData, usersMap) {
	...
	if (!usersMap.userNameTable[username]) {
		if (userData && username === userData.username) {
			...
			usersMap.userNameTable[username] = `User_${
				Object.keys(usersMap.userNameTable).length + 1
			}`;
		}
	}
	...
	const messageObject = {
		...
		...msg.attachments && {
			attachments: msg.attachments.map((attachment) =>
				getAttachmentData(attachment, msg),
			),
		},
	};
...
export const copyFile = async function(attachmentData, assetsPath) {
	...
	FileUpload.copy(
		file,
		joinPath(assetsPath, `${ attachmentData._id }-${ attachmentData.name }`),
	);
};
...
const exportMessageObject = (type, messageObject, messageFile) => {
	...
	const timestamp = messageObject.ts
		? new Date(messageObject.ts).toUTCString()
		: '';
	...
	file.push(
		`<p><strong>${ messageObject.username }</strong> (${ timestamp }):<br/>`,
	);
	file.push(message);
	...
	if (messageFile?._id) {
		const attachment = messageObject.attachments.find(
			(att) =>
				att.type === 'file' && att.title_link.includes(messageFile._id),
		);

		const description =			attachment?.description
			|| attachment?.title
			|| TAPi18n.__('Message_Attachments');
...
export async function exportRoomMessages(
	...
	results.forEach(
		Meteor.bindEnvironment((msg) => {
			const messageObject = getMessageData(
				msg,
				hideUsers,
				userData,
				usersMap,
			...
			result.messages.push(
				exportMessageObject(exportType, messageObject, msg.file),
			);
		}),
	);
...
export const uploadZipFile = async function(filePath, userId, exportType) {
	...
	const newFileName = encodeURIComponent(
		`${ utcDate }-${ userDisplayName }${ fileSuffix }.zip`,
	);
...
export const exportRoomMessagesToFile = async function(
	...
	const limit =		settings.get('UserData_MessageLimitPerRequest') > 0
...
const continueExportOperation = async function(exportOperation) {
	...
	try {
		...
		if (exportOperation.status === 'compressing') {
			...
			exportOperation.generatedFile = joinPath(
				zipFolder,
...
Meteor.startup(function() {
	...
	settings.watch('Troubleshoot_Disable_Data_Exporter_Processor', (value) => {
		...
		SyncedCron.add({
			name,
			schedule: (parser) =>
				parser.cron(`*/${ processingFrequency } * * * *`),
