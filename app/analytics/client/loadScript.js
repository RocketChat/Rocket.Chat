...
Template.body.onRendered(function() {
	this.autorun((c) => {
		const piwikUrl =			settings.get('PiwikAnalytics_enabled')
		...
		const googleId =			settings.get('GoogleAnalytics_enabled')
			&& settings.get('GoogleAnalytics_ID');
				...
				(() => {
					try {
						if (/\S/.test(piwikAdditionalTracker)) {
							...
							const addTrackers = JSON.parse(
								piwikAdditionalTracker,
							);
