...
import './omnichannelDepartmentForm.html';
import { APIClient } from '../../../../utils/client';

Template.omnichannelDepartmentForm.helpers({
	department() {
	...
	customFieldsTemplate() {
		return getCustomFormTemplate('omnichannelDepartmentForm');
	},
	data() {
	...
	tabBarVisible() {
		return Object.values(TabBar.buttons.get())
			.some((button) => button.groups
				.some((group) => group.startsWith('omnichannel-department')));
	},
});

Template.omnichannelDepartmentForm.events({
	'submit #department-form'(e, instance) {
		...
		const callback = (error) => {
			...
			FlowRouter.go('omnichannel-departments');
		};
...
Template.omnichannelDepartmentForm.onCreated(async function() {
