...
import './omnichannelAnalytics.html';
...
const updateAnalyticsChart = () => {
	...
	Meteor.call('livechat:getAnalyticsChartData', options, async function(error, result) {
		...
		chartContext = await drawLineChart(document.getElementById('omnichannel-analytics-chart'), chartContext, [result.chartLabel], result.dataLabels, [result.dataPoints]);
	});
...
Template.omnichannelAnalytics.helpers({
...
Template.omnichannelAnalytics.onCreated(function() {
...
Template.omnichannelAnalytics.onRendered(() => {
	Tracker.autorun(() => {
...
Template.omnichannelAnalytics.events({
	'click .omnichannel-date-picker-btn'(e) {
		e.preventDefault();
		const options = [];
		const config = {
			template: 'omnichannelAnalyticsDaterange',
	...
	'click .omnichannel-daterange-prev'(e) {
		e.preventDefault();

		templateInstance.daterange.set(updateDateRange(templateInstance.daterange.get(), -1));
	},
	'click .omnichannel-daterange-next'(e) {
		e.preventDefault();

		templateInstance.daterange.set(updateDateRange(templateInstance.daterange.get(), 1));
	},
	'change #omnichannel-analytics-options'(e) {
		e.preventDefault();
	...
	'change #omnichannel-analytics-chart-options'(e) {
		e.preventDefault();
