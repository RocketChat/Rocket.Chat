...
import './omnichannelAnalyticsDaterange.html';

Template.omnichannelAnalyticsDaterange.helpers({
	bold(prop) {
		return prop === Template.currentData().daterange.get().value ? 'rc-popover__item--bold' : '';
	},
});

Template.omnichannelAnalyticsDaterange.events({
	'change input'(e) {
		e.preventDefault();
		...
		switch (value) {
			case 'custom':
				const target = document.getElementsByClassName('omnichannel-date-picker-btn')[0];
				const options = [];
				const config = {
					template: 'omnichannelAnalyticsCustomDaterange',
