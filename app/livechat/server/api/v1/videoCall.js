...
API.v1.addRoute('livechat/video.call/:token', {
	get() {
		try {
			...
			if (
				!config.theme
				|| !config.theme.actionLinks
				|| !config.theme.actionLinks.jitsi
			) {
			...
			const videoCall = {
				...
				provider: 'jitsi',
				room:
					rcSettings.get('Jitsi_URL_Room_Prefix')
					+ rname
					+ rcSettings.get('Jitsi_URL_Room_Suffix'),
	...
	{ authRequired: true },
	{
		get() {
			try {
				...
				const webrtcCallingAllowed =					rcSettings.get('WebRTC_Enabled') === true
				...
				if (
					!config.theme
					|| !config.theme.actionLinks
					|| !config.theme.actionLinks.webrtc
				) {
					throw new Meteor.Error('invalid-livechat-config');
				}
				...
				if (
					...
					callStatus = 'ringing';
					Promise.await(
						Rooms.setCallStatusAndCallStartTime(
							room._id,
							callStatus,
						),
					);
