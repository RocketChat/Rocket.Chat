	...
	post() {
		try {
			...
			if (
				...
				&& msg.length
					> parseInt(settings.get('Livechat_message_character_limit'))
			) {
			...
			const sendMessage = {
				...
				roomInfo: {
					source: {
						type: this.isWidget()
							? OmnichannelSourceType.WIDGET
							: OmnichannelSourceType.API,
					},
				},
			};
	...
	put() {
		try {
			...
			if (result) {
				let message = Messages.findOneById(_id);
				if (message.file) {
					message = Promise.await(
						normalizeMessageFileUpload(message),
					);
				}
	...
	get() {
		try {
			...
			const messages = loadMessageHistory({
				...
				Promise.await(normalizeMessageFileUpload(...args)),
			);
	...
	{ authRequired: true },
	{
		post() {
			...
			const sentMessages = this.bodyParams.messages.map((message) => {
				...
				const sentMessage = Promise.await(
					Livechat.sendMessage(sendMessage),
				);
				return {
