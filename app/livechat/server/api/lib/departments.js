...
export async function findDepartmentById({
	...
	const canViewLivechatDepartments = await hasPermissionAsync(
		userId,
	...
	if (includeAgents && canViewLivechatDepartments) {
		result.agents = await LivechatDepartmentAgents.find({
			departmentId,
		}).toArray();
	}
...
export async function findDepartmentAgents({
	...
	const cursor = LivechatDepartmentAgents.findAgentsByDepartmentId(
		departmentId,
		{
