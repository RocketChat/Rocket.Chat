...
const getSecondsWhenOfficeHoursIsDisabled = (room, agentLastMessage) =>
	moment(new Date(room.closedAt)).diff(
		moment(new Date(agentLastMessage.ts)),
		'seconds',
	);
...
const getSecondsSinceLastAgentResponse = async (room, agentLastMessage) => {
	...
	const department =		room.departmentId
		&& await LivechatDepartment.findOneById(room.departmentId);
	if (department && department.businessHourId) {
		const businessHour = await LivechatBusinessHours.findOneById(
			department.businessHourId,
		);
		officeDays = (
			await businessHourManager.getBusinessHour(
				businessHour._id,
				businessHour.type,
			)
		).workHours.reduce(parseDays, {});
	} else {
		officeDays = (
			await businessHourManager.getBusinessHour()
		).workHours.reduce(parseDays, {});
	...
	const daysOfInactivity = endOfConversation
		.clone()
		.startOf('day')
		.diff(startOfInactivity.clone().startOf('day'), 'days');
	...
	for (let index = 0; index <= daysOfInactivity; index++) {
		...
		const startTodaysOfficeHour = moment(
			`${ officeDay.start.day }:${ officeDay.start.time }`,
			'dddd:HH:mm',
		).add(index, 'days');
		const endTodaysOfficeHour = moment(
			`${ officeDay.finish.day }:${ officeDay.finish.time }`,
			'dddd:HH:mm',
		).add(index, 'days');
		if (officeDays[today].open) {
			const firstDayOfInactivity =				startOfInactivity.clone().format('D')
				=== inactivityDay.clone().format('D');
			const lastDayOfInactivity =				endOfConversation.clone().format('D')
				=== inactivityDay.clone().format('D');

			if (!firstDayOfInactivity && !lastDayOfInactivity) {
				totalSeconds += endTodaysOfficeHour
					.clone()
					.diff(startTodaysOfficeHour, 'seconds');
			} else {
				const end = endOfConversation.isBefore(endTodaysOfficeHour)
					? endOfConversation
					: endTodaysOfficeHour;
				const start = firstDayOfInactivity
					? inactivityDay
					: startTodaysOfficeHour;
...
callbacks.add(
	...
	async (room) => {
		...
		const agentLastMessage =			Messages.findAgentLastMessageByVisitorLastMessageTs(
			room._id,
		...
		LivechatRooms.setVisitorInactivityInSecondsById(
			room._id,
			secondsSinceLastAgentResponse,
		);
	},
