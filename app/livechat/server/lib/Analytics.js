...
export const Analytics = {
	...
	ChartData: {
		...
		Total_conversations(date) {
			return Rooms.getTotalLivechatConversationsBetweenDate('l', date);
		},
		...
		Avg_chat_duration(date) {
			...
			Rooms.getLivechatAnalyticsMetricsBetweenDate('l', date).forEach(({ metrics }) => {
		...
		Total_messages(date) {
			...
			Rooms.getLivechatAnalyticsMetricsBetweenDate('l', date).forEach(({ msgs }) => {
				if (msgs) {
					total += msgs;
				}
			});
	...
	OverviewData: {
		...
		Conversations(from, to) {
			...
			for (let m = moment(from); m.diff(to, 'days') <= 0; m.add(1, 'days')) {
				...
				const result = Rooms.getLivechatAnalyticsMetricsBetweenDate('l', date);
			...
			for (let m = moment(from).day(busiestDay); m <= to; m.add(7, 'days')) {
				...
				for (let h = moment(m); h.diff(m, 'days') <= 0; h.add(1, 'hours')) {
					...
					Rooms.getLivechatAnalyticsMetricsBetweenDate('l', date).forEach(({
						msgs,
					}) => {
