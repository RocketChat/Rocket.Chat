...
import {
	ILivechatVisitor,
	ILivechatVisitorConnectionData,
	IVisitorLastChat,
} from '../../../../definition/ILivechatVisitor';
...
export const Livechat = {
   ...
   * @param {string} token the visitor token
   * @returns {Partial<ILivechatVisitor>} the visitor data
   */
	findGuest(token: string): Partial<ILivechatVisitor> {
	...
	/**
   * Returns true if there are agents available (for department or global)
   ...
   */
	online(department?: string): boolean {
		...
		if (settings.get('Livechat_assign_new_conversation_to_bot')) {
			Livechat.logger.debug(
				`Fetching online bot agents for department ${ department }`,
			);
			const botAgents = Livechat.getBotAgents(department);
			const onlineBots = botAgents.count() as unknown as number;
	...
	/**
   * Returns next agent available to take an inquiry based on selected Routing method
   * @param {string} department the department ID
   * @returns {ILivechatAgentRecord} the available agent's information
   */
	getNextAgent(department: string): Promise<ILivechatAgentRecord> {
		return RoutingManager.getNextAgent(department);
	},

	/**
   * Returns the list of users which are agents
   * @param {string} [department] the department ID
   * @returns {ILivechatAgentRecord[]} the list of agents
   */
	getAgents(department?: string): ILivechatAgentRecord[] {
		if (department) {
	...
	/**
   * Search for all online agents, scoped by department or agentId
   * @param {string} [department] the department ID
   * @param {Object} [agent] an agent object
   * @returns {Object | Object[]} the list of available agents
   */
	getOnlineAgents(department?: string, agent?: any): ILivechatAgentRecord[] {
		if (agent?.agentId) {
	...
	/**
   * Checks if a given department or agent is online
   * @param {string} [department] the department ID
   * @param {Object} [agent] the agent you want to know if its online
   * @returns {boolean}
   */
	checkOnlineAgents(department?: string, agent?: any): boolean {
		if (agent?.agentId) {
	...
	/**
   * Returns a list of bot agents per department or global
   * @param {string} [department] the department ID
   ...
   */
	...
	/**
   * Returns the first department which is required on registration and has/hasn't online agents
   * @param {boolean} [onlineRequired=true] wether or not it's required for a department to have online agents
   * @returns {Object} the department that meets the criteria
   */
	getRequiredDepartment(onlineRequired = true): ILivechatDepartment {
		const departments = LivechatDepartment.findEnabledWithAgents();

		return departments.fetch().find((dept: any) => {
			...
			const onlineAgents = LivechatDepartmentAgents.getOnlineForDepartment(
				dept._id,
			);
	...
	/**
   * Finds or create a room for a given pair of visitor & agent
   * @param {ILivechatVisitor} guest the visitor object
   * @param {IMessage} message a default message to be sent on room
   * @param {Object} roomInfo an object with extra properties to be added to room creation
   * @param {ILivechatAgentRecord} agent the agent that will be assigned to room
   * @param {Object} [extraData] extra information for room creation
   ...
   */
	async getRoom(
		...
		Livechat.logger.debug(
			`Attempting to find or create a room for visitor ${ guest._id }`,
		);
		...
		if (room && !room.open) {
			Livechat.logger.debug(
				`Last room for visitor ${ guest._id } closed. Creating new one`,
			);
		...
		if (room == null) {
			...
			if (!defaultAgent && !guest.department) {
				...
				Livechat.logger.debug(
					`No department or default agent selected for ${ guest._id }`,
				);

				if (department) {
					Livechat.logger.debug(
						`Assigning ${ guest._id } to department ${ department._id }`,
					);
			...
			Livechat.logger.debug(
				`Calling QueueManager to request a room for visitor ${ guest._id }`,
			);
			...
			Livechat.logger.debug(
				`Room obtained for visitor ${ guest._id } -> ${ room._id }`,
			);
		}

		if (!room || room.v.token !== guest.token) {
			Livechat.logger.debug(
				`Visitor ${ guest._id } trying to access another visitor's room`,
			);
	...
	/**
   * Sends a message to a specified room. If room doesnt exists, will be created
   * @param {Object} the information about the message to be sent
   * @returns {Object} the message sent
   */
	async sendMessage({
		guest,
		message,
	...
	updateMessage({
		...
		const originalMessage = Messages.findOneById(message._id) as IMessage;
	...
	deleteMessage({
		...
		Livechat.logger.debug(
			`Attempting to delete a message by visitor ${ guest._id }`,
		);
		check(message, Match.ObjectIncluding({ _id: String }));

		const msg = Messages.findOneById(message._id) as IMessage;
		if (!msg || !msg._id) {
			return false;
		}
		...
		if (!deleteAllowed || !editOwn) {
			...
			throw new Meteor.Error(
				'error-action-not-allowed',
				'Message deleting not allowed',
				{ method: 'livechatDeleteMessage' },
			);
		}

		// deleteMessage expects a User, but we're passing a Visitor here :(
		deleteMessage(message, guest as unknown as IUser);
	...
	ILivechatVisitor & { connectionData: ILivechatVisitorConnectionData }
	>): string {
		Livechat.logger.debug(
			`No matches found. Attempting to create new user with token ${ token }`,
		);
		if (!username) {
      ...
      && connection.httpHeaders
			...
			userData.ip = connection.httpHeaders['x-real-ip']
        || connection.httpHeaders['x-forwarded-for']
        || connection.clientAddress;
			userData.host = connection.httpHeaders.host;
		}

		return LivechatVisitors.insert(userData);
	},
	...
	registerGuest({
		...
		username?: string;
		email?: string;
	} & { connectionData?: ILivechatVisitorConnectionData } = {}): string {
		check(token, String);
		check(id, Match.Maybe(String));

		Livechat.logger.debug(
			`New incoming conversation: id: ${ id } | token: ${ token }`,
		);
		...
		if (department) {
			Livechat.logger.debug(
				`Attempt to find a department with id/name ${ department }`,
			);
			...
			Livechat.logger.debug(
				`Assigning visitor ${ token } to department ${ dep._id }`,
			);
      ...
      && (existingUser = LivechatVisitors.findOneGuestByEmailAddress(email))
			...
			userId = this.createVisitor({ token, username, _id: id, connectionData });
		}
	...
	setDepartmentForGuest({
		token,
		department,
	}: { token?: string; department?: string } = {}): boolean {
		check(token, String);
		Livechat.logger.debug(
			`Switching departments for user with token ${ token } (to ${ department })`,
		);
		...
		const dep = LivechatDepartment.findOneById(
			department,
	...
	saveCustomFields(fields: Cursor<ILivechatCustomField>, livechatData: any): { [k: string]: ILivechatCustomField }[] {
		...
		fields.forEach((field) => {
			...
			if (value !== '' && field.regexp !== undefined && field.regexp !== '') {
				...
				if (!regexp.test(value)) {
					throw new Meteor.Error(
						TAPi18n.__('error-invalid-custom-field-value', {
							field: field.label,
						}),
					);
				}
	...
	userId: string,
		...
		if (!userId || hasPermission(userId, 'edit-livechat-room-customfields')) {
			const fields = LivechatCustomField
				.find({ scope: 'visitor' }) as Cursor<ILivechatCustomField>;
			updateData.livechatData = this.saveCustomFields(fields, livechatData);
		}
	...
	closeRoom({ user, visitor, room, comment, options = {} }: { user?: IUser; visitor?: ILivechatVisitor; room?: IOmnichannelRoom; comment?: string; options?: any }): boolean {
		...
		Livechat.logger.debug(
			`Room ${ room._id } was closed at ${
				closeData.closedAt
			} (duration ${ closeData.chatDuration })`,
		);
		...
		Messages.createCommandWithRoomIdAndUser(
			'promptTranscript',
			rid,
			closeData.closedBy,
		);
		...
		Meteor.defer(() => {
			...
			Apps.getBridges()
				?.getListenerBridge()
				.livechatEvent(AppEvents.IPostLivechatRoomClosed, room);
		});
	...
	setCustomFields({
		...
		value: string;
		overwrite: boolean;
	} = { token: '', key: '', value: '', overwrite: false }): any {
		check(token, String);
		check(key, String);
		check(value, String);
		check(overwrite, Boolean);
		Livechat.logger.debug(
			`Setting custom fields data for visitor with token ${ token }`,
		);
		...
		if (customField.regexp !== undefined && customField.regexp !== '') {
			...
			if (!regexp.test(value)) {
				throw new Meteor.Error(
					TAPi18n.__('error-invalid-custom-field-value', { field: key }),
				);
			}
		}
		...
		return LivechatVisitors.updateLivechatDataByToken(
			token,
			key,
			value,
			overwrite,
	...
	enabled(): boolean {
		return settings.get('Livechat_enabled') as boolean;
	},

	getInitSettings(): { [k: string]: string } {
		const rcSettings = {} as any;
		...
		rcSettings.Livechat_history_monitor_type = settings.get(
	...
	saveRoomInfo(roomData: any, guestData: any, userId: string): any {
		Livechat.logger.debug(`Saving room information on room ${ roomData._id }`);
		const { livechatData = {} } = roomData;

		if (!userId || hasPermission(userId, 'edit-livechat-room-customfields')) {
			const fields = LivechatCustomField.find({ scope: 'room' });
			roomData.livechatData = this.saveCustomFields(fields, livechatData);
		}
	...
	closeOpenChats(userId: string, comment: string): void {
		Livechat.logger.debug(`Closing open chats for user ${ userId }`);
		const user = Users.findOneById(userId);
		LivechatRooms.findOpenByAgent(userId).forEach((room: any) => {
			this.closeRoom({ user, room, comment });
		});
	},

	forwardOpenChats(userId: string): void {
	...
	savePageHistory(
		...
		Livechat.logger.debug(
			`Saving page movement history for visitor with token ${ token }`,
		);
	...
	saveTransferHistory(room: IOmnichannelRoom, transferData: any): { [k: string]: any } {
		...
		Livechat.logger.debug(
			`Storing new chat transfer of ${
				room._id
			} [Transfered by: ${ _id } to ${ scopeData }]`,
		);
	...
	async transfer(room: IOmnichannelRoom, guest: ILivechatVisitor, transferData: any): Promise<boolean> {
		Livechat.logger.debug(
		...
		if (transferData.departmentId) {
			transferData.department = LivechatDepartment.findOneById(
	...
	returnRoomAsInquiry(rid: string, departmentId: string): boolean {
		Livechat.logger.debug(
			`Transfering room ${ rid } to ${ departmentId ? 'department' : '' } queue`,
		);
		const room = LivechatRooms.findOneById(rid);
		if (!room) {
			throw new Meteor.Error('error-invalid-room', 'Invalid room', {
		...
		Livechat.logger.debug(
			`Transfering room ${ room._id } by user ${ transferredBy._id }`,
		);
		const transferData = {
			roomId: rid,
			scope: 'queue',
		...
		try {
			...
			throw new Meteor.Error(
				...
				'Error returning inquiry to the queue',
				{ method: 'livechat:returnRoomAsInquiry' },
			);
		}
	...
	sendRequest(
		...
		callback: Function,
		attempts = 10,
	): HTTP.HTTPResponse | undefined {
		if (!attempts) {
			return;
		}
		const secretToken = settings.get('Livechat_secret_token') as string;
		...
		try {
			...
			this.webhookLogger.warn('Will try again in 10 seconds ...');
			setTimeout(
	...
	getLivechatRoomGuestInfo(room: IOmnichannelRoom): { [k: string]: any } {
		...
		if (agent) {
			...
			postData.agent = {
				...
				name: agent.name,
				email: null,
				...(customFields as object) && { customFields },
			};
	...
	addAgent(username: string): boolean | IUser {
	...
	addManager(username: string): boolean | IUser {
		check(username, String);
	...
	removeAgent(username: string): boolean {
		check(username, String);
	...
	removeManager(username: string): boolean {
		check(username, String);
	...
	setUserStatusLivechat(userId: string, status: string): IUser {
		const user = Users.setLivechatStatus(userId, status);
		callbacks.runAsync('livechat.setUserStatusLivechat', { userId, status });
		return user;
	},

	cleanGuestHistory(_id: string): void {
		const guest = LivechatVisitors.findOneById(_id);
		if (!guest) {
			throw new Meteor.Error('error-invalid-guest', 'Invalid guest', {
		...
		LivechatRooms.findByVisitorToken(token).forEach((room: any) => {
	...
	saveDepartmentAgents(
		_id: string,
		departmentAgents: { upsert: Partial<ILivechatDepartmentAgents>[]; remove: Partial<ILivechatDepartmentAgents>[] },
	): boolean {
		check(_id, String);
	...
	saveDepartment(
		_id: string,
		departmentData: Omit<ILivechatDepartment, '_id'>,
		departmentAgents: { upsert: Partial<ILivechatDepartmentAgents>[]; remove: Partial<ILivechatDepartmentAgents>[] },
	): ILivechatDepartment {
      ...
      && (!chatClosingTags || chatClosingTags.length === 0)
		) {
			throw new Meteor.Error(
		...
		const departmentDB = LivechatDepartment.createOrUpdateDepartment(
			_id,
			departmentData,
		);
	...
	saveAgentInfo(
		_id: string,
		agentData: any,
		agentDepartments: { upsert: any; remove: any },
	): boolean {
		check(_id, Match.Maybe(String));
	...
	removeDepartment(_id: string): any {
		check(_id, String);
	...
	sendTranscript({
		...
		const timezone = getTimezone(user) as string;
		Livechat.logger.debug(
			`Transcript will be sent using ${ timezone } as timezone`,
		);
		...
		const messages = Messages.findVisibleByRoomIdNotContainingTypes(
			rid,
			ignoredMessageTypes,
			{ sort: { ts: 1 } },
		);
		...
		let fromEmail = settings.get('From_Email') as string;
		const matchingEmail = fromEmail.match(
		...
		Messages.createTranscriptHistoryWithRoomIdMessageAndUser(
			room._id,
			'',
			user,
	...
	notifyGuestStatusChanged(token: string, status: string): void {
	...
	sendOfflineMessage(
		...
		let emailTo = settings.get('Livechat_offline_email') as string;
		...
		const subject = `Livechat offline message from ${ name }: ${
			emailMessage?.toString().substring(0, 20)
		}`;
	...
	notifyAgentStatusChanged(userId: string, status: string): void {
	...
	allowAgentChangeServiceStatus(
		...
		return Promise.await(
			businessHourManager.allowAgentChangeServiceStatus(agentId),
		);
	},

	notifyRoomVisitorChange(roomId: string, visitor: ILivechatVisitor): void {
	...
	changeRoomVisitor(userId: string, roomId: string, visitor: ILivechatVisitor): IOmnichannelRoom {
		const user = Promise.await(Users.findOneById(userId)) as any;
		if (!user) {
		...
		const room = Promise.await(
			LivechatRooms.findOneById(roomId, { _id: 1, t: 1 }),
		) as any;
		if (!room) {
	...
	updateLastChat(contactId: string, lastChat: IVisitorLastChat): void {
		const updateUser = {
			$set: {
				lastChat,
			},
...
settings.get('Livechat_history_monitor_type', (_, value) => {
	Livechat.historyMonitorType = value as string;
});
