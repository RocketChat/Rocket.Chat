...
export const queueInquiry = async (room, inquiry, defaultAgent) => {
	...
	logger.debug(
		`Delegating inquiry with id ${ inquiry._id } to agent ${ defaultAgent?.username }`,
	);
...
export const QueueManager = {
	async requestRoom({ guest, message, roomInfo, agent, extraData }) {
		...
		if (!checkServiceStatus({ guest, agent })) {
			logger.debug(
				`Cannot create room for visitor ${ guest._id }. No online agents`,
			);
		...
		logger.debug(
			`Room for visitor ${ guest._id } created with id ${ room._id }`,
		);
		...
		logger.debug(
			`Generated inquiry for visitor ${ guest._id } with id ${ inquiry._id } [Not queued]`,
		);
	...
	async unarchiveRoom(archivedRoom = {}) {
		...
		if (oldInquiry) {
			logger.debug(
				`Removing old inquiry (${ oldInquiry._id }) for room ${ rid }`,
			);
		...
		logger.debug(
			`Generated inquiry for visitor ${ v._id } with id ${ inquiry._id } [Not queued]`,
		);
