...
export const createLivechatRoom = (
	rid,
	name,
	guest,
	roomInfo = {},
	...
	const extraRoomInfo = callbacks.run(
		'livechat.beforeRoom',
		roomInfo,
		extraData,
	);
	...
	sendMessage(
		guest,
		{ t: 'livechat-started', msg: '', groupable: false },
		room,
	);
...
export const createLivechatSubscription = (
	...
	const existingSubscription = Subscriptions.findOneByRoomIdAndUserId(
		rid,
		agent.agentId,
	);
	...
	const externalCustomFields = () => {
		...
		try {
			...
			return Object.keys(parseCustomFields).filter(
				(customFieldKey) =>
					parseCustomFields[customFieldKey].sendToIntegrations
					=== true,
			);
		} catch (error) {
...
export const dispatchInquiryQueued = (inquiry, agent) => {
	...
	if (!onlineAgents) {
		logger.debug(
			'Cannot notify agents of queued inquiry. No online agents found',
		);
		return;
	}
...
export const updateChatDepartment = ({
	rid,
	newDepartmentId,
	oldDepartmentId,
}) => {
...
export const forwardRoomToDepartment = async (room, guest, transferData) => {
	...
	logger.debug(
		`Attempting to forward room ${ room._id } to department ${ transferData.departmentId }`,
	);
	...
	if (agentId) {
		logger.debug(
			`Forwarding room ${ room._id } to department ${ departmentId } (to user ${ agentId })`,
		);
		...
		user = LivechatDepartmentAgents.findOneByAgentIdAndDepartmentId(
			agentId,
			departmentId,
		);
		if (!user) {
	...
	if (!roomTaken) {
		logger.debug(
			`Cannot forward room ${ room._id }. Unable to delegate inquiry`,
		);
		return false;
	}
	...
	if (oldServedBy) {
		...
		RoutingManager.removeAllRoomSubscriptions(
			room,
			!chatQueued && servedBy,
		);
	}
	...
	if (chatQueued) {
		logger.debug(
			`Forwarding succesful. Marking inquiry ${ inquiry._id } as ready`,
		);
	...
	logger.debug(
		`Department for visitor with token ${ token } was updated to ${ departmentId }`,
	);
...
export const normalizeTransferredByData = (transferredBy, room) => {
	if (!transferredBy || !room) {
		throw new Error(
			'You must provide "transferredBy" and "room" params to "getTransferredByData"',
		);
	}
...
export const userCanTakeInquiry = (user) => {
	...
	return (
		(status !== 'offline' && statusLivechat === 'available')
		|| roles.includes('bot')
	);
};

export const updateDepartmentAgents = (
	departmentId,
	agents,
	departmentEnabled,
) => {
