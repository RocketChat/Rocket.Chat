...
export const Livechat = {
	...
	checkOnlineAgents(department, agent) {
		...
		if (department) {
			return LivechatDepartmentAgents.checkOnlineForDepartment(
				department,
			);
		}
	...
	async getRoom(guest, message, roomInfo, agent, extraData) {
		...
		if (
			guest.department
			&& !LivechatDepartment.findOneById(guest.department)
		) {
	...
	registerGuest({
		...
		if (user) {
			...
			if (
				...
				Livechat.logger.debug(
					`Saving connection data for visitor ${ token }`,
				);
	...
	saveRoomInfo(roomData, guestData, userId) {
		Livechat.logger.debug(
			`Saving room information on room ${ roomData._id }`,
		);
	...
	setUserStatusLivechatIf(userId, status, condition, fields) {
		const user = Promise.await(
			UsersRaw.setLivechatStatusIf(userId, status, condition, fields),
		);
		callbacks.runAsync('livechat.setUserStatusLivechat', {
			userId,
			status,
	...
	getRoomMessages({ rid }) {
		...
		const isLivechat = Promise.await(
			Rooms.findByTypeInIds('l', [rid]),
		).count();
		...
		return Messages.findVisibleByRoomIdNotContainingTypes(
			rid,
			ignoredMessageTypes,
			{ sort: { ts: 1 } },
		).fetch();
