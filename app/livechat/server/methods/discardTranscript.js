...
Meteor.methods({
	'livechat:discardTranscript'(rid) {
		...
		if (!room || !room.open) {
			throw new Meteor.Error('error-invalid-room', 'Invalid room', { method: 'livechat:requestTranscript' });
		}

		if (!room.transcriptRequest) {
			throw new Meteor.Error('error-transcript-not-requested', 'No transcript requested for this chat', { method: 'livechat:requestTranscript' });
		}
