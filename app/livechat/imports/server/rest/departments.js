		...
		post() {
			...
			try {
				...
				const agents = this.bodyParams.agents
					...
					: {};
				const department = Livechat.saveDepartment(
					null,
					this.bodyParams.department,
					agents,
				);
...
API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const { department, agents } = Promise.await(
				findDepartmentById({
					...
					includeAgents:
						this.queryParams.includeAgents
						&& this.queryParams.includeAgents === 'true',
					onlyMyDepartments: onlyMyDepartments === 'true',
				}),
			);
		...
		put() {
			const permissionToSave = hasPermission(
				this.userId,
				'manage-livechat-departments',
			);
			const permissionToAddAgents = hasPermission(
				this.userId,
