...
Meteor.methods({
	async insertOrUpdateUserStatus(userStatusData) {
		...
		if (userStatusData._id) {
			...
			matchingResults = await CustomUserStatus.findByName(
				userStatusData.name,
			).toArray();
		}
		...
		if (!userStatusData._id) {
			...
			const _id = await (
				await CustomUserStatus.create(createUserStatus)
			).insertedId;
		...
		if (userStatusData.name !== userStatusData.previousName) {
			await CustomUserStatus.setName(
				userStatusData._id,
				userStatusData.name,
			);
		}
		...
		if (userStatusData.statusType !== userStatusData.previousStatusType) {
			await CustomUserStatus.setStatusType(
				userStatusData._id,
				userStatusData.statusType,
			);
		}
