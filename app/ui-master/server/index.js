...
Meteor.startup(() => {
	...
	settings.watch('Meta_robots', (value = '') => {
		...
		injectIntoHead(
			'Meta_robots',
			`<meta name="robots" content="${ escapedValue }">`,
		);
	});
	...
	settings.watch('Meta_msvalidate01', (value = '') => {
		...
		injectIntoHead(
			'Meta_msvalidate01',
			`<meta name="msvalidate.01" content="${ escapedValue }">`,
		);
	});
	...
	settings.watch('Meta_fb_app_id', (value = '') => {
		...
		injectIntoHead(
			'Meta_fb_app_id',
			`<meta property="fb:app_id" content="${ escapedValue }">`,
		);
	});
...
const renderDynamicCssList = _.debounce(
	Meteor.bindEnvironment(() => {
		...
		injectIntoBody(
			'dynamic-variables',
			`<style id='css-variables'> :root {${ css }}</style>`,
		);
	}),
	500,
);
