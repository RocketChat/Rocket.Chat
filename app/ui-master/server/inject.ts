...
const callback: NextHandleFunction = (req, res, next) => {
	...
	try {
		...
		const pathname =			rawPath !== undefined
			&& rawPath.pathname
			&& decodeURIComponent(rawPath.pathname);
		...
		const injection = headInjections.get(pathname.replace(/^\//, '')) as
			| Injection
			| undefined;
...
export const applyHeadInjections = (
