...
export const getSamlConfigs = function(service: string): Record<string, any> {
	const configs = {
		...
		secret: {
			...
			cert: SAMLUtils.normalizeCert(
				(settings.get(`${ service }_cert`) as string) || '',
			),
		},
		signatureValidationType: settings.get(
			`${ service }_signature_validation_type`,
		),
		...
		includePrivateChannelsInUpdate: settings.get(
			`${ service }_include_private_channels_update`,
		),
	};
...
export const configureSamlService = function(
	...
	if (samlConfigs.secret.privateKey && samlConfigs.secret.publicCert) {
		...
		SAMLUtils.error(
			'SAML Service: You must specify both cert and key files.',
		);
	}
	...
	return {
		...
		callbackUrl: Meteor.absoluteUrl(
			`_saml/validate/${ samlConfigs.clientConfig.provider }`,
		),
	};
};
						...
						'All',
						{
							...
							values: [
								...
								},
								{
									key: 'All',
									i18nLabel:
										'SAML_Custom_signature_validation_all',
								},
							],
			...
			function() {
				...
				this.section('SAML_Section_3_Behavior', function() {
						...
						values: [
							{
								key: 'None',
								i18nLabel:
									'SAML_Custom_Username_Normalize_None',
							},
							{
								key: 'Lowercase',
								i18nLabel:
									'SAML_Custom_Username_Normalize_Lowercase',
							},
						],
						...
						values: [
							{
								key: 'SAML',
								i18nLabel:
									'SAML_Custom_Logout_Behaviour_Terminate_SAML_Session',
							},
							{
								key: 'Local',
								i18nLabel:
					...
					this.add(`SAML_Custom_${ name }_channels_update`, false, {
						...
						i18nDescription:
							'SAML_Custom_channels_update_description',
					});
					this.add(
						...
						false,
						{
							type: 'boolean',
							i18nLabel:
								'SAML_Custom_include_private_channels_update',
							i18nDescription:
