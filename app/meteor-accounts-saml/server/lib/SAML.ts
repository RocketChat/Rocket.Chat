...
export class SAML {
	public static processRequest(
		...
		if (!service) {
			if (samlObject.actionName === 'metadata') {
				showErrorMessage(
					res,
					`Unexpected SAML service ${ samlObject.serviceName }`,
				);
				return;
			}
		...
		switch (samlObject.actionName) {
			...
			case 'validate':
				return this.processValidateAction(
					req,
					res,
					service,
					samlObject,
			...
			default:
				throw new Error(
					`Unexpected SAML action ${ samlObject.actionName }`,
				);
		}
	}

	public static async hasCredential(
		credentialToken: string,
	): Promise<boolean> {
		return (
			await CredentialTokens.findOneNotExpiredById(credentialToken)
			!= null
		);
	}
	...
	public static async retrieveCredential(
		...
		const data = await CredentialTokens.findOneNotExpiredById(
			credentialToken,
		);
		if (data) {
	...
	public static insertOrUpdateSAMLUser(userObject: ISAMLUser): {
		userId: string;
		token: string;
	} {
		const {
		...
		if (!user) {
			const expression = userObject.emailList
	...
	private static processLogoutRequest(
		...
		const serviceProvider = new SAMLServiceProvider(service);
		serviceProvider.validateLogoutRequest(
			...
			(err, result) => {
				...
				if (!result) {
					throw new Meteor.Error(
						'Unable to process Logout Request: missing request data.',
					);
				}
				...
				fiber(() => {
					try {
						const cursor = Users.findBySAMLNameIdOrIdpSession(
							result.nameID,
							result.idpSession,
						);
						...
						if (count > 1) {
							throw new Meteor.Error(
								'Found multiple users matching SAML session',
							);
						}

						if (count === 0) {
							throw new Meteor.Error(
								'Invalid logout request: no user associated with session.',
							);
						}
						...
						serviceProvider.logoutResponseToUrl(
							response,
							(err, url) => {
								if (err) {
									SystemLogger.error({ err });
	...
	private static processLogoutResponse(
		...
		if (!req.query.SAMLResponse) {
			SAMLUtils.error(
				'Invalid LogoutResponse, missing SAMLResponse',
				req.query,
			);
		...
		const serviceProvider = new SAMLServiceProvider(service);
		serviceProvider.validateLogoutResponse(
			...
			(err, inResponseTo) => {
				...
				if (!inResponseTo) {
					throw new Meteor.Error(
						'Invalid logout request: no inResponseTo value.',
					);
				}

				const logOutUser = (inResponseTo: string): void => {
					SAMLUtils.log(
						`Logging Out user via inResponseTo ${ inResponseTo }`,
					);
					...
					if (count > 1) {
						throw new Meteor.Error(
							'Found multiple users matching SAML inResponseTo fields',
						);
					}

					if (count === 0) {
						throw new Meteor.Error(
							'Invalid logout request: no user associated with inResponseTo.',
						);
					}
	...
	private static processSLORedirectAction(
		req: IIncomingMessage,
		res: ServerResponse,
	): void {
		res.writeHead(302, {
		...
		serviceProvider.validateResponse(
			...
			async (err, profile /* , loggedOut*/) => {
				try {
					...
					if (!profile) {
						throw new Error(
							'No user data collected from IdP response.',
						);
					}
					...
					const url = `${ Meteor.absoluteUrl(
						'home',
					) }?saml_idp_credentialToken=${ credentialToken }`;
					res.writeHead(302, {
						Location: url,
					});
					res.end();
	...
	private static subscribeToSAMLChannels(
