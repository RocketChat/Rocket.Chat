...
const reloadUsersFromRoomMessages = (rid, template) => {
	const user =		Meteor.userId()
		&& Meteor.users.findOne(Meteor.userId(), { fields: { username: 1 } });
	if (!rid || !user) {
		return;
	}
...
const fetchUsersFromServer = _.throttle(
	async (filterText, records, rid, cb) => {
		...
		users
			// .slice(0, 5)
			.forEach(
				({ username, nickname, name, status, avatarETag, outside }) => {
					// if (records.length < 5) {
					records.push({
						_id: username,
						username,
...
const fetchRoomsFromServer = _.throttle(
	async (filterText, records, rid, cb) => {
...
Template.messagePopupConfig.helpers({
	...
	popupUserConfig() {
		const template = Template.instance();
		const suggestionsCount = settings.get(
		...
		return {
			...
			getFilter: (collection, filter = '', cb) => {
				...
				const filterRegex =					filterText !== ''
					&& new RegExp(`${ escapeRegExp(filterText) }`, 'i');
