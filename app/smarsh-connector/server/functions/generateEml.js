	...
	Meteor.defer(() => {
		...
		Rooms.find().forEach(async (room) => {
			...
			const data = {
				...
				time: smarshHistory
					? moment(date).diff(
						moment(smarshHistory.lastRan),
						'minutes',
					  )
					: moment(date).diff(moment(room.ts), 'minutes'),
				room: room.name
					? `#${ room.name }`
					: `Direct Message Between: ${ room.usernames.join(' & ') }`,
			};
			...
			Messages.find(query).forEach((message) => {
				...
				if (message.t) {
					...
					rows.push(
						`${ message.attachments[0].title } (${ _getLink(
					...
					_.each(
						message.attachments,
						function _loopThroughMessageAttachments(a) {
							if (a.image_url) {
