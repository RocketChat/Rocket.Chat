...
export interface ICachedSettings {
	 ...
	 */
	/* @deprecated */
	getByRegexp<T extends SettingValue = SettingValue>(
		_id: RegExp
	): [string, T][];
...
export class CachedSettings
	 ...
	 */
	/* @deprecated */
	public getByRegexp<T extends SettingValue = SettingValue>(
		_id: RegExp,
	): [string, T][] {
		if (!this.ready && warn) {
			SystemLogger.warn(`Settings not initialized yet. getting: ${ _id }`);
	...
	public changeMultiple<T extends SettingValue = SettingValue>(
		...
		const fns = _ids.map((id) =>
			this.change(
				...
				(): void => {
					callback(
						_ids.map((id) => this.store.get(id)?.value) as T[],
					);
				},
				config,
			),
	...
	public set(record: ISetting): void {
		if (
			this.store.has(record._id)
			&& this.store.get(record._id)?.value === record.value
		) {
			return;
		}
