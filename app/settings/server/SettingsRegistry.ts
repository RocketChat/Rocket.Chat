...
const getGroupDefaults = (
	_id: string,
	options: ISettingAddGroupOptions = {},
): ISettingGroup => ({
	_id,
	i18nLabel: _id,
...
const compareSettingsIgnoringKeys =	(keys: Array<keyof ISetting>) =>
	(a: ISetting, b: ISetting): boolean =>
		[...new Set([...Object.keys(a), ...Object.keys(b)])]
			...
			.every((key) =>
				isEqual(a[key as keyof ISetting], b[key as keyof ISetting]),
			);
...
export class SettingsRegistry {
	...
	constructor({
		...
		store: ICachedSettings;
		model: typeof SettingsModel;
	}) {
		this.store = store;
		this.model = model;
	}
	...
	add(
		...
		const settingFromCode = getSettingDefaults(
			{
				...
				value,
				sorter:
					sorter ?? (sorterKey?.length && this._sorter[sorterKey]++),
				group,
				section,
				...options,
			},
		...
		if (
			isSettingEnterprise(settingFromCode)
			&& !('invalidValue' in settingFromCode)
		) {
			SystemLogger.error(
				`Enterprise setting ${ _id } is missing the invalidValue option`,
			);
			throw new Error(
				`Enterprise setting ${ _id } is missing the invalidValue option`,
			);
		...
		try {
			validateSetting(
				settingFromCode._id,
				settingFromCode.type,
				settingFromCode.value,
			);
		} catch (e) {
			IS_DEVELOPMENT
				&& SystemLogger.error(
					`Invalid setting code ${ _id }: ${ (e as Error).message }`,
				);
		}
		...
		if (
			settingStored
			&& !compareSettings(settingStored, settingOverwritten)
		) {
			...
			const removedKeys = Object.keys(settingStored).filter(
				(key) =>
					!['_updatedAt'].includes(key)
					&& !overwrittenKeys.includes(key),
			);
			...
			this.model.upsert(
				{ _id },
				{
					...
					...removedKeys.length && {
						$unset: removedKeys.reduce(
							(unset, key) => ({ ...unset, [key]: 1 }),
							{},
						),
					},
				},
		...
		if (settingStored) {
			try {
				...
				IS_DEVELOPMENT
					&& SystemLogger.error(
						`Invalid setting stored ${ _id }: ${ (e as Error).message }`,
					);
			}
			return;
		}
		...
		const setting = isOverwritten
			? settingOverwritten
			: settingOverwrittenDefault;
