...
const isWithoutProjection = <T>(props: T): props is WithoutProjection<T> =>
	!('projection' in props) && !('fields' in props);
...
implements IBaseRaw<T> {
	...
	private ensureDefaultFields<P>(
		...
		return {
			...
			...fields
				&& Object.values(fields).length && { projection: fields },
			...rest,
		};
	}
	...
	insertMany(
		...
		return this.col.insertMany(
			docs as unknown as Array<OptionalId<T>>,
			options,
		);
	}
	...
	async trashFindOneById<P extends T>(
		_id: string,
		options?:
		| undefined
		| WithoutProjection<FindOneOptions<T>>
		| FindOneOptions<P extends T ? T : P>,
	): Promise<T | P | null> {
		const query = {
			_id,
			__collection__: this.name,
	...
	trashFindDeletedAfter<P extends T>(
		deletedAt: Date,
		query: FilterQuery<T> = {},
		options?: any,
	): Cursor<T | P> {
