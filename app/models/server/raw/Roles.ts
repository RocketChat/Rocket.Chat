...
export class RolesRaw extends BaseRaw<IRole> {
	constructor(
		...
		public readonly trash?: Collection<IRole>,
	) {
		super(col, trash);
	}
	...
	async addUserRoles(
		...
		if (!Array.isArray(roles)) {
			...
			process.env.NODE_ENV === 'development'
				&& console.warn(
					'[WARN] RolesRaw.addUserRoles: roles should be an array',
				);
		}
		...
		for await (const name of roles) {
			const role = await this.findOne({ name }, {
				scope: 1,
			} as FindOneOptions<IRole>);

			if (!role) {
				process.env.NODE_ENV === 'development'
					&& console.warn(
						`[WARN] RolesRaw.addUserRoles: role: ${ name } not found`,
					);
				continue;
			}
			switch (role.scope) {
				case 'Subscriptions':
					await this.models.Subscriptions.addRolesByUserId(
						userId,
						[name],
						scope,
					);
	...
	async isUserInRoles(
		...
		if (!Array.isArray(roles)) {
			...
			process.env.NODE_ENV === 'development'
				&& console.warn(
					'[WARN] RolesRaw.isUserInRoles: roles should be an array',
				);
		}
		...
		for await (const roleName of roles) {
			...
			switch (role.scope) {
				case 'Subscriptions':
					if (
						await this.models.Subscriptions.isUserInRole(
							userId,
							roleName,
							scope,
						)
				...
				default:
					if (
						await this.models.Users.isUserInRole(userId, roleName)
					) {
						return true;
					}
			}
	...
	async removeUserRoles(
		...
		if (!Array.isArray(roles)) {
			...
			process.env.NODE_ENV === 'development'
				&& console.warn(
					'[WARN] RolesRaw.removeUserRoles: roles should be an array',
				);
		}
		for await (const roleName of roles) {
			const role = await this.findOne({ name: roleName }, {
			...
			switch (role.scope) {
				case 'Subscriptions':
					scope
						&& await this.models.Subscriptions.removeRolesByUserId(
							userId,
							[roleName],
							scope,
						);
				...
				case 'Users':
				default:
					await this.models.Users.removeRolesByUserId(userId, [
						roleName,
					]);
			}
		}
	...
	async findUsersInRole<P>(
		...
		switch (role.scope) {
			case 'Subscriptions':
				return this.models.Subscriptions.findUsersInRoles(
					[name],
					scope,
					options,
				);
