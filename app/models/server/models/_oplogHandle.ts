...
class CustomOplogHandle {
	...
	async start(): Promise<CustomOplogHandle> {
		...
		const oplogUrl = this.usingChangeStream
		...
		if (
			!this.usingChangeStream
			&& (!oplogUrl || urlParsed.defaultDatabase !== 'local')
		) {
			throw Error(
						...
						case 'update':
							callback({
								...
								op: {
									...
									// o: event.fullDocument,
									o: {
										$set: event.updateDescription
											.updatedFields,
										$unset: event.updateDescription.removedFields.reduce(
											(obj, field) => {
...
export const getOplogHandle = async (): Promise<
OplogHandle | CustomOplogHandle | undefined
> => {
	if (process.env.DISABLE_DB_WATCH) {
		return;
	}
