...
export class Rooms extends Base {
	...
	getDirectConversationsByUserId(_id, options) {
		return this.findInGroup({ t: 'd', uids: { $size: 2, $in: [_id] } }, options);
	}
	...
	countDiscussions() {
		return this.findInGroup({ prid: { $exists: true } }).count();
	}

	addGroupFilter(query, groupId)	{
		if (groupId && query) {
			query.groupId = { $in: [groupId, `${ groupId }`] };
		}
	}
}
