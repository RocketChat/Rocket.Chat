...
export function messageContext({ rid } = Template.instance()) {
	...
	return {
		...
		settings: {
			...
			showMessageInMainThread: getUserPreference(
				user,
				'showMessageInMainThread',
			),
			autoImageLoad: getUserPreference(user, 'autoImageLoad'),
			enableMessageParserEarlyAdoption: getUserPreference(
				user,
				'enableMessageParserEarlyAdoption',
			),
			saveMobileBandwidth:
				Meteor.Device.isPhone()
				&& getUserPreference(user, 'saveMobileBandwidth'),
			collapseMediaByDefault: getUserPreference(
				user,
			...
			hideRoles:
				!settings.get('UI_DisplayRoles')
				|| getUserPreference(user, 'hideRoles'),
			...
			Message_GroupingPeriod:
				settings.get('Message_GroupingPeriod') * 1000,
		},
	};
}
