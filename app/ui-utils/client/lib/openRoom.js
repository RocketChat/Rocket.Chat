...
export const openRoom = async function(type, name, render = true) {
	...
	window.currentTracker = Tracker.autorun(async function(c) {
		...
		if ((user && user.username == null) || (!user && !settings.get('Accounts_AllowAnonymousRead'))) {
		...
		try {
			...
			call('setRecentlyVisitedRoom', room._id);
			...
			if (type === 'd') {
				...
				if (result) {
					return Meteor.setTimeout(() => FlowRouter.go('direct', { rid: result.rid }, FlowRouter.current().queryParams), 100);
				}
			}
