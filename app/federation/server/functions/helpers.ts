...
export const isFullyQualified = (name: string): boolean =>
	name.indexOf('@') !== -1;
...
export async function isRegisteringOrEnabled(): Promise<boolean> {
	...
	return (
		typeof value === 'string'
		&& [STATUS_ENABLED, STATUS_REGISTERING].includes(value)
	);
}
...
export const checkRoomType = (room: IRoom): boolean =>
	room.t === 'p' || room.t === 'd';
...
export const getFederatedRoomData = (
	...
	const s = Subscriptions.findByRoomIdWhenUsernameExists(
		room._id,
	).fetch() as ISubscription[];
