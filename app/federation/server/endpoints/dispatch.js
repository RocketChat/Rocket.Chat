...
import {
	contextDefinitions,
	eventTypes,
} from '../../../models/server/models/FederationEvents';
import {
...
const eventHandlers = {
	...
	async [eventTypes.GENESIS](event) {
		switch (event.data.contextType) {
			case contextDefinitions.ROOM.type:
				const eventResult = await FederationRoomEvents.addEvent(
					event.context,
					event,
				);
				...
				if (eventResult.success) {
					...
					if (persistedRoom) {
						...
						Rooms.update(
							...
							{ $set: { federation: room.federation } },
						);
					} else {
						// Denormalize room
	...
	async [eventTypes.ROOM_ADD_USER](event) {
		...
		if (eventResult.success) {
			...
			if (persistedUser) {
				...
				if (!persistedUser.federation) {
					Users.update(
						...
						{ $set: { federation: user.federation } },
					);
			...
			if (federationAltered) {
				...
				Rooms.update(
					{ _id: roomId },
					{ $set: { 'federation.domains': domainsAfterAdd } },
				);
			}
		}
	...
	async [eventTypes.ROOM_REMOVE_USER](event) {
		...
		if (eventResult.success) {
			...
			Rooms.update(
				{ _id: roomId },
				{ $set: { 'federation.domains': domainsAfterRemoval } },
			);
		}
		...
		if (eventResult.success) {
			...
			if (persistedMessage) {
				...
				if (denormalizedMessage.file) {
					...
					const { upload, buffer } = await getUpload(
						origin,
						denormalizedMessage.file._id,
					);
					...
					for (const attachment of denormalizedMessage.attachments) {
						attachment.title_link = attachment.title_link.replace(
							oldUploadId,
							upload._id,
						);
						if (/^image\/.+/.test(denormalizedMessage.file.type)) {
							attachment.image_url = attachment.image_url.replace(
								oldUploadId,
								upload._id,
							);
						} else if (
							/^audio\/.+/.test(denormalizedMessage.file.type)
						) {
							attachment.audio_url = attachment.audio_url.replace(
								oldUploadId,
								upload._id,
							);
						} else if (
							/^video\/.+/.test(denormalizedMessage.file.type)
						) {
							attachment.video_url = attachment.video_url.replace(
								oldUploadId,
								upload._id,
							);
						}
				...
				try {
					...
					serverLogger.debug(
						`Error on creating message: ${ message._id }`,
					);
				}
			}
		}
	...
	async [eventTypes.ROOM_SET_MESSAGE_REACTION](event) {
		...
		if (eventResult.success) {
			...
			Messages.update(
				{ _id: messageId },
				{ $set: { [`reactions.${ reaction }`]: reactionObj } },
			);
		}
...
API.v1.addRoute(
	...
	{
		authRequired: false,
		rateLimiterOptions: { numRequestsAllowed: 30, intervalTimeInMS: 1000 },
	},
	{
		post() {
			...
			try {
				payload = Promise.await(
					decryptIfNeeded(this.request, this.bodyParams),
				);
			} catch (err) {
