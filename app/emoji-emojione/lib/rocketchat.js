		...
		function(shortname) {
			...
			if (!ns.emojioneList[shortname]) {
				for (const emoji in ns.emojioneList) {
					if (
						!ns.emojioneList.hasOwnProperty(emoji)
						|| emoji === ''
					) {
						continue;
					}
					if (
						ns.emojioneList[emoji].shortnames.indexOf(shortname)
						=== -1
					) {
						continue;
					}
			...
			const category =				fname.indexOf('-1f3f') >= 0
				? 'diversity'
				: ns.emojioneList[shortname].category;
			...
			const ePath =				ns.defaultPathPNG !== ns.imagePathPNG
				? ns.imagePathPNG
				: `${ ns.defaultPathPNG + ns.emojiSize }/`;

			// depending on the settings, we'll either add the native unicode as the alt tag, otherwise the shortname
			const alt = ns.unicodeAlt
				? ns.convert(unicode.toUpperCase())
				: shortname;
	...
	const convertUnicode = mem(
		function(entire, m1, m2, m3) {
			...
			if (
				typeof m3 === 'undefined'
				|| m3 === ''
				|| !(ns.unescapeHTML(m3) in ns.asciiList)
			) {
			...
			const category =				unicode.indexOf('-1f3f') >= 0
				? 'diversity'
				: ns.emojioneList[shortname].category;
			const title = ns.imageTitleTag
				? `title="${ ns.escapeHTML(m3) }"`
				: '';
			// const size = ns.spriteSize === '32' || ns.spriteSize === '64' ? ns.spriteSize : '32';
			// if the emoji path has been set, we'll use the provided path, otherwise we'll use the default path
			const ePath =				ns.defaultPathPNG !== ns.imagePathPNG
...
Meteor.startup(function() {
	Tracker.autorun(function() {
		if (isSetNotNull(() => emoji.packages.emojione)) {
			if (
				isSetNotNull(() =>
					getUserPreference(Meteor.userId(), 'convertAsciiEmoji'),
				)
			) {
				emoji.packages.emojione.ascii = getUserPreference(
					Meteor.userId(),
					'convertAsciiEmoji',
				);
			} else {
