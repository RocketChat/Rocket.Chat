...
function _OAuthServicesUpdate() {
	const services = settings.getByRegexp(
		/^(Accounts_OAuth_|Accounts_OAuth_Custom-)[a-z0-9_]+$/i,
	);
	services
		...
		.forEach(([key, value]) => {
			...
			if (value === true) {
				...
				if (/Accounts_OAuth_Custom-/.test(key)) {
					...
					data.accessTokenParam = settings.get(
						`${ key }-access_token_param`,
					);
					data.buttonLabelText = settings.get(
						`${ key }-button_label_text`,
					);
					data.buttonLabelColor = settings.get(
						`${ key }-button_label_color`,
					);
					...
					data.identityTokenSentVia = settings.get(
						`${ key }-identity_token_sent_via`,
					);
					...
					data.channelsMap = settings.get(
						`${ key }-groups_channel_map`,
					);
...
const OAuthServicesUpdate = _.debounce(
	Meteor.bindEnvironment(_OAuthServicesUpdate),
	2000,
);
		...
		if (/Accounts_OAuth_Custom_[a-zA-Z0-9_-]+$/.test(key)) {
			...
			if (key === serviceKey) {
				const values = {
					...
					accessTokenParam:
						process.env[`${ serviceKey }_access_token_param`],
					buttonLabelText:
						process.env[`${ serviceKey }_button_label_text`],
					buttonLabelColor:
						process.env[`${ serviceKey }_button_label_color`],
					...
					identityTokenSentVia:
						process.env[`${ serviceKey }_identity_token_sent_via`],
					...
					channelsMap:
						process.env[`${ serviceKey }_groups_channel_map`],
					channelsAdmin: process.env[`${ serviceKey }_channels_admin`],
					mergeUsers:
						process.env[`${ serviceKey }_merge_users`] === 'true',
					mapChannels: process.env[`${ serviceKey }_map_channels`],
					mergeRoles:
						process.env[`${ serviceKey }_merge_roles`] === 'true',
