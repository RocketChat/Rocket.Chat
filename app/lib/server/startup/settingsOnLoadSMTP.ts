	...
	function() {
		...
		if (settings.get('SMTP_Host')) {
			...
			if (
				...
				process.env.MAIL_URL += `${ encodeURIComponent(
					settings.get('SMTP_Username'),
				) }:${ encodeURIComponent(settings.get('SMTP_Password')) }@`;
			}
			...
			if (
				settings.get('SMTP_Protocol') === 'smtp'
				&& settings.get('SMTP_IgnoreTLS')
			) {
