...
callbacks.add(
	...
	function(message) {
		...
		if (
			message.editedAt
			|| (message.ts && Math.abs(moment(message.ts).diff()) > 60000)
		) {
			return message;
		}
		...
		if (
			...
			if (
				!hasPermission(message.u._id, 'mention-here')
				&& !hasPermission(message.u._id, 'mention-here', message.rid)
			) {
				// Get the language of the user for the error notification.
				const { language } = Users.findOneById(message.u._id);
				const action = TAPi18n.__(
				...
				throw new Meteor.Error(
					'error-action-not-allowed',
					'Notify here in this room not allowed',
					{
