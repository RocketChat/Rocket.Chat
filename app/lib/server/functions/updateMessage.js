...
export const updateMessage = function(message, user, originalMessage) {
	...
	if (message && Apps && Apps.isLoaded()) {
		...
		const prevent = Promise.await(
			Apps.getBridges()
				.getListenerBridge()
				.messageEvent('IPreMessageUpdatedPrevent', appMessage),
		);
		if (prevent) {
		...
		result = Promise.await(
			Apps.getBridges()
				.getListenerBridge()
				.messageEvent('IPreMessageUpdatedExtend', appMessage),
		);
		result = Promise.await(
			Apps.getBridges()
				.getListenerBridge()
				.messageEvent('IPreMessageUpdatedModify', result),
		);
	...
	if (Apps && Apps.isLoaded()) {
		...
		Apps.getBridges()
			.getListenerBridge()
			.messageEvent('IPostMessageUpdated', message);
	}

	Meteor.defer(function() {
		callbacks.run(
			'afterSaveMessage',
			Messages.findOneById(tempid),
			room,
			user._id,
		);
	});
