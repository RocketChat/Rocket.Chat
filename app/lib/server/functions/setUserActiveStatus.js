...
function reactivateDirectConversations(userId) {
	...
	const userIds = directConversations.reduce(
		(acc, r) => acc.push(...r.uids) && acc,
		[],
	);
...
export function setUserActiveStatus(userId, active, confirmRelinquish = false) {
	...
	if (user.username != null && !active) {
		...
		const chatSubscribedRooms = subscribedRooms.filter(
			({ t }) => t !== 'l',
		);
		const livechatSubscribedRooms = subscribedRooms.filter(
			({ t }) => t === 'l',
		);

		if (
			shouldRemoveOrChangeOwner(chatSubscribedRooms)
			&& !confirmRelinquish
		) {
	...
	const destinations =		Array.isArray(user.emails)
		&& user.emails.map(
			(email) => `${ user.name || user.username }<${ email.address }>`,
		);
