...
const ValidPartialURLParam = Match.Where((value) => {
	...
	if (
		!isRelativeURL(value)
		&& !isURL(value)
		&& !value.startsWith(FileUpload.getPath())
	) {
		throw new Error('Invalid href value provided');
	}
...
const validateMessage = (message, room, user) => {
	...
	if (message.alias || message.avatar) {
		...
		if (
			!isLiveChatGuest
			&& !hasPermission(user._id, 'message-impersonate', room._id)
		) {
...
export const sendMessage = function(user, message, room, upsert = false) {
	...
	if (message) {
		if (message._id && upsert) {
			...
			const messageAlreadyExists =				message._id
				&& Messages.findOneById(message._id, { fields: { _id: 1 } });
