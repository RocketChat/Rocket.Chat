...
function getEmailContent({ message, user, room }) {
	...
	const header = TAPi18n.__(room.t === 'd' && room.usersCount === 2 ? 'User_sent_a_message_to_you' : 'User_sent_a_message_on_channel', {
		username: userName,
		channel: roomName,
		lng,
	});
	...
	if (message.file) {
		const fileHeader = TAPi18n.__(room.t === 'd' && room.usersCount === 2 ? 'User_uploaded_a_file_to_you' : 'User_uploaded_a_file_on_channel', {
			username: userName,
			channel: roomName,
			lng,
		});
