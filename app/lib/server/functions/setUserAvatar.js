...
export const setUserAvatar = function(
	...
	if (service === 'url') {
		...
		try {
			...
			if (!result) {
				SystemLogger.info(
					`Not a valid response, from the avatar url: ${ encodeURI(
						dataURI,
					) }`,
				);
			...
			if (!error.response || error.response.statusCode !== 404) {
				SystemLogger.info(
					`Error while handling the setting of the avatar from a url (${ encodeURI(
						dataURI,
					) }) for ${ user.username }:`,
					error,
				);
				throw new Meteor.Error(
					...
					`Error while handling avatar setting from a URL (${ encodeURI(
						dataURI,
					) }) for ${ user.username }`,
					{
		...
		if (!/image\/.+/.test(result.headers['content-type'])) {
			SystemLogger.info(
				`Not a valid content-type from the provided url, ${ result.headers['content-type'] }, from the avatar url: ${ dataURI }`,
			);
			throw new Meteor.Error(
				'error-avatar-invalid-url',
				`Invalid avatar URL: ${ dataURI }`,
