...
export function getMentions(message) {
	...
	const userMentions = mentions.filter(
		(mention) => !mention.type || mention.type === 'user',
	);
...
const incGroupMentions = (rid, roomType, excludeUserId, unreadCount) => {
	...
	Subscriptions.incGroupMentionsAndUnreadForRoomIdExcludingUserId(
		rid,
		excludeUserId,
		1,
		incUnread,
...
const incUserMentions = (rid, roomType, uids, unreadCount) => {
	...
	Subscriptions.incUserMentionsAndUnreadForRoomIdAndUserIds(
		rid,
		uids,
		1,
		incUnread,
...
const getUserIdsFromHighlights = (rid, message) => {
	...
	const subs = Subscriptions.findByRoomWithUserHighlights(
		rid,
		highlightOptions,
	).fetch();
...
export function updateUsersSubscriptions(message, room) {
	...
	if (room != null && !message.tmid) {
		...
		getUserIdsFromHighlights(room._id, message).forEach((uid) =>
			userIds.add(uid),
		);
		...
		if (!toAll && !toHere && unreadCount === 'all_messages') {
			Subscriptions.incUnreadForRoomIdExcludingUserIds(room._id, [
				...userIds,
				message.u._id,
			]);
		}
...
export function updateThreadUsersSubscriptions(message, room, replies) {
	...
	Subscriptions.setLastReplyForRoomIdAndUserIds(
		message.rid,
		repliesPlusSender,
		new Date(),
	);
