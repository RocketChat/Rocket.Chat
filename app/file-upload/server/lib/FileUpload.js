...
const UserDataFilesModel = new Mongo.Collection(
	UserDataFiles.col.collectionName,
);
...
export const FileUpload = {
	...
	requestCanAccessFiles({ headers = {}, query = {} }) {
		...
		const isAuthorizedByCookies =			rc_uid
			&& rc_token
	...
	getStoreByName(handlerName) {
		if (this.handlers[handlerName] == null) {
			SystemLogger.error(
				`Upload handler "${ handlerName }" does not exists`,
			);
		}
	...
	getBufferSync: Meteor.wrapAsync((file, cb) =>
		FileUpload.getBuffer(file, cb),
	),
