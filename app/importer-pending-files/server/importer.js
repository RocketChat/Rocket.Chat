...
export class PendingFileImporter extends Base {
	...
	startImport() {
		...
		const waitForFiles = () => {
			if (
				count + 1 < maxFileCount
				&& currentSize + nextSize < maxFileSize
			) {
				return;
			}
										...
										fileStore._doInsert(
											...
											function(error, file) {
												...
												if (
													...
													attachment.image_dimensions =														file.identify != null
														? file.identify.size
														: undefined;
												}
												...
												Messages.setImportFileRocketChatAttachment(
													_importFile.id,
													url,
													attachment,
												);
