...
WebApp.rawConnectHandlers.use(function(req, res, next) {
	...
	if (settings.get('API_Enable_CORS')) {
		...
		res.setHeader('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept, X-User-Id, X-Auth-Token, x-visitor-token, Authorization');
		...
		const allowedOrigin = CORSOriginSetting
			.trim()
			.split(',')
			.map((origin) => String(origin).trim().toLocaleLowerCase())
			.find((origin) => req.headers.origin === origin);
