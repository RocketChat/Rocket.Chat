...
export const notifyFailedLogin = async (
	...
	const channelToNotify = settings.get(
		'Block_Multiple_Failed_Logins_Notify_Failed_Channel',
	);
	if (!channelToNotify) {
		logger.error(
			'Cannot notify failed logins: channel provided is invalid',
		);
		return;
	}
	...
	if (!room) {
		logger.error(
			"Cannot notify failed logins: channel provided doesn't exists",
		);
		return;
	}
...
export const isValidLoginAttemptByIp = async (ip: string): Promise<boolean> => {
	const whitelist = String(
		settings.get('Block_Multiple_Failed_Logins_Ip_Whitelist'),
	).split(',');
	...
	const lastAttemptAt = (await ServerEvents.findLastFailedAttemptByIp(ip))
		?.ts;
	...
	const willBeBlockedUntil = addMinutesToADate(
	...
	if (
		settings.get('Block_Multiple_Failed_Logins_Notify_Failed')
		&& !isValid
	) {
...
export const isValidAttemptByUser = async (
	login: ILoginAttempt,
): Promise<boolean> => {
	if (
		!settings.get('Block_Multiple_Failed_Logins_Enabled')
	...
	const lastAttemptAt = (
		await ServerEvents.findLastFailedAttemptByUsername(
...
export const saveFailedLoginAttempts = async (
	login: ILoginAttempt,
): Promise<void> => {
	const user: IServerEvent['u'] = {
		_id: login.user?._id,
		username:
			login.user?.username || login.methodArguments[0].user?.username,
	};
...
export const saveSuccessfulLogin = async (
	login: ILoginAttempt,
): Promise<void> => {
	const user: IServerEvent['u'] = {
		_id: login.user?._id,
