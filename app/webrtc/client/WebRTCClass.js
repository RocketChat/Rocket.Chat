...
class WebRTCTransportClass extends Emitter {
	...
	sendStatus(data) {
		this.log(
			'WebRTCTransportClass - sendStatus',
			data,
			this.webrtcInstance.room,
		);
		data.from = this.webrtcInstance.selfId;
...
class WebRTCClass {
   ...
   */
	...
	constructor(selfId, room, autoAccept = false) {
		...
		this.screenShareAvailable = ['chrome', 'firefox', 'electron'].includes(
			this.navigator,
		);
		this.media = {
			video: true,
   ...
   */
	...
	onRemoteStatus(data) {
		...
		this.callInProgressTimeout = Meteor.setTimeout(
   ...
   */
	...
	getUserMedia(media, onSuccess, onError = this.onError) {
		...
		const getScreen = (audioStream) => {
			...
			const isFirefoxExtensionInstalled =				this.navigator === 'firefox'
				&& window.rocketchatscreenshare != null;
				...
				modal.open(
					{
						...
						text: TAPi18n.__(
							'You_need_install_an_extension_to_allow_screen_sharing',
						),
						html: true,
   ...
   */
	...
	onRemoteOffer(data, ...args) {
		...
		if (
			['have-local-offer', 'stable'].includes(
