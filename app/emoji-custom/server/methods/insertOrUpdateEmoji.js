	...
	async insertOrUpdateEmoji(emojiData) {
		...
		if (!s.trim(emojiData.name)) {
			throw new Meteor.Error(
				...
				{ method: 'insertOrUpdateEmoji', field: 'Name' },
			);
		}
		...
		if (nameValidation.test(emojiData.name)) {
			throw new Meteor.Error(
				...
				`${ emojiData.name } is not a valid name`,
				{
		...
		if (emojiData.aliases) {
			if (aliasValidation.test(emojiData.aliases)) {
				throw new Meteor.Error(
					'error-input-is-not-a-valid-field',
					`${ emojiData.aliases } is not a valid alias set`,
					{
						method: 'insertOrUpdateEmoji',
		...
		if (emojiData._id) {
			matchingResults = await EmojiCustom.findByNameOrAliasExceptID(
				emojiData.name,
				emojiData._id,
			).toArray();
			...
			matchingResults = await EmojiCustom.findByNameOrAlias(
				emojiData.name,
			).toArray();
			for await (const alias of emojiData.aliases) {
				matchingResults = matchingResults.concat(
					await EmojiCustom.findByNameOrAlias(alias).toArray(),
				);
			}
		}
		...
		if (typeof emojiData.extension === 'undefined') {
			throw new Meteor.Error(
				'error-the-field-is-required',
				'The custom emoji file is required',
				{ method: 'insertOrUpdateEmoji' },
			);
		}
		...
		if (emojiData.newFile) {
			...
			RocketChatFileEmojiCustomInstance.deleteFile(
				encodeURIComponent(
					`${ emojiData.name }.${ emojiData.previousExtension }`,
				),
			);
			RocketChatFileEmojiCustomInstance.deleteFile(
				encodeURIComponent(
					`${ emojiData.previousName }.${ emojiData.extension }`,
				),
			);
			RocketChatFileEmojiCustomInstance.deleteFile(
				encodeURIComponent(
					`${ emojiData.previousName }.${ emojiData.previousExtension }`,
				),
			);
			...
			if (rs !== null) {
				RocketChatFileEmojiCustomInstance.deleteFile(
					encodeURIComponent(
						`${ emojiData.name }.${ emojiData.extension }`,
					),
				);
