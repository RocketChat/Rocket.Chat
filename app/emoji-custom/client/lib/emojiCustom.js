...
export const getEmojiUrlFromName = function(name, extension) {
	...
	return `${ path }/emoji-custom/${ encodeURIComponent(
		name,
	) }.${ extension }?_dc=${ random }`;
};
...
export const deleteEmojiCustom = function(emojiData) {
	...
	if (arrayIndex !== -1) {
		emoji.packages.emojiCustom.emojisByCategory.rocket.splice(
			arrayIndex,
			1,
		);
	}
	const arrayIndexList = emoji.packages.emojiCustom.list.indexOf(
		`:${ emojiData.name }:`,
	);
	if (arrayIndexList !== -1) {
	...
	if (isSetNotNull(() => emojiData.aliases)) {
		for (const alias of emojiData.aliases) {
			...
			const aliasIndex = emoji.packages.emojiCustom.list.indexOf(
				`:${ alias }:`,
			);
			if (aliasIndex !== -1) {
...
export const updateEmojiCustom = function(emojiData) {
	...
	if (
		previousExists
		&& isSetNotNull(() => emoji.list[`:${ emojiData.previousName }:`].aliases)
	) {
	...
	if (categoryIndex === -1) {
		emoji.packages.emojiCustom.emojisByCategory.rocket.push(
			`${ emojiData.name }`,
		);
	...
	if (previousExists && emojiData.name !== emojiData.previousName) {
		$(document)
			.find(
				`.emojiAdminPreview-image[data-emoji='${ emojiData.previousName }']`,
			)
...
const customRender = (html) => {
	const emojisMatchGroup = emoji.packages.emojiCustom.list
		.map(escapeRegExp)
		.join('|');
