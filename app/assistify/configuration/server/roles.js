...
const assignPermissions = function(role, permissions) {
	permissions.forEach((permission) => Permissions.addRole(permission, role));
};

const revokePermissions = function(role, permissions) {
	permissions.forEach((permission) => Permissions.removeRole(permission, role));
};

const createConfigurationRole = function() {
	Roles.createOrUpdate(CONFIGURATION_ROLE_NAME, 'Users', CONFIGURATION_ROLE_NAME);
...
const createManagerRole = function() {
	Roles.createOrUpdate(MANAGER_ROLE_NAME, 'Users', MANAGER_ROLE_NAME);
