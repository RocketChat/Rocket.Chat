...
class IrcClient {
	...
	initRoomList() {
		const roomIds = RocketChat.models.Subscriptions.findByUserIdAndType(this.user._id, 'c', {fields: {rid: 1}}).fetch().map(s => s.rid);
		const roomsCursor = RocketChat.models.Rooms.findByIds(roomIds, { fields: { name: 1, t: 1 }});
