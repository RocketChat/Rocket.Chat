...
export type AuthorizationContextValue = {
  queryPermission(permission: string, scope?: string): Subscription<boolean>;
  ...
  queryRole(role: string): Subscription<boolean>;
  roleStore: RoleStore;
};
...
export const usePermission = (permission: string, scope?: string): boolean => {
...
export const useRolesDescription = (): ((ids: Array<string>) => [string]) => {
  ...
  return useCallback(
    (values) =>
      values.map(
        (role) => roles[role]?.description || roles[role]?.name || role
      ),
    [roles]
