...
const hasSubarray = (a: unknown): a is Pick<TypedArray, 'subarray'> =>
	typeof a === 'object' && a !== null && 'subarray' in a && typeof a.subarray === 'function';

const hasSet = (a: unknown): a is Pick<TypedArray, 'set'> =>
	typeof a === 'object' && a !== null && 'set' in a && typeof a.set === 'function';

export function copyArray<T>(src: ArrayLike<T>, srcPos: number, dest: { [index: number]: T }, destPos: number, length: number) {
...
export function fillArray(a: TypedArray, fromIndex: number, toIndex: number, val: number): void;
export function fillArray(...args: [a: TypedArray, val: number] | [a: TypedArray, fromIndex: number, toIndex: number, val: number]) {
	if (args.length === 2) {
		const [a, val] = args;
