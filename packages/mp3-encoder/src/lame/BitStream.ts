...
export class BitStream {
	...
	getframebits(gfp: LameGlobalFlags) {
		...
		const bytes = Math.trunc(((gfp.version + 1) * 72000 * bit_rate) / gfp.out_samplerate + gfc.padding);
		return 8 * bytes;
	}
	...
	private encodeSideInfo2(gfp: LameGlobalFlags, bitsPerFrame: number) {
		...
		if (gfp.version === 1) {
			...
			if (gfc.channels_out === 2) this.writeheader(gfc, l3_side.private_bits, 3);
	...
	private huffmancode(gfc: LameInternalFlags, tableindex: number, start: number, end: number, gi: GrInfo) {
	...
	private compute_flushbits(gfp: LameGlobalFlags, total_bytes_output: TotalBytes) {
		const gfc = gfp.internal_flags;
		let flushbits;
	...
	flush_bitstream(gfp: LameGlobalFlags) {
		...
		if (gfc.findReplayGain) {
			...
			assert(!isCloseToEachOther(radioGain, GainAnalysis.GAIN_NOT_ENOUGH_SAMPLES));
		...
		if (gfc.findPeakSample) {
			...
			if (gfc.noclipGainChange > 0) {
				if (isCloseToEachOther(gfp.scale, 1.0) || isCloseToEachOther(gfp.scale, 0.0)) {
	...
	copyMetadata(gfc: LameInternalFlags, buffer: Uint8Array, bufferPos: number, size: number) {
		return this.copyBuffer(gfc, buffer, bufferPos, size, false);
	}

	copyFrameData(gfc: LameInternalFlags, buffer: Uint8Array, bufferPos: number, size: number) {
		return this.copyBuffer(gfc, buffer, bufferPos, size, true);
	}

	private copyBuffer(gfc: LameInternalFlags, buffer: Uint8Array, bufferPos: number, size: number, mp3data: boolean) {
