...
export class GainAnalysis {
	...
	static readonly MAX_SAMPLES_PER_WINDOW =
		(GainAnalysis.MAX_SAMP_FREQ * GainAnalysis.RMS_WINDOW_TIME_NUMERATOR) / GainAnalysis.RMS_WINDOW_TIME_DENOMINATOR + 1;
	...
	private ABYule = [
		[
			0.038575994352, -3.84664617118067, -0.02160367184185, 7.81501653005538, -0.00123395316851, -11.34170355132042, -0.00009291677959,
			13.05504219327545, -0.01655260341619, -12.28759895145294, 0.02161526843274, 9.4829380631979, -0.02074045215285, -5.87257861775999,
			0.00594298065125, 2.75465861874613, 0.00306428023191, -0.86984376593551, 0.00012025322027, 0.13919314567432, 0.00288463683916,
		],
		[
		...
		[
			...
			0.06276101321749, 0.04500235387352, -0.00000828086748, 0.02005851806501, 0.00205861885564, 0.00302439095741, -0.02950134983287,
		],
		[
	...
	private filterButter(input: Float32Array, inputPos: number, output: Float32Array, outputPos: number, nSamples: number, kernel: number[]) {
		while (nSamples-- !== 0) {
			output[outputPos] =
				input[inputPos + 0] * kernel[0] -
				output[outputPos - 1] * kernel[1] +
	...
	private resetSampleFrequency(rgData: ReplayGain, samplefreq: number) {
		...
		rgData.sampleWindow = Math.trunc(
			(samplefreq * GainAnalysis.RMS_WINDOW_TIME_NUMERATOR + GainAnalysis.RMS_WINDOW_TIME_DENOMINATOR - 1) /
	...
	analyzeSamples(
		...
		while (batchsamples > 0) {
			...
			if (cursamplepos < GainAnalysis.MAX_ORDER) {
				...
				if (cursamples > GainAnalysis.MAX_ORDER - cursamplepos) cursamples = GainAnalysis.MAX_ORDER - cursamplepos;
			} else {
			...
			if (rgData.totsamp === rgData.sampleWindow) {
				const val = GainAnalysis.STEPS_per_dB * 10 * Math.log10(((rgData.lsum + rgData.rsum) / rgData.totsamp) * 0.5 + 1e-37);
