...
export class FFT {
	...
	private readonly costab = [
		9.238795325112867e-1, 3.826834323650898e-1, 9.951847266721969e-1, 9.80171403295606e-2, 9.996988186962042e-1, 2.454122852291229e-2,
	...
	fft_short(_gfc: LameInternalFlags, x_real: Float32Array[], chn: number, buffer: Float32Array[], bufPos: number) {
		for (let b = 0; b < 3; b++) {
			let x = BLKSIZE_s / 2;
	...
	fft_long(_gfc: LameInternalFlags, y: Float32Array, chn: number, buffer: Float32Array[], bufPos: number) {
		let jj = BLKSIZE / 8 - 1;
		let x = BLKSIZE / 2;
	...
	init() {
		for (let i = 0; i < BLKSIZE; i++) {
			this.window[i] = 0.42 - 0.5 * Math.cos((2 * Math.PI * (i + 0.5)) / BLKSIZE) + 0.08 * Math.cos((4 * Math.PI * (i + 0.5)) / BLKSIZE);
		}

		for (let i = 0; i < BLKSIZE_s / 2; i++) {
			this.window_s[i] = 0.5 * (1.0 - Math.cos((2.0 * Math.PI * (i + 0.5)) / BLKSIZE_s));
		}
	}
}
