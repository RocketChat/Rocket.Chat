...
import { BPC, CBANDS, ENCDELAY, MAX_HEADER_BUF, MDCTDELAY, MFSIZE, POSTDELAY, SBLIMIT, SBMAX_l, SBMAX_s, SFBMAX } from './constants';
...
export class LameInternalFlags {
	...
	readonly blackfilt = Array.from({ length: 2 * BPC + 1 }, () => new Float32Array());
	...
	readonly sb_sample = Array.from({ length: 2 }, () =>
		Array.from({ length: 2 }, () => Array.from({ length: 18 }, () => new Float32Array(SBLIMIT))),
	);
	...
	readonly bitrate_stereoMode_Hist = Array.from({ length: 16 }, () => new Int32Array(4 + 1));

	readonly bitrate_blockType_Hist = Array.from({ length: 16 }, () => new Int32Array(4 + 1 + 1));
