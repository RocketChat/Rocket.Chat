...
import { BLKSIZE, BPC, ENCDELAY, FFTOFFSET, LAME_ID, MFSIZE, MPG_MD_MS_LR, POSTDELAY, PSFB12, PSFB21, SBMAX_l, SBMAX_s } from './constants';
...
export class Lame {
	...
	private lame_init_params(gfp: LameGlobalFlags): void {
		...
		if (gfp.VBR === VbrMode.vbr_off && gfp.compression_ratio > 0) {
			...
			gfp.brate = Math.trunc((gfp.out_samplerate * 16 * gfc.channels_out) / (1e3 * gfp.compression_ratio));
		...
		if (gfp.lowpassfreq === 0) {
			...
			switch (gfp.VBR) {
				...
				case VbrMode.vbr_rh: {
					const x = [19500, 19000, 18600, 18000, 17500, 16000, 15600, 14900, 12500, 10000, 3950];
				...
				default: {
					const x = [19500, 19000, 18500, 18000, 17500, 16500, 15500, 14500, 12500, 9500, 3950];
					if (gfp.VBR_q >= 0 && gfp.VBR_q <= 9) {
						lowpass = x[gfp.VBR_q];
					} else {
						lowpass = 19500;
		...
		if (gfc.findReplayGain) {
			if (this.bs.ga.initGainAnalysis(gfc.rgdata, gfp.out_samplerate) === GainAnalysis.INIT_GAIN_ANALYSIS_ERROR) {
		...
		for (let i = 0; i < SBMAX_l + 1; i++) gfc.scalefac_band.l[i] = this.sfBandIndex[j].l[i];
		...
		gfc.frac_SpF = 0;
		if (gfp.VBR === VbrMode.vbr_off) {
			gfc.frac_SpF = ((gfp.version + 1) * 72000 * gfp.brate) % Math.trunc(gfp.out_samplerate);
	...
	private optimum_samplefreq(lowpassfreq: number, input_samplefreq: number): SampleRate {
	...
	lame_encode_flush(gfp: LameGlobalFlags, mp3buffer: Uint8Array, mp3bufferPos: number, mp3buffer_size: number) {
		const gfc = gfp.internal_flags;
	...
	lame_encode_buffer(
		...
		if (gfc.in_buffer_0 === null || gfc.in_buffer_1 === null || gfc.in_buffer_nsamples < nsamples) {
	...
	private lame_encode_buffer_sample(
		...
		if (!isCloseToEachOther(gfp.scale, 0) && !isCloseToEachOther(gfp.scale, 1.0)) {
			for (let i = 0; i < nsamples; ++i) {
		...
		let in_bufferPos = 0;
		while (nsamples > 0) {
			const in_buffer_ptr: [Float32Array, Float32Array] = [in_buffer[0], in_buffer[1]];
			...
			if (gfc.findReplayGain)
				if (
					this.bs.ga.analyzeSamples(gfc.rgdata, mfbuf[0], gfc.mf_size, mfbuf[1], gfc.mf_size, n_out, gfc.channels_out) ===
			...
			if (gfc.mf_size >= mf_needed) {
				...
				const ret = this.lame_encode_frame(gfp, mfbuf[0], mfbuf[1], mp3buf, mp3bufPos, buf_size);
	...
	private static fill_buffer_resample(
		...
		const intratio = Math.abs(gfc.resample_ratio - Math.floor(0.5 + gfc.resample_ratio)) < 0.0001 ? 1 : 0;
