...
export class ABRPresets {
	...
	private findNearestFullBitrateIndex(bitrate: number): FullBitrateIndex {
		...
		for (let maybeLowerRange = 0; maybeLowerRange < this.fullBitrates.length - 1; maybeLowerRange++) {
		...
		return bitrate - this.fullBitrates[lowerRange] < this.fullBitrates[upperRange] - bitrate
