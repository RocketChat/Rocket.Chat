...
export class Quantize {
	...
	private init_xrpow_core(cod_info: GrInfo, xrpow: Float32Array, upper: number, sum: number) {
		sum = 0;
		...
		if (cod_info.block_type !== SHORT_TYPE) {
			...
			for (let block = 0; block < 3; block++) {
				...
				for (let gsfb = PSFB12 - 1; gsfb >= 0 && !stop; gsfb--) {
					...
					const end = start + (gfc.scalefac_band.psfb12[gsfb + 1] - gfc.scalefac_band.psfb12[gsfb]);
	...
	init_outer_loop(gfc: LameInternalFlags, cod_info: GrInfo) {
		...
		if (cod_info.block_type === SHORT_TYPE) {
			...
			cod_info.psymax = cod_info.sfb_lmax + 3 * ((gfc.sfb21_extra ? SBMAX_s : SBPSY_s) - cod_info.sfb_smin);
	...
	private bin_search_StepSize(gfc: LameInternalFlags, cod_info: GrInfo, desired_rate: number, ch: number, xrpow: Float32Array) {
	...
	trancate_smallspectrums(gfc: LameInternalFlags, gi: GrInfo, l3_xmin: Float32Array, work: Float32Array) {
		const distort = new Float32Array(SFBMAX);

		if (((gfc.substep_shaping & 4) === 0 && gi.block_type === SHORT_TYPE) || (gfc.substep_shaping & 0x80) !== 0) return;
		...
		do {
			...
			do {
				for (nsame = 1; start + nsame < width; nsame++)
					if (!isCloseToEachOther(work[start + j - width], work[start + j + nsame - width])) break;
	...
	private get_klemm_noise(distort: Float32Array, gi: GrInfo) {
		...
		for (let sfb = 0; sfb < gi.psymax; sfb++) klemm_noise += this.penalties(distort[sfb]);

		return Math.max(1e-20, klemm_noise);
	}

	// eslint-disable-next-line complexity
	private quant_compare(quant_comp: number, best: CalcNoiseResult, calc: CalcNoiseResult, gi: GrInfo, distort: Float32Array) {
		let better;
		...
		switch (quant_comp) {
			...
			case 0:
				better =
					...
					(calc.over_count === best.over_count && isCloseToEachOther(calc.over_noise, best.over_noise) && calc.tot_noise < best.tot_noise);
				break;
			...
			case 5:
				better =
					calc.over_noise < best.over_noise || (isCloseToEachOther(calc.over_noise, best.over_noise) && calc.tot_noise < best.tot_noise);
				break;
			case 6:
				better =
					calc.over_noise < best.over_noise ||
					(isCloseToEachOther(calc.over_noise, best.over_noise) &&
						(calc.max_noise < best.max_noise || (isCloseToEachOther(calc.max_noise, best.max_noise) && calc.tot_noise <= best.tot_noise)));
				break;
			case 7:
	...
	private amp_scalefac_bands(gfp: LameGlobalFlags, cod_info: GrInfo, distort: Float32Array, xrpow: Float32Array, bRefine: boolean) {
	...
	private inc_subblock_gain(gfc: LameInternalFlags, cod_info: GrInfo, xrpow: Float32Array) {
		let sfb;
	...
	private balance_noise(gfp: LameGlobalFlags, cod_info: GrInfo, distort: Float32Array, xrpow: Float32Array, bRefine: boolean) {
		const gfc = gfp.internal_flags;
	...
	// eslint-disable-next-line complexity
	outer_loop(gfp: LameGlobalFlags, cod_info: GrInfo, l3_xmin: Float32Array, xrpow: Float32Array, ch: number, targ_bits: number) {
		const gfc = gfp.internal_flags;
		...
		while (!bEndOfSearch) {
			do {
				...
				if (gfc.sfb21_extra) {
					...
					if (cod_info_w.block_type === SHORT_TYPE && (distort[cod_info_w.sfbmax + 1] > 1.0 || distort[cod_info_w.sfbmax + 2] > 1.0)) break;
				}

				if (!this.balance_noise(gfp, cod_info_w, distort, xrpow, bRefine)) break;
				...
				while (
					(cod_info_w.part2_3_length = this.tak.count_bits(gfc, xrpow, cod_info_w, prev_noise)) > huff_bits &&
				...
				if (best_noise_info.over_count === 0) {
					while (
						(cod_info_w.part2_3_length = this.tak.count_bits(gfc, xrpow, cod_info_w, prev_noise)) > best_part2_3_length &&
						cod_info_w.global_gain <= maxggain
					)
						cod_info_w.global_gain++;
				...
				better = this.quant_compare(better, best_noise_info, noise_info, cod_info_w, distort) ? 1 : 0;
	...
	private calc_noise_core(cod_info: GrInfo, startline: StartLine, l: number, step: number) {
		let noise = 0;
		let j = startline.s;
	...
	private calc_noise(
		...
		for (sfb = 0; sfb < cod_info.psymax; sfb++) {
			const s =
				cod_info.global_gain -
				((scalefac[scalefacPos++] + (cod_info.preflag !== 0 ? this.qupvt.pretab[sfb] : 0)) << (cod_info.scalefac_scale + 1)) -
