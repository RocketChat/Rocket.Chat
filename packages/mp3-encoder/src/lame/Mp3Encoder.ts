...
export class Mp3Encoder {
	...
	encodeBuffer(left: Int16Array, right: Int16Array = left) {
		...
		const size = this.lame.lame_encode_buffer(this.gfp, left, right, left.length, this.mp3buf, 0, this.mp3buf_size);
		return new Uint8Array(this.mp3buf.subarray(0, size));
	}

	flush() {
		const size = this.lame.lame_encode_flush(this.gfp, this.mp3buf, 0, this.mp3buf_size);
		return new Uint8Array(this.mp3buf.subarray(0, size));
	}
}
