...
export class Reservoir {
	...
	ResvFrameBegin(gfp: LameGlobalFlags, mean_bits: MeanBits) {
		...
		if (gfp.brate > 320) {
			maxmp3buf = 8 * Math.trunc((gfp.brate * 1000) / (gfp.out_samplerate / 1152) / 8 + 0.5);
		} else {
			maxmp3buf = 8 * 1440;
		}

		let fullFrameBits = mean_bits.bits * gfc.mode_gr + Math.min(gfc.ResvSize, 0);
	...
	ResvMaxBits(gfp: LameGlobalFlags, mean_bits: number, targ_bits: MeanBits, cbr: number) {
		const gfc = gfp.internal_flags;
		let add_bits;
		let { ResvSize } = gfc;
