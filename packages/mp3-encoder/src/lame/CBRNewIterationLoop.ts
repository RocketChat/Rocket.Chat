...
import { MAX_BITS_PER_CHANNEL, MAX_BITS_PER_GRANULE, MPG_MD_MS_LR, SFBMAX, SHORT_TYPE } from './constants';

export class CBRNewIterationLoop {
	constructor(public quantize: Quantize) {}

	iteration_loop(gfp: LameGlobalFlags, pe: number[][], ms_ener_ratio: number[], ratio: III_psy_ratio[][]) {
	...
	private on_pe(gfp: LameGlobalFlags, pe: number[][], targ_bits: Int32Array, mean_bits: number, gr: number, cbr: number) {
		const gfc = gfp.internal_flags;
		let tbits = 0;
		let bits;
		...
		for (bits = 0, ch = 0; ch < gfc.channels_out; ++ch) {
			...
			if (add_bits[ch] + targ_bits[ch] > MAX_BITS_PER_CHANNEL) add_bits[ch] = Math.max(0, MAX_BITS_PER_CHANNEL - targ_bits[ch]);
