...
export class PsyModel {
	...
	private readonly tab = [1.0, 0.79433, 0.63096, 0.63096, 0.63096, 0.63096, 0.63096, 0.25119, 0.11749] as const;
	...
	private mask_add(m1: number, m2: number, kk: number, b: number, gfc: LameInternalFlags, shortblock: number) {
		let ratio;
	...
	private msfix1(gfc: LameInternalFlags) {
		...
		for (let sb = 0; sb < SBMAX_s; sb++) {
			for (let sblock = 0; sblock < 3; sblock++) {
				...
				const rmid = Math.max(gfc.thm[2].s[sb][sblock], Math.min(gfc.thm[3].s[sb][sblock], mld));

				mld = gfc.mld_s[sb] * gfc.en[2].s[sb][sblock];
				const rside = Math.max(gfc.thm[3].s[sb][sblock], Math.min(gfc.thm[2].s[sb][sblock], mld));
	...
	private ns_msfix(gfc: LameInternalFlags, msfix: number, athadjust: number) {
		...
		for (let sb = 0; sb < SBMAX_l; sb++) {
			...
			const thmLR = Math.min(Math.max(gfc.thm[0].l[sb], ath), Math.max(gfc.thm[1].l[sb], ath));
		...
		for (let sb = 0; sb < SBMAX_s; sb++) {
			for (let sblock = 0; sblock < 3; sblock++) {
				...
				const thmLR = Math.min(Math.max(gfc.thm[0].s[sb][sblock], ath), Math.max(gfc.thm[1].s[sb][sblock], ath));
	...
	private convert_partition2scalefac_s(gfc: LameInternalFlags, eb: Float32Array, thr: Float32Array, chn: number, sblock: number) {
		let sb = 0;
		let b = 0;
		let enn = 0.0;
		let thmm = 0.0;
	...
	private convert_partition2scalefac_l(gfc: LameInternalFlags, eb: Float32Array, thr: Float32Array, chn: number) {
		let sb = 0;
		let b = 0;
		let enn = 0.0;
		let thmm = 0.0;
	...
	private block_type_set(gfp: LameGlobalFlags, uselongblock: Int32Array, blocktype_d: Int32Array, blocktype: Int32Array) {
		const gfc = gfp.internal_flags;
		...
		for (let chn = 0; chn < gfc.channels_out; chn++) {
			...
			if (uselongblock[chn] !== 0) {
				...
				if (gfc.blocktype_old[chn] === STOP_TYPE) gfc.blocktype_old[chn] = SHORT_TYPE;
			}
	...
	private readonly regcoef_s = [11.8, 13.6, 17.2, 32, 46.5, 51.3, 57.5, 67.1, 71.5, 84.6, 97.6, 130] as const;
	...
	private readonly regcoef_l = [
		6.8, 5.8, 5.8, 6.4, 6.5, 9.9, 12.1, 14.4, 15, 18.9, 21.6, 26.9, 34.2, 40.2, 46.8, 56.5, 60.7, 73.9, 85.7, 93.4, 126.1,
	] as const;
	...
	private calc_energy(gfc: LameInternalFlags, fftenergy: Float32Array, eb: Float32Array, max: Float32Array, avg: Float32Array) {
		let b = 0;
		let j = 0;
	...
	private calc_mask_index_l(gfc: LameInternalFlags, max: Float32Array, avg: Float32Array, mask_idx: Int32Array) {
	...
	L3psycho_anal_ns(
		...
		const wsamp_S = Array.from({ length: 2 }, () => Array.from({ length: 3 }, () => new Float32Array(BLKSIZE_s)));
		...
		for (chn = 0; chn < gfc.channels_out; chn++) {
			...
			for (i = 0; i < 576; i++) {
				...
				for (j = 0; j < (PsyModel.NSFIRLEN - 1) / 2 - 1; j += 2) {
					sum1 += this.fircoef[j] * (firbuf[firbufPos + i + j] + firbuf[firbufPos + i + PsyModel.NSFIRLEN - j]);
					sum2 += this.fircoef[j + 1] * (firbuf[firbufPos + i + j + 1] + firbuf[firbufPos + i + PsyModel.NSFIRLEN - j - 1]);
				}
		...
		for (chn = 0; chn < numchn; chn++) {
			...
			const fftenergy_s = Array.from({ length: 3 }, () => new Float32Array(HBLKSIZE_s));
			...
			const attackThreshold = chn === 3 ? gfc.nsPsy.attackthre_s : gfc.nsPsy.attackthre;
			for (i = 0; i < 12; i++) {
			...
			for (sblock = 0; sblock < 3; sblock++) {
				...
				for (sb = 0; sb < SBMAX_s; sb++) {
					...
					if (ns_attacks[sblock] >= 2 || ns_attacks[sblock + 1] === 1) {
						...
						const p = this.nsInterp(gfc.thm[chn].s[sb][idx], thmm, PsyModel.NS_PREECHO_ATT1 * pcfact);
						thmm = Math.min(thmm, p);
					}

					if (ns_attacks[sblock] === 1) {
						const idx = sblock !== 0 ? sblock - 1 : 2;
						const p = this.nsInterp(gfc.thm[chn].s[sb][idx], thmm, PsyModel.NS_PREECHO_ATT2 * pcfact);
						thmm = Math.min(thmm, p);
			...
			for (b = 0; b < gfc.npart_l; b++) {
				...
				else thr[b] = this.nsInterp(Math.min(ecb, Math.min(this.rpelev * gfc.nb_1[chn][b], this.rpelev2 * gfc.nb_2[chn][b])), ecb, pcfact);
		...
		if (gfp.mode === MPEGMode.JOINT_STEREO) {
			this.msfix1(gfc);
			const { msfix } = gfp;
			if (Math.abs(msfix) > 0.0) this.ns_msfix(gfc, msfix, gfp.ATHlower * gfc.ATH.adjust);
		}
		...
		for (chn = 0; chn < numchn; chn++) {
			...
			if (chn > 1) {
				...
				ppePos = -2;
				type = NORM_TYPE;
				if (blocktype_d[0] === SHORT_TYPE || blocktype_d[1] === SHORT_TYPE) type = SHORT_TYPE;
			...
			if (type === SHORT_TYPE) ppe[ppePos + chn] = this.pecalc_s(mr, gfc.masking_lower);
	...
	private vbrpsy_compute_loudness_approximation_l(gfp: LameGlobalFlags, gr_out: number, chn: number, fftenergy: Float32Array) {
		const gfc = gfp.internal_flags;
		if (gfp.athaa_loudapprox === 2 && chn < 2) {
			gfc.loudness_sq[gr_out][chn] = gfc.loudness_sq_save[chn];
			gfc.loudness_sq_save[chn] = this.psycho_loudness_approx(fftenergy, gfc);
	...
	private vbrpsy_attack_detection(
		...
		for (let chn = 0; chn < n_chn_out; chn++) {
			...
			for (let i = 0; i < 576; i++) {
				...
				for (let j = 0; j < (PsyModel.NSFIRLEN - 1) / 2 - 1; j += 2) {
					sum1 += this.fircoef_[j] * (firbuf[firbufPos + i + j] + firbuf[firbufPos + i + PsyModel.NSFIRLEN - j]);
					sum2 += this.fircoef_[j + 1] * (firbuf[firbufPos + i + j + 1] + firbuf[firbufPos + i + PsyModel.NSFIRLEN - j - 1]);
				}
				ns_hpfsmpl[chn][i] = sum1 + sum2;
			}
			masking_ratio[gr_out][chn].en.assign(gfc.en[chn]);
		...
		for (let chn = 0; chn < n_chn_psy; chn++) {
			...
			for (let i = 0; i < 3; ++i) {
				const enn = en_subshort[i * 3 + 3] + en_subshort[i * 3 + 4] + en_subshort[i * 3 + 5];
				let factor = 1;
	...
	private vbrpsy_skip_masking_s(gfc: LameInternalFlags, chn: number, sblock: number) {
		if (sblock === 0) {
	...
	private psyvbr_calc_mask_index_s(gfc: LameInternalFlags, max: number[], avg: Float32Array, mask_idx: number[]) {
		const last_tab_entry = this.tab.length - 1;
		let b = 0;
		let a = avg[b] + avg[b + 1];
		assert(a >= 0);
	...
	private vbrpsy_compute_masking_l(gfc: LameInternalFlags, fftenergy: Float32Array, eb_l: Float32Array, thr: Float32Array, chn: number) {
		const max = new Float32Array(CBANDS);
		const avg = new Float32Array(CBANDS);
		const mask_idx_l = new Int32Array(CBANDS + 2);
		let b;
	...
	private vbrpsy_compute_block_type(gfp: LameGlobalFlags, uselongblock: Int32Array) {
		const gfc = gfp.internal_flags;
	...
	private vbrpsy_apply_block_type(gfp: LameGlobalFlags, uselongblock: Int32Array, blocktype_d: Int32Array) {
		const gfc = gfp.internal_flags;
	...
	L3psycho_anal_vbr(
		...
		const sub_short_factor = Array.from({ length: 4 }, () => new Float32Array(3));
		const pcfact = 0.6;
	...
	private s3_func(bark: number) {
		...
		tempx += 0.474;
		const tempy = 15.811389 + 7.5 * tempx - 17.5 * Math.sqrt(1.0 + tempx * tempx);
	...
	private init_numline(
		...
		for (i = 0; i < CBANDS; i++) {
			...
			for (j2 = j; this.freq2bark(sfreq * j2) - bark1 < PsyModel.DELBARK && j2 <= blksize / 2; j2++);
	...
	private init_s3_values(
		...
		if (use_old_s3) {
			...
			for (j = 0; j < npart; j++) {
				...
				for (let i = 0; i < npart; i++) {
					const v = s3_x_norm * this.s3_func_x(bval[i] - bval[j], hf_slope) * bval_width[j];
					s3[i][j] = v * norm[i];
				}
			}
		}
		...
		let k = 0;
		for (let i = 0; i < npart; i++) for (j = s3ind[i][0]; j <= s3ind[i][1]; j++) p[k++] = s3[i][j];
