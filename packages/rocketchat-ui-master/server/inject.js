...
const renderDynamicCssList = _.debounce(Meteor.bindEnvironment(() => {
	...
	const colors = RocketChat.models.Settings.find({_id:/theme-color-color-/i}, {fields: { value: 1, editor: 1}}).fetch().filter(color => color && color.value);
...
RocketChat.models.Settings.find({_id:/theme-color-color-/i}, {fields: { value: 1}}).observe({
	changed: renderDynamicCssList
});
