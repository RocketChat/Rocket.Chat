...
const detectEmail = (
  ...
  if (
    tld.length >= 4 &&
    /[a-z]f$/i.test(tld) &&
    !['pdf', 'gif'].includes(tld.toLowerCase())
  ) {
    return null;
  }
...
const getEmoticonAt = (
  text: string,
  position: number,
): { emoticon: string; length: number } | null => {
  for (const emoticon of EMOTICON_KEYS_DESC) {
    if (text.slice(position, position + emoticon.length) === emoticon) {
      ...
      const nextChar =
        position + emoticon.length < text.length
          ? text[position + emoticon.length]
          : '';
      ...
      const nextBoundary =
        position + emoticon.length >= text.length || /\s/.test(nextChar);
 ...
 */
const shouldSkipConsecutiveEmoticons = (
  text: string,
  startPos: number,
): boolean => {
  ...
  return (
    emoticonCount >= 4 ||
    (emoticonCount >= 2 && (hasNonWhitespaceBefore || hasNonWhitespaceAfter))
  );
};
  ...
  if (isBold && thirdChar === '*') {
    ...
    if (doubleEndStart !== -1 && doubleEndStart > i + 3) {
      ...
      if (content.trim().length > 0) {
        ...
        const charAfterDouble =
          doubleEndStart + 2 < text.length ? text[doubleEndStart + 2] : '';
        if (charAfterDouble === '*') {
          // ***Hello*** case
          trailingAsterisk = '*';
...
const parseItalicMarkup = (
      ...
      if (contentBeforeDouble.trim().length > 0) {
        ...
        if (nextSingleEnd !== -1 && nextSingleEnd > doublePos + 2) {
          ...
          if (secondContent.trim().length > 0) {
            ...
            const afterSecond =
              nextSingleEnd + 1 < text.length ? text[nextSingleEnd + 1] : '';
            const atSecondEndBoundary =
              nextSingleEnd + 1 >= text.length ||
              /[\s\n\r\t\(\)\[\]{}.,;:!?*~`-]/.test(afterSecond);
            if (atSecondEndBoundary) {
              const firstInlineContent = parseInlineContent(
                contentBeforeDouble,
                options,
              );
              const secondInlineContent = parseInlineContent(
                secondContent,
                options,
              );
              return {
  ...
  while (searchPos < text.length) {
    ...
    if (options?.emoticons && nextUnderscore > 0) {
      ...
      const charAfter =
        nextUnderscore + 1 < text.length ? text[nextUnderscore + 1] : '';
    ...
    const charAfterUnderscore =
      nextUnderscore + 1 < text.length ? text[nextUnderscore + 1] : '';
    const looksLikeDelimiter =
      charAfterUnderscore === ' ' || nextUnderscore + 1 >= text.length;
  ...
  if (singleEnd !== -1 && singleEnd > i + 1) {
    ...
    const nextCharAfterSingle =
      singleEnd + 1 < text.length ? text[singleEnd + 1] : '';
    const atSingleEndWordBoundary =
      singleEnd + 1 >= text.length ||
  ...
  if (nextChar === '_') {
    ...
    if (doubleEnd !== -1 && doubleEnd > i + 2) {
      const nextCharAfterDouble =
        doubleEnd + 2 < text.length ? text[doubleEnd + 2] : '';
      const atDoubleEndWordBoundary =
        doubleEnd + 2 >= text.length ||
  ...
  while (i < n) {
    ...
    if (char === '<' && text.slice(i, i + 3) === '<t:') {
      ...
      if (closeIndex !== -1) {
        ...
        if (
          lastColonIndex !== -1 &&
          lastColonIndex < timestampContent.length - 1
        ) {
        ...
        if (RE_TIMESTAMP_ALL_DIGITS.test(timestampValue)) {
          ...
          if (timestampValue.length >= 9) {
            ...
            tokens.push(
              ast.timestamp(
                timestampValue,
                format as 't' | 'T' | 'd' | 'D' | 'f' | 'F' | 'R' | undefined,
              ),
            );
            i = closeIndex + 1;
            continue;
          ...
          const unixTimestamp = Math.floor(
            new Date(timestampValue).getTime() / 1000,
          ).toString();
          ...
          if (timeMatch) {
            const [, hours, minutes, seconds = '00', timezone = '+00:00'] =
              timeMatch;
          ...
          if (closeParens !== -1) {
            ...
            if (url.startsWith('+') && /^[+0-9()\-]+$/.test(url)) {
              ...
              if (digitsOnly.length >= 5) {
                ...
                if (labelText) {
                  const parsedTokens = parseInlineContent(
                    labelText,
                    options,
                    true,
                  );
                  ...
                  if (
                    ...
                    filteredTokens.some((t) => t.type !== 'PLAIN_TEXT')
                  ) {
            ...
            if (labelText) {
              ...
              const filteredTokens = parsedTokens.filter(
                (
                  token,
                ): token is AST.Plain | AST.Bold | AST.Italic | AST.Strike =>
                  token.type === 'PLAIN_TEXT' ||
                  token.type === 'BOLD' ||
                  token.type === 'ITALIC' ||
                  token.type === 'STRIKE',
    ...
    if (options?.emoticons) {
      for (const emoticon of EMOTICON_KEYS_DESC) {
        if (text.slice(i, i + emoticon.length) === emoticon) {
          ...
          const prevChar = i > 0 ? text[i - 1] : '';
          const nextChar =
            i + emoticon.length < n ? text[i + emoticon.length] : '';

          // Boundary check for emoticons
          const prevBoundary =
            i === 0 || /[\s\n\r\t\(\)\[\]{}.,;*_]/.test(prevChar);
          const nextBoundary =
            i + emoticon.length >= text.length ||
            /[\s\n\r\t\(\)\[\]{}.,;*_]/.test(nextChar);
          ...
          if (
            ...
            (isKissingEmoticon && afterEmphasis)
          ) {
            ...
            if (
              adjacentToText &&
              shouldSkipConsecutiveEmoticons(text, i) &&
              !isKissingEmoticon
            ) {
    ...
    while (tempI < n) {
      ...
      if (currentChar === '@') {
        ...
        const emailBefore =
          ...
          (lastCharBefore &&
            lastCharBefore.charCodeAt(0) > 127 &&
...
const detectEmailsInText = (text: string): AST.Inlines[] => {
  ...
  while (i < n) {
    ...
    if (
      emailStart >= 7 &&
      text.slice(emailStart - 7, emailStart) === 'mailto:'
    ) {
      emailStart -= 7;
    }
...
export const parse = (input: string, options?: Options): AST.Root => {
  ...
  const matchesConsecutiveEmoticons =
    RE_CONSECUTIVE_EMOTICONS_2PLUS.test(normalizedInput);
  const isFourOrMoreConsecutiveEmoticons = RE_CONSECUTIVE_EMOTICONS_4PLUS.test(
  ...
  if (
    matchesConsecutiveEmoticons &&
    options?.emoticons &&
    !isFourOrMoreConsecutiveEmoticons
  ) {
    ...
    while (position < normalizedInput.length) {
      // Skip whitespace
      while (
        position < normalizedInput.length &&
        /\s/.test(normalizedInput[position])
      ) {
        position++;
      }
  ...
  if (normalizedInput.trim().length > 0) {
    ...
    if (containsOnlyEmojisAndWhitespace && unicodeEmojis.length > 0) {
      ...
      if (unicodeEmojis.length === 1) {
        ...
        return [
          ast.bigEmoji([unicodeEmojis[0], unicodeEmojis[1], unicodeEmojis[2]]),
        ];
      }
    }
  }
  ...
  for (const line of lines) {
    ...
    const emojiTokens = inlineContent.filter(
      (token) => token.type === 'EMOJI',
    ) as AST.Emoji[];
  ...
  if (
    allLinesAreEmojiOnly &&
    allEmojiTokens.length >= 1 &&
    allEmojiTokens.length <= 3
  ) {
    if (allEmojiTokens.length === 1) {
      ...
      return [
        ast.bigEmoji([allEmojiTokens[0], allEmojiTokens[1], allEmojiTokens[2]]),
      ];
    }
  }
  ...
  while (i < lines.length) {
    ...
    let __ns = 0;
    while (__ns < line.length && (line[__ns] === ' ' || line[__ns] === '\t'))
      __ns++;
    ...
    if (starter === '>' && RE_BLOCKQUOTE_LINE.test(line)) {
      ...
      while (
        ...
        RE_BLOCKQUOTE_LINE.test(lines[quoteIndex])
      ) {
    ...
    if (starter === '-' && RE_TASK_ITEM_LINE.test(line)) {
      ...
      while (
        taskIndex < lines.length &&
        RE_TASK_ITEM_LINE.test(lines[taskIndex])
      ) {
    ...
    if (
      ...
      while (
        ...
        RE_UNORDERED_BULLET.test(lines[listIndex])
      ) {
    ...
    if (starter >= '0' && starter <= '9' && RE_ORDERED_LINE.test(line)) {
      ...
      while (
        listIndex < lines.length &&
        RE_ORDERED_LINE.test(lines[listIndex])
      ) {
        const listLine = lines[listIndex];
