...
function findDirectMessageRoom(params, user) {
	...
	if (!canAccess || !room || room.t !== "d") {
		throw new Meteor.Error(
			...
			'The required "roomId" or "username" param provided does not match any dirct message'
		);
	}

	const subscription = RocketChat.models.Subscriptions.findOneByRoomIdAndUserId(
		room._id,
		user._id
	);
		...
		post() {
			...
			if (!findResult.subscription.open) {
				return RocketChat.API.v1.failure(
					`The direct message room, ${
						this.bodyParams.name
					}, is already closed to the sender`
				);
			}
...
RocketChat.API.v1.addRoute(
	...
	{
		get() {
			const access = RocketChat.authz.hasPermission(
				this.userId,
...
RocketChat.API.v1.addRoute(
	["dm.history.others", "im.history.others"],
	{ authRequired: true },
	{
		get() {
			if (
...
RocketChat.API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const cursor = RocketChat.models.Subscriptions.findByRoomId(
				findResult.room._id,
				{
		...
		get() {
			...
			return RocketChat.API.v1.success({
				messages: messages.map(message =>
					RocketChat.composeMessageObjectWithUser(message, this.userId)
				),
...
RocketChat.API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			if (
				!RocketChat.authz.hasPermission(this.userId, "view-room-administration")
			) {
				return RocketChat.API.v1.unauthorized();
			}
...
RocketChat.API.v1.addRoute(
	...
	{ authRequired: true },
	{
		get() {
			...
			const cursor = RocketChat.models.Rooms.findBySubscriptionTypeAndUserId(
				"d",
				this.userId,
				{
					sort,
