...
RocketChat.API.v1.addRoute('shield.svg', { authRequired: false }, {
	get() {
		const { type, channel, name } = this.queryParams;
		let text;
		switch (type) {
			case 'online':
		...
		const leftSize = name ? name.length * 6 + 32 : 24;
			...
			body: `
				<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${ width }" height="${ height }">
				  ...
				  <g fill="#fff" font-family="DejaVu Sans,Verdana,Geneva,sans-serif" font-size="11">
						${ name ? `<text x="24" y="15" fill="#010101" fill-opacity=".3">${ name }</text>
				    <text x="24" y="14">${ name }</text>` : '' }
