...
const OverflowElement = ({ block, context }: OverflowElementProps): ReactElement => {
	...
	const fireChange = useCallback(([value]: [UiKit.ActionOf<UiKit.OverflowElement>, string]) => action({ target: { value } }), [action]);

	const options = useMemo<OptionType[]>(
		() => block.options.map(({ value, text, url }) => [value, fromTextObjectToString(text) ?? '', undefined, undefined, undefined, url]),
	...
	return (
		<>
			<IconButton
				...
				icon='kebab'
			/>
			<PositionAnimated width='auto' visible={visible} anchor={ref} placement='bottom-start'>
