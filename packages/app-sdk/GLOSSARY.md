...
| **store** | app-private persistence — `ctx.store`, typed collections. **Not** platform data | [18](rfc/18-surface-store.md) |
| **association** | an optional per-record relation tag on a store write. Unsettled: A drop / B keep / C declare per collection | [18a](rfc/18a-surface-store-associations.md) |
...
| **index spec** | one declared index on a store collection: `{ on, unique?, ttl? }`. The host builds it at install | [18](rfc/18-surface-store.md) |
| **the index contract** | the declared index set *is* the query surface: `find` accepts only a key set a declared index serves as a prefix | [18](rfc/18-surface-store.md), [host 31](rfc-host/31-store-persistence.md) |
| **served query** | a `find` argument the index contract admits — the prefix of some declared index. `ServedQuery` in the sketch | [18](rfc/18-surface-store.md) |
...
| **store** | app-private persistence, `ctx.store` ([18](rfc/18-surface-store.md)) | platform data, reached through `ctx.rooms` / `ctx.messages` ([20](rfc/20-data-overview.md)); or Mongo, which is **storage**. One declared collection inside a store is a **store collection** ([host 31](rfc-host/31-store-persistence.md)) |
