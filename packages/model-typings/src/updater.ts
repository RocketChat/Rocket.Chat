...
export interface Updater<T extends { _id: string }> {
	...
	addUpdateFilter(update: UpdateFilter<T>): Updater<T>;
}
...
export type UpdaterCompatibleUpdateFilter<T extends { _id: string }> = Pick<UpdateFilter<T>, '$set' | '$unset' | '$inc' | '$addToSet'>;
