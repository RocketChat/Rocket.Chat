...
): [(node: HTMLVideoElement | null) => void, { current: HTMLVideoElement | null }] => {
	const remoteStream = getRemoteStream(instance);
	const actualRef = useRef<HTMLVideoElement | null>(null);
			...
			useCallback(
				(node) => {
					...
					node.play().catch((error) => {
						console.error('MediaCall: useVideoStream - Error playing media stream', error);
					});
