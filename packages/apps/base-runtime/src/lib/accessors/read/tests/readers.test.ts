...
describe('Reader family (base-runtime)', () => {
	describe('MessageRead', () => {
		it('getById forwards to getMessageBridge:doGetById with the APP_ID sentinel', async () => {
			const { rec, bridges } = setup({ 'bridges:getMessageBridge:doGetById': { id: 'm1' } });
			const result = await new MessageRead(bridges).getById('m1');
		...
		it('getSenderUser returns the message sender, or undefined when the message is missing', async () => {
			...
			assert.deepStrictEqual(await new MessageRead(withMsg.bridges).getSenderUser('m1'), { id: 'u1' });

			const noMsg = setup({ 'bridges:getMessageBridge:doGetById': null });
			assert.strictEqual(await new MessageRead(noMsg.bridges).getSenderUser('m1'), undefined);
		});
		...
		it('getRoom returns the message room, or undefined when the message is missing', async () => {
			...
			assert.deepStrictEqual(await new MessageRead(withMsg.bridges).getRoom('m1'), { id: 'r1' });

			const noMsg = setup({ 'bridges:getMessageBridge:doGetById': undefined });
			assert.strictEqual(await new MessageRead(noMsg.bridges).getRoom('m1'), undefined);
		});
	});

	describe('RoomRead', () => {
		it('getMessages applies limit/showThreadMessages defaults', async () => {
			const { rec, bridges } = setup({ 'bridges:getRoomBridge:doGetMessages': [] });
			await new RoomRead(bridges).getMessages('r1');
		...
		it('getMessages rejects a limit above 100', async () => {
			const { bridges } = setup();
			assert.throws(() => new RoomRead(bridges).getMessages('r1', { limit: 101 }), /Expected number <= 100, got 101/);
		});

		it('getMessages rejects an invalid sort key and direction', async () => {
			const { bridges } = setup();
			assert.throws(() => new RoomRead(bridges).getMessages('r1', { sort: { bogus: 'asc' } as any }), /Invalid key "bogus"/);
			// `createdAt` is a valid sortable field, so this exercises the direction check specifically.
			assert.throws(() => new RoomRead(bridges).getMessages('r1', { sort: { createdAt: 'sideways' } as any }), /Invalid sort direction/);
		});

		it('getAllRooms validates limit and skip and forwards normalized options', async () => {
			const { rec, bridges } = setup({ 'bridges:getRoomBridge:doGetAllRooms': [] });
			await new RoomRead(bridges).getAllRooms();
			assert.deepStrictEqual(rec.emitted()[0], {
				method: 'bridges:getRoomBridge:doGetAllRooms',
				params: [{}, { limit: 100, skip: 0 }, 'APP_ID'],
			});

			assert.throws(() => new RoomRead(setup().bridges).getAllRooms({}, { limit: 0 } as any), /between 1 and 100/);
			assert.throws(() => new RoomRead(setup().bridges).getAllRooms({}, { skip: -1 } as any), /Expected number >= 0/);
		});

		it('getUnreadByUser validates roomId and fills option defaults', async () => {
			const { rec, bridges } = setup({ 'bridges:getRoomBridge:doGetUnreadByUser': [] });
			await new RoomRead(bridges).getUnreadByUser('r1', 'u1');
			assert.deepStrictEqual(rec.emitted()[0], {
			...
			await assert.rejects(() => new RoomRead(setup().bridges).getUnreadByUser('   ', 'u1'), /non-empty string/);
		});

		it('passthrough getters forward verbatim', async () => {
			const { rec, bridges } = setup();
			const room = new RoomRead(bridges);
	...
	describe('UserRead', () => {
		it('getBySipExtension short-circuits to undefined for an empty extension (no bridge call)', async () => {
			const { rec, bridges } = setup();
			assert.strictEqual(await new UserRead(bridges).getBySipExtension(''), undefined);
	...
	describe('PersistenceRead', () => {
		it('readByAssociation wraps the single association into an array', async () => {
			const { rec, bridges } = setup({ 'bridges:getPersistenceBridge:doReadByAssociations': [] });
			const assoc = { type: 1, id: 'x' } as any;
			await new PersistenceRead(bridges).readByAssociation(assoc);
			assert.deepStrictEqual(rec.emitted()[0], {
	...
	describe('UploadRead', () => {
		it('getBufferById fetches the upload then its buffer (two bridge calls)', async () => {
			const { rec, bridges } = setup({ 'bridges:getUploadBridge:doGetById': { _id: 'up1' } });
			await new UploadRead(bridges).getBufferById('up1');
	...
	describe('LivechatRead', () => {
		it('_fetchLivechatRoomMessages sends the app id first, then the room id', async () => {
			const { rec, bridges } = setup({ 'bridges:getLivechatBridge:do_fetchLivechatRoomMessages': [] });
			await new LivechatRead(bridges)._fetchLivechatRoomMessages('r1');
			assert.deepStrictEqual(rec.emitted()[0], {
	...
	describe('simple passthrough readers', () => {
		it('route to their matching bridge method', async () => {
			const { rec, bridges } = setup();
			await new CloudWorkspaceRead(bridges).getWorkspaceToken('scope');
			new VideoConferenceRead(bridges).getById('c1');
			await new OAuthAppsReader(bridges).getOAuthAppById('o1');
			new ContactRead(bridges).getById('ct1');
			await new ThreadRead(bridges).getThreadById('t1');
			await new RoleRead(bridges).getCustomRoles();
