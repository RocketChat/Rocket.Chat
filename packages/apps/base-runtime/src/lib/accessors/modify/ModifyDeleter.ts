...
export class ModifyDeleter implements IModifyDeleter {
	...
	public async deleteRoom(roomId: string): Promise<void> {
		await this.bridges.getRoomBridge().doDelete(roomId, 'APP_ID');
	}
	...
	public async deleteMessage(message: IMessage, user: IUser): Promise<void> {
		await this.bridges.getMessageBridge().doDelete(message, user, 'APP_ID');
	}
	...
	public async removeUsersFromRoom(roomId: string, usernames: Array<string>) {
		...
		return this.bridges.getRoomBridge().doRemoveUsers(roomId, usernames, 'APP_ID') as Promise<void>;
	}
}
