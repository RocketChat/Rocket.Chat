...
describe('createPredicateFromFilter', () => {
	it('matches simple equality', () => {
		...
		const fn = createPredicateFromFilter<any>(filter);
	...
	it('matches $lt/$lte/$gt/$gte', () => {
		...
		const fn2 = createPredicateFromFilter<any>(filter2);
		expect(fn2({ foo: 5 })).toBe(true);
		const filter3 = { foo: { $gt: 5 } };
		const fn3 = createPredicateFromFilter<any>(filter3);
		expect(fn3({ foo: 6 })).toBe(true);
		const filter4 = { foo: { $gte: 5 } };
		const fn4 = createPredicateFromFilter<any>(filter4);
	...
	it('returns true for empty filter', () => {
		const fn = createPredicateFromFilter<any>({});
		expect(fn({})).toBe(true);
		expect(fn({ foo: 1 })).toBe(true);
	});
	...
	it('throws error for unsupported operators', () => {
		...
		expect(() => createPredicateFromFilter<any>(filter)).toThrow('Unrecognized logical operator: $unsupported');
	});
});
