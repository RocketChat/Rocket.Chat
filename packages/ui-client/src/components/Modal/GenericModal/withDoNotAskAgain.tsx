...
import { useRememberDontAskAgain } from '../../../hooks/useRememberDontAskAgain';
...
function withDoNotAskAgain<T extends RequiredModalProps>(Component: ComponentType<any>) {
	...
	const WrappedComponent = function ({ onConfirm, dontAskAgain, ...props }: WrappedComponentProps) {
		...
		const rememberDontAskAgain = useRememberDontAskAgain();
		const [state, setState] = useState<boolean>(false);

		const handleConfirm = async (): Promise<void> => {
			try {
				if (state) {
					await rememberDontAskAgain(dontAskAgain);
				}
				await onConfirm();
			} catch (e) {
				console.error(e);
