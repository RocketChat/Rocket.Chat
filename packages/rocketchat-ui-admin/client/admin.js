...
const setFieldValue = function(settingId, value, type, editor) {
	...
	switch (type) {
		...
		case 'roomPick':
			...
			TempSettings.update({ _id: settingId }, { $set: { value: value, changed: JSON.stringify(RocketChat.settings.collectionPrivate.findOne(settingId).value) isnt JSON.stringify(value) } });
			break;
		default:
			input.val(value).change();
	}
