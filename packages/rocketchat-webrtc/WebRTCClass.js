...
class WebRTCClass {
	...
	checkPeerConnections() {
		...
		Object.keys(peerConnections).forEach((id) => {
			...
			if (peerConnection.iceConnectionState !== 'connected' && peerConnection.iceConnectionState !== 'completed' && peerConnection.createdAt + 5000 < Date.now()) {
				this.stopPeerConnection(peerConnection.id);
			}
		});
	}
   ...
   */
	...
	getPeerConnection(id) {
		...
		peerConnection.addEventListener('icecandidate', (e) => {
			...
			return this.transport.sendCandidate({
				to: id,
				candidate: {
	...
	getUserMedia(media, onSuccess, onError) {
		...
		const getScreen = (audioStream) => {
			...
			const getScreenSuccess = function(stream) {
				if (audioStream != null) {
			...
			if (this.navigator === 'firefox') {
				...
				ChromeScreenShare.getSourceId(this.navigator, function(id) {
					media = {
						audio: false,
						video: {
							mandatory: {
					...
					return this._getUserMedia(media, getScreenSuccess, onError);
				});
			}
		};
	...
	onRemoteJoin(data) {
		...
		const onOffer = () => {
			return function(offer) {
				const onLocalDescription = function() {
   ...
   */
	...
	onRemoteOffer(data) {
		...
		let peerConnection = this.getPeerConnection(data.from);
		if ((peerConnection.signalingState === 'have-local-offer' || peerConnection.signalingState === 'stable') && peerConnection.createdAt < data.ts) {
			this.stopPeerConnection(data.from);
			peerConnection = this.getPeerConnection(data.from);
		}
		if (peerConnection.iceConnectionState !== 'new') {
...
Meteor.startup(function() {
	return Tracker.autorun(function() {
		if (Meteor.userId()) {
			return RocketChat.Notifications.onUser('webrtc', () => {
