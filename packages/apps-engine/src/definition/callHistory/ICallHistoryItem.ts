...
export type CallHistoryState =
	...
	| 'transferred'
	/**
	 * An app refused the call before anyone's device rang. Read
	 * `ICallHistoryCallDetails.preventedBy` for which app refused it, and why.
	 */
	| 'prevented';
 ...
 *
 * A `'prevented'` call is the exception. The callee never learned about it, so only the
 * caller's `'outbound'` item exists.
 */
