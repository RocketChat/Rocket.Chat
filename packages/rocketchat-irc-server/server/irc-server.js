// TODO Refactoring - transfer IrcServer class (irc-server-bridge) to your specific file
...
export class IrcServer {
	...
	onConnect = () => {
		this.writeCommand({
			command: 'PASS',
			parameters: [this.sendPassword, 'TS', 6, this.serverId] // password, 'TS', TS version, SID
		});

		this.writeCommand({
			command: 'CAPAB',
			trailer: 'TBURST EOB QS ENCAP EX CHW IE KNOCK'
		});
	...
	sendMessage = (message, room) => {
		if (this.state !== 'connected' || this.localUsersById[message.u._id] === undefined) {
			return;
		}
