RocketChat.OTR.Room = class {
	...
	encrypt(message) {
		var data = new TextEncoder('UTF-8').encode(EJSON.stringify({ text: message, userId: this.userId, ack: Random.id((Random.fraction()+1)*20), ts: new Date(Date.now() + TimeSync.serverOffset()) }));
