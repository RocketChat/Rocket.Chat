export class RocketletMessagesConverter {
	...
	convertMessage(msgObj) {
		...
		const attachments = this._convertAttachmentsToRocketlet(msgObj.attachments);
	...
	convertRocketletMessage(message) {
		...
		const attachments = this._convertRocketletAttachments(message.attachments);
	...
	_convertRocketletAttachments(attachments) {
	...
	_convertAttachmentsToRocketlet(attachments) {
		if (typeof attachments === 'undefined' || !Array.isArray(attachments)) {
			return undefined;
		}
