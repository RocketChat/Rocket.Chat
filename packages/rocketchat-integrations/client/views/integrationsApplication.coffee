Template.integrationsApplication.onCreated ->
	@record = new ReactiveVar
		username: 'rocket.cat'
		token: Random.id(24)


Template.integrationsApplication.helpers
		...
		if params?.id?
			...
			if data?
				if data.permissions?
					...
					for key, value of data.permissions
						value.permission = key
						data.permissions_array.push value
			# 	if not data.token?
...
Template.integrationsApplication.events
	"blur input": (e, t) ->
		t.record.set
