...
const successResults = [200, 201, 202];
const compiledScripts = {};
const triggers = {};

const getIntegrationScript = function _getIntegrationScript(integration) {
...
const hasScriptAndMethod = (integration, method) => {
...
const executeScript = (integration, method, params) => {
	let script;
	try {
		script = getIntegrationScript(integration);
	} catch (e) {
...
const removeIntegration = (record) => {
...
const ExecuteTriggerUrl = (url, trigger, message, room, tries=0) => {
	let word;
	...
	HTTP.call(opts.method, opts.url, opts, (error, result) => {
		...
		if (hasScriptAndMethod(trigger, 'process_outgoing_response')) {
			...
			const scriptResult = executeScript(trigger, 'process_outgoing_response', sandbox);
		...
		if (!result || !successResults.includes(result.statusCode)) {
			if (error) {
...
const ExecuteTrigger = (trigger, message, room) => {
...
const ExecuteTriggers = (message, room) => {
	if (!room) {
		return;
	}
	...
	switch (room.t) {
		case 'd':
			...
			if (triggers['@'+id]) {
				for (const trigger of Object.values(triggers['@'+id])) {
					triggersToExecute.push(trigger);
				}
			}

			if (triggers.all_direct_messages) {
				for (const trigger of Object.values(triggers.all_direct_messages)) {
					triggersToExecute.push(trigger);
				}
			}

			if (id !== username && triggers['@'+username]) {
				for (const trigger of Object.values(triggers['@'+username])) {
					triggersToExecute.push(trigger);
				}
			}
			break;
		...
		case 'c':
			if (triggers.__any) {
				for (const trigger of Object.values(triggers.__any)) {
					triggersToExecute.push(trigger);
				}
			}

			if (triggers.all_public_channels) {
				for (const trigger of Object.values(triggers.all_public_channels)) {
					triggersToExecute.push(trigger);
				}
			}

			if (triggers['#'+room._id]) {
				for (const trigger of Object.values(triggers['#'+room._id])) {
					triggersToExecute.push(trigger);
				}
			}

			if (room._id !== room.name && triggers['#'+room.name]) {
				for (const trigger of Object.values(triggers['#'+room.name])) {
					triggersToExecute.push(trigger);
				}
			}
			break;
		...
		default:
			if (triggers.all_private_groups) {
				for (const trigger of Object.values(triggers.all_private_groups)) {
					triggersToExecute.push(trigger);
				}
			}
