...
import { _setRealName, _setUsername } from 'meteor/rocketchat:lib';
...
export function syncUserData(user, ldapUser) {
	...
	if (user && user._id && settings.get('LDAP_Sync_User_Avatar') === true) {
		const avatar = ldapUser._raw[settings.get('LDAP_Avatar_Field')] || ldapUser._raw.thumbnailPhoto || ldapUser._raw.jpegPhoto;
