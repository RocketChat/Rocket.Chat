...
import { queues } from '../../services/utils';
export default {
	name:'mailer',
	mixins:[queues()],
	actions: {
		sendEmail(ctx) {
			const { to, from, subject, html, data, headers } = ctx.params;
			this.createJob('email', { to, from, subject, html, data, headers });
		},
	},
	events :{
	...
	queue: {
		email({ to, from, subject, html, data, headers }) {
			return send({ to, from, subject, html, data, headers });
		},
	},
};
