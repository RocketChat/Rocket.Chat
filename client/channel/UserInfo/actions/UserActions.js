...
const UserActions = ({ user, rid }) => {
	...
	const menu = menuOptions && <Menu small={false} renderItem={({ label: { label, icon }, ...props }) => <Option {...props} label={label} icon={icon} />} flexShrink={0} key='menu' options={menuOptions} />;
	...
	return <ButtonGroup flexShrink={0} flexWrap='nowrap' withTruncatedText justifyContent='center' flexShrink={0}>
		{actions}
	</ButtonGroup>;
};
