...
export const useRoomIcon = (room: IRoom, small = true): JSX.Element | { name: string; color?: string } | null => {
	...
	if (room.prid) {
		return { name: 'baloons', ...!room.hideUnreadStatus && (room.alert || room.unread) && { color: unreadColor } };
	}

	if (room.teamMain) {
		return { name: room.t === 'p' ? 'team-lock' : 'team', ...!room.hideUnreadStatus && (room.alert || room.unread) && { color: unreadColor } };
	}

	switch (room.t) {
		case 'p':
			return { name: 'hashtag-lock', ...!room.hideUnreadStatus && (room.alert || room.unread) && { color: unreadColor } };
		case 'c':
			return { name: 'hash', ...!room.hideUnreadStatus && (room.alert || room.unread) && { color: unreadColor } };
		case 'l':
		...
		case 'd':
			...
			if (direct.uids && direct.uids.length > 2) {
				return { name: 'balloon', ...!room.hideUnreadStatus && (room.alert || room.unread) && { color: unreadColor } };
			}
