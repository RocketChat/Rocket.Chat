...
export function EditCustomUserStatusWithData({ _id, cache, ...props }) {
	...
	if (error) {
		return <Box fontScale='h1' pb='x20'>{t('Custom_Emoji_Error_Invalid_Emoji')}</Box>;
	}
...
export function EditCustomUserStatus({ close, onChange, data, ...props }) {
	...
	const hasUnsavedChanges = useMemo(() => previousName !== name || previousAliases !== aliases, [name, aliases]);
		...
		<Box display='flex' flexDirection='column' fontScale='p1' color='default' mbs='x20' {...props}>
			<Margins block='x4'>
				...
				<Field>
					...
					<Field.Row>
						<TextInput value={aliases} onChange={(value) => setAliases(value)} placeholder={t('Aliases')} />
					</Field.Row>
				</Field>
				<Field>
					<Field.Row>
