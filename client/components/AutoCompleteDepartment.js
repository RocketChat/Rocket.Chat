...
export const AutoCompleteDepartment = React.memo((props) => {
	...
	const options = useMemo(() => (data && [{ value: 'all', label: label && label }, ...data.departments.map((department) => ({ value: department._id, label: department.name }))]) || [{ value: 'all', label: t('All') }], [data, t]);
