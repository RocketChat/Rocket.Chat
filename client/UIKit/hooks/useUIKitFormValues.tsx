...
const filterInputFields = (value: any): value is IInputBlock | IActionsBlock => {
	...
	if (
		isActionsOrContextBlock(value) &&
		value.elements.length &&
		value.elements.map((element) => ({ element } as unknown as IBlock)).filter(filterInputFields)
			.length
	) {
		return true;
	}
...
const mapElementToState = (block: IInputBlock | IActionsBlock): UIKitFormState[] => {
	...
	return [
		{
			[(block as IInputBlock).element.actionId]: {
				value: (block as IInputBlock).element.initialValue,
				blockId,
				appId,
			},
		},
...
const reducer = (
	values: UIKitFormState,
	{ actionId, payload }: UIKitFormStateChangeDispatch,
): UIKitFormState => ({
	...values,
	[actionId]: payload,
});

const useUIKitFormValues = (
	view: UiKitPayload,
): [UIKitFormState, Dispatch<UIKitFormStateChangeDispatch>] => {
	const initializer = useMutableCallback(() => {
		const filteredBlock = view.blocks.filter(filterInputFields) as unknown as (
			| IInputBlock
			| IActionsBlock
		)[];
