							...
							<CheckBox
								...
								onChange={(): void => {
									setChannels((channels) => {
										const hasCheckedArchivedChannels = channels.some(
											({ is_archived, do_import }) => is_archived && do_import,
										);
										...
										if (hasCheckedArchivedChannels) {
											return channels.map((channel) =>
												channel.is_archived ? { ...channel, do_import: false } : channel,
											);
										}
					...
					{channels.slice(current, current + itemsPerPage).map((channel) => (
						<Table.Row key={channel.channel_id}>
									...
									onChange={(event: ChangeEvent<HTMLInputElement>): void => {
										...
										setChannels((channels) =>
											channels.map((_channel) =>
												_channel === channel ? { ..._channel, do_import: checked } : _channel,
											),
										);
									}}
								/>
							...
							<Table.Cell align='end'>
								{channel.is_archived && <Tag variant='danger'>{t('Importer_Archived')}</Tag>}
							</Table.Cell>
						</Table.Row>
					))}
				</Table.Body>
