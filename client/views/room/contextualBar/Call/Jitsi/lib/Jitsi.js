 ...
 * @constructor
 */
export function JitsiMeetExternalAPI(domain, room_name, width, height, parentNode, configOverwrite, interfaceConfigOverwrite, noSsl, token) {
...
// 	const windowForEventListening = options.windowForEventListening || window;
// 	const listeners = {};
// 	let sendBuffer = [];
// 	const listenBuffer = {};
// 	let ready = false;
// 	const readyMethod = '__ready__';
// 	let readynessCheck;
...
// 			const listenersForMethod = listeners[data.method];
...
// 			const listenBufferForMethod = listenBuffer[method];
// 			if (listenBufferForMethod) {
// 				const listenersForMethod = listeners[method];
// 				for (let i = 0; i < listenersForMethod.length; i++) {
...
// 	const readyCheckID = `${ +new Date() + Math.random() }`;
