		...
		it('should set the topic of the DM with a string', (done) => {
			request
				...
				.expect((res) => {
					...
					expect(res.body).to.have.nested.property(
						'topic',
						'a direct message with rocket.cat',
					);
				})
				.end(done);
		});
			...
			request
				...
				.expect((res) => {
					...
					const lastMessage = messages.filter(
						(message) => message._id === dmMessage._id,
					)[0];
					expect(lastMessage)
						.to.have.property('starred')
						.and.to.be.an('array');
					expect(lastMessage.starred[0]._id).to.be.equal(
						adminUsername,
					);
				})
				.end(done);
	...
	describe('fname property', () => {
		...
		it('should have fname property', (done) => {
			request
				...
				.expect((res) => {
					expect(res.body).to.have.property('success', true);
					expect(res.body.subscription).to.have.property(
						'name',
						username,
					);
					expect(res.body.subscription).to.have.property(
		...
		it('should create a self-DM', (done) => {
			request
				...
				.expect((res) => {
					...
					expect(res.body.room)
						.to.have.property('usernames')
						.and.to.have.members([adminUsername]);
				})
				.end(done);
		});
	});
