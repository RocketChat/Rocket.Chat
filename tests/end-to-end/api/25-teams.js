...
describe('[Teams]', () => {
				...
				.then((response) => {
					...
					return request
						...
						.expect((response) => {
							...
							expect(member.createdBy).to.have.property(
								'username',
							);
	...
	describe('/teams.convertToChannel', () => {
		...
		before(
			'create channel (to erase after its team is converted to a channel)',
			(done) => {
				request
		...
		before('add first channel to team', (done) => {
			request
				...
				.expect((res) => {
					...
					expect(res.body.rooms[0]).to.have.property(
						'teamId',
						testTeam._id,
					);
					expect(res.body.rooms[0]).to.not.have.property(
						'teamDefault',
					);
				})
				.then(() => done())
		...
		before(
			'create channel (to keep after its team is converted to a channel)',
			(done) => {
				request
					.post(api('channels.create'))
					.set(credentials)
				...
				.then(() => {
					request
						...
						.expect((response) => {
							...
							expect(response.body.channel).to.have.property(
								'_id',
								channelToKeepId,
							);
							expect(response.body.channel).to.not.have.property(
								'teamId',
							);
						});
				})
				...
				.then(() =>
					request
						...
						.expect((response) => {
							...
							expect(response.body.members[1]).to.have.property(
								'user',
							);
							expect(response.body.members[1]).to.have.property(
								'roles',
							...
							const members = response.body.members.map(
								({ user, roles }) => ({
									_id: user._id,
		...
		it('should list all teams', (done) => {
			request
				...
				.expect((res) => {
					...
					expect(res.body.teams[0].createdBy).to.include.property(
						'_id',
					);
					expect(res.body.teams[0].createdBy).to.include.property(
						'username',
					...
					expect(res.body.teams[0]).to.include.property(
						'numberOfUsers',
					);
				})
				.end(done);
			...
			before('create team', (done) => {
				request
					...
					.expect((resp) => {
						...
						expect(resp.body.team).to.have.property(
							'name',
							tempTeamName,
						);
							...
							.then(() => {
								request
									...
									.expect((response) => {
										...
										).to.have.property('_id', channel1Id);
										expect(
											response.body.channel,
										).to.not.have.property('teamId');
									})
									.then(() => done());
							});
					})
			...
			updatePermission('add-team-channel', ['admin']).then(() => {
				request
					...
					.expect((res) => {
						...
						expect(res.body.rooms[1]).to.have.property(
							'teamId',
							publicTeam._id,
						);
			...
			updatePermission('edit-team-channel', ['admin']).then(() => {
				request
					...
					.expect((res) => {
						...
						expect(res.body.room).to.have.property(
							'teamId',
							publicTeam._id,
						);
						expect(res.body.room).to.have.property(
			...
			updatePermission('remove-team-channel', ['admin']).then(() => {
				request
					...
					.expect((res) => {
						...
						expect(res.body.room).to.not.have.property(
							'teamDefault',
						);
					})
					.end(done);
	...
	describe('/teams.update', () => {
		...
		it('should not update team if permissions are not met', async () => {
			...
			const unauthorizedUserCredentials = await login(
