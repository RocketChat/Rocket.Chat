...
describe('[Rooms]', function() {
	...
	describe('update group dms name', () => {
		...
		before(async () => {
			...
			const genius = 'genius';
			const usernames = [testUser.username, genius].join(',');
					...
					.end(() => {
						request.get(api('subscriptions.getOne'))
							...
							.end((err, res) => {
								...
								expect(subscription.name).to.equal(`genius,changed.username.${ testUser.username }`);
								done();
							});
					});
			});
				...
				request.post(api('users.update'))
					...
					.end(() => {
						request.get(api('subscriptions.getOne'))
							...
							.end((err, res) => {
								const { subscription } = res.body;
								expect(subscription.fname).to.equal(`changed.name.${ testUser.username }, Genius`);
								done();
							});
					});
			});
