...
describe('[Users]', function() {
		...
		[
			{
				...
				value: '',
				reason: 'is required and missing',
			},
			{
				name: 'customFieldText',
				value: '0',
				reason: 'length is less than minLength',
			},
			{
				name: 'customFieldText',
				value: '0123456789-0',
				reason: 'length is more than maxLength',
			},
		].forEach((field) => {
			...
			it('should create a new user with default roles', (done) => {
				...
				request
					...
					.expect((res) => {
						...
						expect(res.body.user.roles).to.have.members([
							'user',
							'admin',
						]);
					})
			...
			request
				...
				.expect((res) => {
					...
					expect(res.body)
						...
						.and.to.be.equal('Username is already in use');
				})
				.end(done);
		});
	});
		...
		it('should return "rooms" property when user request it and the user has the necessary permission (admin, "view-other-user-channels")', (done) => {
			request
				...
				.expect((res) => {
					expect(res.body).to.have.property('success', true);
					expect(res.body)
						.to.have.nested.property('user.rooms')
						.and.to.be.an('array');
			...
			updatePermission('view-other-user-channels', []).then(() => {
				request
					...
					.expect((res) => {
						...
						expect(res.body.user.rooms[0]).to.have.property(
							'unread',
						);
					})
					.end(done);
		...
		it('should query all users in the system', (done) => {
			request
				...
				.expect((res) => {
					...
					const myself = res.body.users.find(
	...
	describe('[/users.setAvatar]', () => {
		...
		before((done) => {
			updateSetting('Accounts_AllowUserAvatarChange', true).then(() => {
				updatePermission('edit-other-user-avatar', [
					'admin',
					'user',
				]).then(done);
			});
	...
	describe('[/users.setPreferences]', () => {
				...
				() => {
					request
						...
						.expect((res) => {
							...
							expect(res.body.user.settings).to.have.property(
								'preferences',
							);
		...
		it('should set some preferences and language preference by user when execute successfully', (done) => {
			...
			request
				...
				.expect((res) => {
					expect(res.body).to.have.property('success', true);
					expect(res.body.user.settings.preferences).to.have.property(
						'language',
						'en',
					);
				})
		...
		it('should assign a new owner to the room if the last room owner is deleted', async () => {
			...
			await request
				...
				.expect((res) => {
					...
					expect(res.body.roles[0].u).to.have.property(
						'_id',
						credentials['X-User-Id'],
					);
				});
		});
	});
	...
	describe('[/users.setActiveStatus]', () => {
		...
		before((done) => {
			updatePermission('edit-other-user-active-status', [
				'admin',
				'user',
			]).then(done);
		});
		...
		it('should set other user as room owner if the last owner of a room is deactivated and `confirmRelinquish` is set to `true`', async () => {
			...
			await request
				...
				.expect((res) => {
					...
					expect(res.body.roles[1].u).to.have.property(
						'_id',
						credentials['X-User-Id'],
					);
				});
		...
		it('should return the request data with fullExport false when no query parameter was send', (done) => {
			request
				...
				.expect((res) => {
					...
					expect(res.body)
						.to.have.property('exportOperation')
						.and.to.be.an('object');
					expect(res.body.exportOperation).to.have.property(
						'fullExport',
						false,
					);
				})
					...
					request
						...
						.expect((res) => {
							...
							expect(res.body.error).to.be.equal(
								"The 'selector' param is required",
							);
						})
						.end(done);
				},
				...
				() => {
					request
						...
						.expect((res) => {
							...
							expect(res.body.error).to.be.equal(
								'Change status is not allowed [error-not-allowed]',
							);
						})
						.end(done);
				},
			...
			updateSetting('Accounts_AllowUserStatusMessageChange', true).then(
				() => {
					request
						...
						.expect((res) => {
							expect(res.body).to.have.property('success', true);
							getUserStatus(credentials['X-User-Id']).then(
								(status) =>
