...
for (const target of targets) {
	...
	if (Object.keys(collectedExamples).length) {
		...
		writeFileSync(
			...
			[
				`/**`,
				` * Request and response examples for the ${moduleName} endpoints, imported from`,
				...
				`};`,
				'',
				`export const ${exampleName}: Record<string, PayloadExamples> = ${JSON.stringify(collectedExamples, null, '\t')};`,
				'',
			].join('\n'),
		);
