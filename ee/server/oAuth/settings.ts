...
export function addEnterpriseOAuthSettings(name: string, values: any): void {
	onLicense('oAuth-enterprise', () => {
		settings.add(`Accounts_OAuth_Custom-${ name }-groups_claim`, values.groupsClaim || 'groups', { type: 'string', group: 'OAuth', section: `Custom OAuth: ${ name }`, i18nLabel: 'Accounts_OAuth_Custom_Groups_Claim', enterprise: true, invalidValue: false });
		settings.add(`Accounts_OAuth_Custom-${ name }-roles_claim`, values.rolesClaim || 'roles', { type: 'string', group: 'OAuth', section: `Custom OAuth: ${ name }`, i18nLabel: 'Accounts_OAuth_Custom_Roles_Claim', enterprise: true, invalidValue: false });
