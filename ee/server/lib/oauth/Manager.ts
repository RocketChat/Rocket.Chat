...
export class OAuthEEManager {
				...
				if (typeof ssoGroup === 'string') {
					...
					for (const channel of channels) {
						...
						if (!room) {
							...
							if (!room || !room.rid) {
								logger.error(
									`could not create channel ${ channel }`,
								);
								return;
							}
						}
	...
	static updateRolesFromSSO(
		...
		if (user && identity && roleClaimName) {
			...
			const toRemove = user.roles.filter(
				(val: any) =>
					!rolesFromSSO.includes(val) && rolesToSync.includes(val),
			);
			...
			const toAdd = rolesFromSSO.filter(
				(val: any) =>
					...
					&& (!rolesToSync.length || rolesToSync.includes(val)),
			);
	...
	static mapRolesFromSSO(
		...
		if (identity && roleClaimName) {
			// Adding roles
			if (
				identity[roleClaimName]
				&& Array.isArray(identity[roleClaimName])
			) {
