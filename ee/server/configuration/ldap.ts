...
Meteor.startup(() =>
	onLicense('ldap-enterprise', () => {
		...
		function configureBackgroundSync(
			...
			return function addCronJobDebounced(): void {
				if (
					...
					|| settings.get(enableSetting) !== true
				) {
		...
		settings.watchMultiple(
			['LDAP_Background_Sync', 'LDAP_Background_Sync_Interval'],
			addCronJob,
		);
		settings.watchMultiple(
			[
			...
			(
				{
					...
					ldapUser,
					isNewUser,
				}: { user: IUser; ldapUser: ILDAPEntry; isNewUser: boolean },
				ldap: LDAPConnection,
			) => {
				Promise.await(
					LDAPEEManager.advancedSyncForUser(
						ldap,
						user,
						isNewUser,
						ldapUser.dn,
