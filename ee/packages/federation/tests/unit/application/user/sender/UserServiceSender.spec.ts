...
const { FederationUserServiceSender } = proxyquire.noCallThru().load('../../../../../src/application/user/sender/UserServiceSender', {
	mongodb: {
...
const { FederatedRoom } = proxyquire.noCallThru().load('../../../../../src/domain/FederatedRoom', {
	mongodb: {
		'ObjectId': class ObjectId {
			toHexString(): string {
				return 'hexString';
