...
import { VerificationStatus } from '../../../../src/infrastructure/matrix/helpers/MatrixIdVerificationTypes';

const fetchStub = {
	serverFetch: () => Promise.resolve({}),
};

const { MatrixBridge } = proxyquire.noCallThru().load('../../../../src/infrastructure/matrix/Bridge', {
...
describe('Federation - Infrastructure - Matrix - Bridge', () => {
	...
	describe('#isUserIdFromTheSameHomeserver()', () => {
		it('should return true if the userId is from the same homeserver', () => {
			expect(bridge.isUserIdFromTheSameHomeserver('@user:server.com', 'server.com', 'different:server.com')).to.be.true;
		});

		it('should return false if the userId is from a different homeserver', () => {
			expect(bridge.isUserIdFromTheSameHomeserver('@user:server2.com', 'server.com', 'different:server2.com')).to.be.false;
		});
	...
	describe('#extractHomeserverOrigin()', () => {
		it('should return the proxy homeserver origin if there is no server in the userId', () => {
			expect(bridge.extractHomeserverOrigin('@user', defaultProxyDomain)).to.be.equal(defaultProxyDomain);
		});
