...
export class RocketChatMessageAdapter {
	public async sendMessage(
		...
		_room: FederatedRoom,
		_rawMessage: string,
		_externalFormattedMessage: string,
		_externalEventId: string,
		_homeServerDomain: string,
	): Promise<void> {
		// await sendMessage(
	...
	public async sendQuoteMessage(
		_user: FederatedUser,
		_federatedRoom: FederatedRoom,
		_externalFormattedText: string,
		_rawMessage: string,
		_externalEventId: string,
		_messageToReplyTo: IMessage,
		_homeServerDomain: string,
	): Promise<void> {
	...
	public async editMessage(
		_user: FederatedUser,
		_newRawMessageText: string,
		_newExternalFormattedMessage: string,
		_originalMessage: IMessage,
		_homeServerDomain: string,
	): Promise<void> {
	...
	public async sendFileMessage(
		_user: FederatedUser,
		_room: FederatedRoom,
		_files: IMessage['files'],
		_attachments: IMessage['attachments'],
		_externalEventId: string,
	): Promise<void> {
		// await sendMessage(
		// 	user.getInternalReference(),
	...
	public async deleteMessage(_message: IMessage, _user: FederatedUser): Promise<void> {
		// await deleteMessage(message, user.getInternalReference());
	}

	public async reactToMessage(_user: FederatedUser, _message: IMessage, _reaction: string, _externalEventId: string): Promise<void> {
		// try {
	...
	public async unreactToMessage(_user: FederatedUser, _message: IMessage, _reaction: string, _externalEventId: string): Promise<void> {
		// await executeSetReaction(user.getInternalId(), reaction, message._id);
