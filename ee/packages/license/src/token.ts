...
const convertV2KeyToV3 = (key: string): string => Buffer.from(key, 'base64').toString('utf-8');
...
const PUBLIC_LICENSE_KEY_V3 = convertV2KeyToV3(PUBLIC_LICENSE_KEY_V2);
...
export async function decryptStatsToken(encrypted: string): Promise<string> {
	...
	const [payload] = await verify(encrypted, convertV2KeyToV3(PUBLIC_STATS_KEY_V2));
