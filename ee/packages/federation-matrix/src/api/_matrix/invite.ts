...
import { NotAllowedError, federationSDK, eventIdSchema } from '@rocket.chat/federation-sdk';
...
export const getMatrixInviteRoutes = () => {
	...
	return new Router('/federation').put(
		...
		async (c) => {
			...
			try {
				const inviteEvent = await federationSDK.processInvite(event, eventIdSchema.parse(eventId), roomVersion);
