...
export class FederationMatrix extends ServiceClass implements IFederationMatrixService {
	...
	@traced((room: IRoomNativeFederated, unbannedUser: IUser, userWhoUnbanned: IUser) => ({
		roomId: room?._id,
		matrixRoomId: room?.federation?.mrid,
		unbannedUsername: unbannedUser?.username,
		performerUsername: userWhoUnbanned?.username,
	}))
	async unbanUser(room: IRoomNativeFederated, unbannedUser: IUser, userWhoUnbanned: IUser): Promise<void> {
		try {
			...
			addSpanAttributes({
				unbannedMatrixUserId: actualUnbannedMatrixUserId,
				senderMatrixUserId: actualSenderMatrixUserId,
			});
	...
	@traced((room: IRoomNativeFederated, bannedUser: IUser, userWhoBanned: IUser) => ({
		roomId: room?._id,
		matrixRoomId: room?.federation?.mrid,
		bannedUsername: bannedUser?.username,
		performerUsername: userWhoBanned?.username,
	}))
	async banUser(room: IRoomNativeFederated, bannedUser: IUser, userWhoBanned: IUser): Promise<void> {
		try {
			...
			addSpanAttributes({
				bannedMatrixUserId: actualBannedMatrixUserId,
				senderMatrixUserId: actualSenderMatrixUserId,
			});
