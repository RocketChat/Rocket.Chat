...
(IS_EE ? describe : describe.skip)('Federation', () => {
	...
	beforeAll(async () => {
		...
		rc1User1RequestConfig = await getRequestConfig(federationConfig.rc1.apiUrl, rc1User1Username, rc1User1Password);
		...
		hs1User1App = new SynapseClient(federationConfig.hs1.url, hs1User1MatrixId, hs1User1Password);
		...
		describe('Create a room on RC as private and federated and: ', () => {
			...
			describe('Add 2 or more federated users in the creation modal', () => {
				...
				beforeAll(async () => {
					...
					const createResponse = await createRoom({
						type: 'p',
						name: channelName,
						members: [federationConfig.hs1.adminMatrixUserId, hs1User1MatrixId],
						extraData: {
							federated: true,
						},
						config: rc1AdminRequestConfig,
				...
				it('It should show the new users in the members list of all RCs involved', async () => {
					...
					const hs1User1InRC = await findRoomMember(federatedChannel._id, hs1User1MatrixId, {}, rc1AdminRequestConfig);
					...
					const hs1User1InSynapseAdmin = await hs1AdminApp.findRoomMember(channelName, hs1User1MatrixId, { initialDelay: 2000 });
					...
					const hs1User1InSynapseUser1 = await hs1User1App.findRoomMember(channelName, hs1User1MatrixId);
				...
				it('It should show the system messages that the user added on all RCs involved', async () => {
					...
					const hs1User1JoinMessage = historyResponse.messages.find(
						(message: IMessage) => message.t === 'uj' && message.msg && message.msg === hs1User1MatrixId,
					);
			...
			describe('Add 1 federated user and 1 local user in the creation modal', () => {
				...
				it('It should show the 2 new users in the members list', async () => {
					...
					const rc1AdminUserInRC = await findRoomMember(federatedChannel._id, federationConfig.rc1.adminUser, {}, rc1AdminRequestConfig);
					const rc1User1InRC = await findRoomMember(federatedChannel._id, rc1User1Username, {}, rc1AdminRequestConfig);
					const hs1AdminUserInRC = await findRoomMember(
						federatedChannel._id,
						federationConfig.hs1.adminMatrixUserId,
						{},
					...
					const rc1User1InSynapse = await hs1AdminApp.findRoomMember(channelName, rc1User1MatrixId, { initialDelay: 2000 });
					const hs1AdminUserInSynapse = await hs1AdminApp.findRoomMember(channelName, federationConfig.hs1.adminMatrixUserId, {
						initialDelay: 2000,
					});
				...
				it('It should show the 2 system messages that the user added', async () => {
					...
					const localUserJoinMessage = historyResponse.messages.find(
						(message: IMessage) => message.t === 'uj' && message.msg && message.msg === rc1User1Username,
					);
