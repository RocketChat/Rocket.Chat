...
(IS_EE ? describe : describe.skip)('Federation Ban/Unban', () => {
	...
	const rc1User1Name = `fed-ban-user-${Date.now()}`;
	...
	describe('Ban from RC side', () => {
		...
		beforeAll(async () => {
			...
			await retry(
				...
				async () => {
					const member = await hs1AdminApp.findRoomMember(channelName, rc1User1MatrixId);
	...
	describe('Ban and unban invited user (invite status preservation)', () => {
		let channelName: string;
		let federatedChannelId: string;

		beforeAll(async () => {
			channelName = `fed-ban-invite-${Date.now()}`;
		...
		it('should have the user listed as banned', async () => {
			const response = await rc1AdminRequestConfig.request
				.get(api('rooms.bannedUsers'))
				.set(rc1AdminRequestConfig.credentials)
				.query({ roomId: federatedChannelId });
		...
		it('should unban the user (removing the subscription entirely)', async () => {
			const response = await rc1AdminRequestConfig.request.post(api('rooms.unbanUser')).set(rc1AdminRequestConfig.credentials).send({
				roomId: federatedChannelId,
				userId: rc1User1RequestConfig.credentials['X-User-Id'],
			});
		...
		it('should have no subscription after unban (user must be re-invited)', async () => {
	...
	describe('Ban from Synapse side', () => {
		...
		beforeAll(async () => {
			...
			await hs1AdminApp.inviteUserToRoom(synapseRoomId, rc1User1MatrixId);
		...
		it('should ban the RC user from Synapse', async () => {
			await hs1AdminApp.banUser(synapseRoomId, rc1User1MatrixId, 'federation ban test');
		});
		...
		it('should unban the RC user from Synapse', async () => {
			await hs1AdminApp.unbanUser(synapseRoomId, rc1User1MatrixId);
		});
	...
	describe('Ban from Synapse, re-invite and messaging after unban', () => {
		...
		it('should ban the RC user from Synapse', async () => {
			await hs1AdminApp.banUser(synapseRoomId, rc1User1MatrixId, 'ban before re-invite test');
