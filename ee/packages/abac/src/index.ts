...
export class AbacService extends ServiceClass implements IAbacService {
	...
	async setRoomAbacAttributes(rid: string, attributes: Record<string, string[]>): Promise<void> {
		const room = await Rooms.findOneByIdAndType<Pick<IRoom, '_id' | 'abacAttributes' | 't' | 'teamMain' | 'teamDefault' | 'default'>>(rid, 'p', {
	...
	async removeRoomAbacAttribute(rid: string, key: string): Promise<void> {
		const room = await Rooms.findOneByIdAndType<Pick<IRoom, '_id' | 'abacAttributes' | 'teamDefault' | 'default'>>(rid, 'p', { projection: { abacAttributes: 1, default: 1, teamDefault: 1 } });
		if (!room) {
			throw new Error('error-room-not-found');
		}
	...
	async addRoomAbacAttributeByKey(rid: string, key: string, values: string[]): Promise<void> {
		...
		const room = await Rooms.findOneByIdAndType<Pick<IRoom, '_id' | 'abacAttributes' | 't' | 'teamMain' | 'default' | 'teamDefault'>>(rid, 'p', {
			projection: { abacAttributes: 1, t: 1, teamMain: 1, teamDefault: 1, default: 1 },
		});
		if (!room) {
			throw new Error('error-room-not-found');
