...
describe('virtru/identity', () => {
	...
	it('buildAttributeFqns round-trips with parseAttributeFqns', () => {
		...
		expect(parseAttributeFqns(fqns)).toEqual([{ key: 'clearance', values: ['secret', 'topsecret'] }]);
	});
	...
	it('parseAttributeFqns ignores malformed FQNs', () => {
		expect(parseAttributeFqns(['not-an-fqn', 'https://x/attr/k/value/v'])).toEqual([{ key: 'k', values: ['v'] }]);
	});
});
