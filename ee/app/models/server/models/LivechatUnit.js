...
const addQueryRestrictions = (originalQuery = {}) => {
	...
	if (Array.isArray(units)) {
		...
		const condition = {
			$or: [{ ancestors: { $in: units } }, { _id: { $in: units } }],
		};
...
export class LivechatUnit extends LivechatDepartment {
	...
	createOrUpdateUnit(
		...
		const savedMonitors = _.pluck(
			LivechatUnitMonitors.findByUnitId(_id).fetch(),
			'monitorId',
		);
		...
		_.difference(savedDepartments, departmentsToSave).forEach(
			(departmentId) => {
