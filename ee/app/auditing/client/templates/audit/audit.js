...
const loadMessages = async function({ rid, users, startDate, endDate = new Date(), msg, type, visitor, agent }) {
	...
	try {
		...
		let messages;
		if (type === 'l') {
			messages = await call('auditGetOmnichannelMessages', { rid, users, startDate, endDate, msg, type, visitor, agent });
		} else {
			messages = await call('auditGetMessages', { rid, users, startDate, endDate, msg, type, visitor, agent });
		}
