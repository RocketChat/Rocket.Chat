...
class CustomBusinessHour
	...
	async saveBusinessHour(
		...
		if (
			...
			&& existingBusinessHour._id !== businessHour._id
		) {
		...
		const businessHourToReturn = {
			...businessHourData,
			departmentsToApplyBusinessHour,
		};
		delete businessHourData.departments;
		const businessHourId = await this.baseSaveBusinessHour(
			businessHourData,
		);
		const currentDepartments = (
			await this.DepartmentsRepository.findByBusinessHourId(
				businessHourId,
				{
					projection: { _id: 1 },
				},
		...
		const toRemove = [
			...currentDepartments.filter((dept) => !departments.includes(dept)),
		];
		const toAdd = [
			...departments.filter(
				(dept: string) => !currentDepartments.includes(dept),
			),
		];
		await this.removeBusinessHourFromDepartmentsIfNeeded(
			businessHourId,
			toRemove,
		);
	...
	private async removeBusinessHourFromAgents(
		...
		this.UsersRepository.removeBusinessHourByAgentIds(
			agentIds,
			businessHourId,
		);
	}
