...
export class MultipleBusinessHoursBehavior
	...
	async onStartBusinessHours(): Promise<void> {
		...
		const currentTime = moment.utc(
			moment().utc().format('dddd:HH:mm'),
			'dddd:HH:mm',
		);
		const day = currentTime.format('dddd');
		const activeBusinessHours =			await this.BusinessHourRepository.findActiveAndOpenBusinessHoursByDay(
			day,
			{
				fields: {
					workHours: 1,
		...
		const businessHoursToOpen = await filterBusinessHoursThatMustBeOpened(
			activeBusinessHours,
		);
	...
	async closeBusinessHoursByDayAndHour(
		day: string,
		hour: string,
	): Promise<void> {
	...
	async afterSaveBusinessHours(
		businessHourData: IBusinessHoursExtraProperties,
	): Promise<void> {
		const departments = businessHourData.departmentsToApplyBusinessHour
			?.split(',')
			.filter(Boolean);
		const currentDepartments = businessHourData.departments?.map(
			(dept: any) => dept._id
		);
		const toRemove = [
		...
		const businessHour = await this.BusinessHourRepository.findOneById(
			businessHourData._id,
		);
		if (!businessHour) {
			return;
		}
		const businessHourIdToOpen = (
			await filterBusinessHoursThatMustBeOpened([businessHour])
	...
	async onAddAgentToDepartment(
		...
		const businessHourToOpen = await filterBusinessHoursThatMustBeOpened([
			businessHour,
		]);
	...
	async onRemoveAgentFromDepartment(
		...
		return this.handleRemoveAgentsFromDepartments(
			department,
			agentsId,
			options,
		);
	...
	async onRemoveDepartment(options: Record<string, any> = {}): Promise<any> {
		...
		const deletedDepartment = LivechatDepartment.trashFindOneById(
			department._id,
		);
		return this.handleRemoveAgentsFromDepartments(
			deletedDepartment,
	...
	private async handleRemoveAgentsFromDepartments(
		...
		for await (const agentId of agentsIds) {
			if (
				await this.DepartmentsAgentsRepository.findByAgentId(
					agentId,
				).count() === 0
			) {
	...
	private async openBusinessHour(
	...
	private async removeBusinessHourFromRemovedDepartmentsUsersIfNeeded(
		...
		const agentIds = (
			await this.DepartmentsAgentsRepository.findByDepartmentIds(
				departmentsToRemove,
			).toArray()
	...
	private async closeBusinessHour(
		businessHour: Record<string, any>,
	): Promise<void> {
