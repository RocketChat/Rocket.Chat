...
callbacks.add(
	...
	(_: any, { departmentId }: { departmentId?: string }) => {
		if (departmentId) {
			const departmentLimit =				LivechatDepartment.findOneById(departmentId)
					?.maxNumberSimultaneousChat || 0;
			if (departmentLimit > 0) {
				cbLogger.debug(
					`Applying department filters. Max chats per department ${ departmentLimit }`,
				);
				return {
					$match: {
						'queueInfo.chats': { $gte: Number(departmentLimit) },
					},
				};
			}
		}

		const maxChatsPerSetting = settings.get(
