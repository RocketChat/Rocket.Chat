...
const handleOnAgentAssignmentFailed = async ({
	...
	if (room.onHold) {
		cbLogger.debug(
			'Room is on hold. Removing current assignations before queueing again',
		);
	...
	if (!forwardingToDepartment) {
		cbLogger.debug(
			'Skipping callback. Room not being forwarded to department',
		);
		return;
	}
	...
	if (
		...
		cbLogger.debug(
			'Skipping callback. New and old departments are the same',
		);
		return;
	}
