...
callbacks.add(
	...
	async ({
		...
		if (allowAgentSkipQueue(agent)) {
			cbLogger.debug(
				`Callback success. Agent ${ agent.agentId } can skip queue`,
			);
			return agent;
		}
		...
		if (maxNumberSimultaneousChat === 0) {
			cbLogger.debug(
				`Callback success. Agent ${ agentId } max number simultaneous chats on range`,
			);
			return agent;
		}
		...
		if (maxNumberSimultaneousChat <= chats) {
			cbLogger.debug(
				'Callback with error. Agent reached max amount of simultaneous chats',
			);
			callbacks.run(
		...
		cbLogger.debug(
			`Callback success. Agent ${ agentId } can take inquiry ${ inquiry._id }`,
		);
		return agent;
	},
