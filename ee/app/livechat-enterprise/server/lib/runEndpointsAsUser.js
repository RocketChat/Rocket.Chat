...
export function runEndpointsAsUser({
	originalAPIRoutes,
	endpointsToRunAsUser,
}) {
	originalAPIRoutes
		.filter((route) =>
			Object.keys(endpointsToRunAsUser).includes(route.path),
		)
		.forEach((route) =>
			replaceOriginalActionToRunAsUser(route, endpointsToRunAsUser),
		);
}
