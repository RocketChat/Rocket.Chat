...
const CannedResponsesForm = ({ values, handlers, errors }) => {
	...
	const { handleShortcut, handleText } = handlers;
	...
	return (
		<FieldGroup>
			<Field>
				...
				{errors && errors.shortcut && <Field.Error>{errors.shortcut}</Field.Error>}
			</Field>
			<Field>
				...
				{errors && errors.text && <Field.Error>{errors.text}</Field.Error>}
			</Field>
		</FieldGroup>
	);
};
