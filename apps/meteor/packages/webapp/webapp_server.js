...
import { removeExistingSocketFile, registerSocketFileCleanup } from './socket_file.js';
		...
		Object.assign(
			{
				...
				baseDataExtension: {
					...
					meteorRuntimeConfig,
					meteorRuntimeHash: sha256(meteorRuntimeConfig),
...
WebAppInternals.staticFilesMiddleware = async function (staticFilesByArch, req, res, next) {
	...
	var serveStaticJs = function (s) {
		if (req.method === 'GET' || req.method === 'HEAD' || Meteor.settings.packages?.webapp?.alwaysReturnContent) {
			res.writeHead(200, {
	...
	if (req.method !== 'HEAD' && req.method !== 'GET' && !Meteor.settings.packages?.webapp?.alwaysReturnContent) {
		const status = req.method === 'OPTIONS' ? 200 : 405;
		res.writeHead(status, {
			'Allow': 'OPTIONS, GET, HEAD',
			'Content-Length': '0',
...
async function runWebAppServer() {
	...
	function generateClientProgram(arch, staticFilesByArch = WebAppInternals.staticFilesByArch) {
		...
		const newProgram = (WebApp.clientPrograms[arch] = {
			...
			versionNonRefreshable: () =>
				WebAppHashing.calculateClientHash(manifest, (type, replaceable) => type !== 'css' && !replaceable, configOverrides),
	...
	app.use(async function (req, res, next) {
		if (!appUrl(req.url)) {
			return next();
		} else if (req.method !== 'HEAD' && req.method !== 'GET' && !Meteor.settings.packages?.webapp?.alwaysReturnContent) {
			const status = req.method === 'OPTIONS' ? 200 : 405;
			res.writeHead(status, {
				'Allow': 'OPTIONS, GET, HEAD',
				'Content-Length': '0',
	...
	Object.assign(WebApp, {
		...
		suppressConnectErrors: () => {
			if (!warnedAboutConnectUsage) {
				Meteor._debug(
					'WebApp.suppressConnectErrors has been renamed to Meteor._suppressExpressErrors and it should be used only in tests.',
				);
