...
Meteor.startup(() => {
	onLoggedIn(() => {
		...
		return sdk.stream('notify-user', [`${accounts.getUserId()}/message`], (msg: IMessage) => {
	...
	onLoggedIn(() => {
		return sdk.stream('notify-user', [`${accounts.getUserId()}/subscriptions-changed`], (_action, sub) => {
