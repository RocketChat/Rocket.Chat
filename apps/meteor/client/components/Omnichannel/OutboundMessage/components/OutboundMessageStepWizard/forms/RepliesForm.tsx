...
import AutoCompleteAgent from '../../AutoCompletAgentLocal';
...
const RepliesForm = forwardRef<RepliesFormRef, RepliesFormProps>((props, ref) => {
	...
	const submit = useEffectEvent(async ({ agentId, departmentId }: RepliesFormData) => {
		if ((departmentId && !department) || (agentId && !agent)) {
			throw Error('error-department-agent-not-found');
		}

		const payload = { departmentId, department, agentId, agent };
	...
	return (
		<form id={repliesFormId} onSubmit={handleSubmit(submit)}>
			<FieldGroup>
				<Field>
					<FieldLabel id={`${repliesFormId}-department`}>{`${t('Department')} (${t('optional')})`}</FieldLabel>
					<FieldRow>
						<Controller
							control={control}
							name='departmentId'
							rules={{
								validate: () => isErrorDepartment && t('Error_loading__name__information', t('department')),
							}}
							render={({ field }) => (
								<AutoCompleteDepartment
									aria-invalid={!!errors.departmentId}
									aria-labelledby={`${repliesFormId}-department`}
									aria-describedby={errors.departmentId && `${repliesFormId}-department-error`}
				...
				<Field>
					...
					<FieldRow>
						<Controller
							...
							name='agentId'
							rules={{
								validate: () => isErrorAgent && t('Error_loading__name__information', t('agent')),
							}}
							render={({ field }) => (
								<AutoCompleteAgent
									aria-invalid={!!errors.agentId}
									aria-describedby={errors.agentId && `${repliesFormId}-agent-error`}
