...
import { playHandRaiseChime, playJoinChime, playMutedReminder, type RemoteParticipantInfo } from '@rocket.chat/ui-voip';
...
import type { ActiveLiveKitCall } from './LiveKitVideoConfContext';
...
type LiveKitRoomHostProps = {
	serverUrl: string;
	token: string;
	callId: string;
	preferences: ActiveLiveKitCall['preferences'];
	onLeave: () => void;
	onContextChange: (value: unknown) => void;
	onDiagnosticsChange: (value: unknown) => void;
};
...
/**
 * The LiveKit room itself, and the only module in the client that pulls the LiveKit SDK.
 *
 * It lives apart from the bridge so that `livekit-client` and `@livekit/components-react` are fetched when a
 * call actually starts rather than on every page load. The bridge used to import this code directly, which put
 * roughly four megabytes of SDK in the boot graph of a channel, of admin, of the login screen — everything —
 * for a feature only a call uses.
 *
 * Mounted inside the bridge's detached portal, so the room is a sibling of the app rather than an ancestor:
 * nothing above it remounts when a call starts or ends.
 */
const LiveKitRoomHost = ({
	serverUrl,
	token,
	callId,
	preferences,
	...
	onContextChange,
	onDiagnosticsChange,
}: LiveKitRoomHostProps) => (
	// Apply preflight mic/cam preferences from the VC popup as the initial `audio` / `video` flags so the
	// LiveKitRoom publishes (or skips) tracks according to what the user chose. Defaults match the legacy
	...
	<LiveKitRoom
		...
		audio={preferences?.mic ?? true}
		video={preferences?.cam ?? false}
		options={captureDefaults(preferences)}
...
export default LiveKitRoomHost;
