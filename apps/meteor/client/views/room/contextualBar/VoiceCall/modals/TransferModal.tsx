...
export const VoiceCallTransferModal = ({ isLoading = false, onCancel, onConfirm }: VoiceCallTransferModalProps) => {
	const user = useUser();
	...
	const { data: targetUser, isLoading: isTargetInfoLoading } = useQuery(['/v1/users.info', username], () => getUserInfo({ username }), {
				...
				<Field>
					...
					<FieldRow>
						<UserAutoComplete
							...
							multiple={false}
							conditions={{
								freeSwitchExtension: { $exists: true },
