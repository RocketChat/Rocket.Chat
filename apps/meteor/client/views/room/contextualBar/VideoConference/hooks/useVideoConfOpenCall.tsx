...
// Shared window name for in-product (same-origin) conferences, so we never stack duplicate tabs even if
		...
		(callUrl: string, providerName?: string) => {
			...
			if (!desktopApp?.openInternalVideoChatWindow) {
				const open = () => {
					...
					// The conference tab is same-origin, so check what it's *actually* showing rather than the URL
					// we last passed (which can differ in string form between the start/join paths). If it's already
					// on this conference, focus it without reloading (empty URL = no navigation).
					if (isAlive) {
