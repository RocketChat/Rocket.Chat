...
export function useOpenRoomById(rid: IRoom['_id']) {
	...
	const tryCacheShortcut = useCallback((): { rid: IRoom['_id'] } | undefined => {
		...
		if (sub && sub.open === false) return undefined;
		return { rid };
	}, [rid, user?._id]);
	...
	return useQuery({
		...
		queryFn: async (): Promise<{ rid: IRoom['_id'] }> => {
			...
			// Subscriptions.state may be empty when used without a pre-populating parent (e.g. ConferenceRoomPreload).
			// Fetch the subscription as a fallback so openRoom.mutateAsync is not silently skipped.
