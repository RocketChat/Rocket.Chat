...
export const useChangeLeaderAction = (user: Pick<IUser, '_id' | 'username'>, rid: IRoom['_id']): UserInfoAction | undefined => {
	...
	const changeLeaderAction = useEffectEvent(async () =>
		toggleOwnerMutation.mutateAsync({ roomId: rid, userId: uid }).then(() => queryClient.invalidateQueries({ queryKey: ['roomRoles'] })),
	);
