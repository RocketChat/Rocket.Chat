...
export const useLiveLocationStopListener = () => {
	...
	const stopLiveLocationSharing = useCallback(
		async (rid?: string, messageId?: string, currentPosition?: GeolocationPosition) => {
	...
	const handleStopCommand = useCallback(
		async (rid: string) => {
			...
			return stopLiveLocationSharing(rid, messageId);
		},
...
export const handleLiveLocationAction = (action: any, message: any, handleStopCommand: (rid: string) => Promise<boolean>): boolean => {
