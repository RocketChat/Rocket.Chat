...
const LiveLocationChatWidget = ({ rid, tmid, onClose }: Props): ReactElement => {
	...
	const [, setGpsUpdateCount] = useState(0);
	...
	const createLiveLocationAttachment = useCallback(
		(pos: GeolocationPosition, isLive = true) => {
	...
	const getStatusIcon = () => {
		...
		if (locationState === 'sharing') return '📍';
		return '❌';
	};
	const getStatusText = () => {
		if (locationState === 'waiting') return 'Getting location...';
		if (locationState === 'sharing') return 'Live sharing active';
		return 'Location error';
	};

	// Provider-aware preview (clean theme + retina)
	const previewUrl = position
		? cacheBust(
				...
				<div style={{ padding: 16 }}>
					{locationState === 'error' && (
						<div style={{ color: '#dc3545', marginBottom: 12 }}>
							...
							<p style={{ margin: '4px 0 0 0', fontSize: 12, color: '#666' }}>Please enable location access in your browser settings.</p>
						</div>
					)}
