...
const ShareLocationModal = ({ rid, tmid, onClose }: ShareLocationModalProps): ReactElement => {
	...
	// Provider instance
	const map: IMapProvider = useMemo(
		() =>
		...
		return (
			...
			>
				<div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
					...
					<label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
						<input type='radio' name='provider' value='google' checked={provider === 'google'} onChange={() => setProvider('google')} />
						Google
					</label>
					<div style={{ fontSize: 12, color: '#666' }}>You can change this later. Your choice is saved for live sharing too.</div>
				</div>
			</GenericModal>
		);
	}
		...
		if (permissionState === 'granted' && positionError) {
			return (
				<GenericModal
					...
					onCancel={onClose}
				>
					<div style={{ marginBottom: 16 }}>{(positionErr as Error | undefined)?.message || 'Unable to fetch your current location.'}</div>
