...
export interface ILocationServiceConfig {
...
export class LiveLocationService {
	private config: ILocationServiceConfig;
	...
	constructor(config: ILocationServiceConfig) {
		this.config = config;
	...
	private setupDevelopmentMock() {
		if (process.env.NODE_ENV === 'development') {
			...
			let lon = -74.006; // starting longitude (NYC)

			navigator.geolocation.getCurrentPosition = (success, _error) => {
				success({
					coords: {
						latitude: lat,
						longitude: lon,
	...
	createLocationAttachment(pos: GeolocationPosition, isLive = false) {
	...
	startWatching(onSuccess: (position: GeolocationPosition) => void, onError: (error: GeolocationPositionError) => void) {
		...
		this.watchId = navigator.geolocation.watchPosition(onSuccess, onError, { enableHighAccuracy: true, timeout: 20000, maximumAge: 0 });
