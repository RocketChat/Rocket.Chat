...
import { Field, FieldGroup, FieldLabel, FieldRow, FieldError, TextInput, TextAreaInput, Icon } from '@rocket.chat/fuselage';
...
interface IEditScheduledMessageModalProps {
...
const EditScheduledMessageModal = ({ message, onSuccess, onClose, updateScheduledMessage }: IEditScheduledMessageModalProps) => {
	...
	const onSubmit = async ({ msg, date, time }: { msg: string; date: string; time: string }) => {
		try {
			...
			dispatchToastMessage({
				type: 'error',
				message: error instanceof Error ? error.message : t('Failed_to_update_scheduled_message'),
			});
		}
	};
		...
		>
			<FieldGroup mbs={16}>
				<Field>
					...
					<FieldRow>
						<Controller
							...
							rules={{
								...
								validate: {
									notEmpty: (value) => value.trim().length > 0 || t('Message_cannot_be_empty'),
									notOnlyWhitespace: (value) => value.trim().length > 0 || t('Message_cannot_be_only_whitespace'),
								},
							}}
							render={({ field }) => (
								<TextAreaInput
					...
					{errors.msg && <FieldError id={messageFieldErrorId}>{errors.msg.message}</FieldError>}
				</Field>
