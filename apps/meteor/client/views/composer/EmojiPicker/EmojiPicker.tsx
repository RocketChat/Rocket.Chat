import { Box, PositionAnimated, Field, TextInput, Icon } from '@rocket.chat/fuselage';
...
const EmojiPicker = ({ reference, composer, onClose }) => {
	...
	const createPickerEmojis = useCallback(() => {
		...
		setEmojiListByCategory(mappedCategories);
	}, [actualTone, customItemsLimit]);
	...
	useEffect(() => {
		...
		createPickerEmojis();
	}, [actualTone, recentEmojis, createPickerEmojis]);
	...
	const handleSelectEmoji = (event: MouseEvent<ButtonElement>) => {
		...
		composer?.insertText(` :${_emoji + tone}: `);
	...
	return (
		<PositionAnimated visible='visible' anchor={reference} placement='top-start'>
			<Box width='x365' height='x300' bg='light' padding='x8' borderRadius={4} elevation='2' display='flex' flexDirection='column'>
				...
				<Box color='default' className='filter' mbe='x8'>
					<Box is='ul' display='flex'>
						{emojiCategories.map((category) => (
							<EmojiPickerCategoryItem key={category.key} category={category} activeCategory={activeCategory} setSearching={setSearching} />
						))}
					</Box>
				</Box>
