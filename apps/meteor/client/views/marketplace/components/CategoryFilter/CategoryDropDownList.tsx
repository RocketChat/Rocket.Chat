...
const CategoryDropDownList = ({ categories, onSelected }: CategoryDropDownListProps) => {
	return (
		<Tile overflow='auto' paddingBlock={12} pi={0} elevation='2' width='full' backgroundColor='light' borderRadius={2}>
			{categories.map((category, index) => (
				<Fragment key={index}>
					{category.label && (
						<Box pi={16} paddingBlockStart={8} paddingBlockEnd={4} fontScale='micro' textTransform='uppercase' color='default'>
							{category.label}
						</Box>
					)}
