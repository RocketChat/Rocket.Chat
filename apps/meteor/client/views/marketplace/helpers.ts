...
type appButtonPropsType = App & { isAdminUser: boolean; hasEndUserRequest?: boolean };
...
export const appButtonProps = ({
	...
	const hasRequestedEndUser = hasEndUserRequest ?? requestedEndUser;

	if (!isAdminUser) {
		if (hasRequestedEndUser) {
			return {
				action: 'request',
				label: 'Requested',
			};
