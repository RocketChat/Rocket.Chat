...
type AppsPagePremiumContentProps = {
  appsResult: {
...
const AppsPagePremiumContent = ({
...
}: AppsPagePremiumContentProps): ReactElement => {
  ...
  const fixFiltersSize = breakpoints.includes('lg') ? { maxWidth: 'x200', minWidth: 'x200' } : null;
	...
	return (
      ...
      {/* Filters */}
      <Box pi={24}>
        <FilterByText placeholder={t('Search_Premium_Apps')} onChange={({ text }): void => setText(text)}>
...
export default AppsPagePremiumContent;
