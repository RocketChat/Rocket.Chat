...
export const useExternalRouteNavigation = () => {
	...
	useEffect(() => {
		...
		const desktop: IRocketChatDesktop | undefined = window.RocketChatDesktop;
		if (desktop?.onNavigateToRoute) {
			desktop.onNavigateToRoute((path) => router.navigate(path as LocationPathname));
		}
