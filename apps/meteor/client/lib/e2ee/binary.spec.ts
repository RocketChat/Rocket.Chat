...
describe('Binary', () => {
	describe('toString', () => {
		it('should convert ArrayBuffer to string', () => {
			...
			const result = Binary.toString(array.buffer);
			expect(result).toBe('Hello');
		});

		it('should handle empty ArrayBuffer', () => {
			const buffer = new ArrayBuffer(0);
			const result = Binary.toString(buffer);
			expect(result).toBe('');
		});
	});
	...
	describe('toArrayBuffer', () => {
		it('should convert string to ArrayBuffer', () => {
			const str = 'Hello';
			const buffer = Binary.toArrayBuffer(str);
		...
		it('should throw RangeError for illegal char code', () => {
			...
			expect(() => Binary.toArrayBuffer(str)).toThrowErrorMatchingInlineSnapshot(`"illegal char code: 256"`);
		});
	});
});
