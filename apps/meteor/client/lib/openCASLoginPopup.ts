...
const getPopupUrl = (credentialToken: string): string => {
	const loginUrl = settings.watch<string | undefined>('CAS_login_url');
...
export const openCASLoginPopup = async (credentialToken: string) => {
	const popupWidth = settings.watch<number>('CAS_popup_width') || 800;
	const popupHeight = settings.watch<number>('CAS_popup_height') || 600;
