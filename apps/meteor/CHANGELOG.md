...
- ([#37732](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue with the build that prevented Deno runtime to run on air-gapped environments
...
- ([#37327](https://medsensehealth.ca)) Adds complexity requirements to end-to-end encryption passphrase

- ([#36807](https://medsensehealth.ca) by [@tiagoevanp](https://github.com/tiagoevanp)) Adds a `deletedRooms` field to the `users.delete` endpoint response, indicating which rooms were deleted as part of the user deletion process.

- ([#37547](https://medsensehealth.ca)) Adds the `getUserRoomIds` method to the `UserRead` accessor in the Apps-Engine, graduating it from the experimental bridge to the stable user bridge.

- ([#37368](https://medsensehealth.ca)) Allows users to enable TOTP-based two factor authentication without requiring a verified email address.

- ([#37119](https://medsensehealth.ca) by [@ergot-rp](https://github.com/ergot-rp)) Adds missing legend for fieldset in profile page to meet WCAG compliance

- ([#37524](https://medsensehealth.ca)) Moves the expandable message composer out of feature preview

- ([#37378](https://medsensehealth.ca)) Introduces in-chat messages for when a voice call ends

- ([#37276](https://medsensehealth.ca)) Disables the delete message confirmation button to prevent the action from being triggered while the request is in progress

- ([#37318](https://medsensehealth.ca)) Fixes the time display in calendar event notifications by converting the UTC time to the local time.

- ([#37167](https://medsensehealth.ca)) Changes a behavior that would store the result of every status transition that happened to apps
...
- ([#37245](https://medsensehealth.ca)) Replaces old `Assign Extension` button and modal by introducing a proper input in the user edit form.

- ([#37505](https://medsensehealth.ca)) Introduces a new user preference to enable/disable desktop voice call notifications.

- ([#36807](https://medsensehealth.ca) by [@tiagoevanp](https://github.com/tiagoevanp)) Fix issue where a team would become orphaned when its last owner was deleted.

- ([#37134](https://medsensehealth.ca) by [@ergot-rp](https://github.com/ergot-rp)) Adds missing legend for fieldset in accessibility page to meet WCAG compliance

- ([#36308](https://medsensehealth.ca) by [@sandranymark](https://github.com/sandranymark)) Improves inline error in report message modal to meet WCAG compliance.

- ([#37313](https://medsensehealth.ca)) Includes the voice call extension to the `users.autocomplete` endpoint resulting users properties

- ([#37250](https://medsensehealth.ca)) Updates AutoTranslate to fetch supported languages dynamically from DeepL API.
...
- ([#37255](https://medsensehealth.ca)) Adds deprecation warning for `livechat:saveUnit`

- ([#36835](https://medsensehealth.ca)) Fixes the capture of lead's email or phone number when the visitor didn't have data already

- ([#37278](https://medsensehealth.ca)) Fixes an issue with LDAP Sync not updating user's VoIP Extensions

- ([#37430](https://medsensehealth.ca)) Fixes an issue where it was not possible to save nickname and bio as empty in user profile

- ([#37387](https://medsensehealth.ca)) Fixes an issue where user data exports requested would remain stuck and never complete.

- ([#37270](https://medsensehealth.ca)) Fixes editing of encrypted message attachment description.

- ([#37493](https://medsensehealth.ca)) Fixes an issue where `user-agent` is not properly extracted from the DDP connection headers

- ([#37350](https://medsensehealth.ca)) Fixes the sidebar collapse breakpoint in enhanced navigation

- ([#37496](https://medsensehealth.ca)) Fixes the ping behavior so it only triggers when the app becomes idle, preventing unnecessary restarts

- ([#37277](https://medsensehealth.ca)) Fixes an issue where updating a private canned response wasn't being shown on canned-response contextualbar.

- ([#37326](https://medsensehealth.ca)) Fixes an issue related to creating new users, it should not auto opt in new users for email two factor authentication if any one of `Accounts_TwoFactorAuthentication_Enabled`, `Accounts_TwoFactorAuthentication_By_Email_Enabled` and `Accounts_TwoFactorAuthentication_By_Email_Auto_Opt_In` setting is disabled.

- ([#37380](https://medsensehealth.ca)) Fixes an issue where apps would never get the bio of a user even though the type has the field defined

- ([#37477](https://medsensehealth.ca)) Fixes an issue where leave room action is available for users without subscription

- ([#37486](https://medsensehealth.ca)) Fixes the User Autocomplete's selected option being misaligned

- ([#37290](https://medsensehealth.ca)) Fixes an issue where private channels that a user belongs to were not shown in Directory search results.

- ([#37395](https://medsensehealth.ca)) Fixes client slowdown for users with large amount of channels

- ([#37384](https://medsensehealth.ca)) Fixes a problem in apps-engine debug logs where only 2 depth levels were displayed for objects, which is often not enough for debugging purposes

- ([#37152](https://medsensehealth.ca)) Fixes a bug that would cause apps to go into `invalid_installation_disabled` in some cases

- ([#37345](https://medsensehealth.ca)) Fixes administration settings page not listing the settings after logging out and back into the workspace

- ([#37340](https://medsensehealth.ca)) Fixes `Conversations by Tag` chart alignment to match similar charts in the `Reports` section of Omnichannel.

- ([#37338](https://medsensehealth.ca)) Improves mandatory role-based two-factor authentication setup to always verify available 2FA methods before enforcement.

- ([#37449](https://medsensehealth.ca)) Fixes an issue where voice call button is not displaying in navbar on mobile screen sizes
...
- ([#37550](https://medsensehealth.ca)) Fixes an issue that caused the server to load upload files' contents in-memory twice
...
- ([#37540](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue where `user-agent` is not properly extracted from the DDP connection headers

- ([#37520](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes client slowdown for users with large amount of channels
...
- ([#37436](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes the capture of lead's email or phone number when the visitor didn't have data already

- ([#37440](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes a problem in apps-engine debug logs where only 2 depth levels were displayed for objects, which is often not enough for debugging purposes
...
- ([#37057](https://medsensehealth.ca)) Adds an experimental API to the apps-engine that retrieves the ids of rooms the user is a member of

- ([#37038](https://medsensehealth.ca)) Introduce the `RangeSettingInput` component, providing a new visual input type for settings that accept a range of numeric values. This improves the user experience for adjusting range-based settings in the administration panel.

- ([#37312](https://medsensehealth.ca)) Replaces old `Assign Extension` button and modal by introducing a proper input in the user edit form.

- ([#37073](https://medsensehealth.ca)) Adds a warning modal in order to prevent the `access-permissions` permission from being removed from the last granted role in the permissions table. This hardening measure ensures that administrators cannot accidentally lock themselves out of the system's permission management screen.

### Patch Changes

- ([#37154](https://medsensehealth.ca)) Fixes a GUI crash happening in the admin user page when attempting to display an invalid custom field
...
- ([#36973](https://medsensehealth.ca)) Adds deprecation warning on `livechat:returnAsInquiry` with new endpoint replacing it; `livechat/inquiries.returnAsInquiry`

- ([#37218](https://medsensehealth.ca)) Adds deprecation warning on `livechat:setupConnection`

- ([#37147](https://medsensehealth.ca)) Fixes client crash on invalid message attachment fields payload.

- ([#37235](https://medsensehealth.ca)) Adds deprecation warning on `sendFileLivechatMessage`

- ([#37158](https://medsensehealth.ca)) Fixes crash in end-to-end encrypted rooms when sending a quote or message link referencing a message outside the room.

- ([#36990](https://medsensehealth.ca)) Change app update strategies to prevent unwanted data changes in database

- ([#37042](https://medsensehealth.ca)) Adjusts custom sound loop on omnichannel continuous notification to play `New room` loop instead of `New message` loop

- ([#37051](https://medsensehealth.ca)) Adds deprecation warning on `removeCannedResponse`;

- ([#37200](https://medsensehealth.ca)) Fixes an issue where messages are not being translated immediately in omnichannel rooms

- ([#37171](https://medsensehealth.ca)) Adds deprecation warning on legacy method: `livechat:removeUnit`

- ([#37229](https://medsensehealth.ca)) Fixes an issue where enabling trace method calls setting was crashing the server.

- ([#37103](https://medsensehealth.ca)) Fixes real-time monitoring displaying incorrect data

- ([#37136](https://medsensehealth.ca)) Adds deprecation warning on `livechat:removeTag` with new endpoint replacing it; `livechat/tags.remove`

- ([#37109](https://medsensehealth.ca)) Fixes an issue where the start call button is available in archived rooms

- ([#36790](https://medsensehealth.ca) by [@tiagoevanp](https://github.com/tiagoevanp)) Fixes an issue where the API method `rooms.delete` inadvertently deletes the main room of a team

- ([#37203](https://medsensehealth.ca)) Fixes an issue where pagination is not working properly in “users in role” table

- ([#37041](https://medsensehealth.ca)) Ensures the formatted volume value is kept between 0 and 1

- ([#36958](https://medsensehealth.ca)) Adds deprecation warning on `livechat:removeRoom` with new endpoint replacing it; `livechat/rooms.delete`
...
- ([#36723](https://medsensehealth.ca) by [@tiagoevanp](https://github.com/tiagoevanp)) Fixes search by name in custom emojis list, by adding a correct parameter to the endpoint `emoji-custom.all`
...
- ([#36923](https://medsensehealth.ca)) Replaces some texts in order to improve the E2E encryption communication on the UI

- ([#36589](https://medsensehealth.ca)) Adds new endpoint to disable Livechat Contacts by its id, with a new permission `delete-livechat-contact`.

- ([#36820](https://medsensehealth.ca)) Fixes login using iframe authentication.

- ([#36945](https://medsensehealth.ca)) It removes the encrypted input from the edit room panel in order to avoid duplicated configurations with the same purpose

- ([#36778](https://medsensehealth.ca)) Introduces the ability to reset the e2e encrypted password from the enter e2e encrypted password modal
...
- ([#36717](https://medsensehealth.ca)) Introduces a new voice call architecture along with a new and improved call widget experience.

- ([#36397](https://medsensehealth.ca)) Introduces a new flow for requesting device permissions for Voice Calling, prompting the user before the request. Also solves a few issues with the device selection menu.

- ([#36974](https://medsensehealth.ca)) Improves the Outbound Message modal’s scrolling on smaller viewports and with large templates

### Patch Changes

- ([#36861](https://medsensehealth.ca)) Enable room search by Cyrillic characters in channel names (e.g. "тест").

- ([#36967](https://medsensehealth.ca)) Fixes an issue where apps that import node native modules with the optional `node:` specifier would fail to construct

- ([#37031](https://medsensehealth.ca)) Removes deprecated search federated rooms
...
- ([#36933](https://medsensehealth.ca)) Adds deprecation warning on `livechat:removeCustomField` with new endpoint replacing it; `livechat/custom-fields.delete`

- ([#36975](https://medsensehealth.ca)) Adds deprecation warning on `livechat:saveAgentInfo` with new endpoint replacing it; `livechat/agents.saveInfo`

- ([#36866](https://medsensehealth.ca)) Fixes an issue with iframe authentication where login should always fallback to iframe authentication in case of any erorrs.

- ([#36661](https://medsensehealth.ca)) Fixes `channels.messages`, `groups.messages`, `dm.messages` and `im.messages` APIs to filter out deleted messages.

- ([#37053](https://medsensehealth.ca)) Fixes stale phone numbers being available for outbound message

- ([#37052](https://medsensehealth.ca)) Adds deprecation warning on `saveCannedResponse`

- ([#36812](https://medsensehealth.ca) by [@tiagoevanp](https://github.com/tiagoevanp)) Prevent `Export Messages` tab from closing when selecting messages by clicking outside of it and preference "Hide Contextual Bar by clicking outside of it" is set true.

- ([#36845](https://medsensehealth.ca)) Adds deprecation warning to `livechat:getAnalyticsChartData`, as well as it adds a new endpoint to replace it; `livechat/analytics/dashboards/charts-data`

- ([#36683](https://medsensehealth.ca)) Fixes an issue where deleted messages remain visible until a manual page refresh when "Message removed" is hidden in hidden system messages settings.

- ([#36982](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#36897](https://medsensehealth.ca)) Adds deprecation warning on `livechat:getRoutingConfig`, as well as it adds new endpoint to replace it; `livechat/config/routing`

- ([#37068](https://medsensehealth.ca)) Changes the feedback message when inviting external users to a federated channel

- ([#36995](https://medsensehealth.ca)) Disables cache for outbound message providers to ensure UI is always up to date

- ([#36813](https://medsensehealth.ca)) Fixes an issue where the omnichannel default priorities are not being translated in the sidebar room menu

- ([#36772](https://medsensehealth.ca) by [@tiagoevanp](https://github.com/tiagoevanp)) Fix issue where resending the welcome email could include unresolved placeholders (e.g., `[name]`, `[email]`).

- ([#37065](https://medsensehealth.ca)) Adds missing tooltip when adding users to federated room, indicating verification status

- ([#36799](https://medsensehealth.ca)) Fixes a missing translation on the create channel/team modal

- ([#36993](https://medsensehealth.ca)) Adds deprecation warning on `livechat:removeRoom`, use `livechat/inquiries.take` instead

- ([#36852](https://medsensehealth.ca)) Fixes `teams.addMembers` API to assign team member roles properly.

- ([#36921](https://medsensehealth.ca)) Adds deprecation warning on `livechat:removeAllClosedRooms` with new endpoint replacing it; `livechat/rooms.removeAllClosedRooms`

- ([#36855](https://medsensehealth.ca)) Changes a strict behavior on reporting slash commands provided by apps

- ([#37007](https://medsensehealth.ca)) Fixes reactivity when creating a federated room, that was wrongly showing Join button on composer, requiring a reload or change of route to be able to send a message.

- ([#36780](https://medsensehealth.ca)) Fixes an issue where isn't possible to scroll the discussion list

- ([#36981](https://medsensehealth.ca)) Adds deprecation warning on `livechat:saveDepartment`

- ([#36797](https://medsensehealth.ca)) Fixes an issue where the encryption toggle was incorrectly reset/disabled/enabled in the Teams creation modal when Broadcast or Private was toggled, or when the user lacked unrelated permissions.

- ([#36919](https://medsensehealth.ca)) Show iframe authentication page, when login through iframe authentication API token fails

- ([#37050](https://medsensehealth.ca)) Adds a validation to external users when creating a channel that is not federated

- ([#36814](https://medsensehealth.ca)) Fixes canned messages contextual bar "Create" button not being affected by the correct permission

- ([#36745](https://medsensehealth.ca) by [@tiagoevanp](https://github.com/tiagoevanp)) Fixes intermittent error "Cannot read properties of undefined" when editing messages

- ([#36776](https://medsensehealth.ca)) Fixes an issue where the omnichannel default priorities are not being translated in priorities list

- ([#36870](https://medsensehealth.ca)) Fixes an issue with inconsistent tooltip display in the permission row

- ([#36943](https://medsensehealth.ca)) Fixes an issue where an object reference leak would cause invalid data to be stored in the database during app installation

- ([#36880](https://medsensehealth.ca)) Fixes issue with Login Detection emails showing undefined values instead of the user agent info

- ([#36668](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat chat.unPinMessage API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36971](https://medsensehealth.ca)) Adds deprecation warning on `livechat:saveCustomField` with new endpoint replacing it; `livechat/custom-fields.save`

- ([#37071](https://medsensehealth.ca)) Prevents adding a external user to a non federated room through Members tab

- ([#36883](https://medsensehealth.ca)) Fixes an issue where users table headers are being displayed when there's no results

- ([#36986](https://medsensehealth.ca)) Adds deprecation warning on `livechat:sendTranscript` with endpoint replacing it; `livechat/trasncript`

- ([#36994](https://medsensehealth.ca)) Fixes button `Add members` not being disabled on a federated room memers tab, when no user is selected
...
- ([#37236](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes real-time monitoring displaying incorrect data
...
- ([#36962](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes login using iframe authentication.

- ([#37000](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Security Hotfix (https://medsensehealth.ca)

- ([#37028](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Changes a strict behavior on reporting slash commands provided by apps
...
- ([#36623](https://medsensehealth.ca)) Overrides the scrollbars auto hide behavior from hiding while not scrolling to hiding while not moving

- ([#36556](https://medsensehealth.ca)) Adds a "Clear Filters" Button to the App Logs Filter Contextual Bar

- ([#36424](https://medsensehealth.ca)) Adds an endpoint to fetch a Outbound Comms Provider's metadata.

- ([#36553](https://medsensehealth.ca)) Creates a new endpoint that allows agents to send an outbound message from a registered app provider

- ([#36558](https://medsensehealth.ca)) Adds a "Collapse All" button to the Apps Logs Filter and moves existing "Expand All" button to a kebab menu

- ([#36049](https://medsensehealth.ca)) Introduces the side navigation with a new filtering system. The update adds new filters for All, Mentions, Favorites, and Discussions, as well as dedicated filters for Omnichannel conversations and grouping by Teams, Channels, and DMs.
  > This change is being tested under `Enhanced navigation experience` feature preview, in order to check it you need to enabled it
...
- ([#36622](https://medsensehealth.ca)) Fixes an issue where audio and video messages would stop playing if left idle past their link expiration. Now the player automatically refreshes expired links so users can continue listening or watching without reloading the chat.

- ([#36507](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat oauth-apps.create API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36585](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat oauth-apps.update API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36703](https://medsensehealth.ca)) Fixes an issue where custom room notification sounds were not applied.

- ([#36738](https://medsensehealth.ca)) Addresses an issue where video conference popups don't receive proper focus because FocusScope is mispositioned

- ([#36577](https://medsensehealth.ca)) Fixes an issue where rooms transferred to a department's queue could get stuck—marked as taken but with no agent assigned.

- ([#36020](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat chat.pinMessage API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36617](https://medsensehealth.ca)) Allows agents to set a default agent when the chat being transferred ends up in the queue

- ([#36716](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat e2e.setRoomKeyID endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36625](https://medsensehealth.ca)) Fixes an issue where app installation would fail if the app package contained JS syntax newer than 2017

- ([#36592](https://medsensehealth.ca)) Fixes `create-p` and `create-c` permissions not being applyed in teams creation

- ([#36714](https://medsensehealth.ca)) Fixes an issue where the Encrypted toggle in the `Create Channel Modal` would change unexpectedly or become disabled after switching the Private or Broadcast options when E2E defaults are enabled.

- ([#36611](https://medsensehealth.ca)) Fix an issue where the report exported in the App logs page would not consider the instance id filter

- ([#36815](https://medsensehealth.ca)) Fixes queued conversations not being sorted in real time based on the room's SLA policy

- ([#36606](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat oauth-apps.delete API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36802](https://medsensehealth.ca)) Fixes an error on apps loading that would cause an unhandled promise rejection crash during startup in some cases

- ([#36080](https://medsensehealth.ca)) Fixes issue preventing the Security Logs from being accessed using the Enhanced Navbar in feature preview

- ([#36749](https://medsensehealth.ca)) Fixes an issue with Omnichannel inquiries where multiple instances could take the same inquiry from the queue resulting in the same room being assined to multiple agents.

- ([#36527](https://medsensehealth.ca)) Fixes an issue that caused some types of messages to generate an empty thread preview

- ([#36672](https://medsensehealth.ca)) fixes an issue where some apps that don't need permission would have grantedPermissions as null making it impossible to activate the app

- ([#36651](https://medsensehealth.ca)) Fixes some locale loading issues for date-time formatting functionality.

- ([#36747](https://medsensehealth.ca)) Fixes the `onlyMyDepartments` flag that some endpoints accept so it works more consistent across different user roles. Now, for monitors, `onlyMyDepartments` will include departments the user is serving as an agent. For agents, it will filter the ones the user is serving. There's no change for managers and admins, which can see anything.

- ([#36601](https://medsensehealth.ca)) Fixes an issue where user custom status text is being overwritten, causing it not being updated in real time

- ([#36591](https://medsensehealth.ca)) Fixes order on featured room header actions

- ([#36677](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat dm.delete/im.delete API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36569](https://medsensehealth.ca)) Fixes "View Logs" button not filtering logs by instance id

- ([#36662](https://medsensehealth.ca)) Fixes scroll issue when moving between channels or DMs

- ([#36586](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat oauth-apps.list API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36598](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat oauth-apps.get API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36676](https://medsensehealth.ca)) Fixes an issue where files containing exif data would fail to upload to S3 when `Message_Attachments_Strip_Exif` is enabled.

- ([#36544](https://medsensehealth.ca)) fixes an issue where bussines hours are not working on weekends when the timezone of bh slip into another day

- ([#36627](https://medsensehealth.ca)) fixes an issue where using `/v1/users.updateOwnBasicInfo`, the user was not be able to set the password (not change), even when required

- ([#35985](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat Permissions API endpoints by migrating to a centralized syntax and utilizing shared AJV schemas for validation. This will enhance API documentation and ensure type safety through response validation.

- ([#36670](https://medsensehealth.ca)) Fixes an issue that would cause the chat server to crash with an unhandled rejection in some cases

- ([#36578](https://medsensehealth.ca)) Fixes a bug where the `/api/v1/users.update` API call was replacing the entire `customFields` object instead of merging only the specified properties. The fix ensures that when updating custom fields, existing values are preserved while only specified fields are updated or added.
...
- ([#36680](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) fixes an issue where some apps that don't need permission would have grantedPermissions as null making it impossible to activate the app
...
- ([#36581](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue where rooms transferred to a department's queue could get stuck—marked as taken but with no agent assigned.

- ([#36612](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes a bug where the `/api/v1/users.update` API call was replacing the entire `customFields` object instead of merging only the specified properties. The fix ensures that when updating custom fields, existing values are preserved while only specified fields are updated or added.
...
- ([#36321](https://medsensehealth.ca)) Adds theme palette to Application error page

- ([#36288](https://medsensehealth.ca)) Adds a new filter to the Logs tab of the App Details page.

- ([#36359](https://medsensehealth.ca)) Adds a new tab on App Details page to see all instances and it's statuses.
  Adds a new variant to the status tag, mixed status.
- ([#36377](https://medsensehealth.ca)) Adds new endpoints for outbound communications

- ([#36142](https://medsensehealth.ca)) Implements new component for Apps Logs View

- ([#36386](https://medsensehealth.ca)) Adds `separateResponse` param to incoming webhooks to return per-channel statuses; when false or omitted, validates all targets and aborts sending to any channel if one fails.

- ([#36339](https://medsensehealth.ca)) Improves the message exportation content for PDF method including images and information about threads and files

- ([#36403](https://medsensehealth.ca)) Adds a button to the apps logs UI that exports logs as a downloadable file

- ([#36413](https://medsensehealth.ca)) Adds 2 new buttons to the App Logs Filters, "Expand all" and "Refresh"

### Patch Changes

- ([#36385](https://medsensehealth.ca)) fixes: v1/updateOwnBasicInfo does not trigger user stream

- ([#36532](https://medsensehealth.ca)) Fixes an issue with SLA policies that caused an inquiry to "dissapear" after its SLA being updated
...
- ([#36099](https://medsensehealth.ca)) Fixes missing UI updates after pruning messages with "Files only" enabled.

- ([#36105](https://medsensehealth.ca)) Fixes an issue that was causing ddp-streamer process to break if the communication with presence service was interrupted for any reason.

- ([#36317](https://medsensehealth.ca)) Fixes an issue that causes legitimate URLs to return '#' in links

- ([#36494](https://medsensehealth.ca)) Fixes a behavior when transferring a room to another department that was not considering the `waiting queue` setting and attempted to route the chat to an agent instead of leaving it on the department's queue

- ([#36230](https://medsensehealth.ca)) Fixes FreeSwitch event parser to automatically reconnect when connection is lost

- ([#36398](https://medsensehealth.ca)) Fixes missing burger menu in embedded layout page header when using enhanced navigation feature preview

- ([#36333](https://medsensehealth.ca)) Adds `imgAlt`, `imgWidth`, and `imgHeight` props to the `GenericUpsellModal` in order to improve the accessibility of upsell modals by providing alternative text for images and preventing layout shifts during image loading by allowing fixed dimensions.

- ([#36406](https://medsensehealth.ca)) add some missing translations for sound names and font sizes

- ([#36332](https://medsensehealth.ca)) Fixes livechat widget attempting to connect to localhost instead of actual site url

- ([#36345](https://medsensehealth.ca)) Fixes an issue that prevented all custom fields from being saved when multiple updates were issued on a single call

- ([#36265](https://medsensehealth.ca)) Fixes an issue where chat.postMessage returned an error when using the parseUrls flag.

- ([#36404](https://medsensehealth.ca)) fixes the selection of room and users performance when using forward message feature

- ([#35277](https://medsensehealth.ca) by [@Anxhul10](https://github.com/Anxhul10)) Fixes email validation in the registration form to prevent users from registering with an already existing email address.

- ([#36364](https://medsensehealth.ca)) Improves the loading state and accessibility of the `AutoCompleteDepartment` component

- ([#35884](https://medsensehealth.ca) by [@ahmed-n-abdeltwab](https://github.com/ahmed-n-abdeltwab)) Add OpenAPI support for the Rocket.Chat Webdav API endpoints by migrating to a modern chained route definition syntax and utilizing shared AJV schemas for validation to enhance API documentation and ensure type safety through response validation.

- ([#36313](https://medsensehealth.ca)) Fixes rate limiter not being enforced for the v1 REST API

- ([#36503](https://medsensehealth.ca)) Fixes missing message text in desktop notifications by ensuring the "Show Message in Notification" setting only affects mobile push notifications.

- ([#35006](https://medsensehealth.ca)) Fixes the parsing of FreeSwitch events to properly generate a history of calls on Rocket.Chat

- ([#36338](https://medsensehealth.ca)) Clears stale data in local storage when login is expired

- ([#36381](https://medsensehealth.ca)) Fixes redirection not being triggered after a required password change

- ([#36449](https://medsensehealth.ca)) Fixes priorities, sla changes & inquiries not being propagated when change streams were not being used

- ([#36256](https://medsensehealth.ca)) Fixes an issue that caused the queue worker to stop processing if something failed when checking MAC limits or while fetching the list of queues

- ([#36259](https://medsensehealth.ca)) Introduces PDF file as an export type for room messages

- ([#36228](https://medsensehealth.ca)) Fixes GDPR contact information removal for Omnichannel.

- ([#36504](https://medsensehealth.ca)) Fixes an issue that would leave an app in an unrecoverable state if the installation failed during the construction of the runtime

- ([#36235](https://medsensehealth.ca)) Fixes the issue of deactivation emails being sent to users that were removed from AD even if they were not active in the workspace

- ([#36346](https://medsensehealth.ca)) Fixes an issue where some error objects sent to apps' method calls would only contain the message '[object Object]'
...
- ([#36538](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue with SLA policies that caused an inquiry to "dissapear" after its SLA being updated

- Bump @rocket.chat/meteor version.

- ([#36534](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes a behavior when transferring a room to another department that was not considering the `waiting queue` setting and attempted to route the chat to an agent instead of leaving it on the department's queue

- ([#36545](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes missing message text in desktop notifications by ensuring the "Show Message in Notification" setting only affects mobile push notifications.

- ([#36543](https://medsensehealth.ca)) Fixes an issue that would leave an app in an unrecoverable state if the installation failed during the construction of the runtime
...
- ([#36474](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes priorities, sla changes & inquiries not being propagated when change streams were not being used
...
- ([#36430](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) fixes: v1/updateOwnBasicInfo does not trigger user stream

- fixes an issue where depending on some race conditions the subscription stream was not being updated

- Bump @rocket.chat/meteor version.

- ([#36432](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes redirection not being triggered after a required password change
...
- ([#36091](https://medsensehealth.ca)) Replaces the cancel button by reset in notification and export messages contextual bar.

- ([#35315](https://medsensehealth.ca) by [@xmaxlad](https://github.com/xmaxlad)) Prevents creation of unnamed Personal Access Tokens by requiring the form's `name` field fullfilment.

- ([#36127](https://medsensehealth.ca)) #### Adds support for hiding the "Expand chat" button in the Livechat widget.
...
- ([#36169](https://medsensehealth.ca)) Fix an issue where action buttons registered by apps would be displayed even if their apps were disabled

- ([#36067](https://medsensehealth.ca)) Adds pagination to the App/Logs list, allowing the user to see all logs, instead of a subset of the most recent ones

- ([#36013](https://medsensehealth.ca)) Resolves some accessibility violations in the rooms file list context bar, including issues with ARIA attributes and focus management.

- ([#36037](https://medsensehealth.ca)) Adds a new setting to select which algorithm to use when signing SAML requests and responses

- ([#36114](https://medsensehealth.ca)) Improves form fields layout for mobile screens in profile page

- ([#36153](https://medsensehealth.ca)) Adds a new setting to override outlook calendar settings per user email domain

### Patch Changes

- ([#36154](https://medsensehealth.ca)) Fixes an issue that prevented the action of saving an agent when editing a department to work.

- ([#36143](https://medsensehealth.ca)) Fixes message's quote chain limit not being respected for End-to-end encrypted rooms.

- ([#36062](https://medsensehealth.ca)) Removes the `Change to Default` language button from the login page
...
- ([#36038](https://medsensehealth.ca)) Fixes departments field being limited to 50 entries when adding or editing agents

- ([#36202](https://medsensehealth.ca)) Fixes the usage of `Livechat_enabled_when_agent_idle` setting across the codebase. Goal is to use it wherever is applicable making the feature more predictable.

- ([#36193](https://medsensehealth.ca)) Fixes the condition for firing the `LivechatSessionTaken` webhook event.

- ([#36105](https://medsensehealth.ca)) Fixes an issue that was causing ddp-streamer process to break if the communication with presence service was interrupted for any reason.

- ([#36097](https://medsensehealth.ca)) Fixes `contact.conflictingFields` being updated with nullish values.

- ([#36263](https://medsensehealth.ca)) Fixes an issue, where multiple reconnections would subscribe multiple times to the same stream, only a frontend issue, since the stream cache prevents sending multiple times to the backend, but does not prevent running the callback multiple times

- ([#36095](https://medsensehealth.ca)) Avoids creating font-size tag multiple times

- ([#36035](https://medsensehealth.ca)) Fixes file filtering by name or type not working for non-private channels.

- ([#36224](https://medsensehealth.ca)) fix: Security Hotfix (https://medsensehealth.ca)

- ([#36163](https://medsensehealth.ca)) Fixes an issue where incoming webhook messages did not respect the maximum allowed characters per message.

- ([#36111](https://medsensehealth.ca)) Fixes Omnichannel Rest API validation schemas that were flagging `Pagination` properties as invalid

- ([#35947](https://medsensehealth.ca)) Fixes an issue where the slash commands options are not truncating correctly in small screen sizes

- ([#35863](https://medsensehealth.ca)) Fixes encoding of special characters displayed by code blocks in markdown text attachments

- ([#36100](https://medsensehealth.ca)) Disables sorting on non-indexed columns in the "Chats" tab of Omnichannel Contact Center

- ([#36116](https://medsensehealth.ca)) Fixed contacts being marked as `known` after editing a custom field, or resolving conflicts by adding a new model function that only updates the `customFields` or `conflictingFields` prop.

- ([#36171](https://medsensehealth.ca)) Fixes an error message triggered whenever a room is selected on secondary navigation for teams.

- ([#36029](https://medsensehealth.ca)) Fixes issue where department autocomplete did not render options with checkboxes when configured

- ([#36236](https://medsensehealth.ca)) Fixes the `channels.counters`, `groups.counters` and `im.counters` endpoint to include only active users in members count.

- ([#36156](https://medsensehealth.ca)) Fixes an issue where the `Show Message in Notification` and `Show Channel/Group/Username in Notification` setting was ignored in desktop notifications. Also ensures users are redirected to the correct room on interacting with the notifications.

- ([#36004](https://medsensehealth.ca)) Fixes an issue with the Thread List displaying unreadable errors.

- ([#36206](https://medsensehealth.ca)) Fixes an issue where overlapping calendar events could cause the user status to stay busy indefinitely

- ([#36256](https://medsensehealth.ca)) Fixes an issue that caused the queue worker to stop processing if something failed when checking MAC limits or while fetching the list of queues

- ([#36346](https://medsensehealth.ca)) Fixes an issue where some error objects sent to apps' method calls would only contain the message '[object Object]'

- ([#36258](https://medsensehealth.ca)) Fixes an issue that prevented the action of removing an agent when editing a department to work.

- ([#36138](https://medsensehealth.ca)) fixes contextual-bar not working by apps interaction after a page refresh.
...
- ([#36360](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue where some error objects sent to apps' method calls would only contain the message '[object Object]'
...
- ([#36323](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue that was causing ddp-streamer process to break if the communication with presence service was interrupted for any reason.

- ([#36250](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) fix: Security Hotfix (https://medsensehealth.ca)

- ([#36270](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue that caused the queue worker to stop processing if something failed when checking MAC limits or while fetching the list of queues

- ([#36298](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue that prevented the action of removing an agent when editing a department to work.
...
- ([#36204](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue that prevented the action of saving an agent when editing a department to work.

- Bump @rocket.chat/meteor version.

- Bump @rocket.chat/meteor version.

- ([#36210](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes the usage of `Livechat_enabled_when_agent_idle` setting across the codebase. Goal is to use it wherever is applicable making the feature more predictable.

- ([#36200](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixed contacts being marked as `known` after editing a custom field, or resolving conflicts by adding a new model function that only updates the `customFields` or `conflictingFields` prop.
...
- ([#35830](https://medsensehealth.ca)) Introduces a new feature which enables dragging the VoIP call widget to change its position on the screen.

- ([#35996](https://medsensehealth.ca)) Improves the alert shown after pruning messages to reflect that files have been pruned when the "files only" option was selected.

- ([#35416](https://medsensehealth.ca)) Improve the `/api/apps/:id/logs` endpoint to accept filters

- ([#35416](https://medsensehealth.ca)) Add a new endpoint `/api/apps/logs` that allows for fetching logs without a filter for app id

- ([#35776](https://medsensehealth.ca)) Improves handling of errors during voice calls
...
- ([#35468](https://medsensehealth.ca) by [@samir0607](https://github.com/samir0607)) Fixes an issue where the composer popup doesn't scroll the results properly

- ([#35984](https://medsensehealth.ca)) Fixes an issue that caused the auto transfer process due to agent inactivity to fail with error `Missing property userType` when the target of the transfer was the queue.

- ([#35981](https://medsensehealth.ca)) Deprecates the use of MongoDB oplog or Change Streams to receive real time data updates.
...
- ([#35976](https://medsensehealth.ca)) Fixes a bug where changing site url breaks `meteor_runtime_config` path until we reset the server

- ([#35963](https://medsensehealth.ca)) Fixes emojis being parsed inside code blocks in text message attachment using `chat.postMessage` API call

- ([#36014](https://medsensehealth.ca)) Fixes an issue where the native scrollbar was rendering with an invalid variable color

- ([#35821](https://medsensehealth.ca)) Fixes invite link validation being called more than once and sometimes being invalidated due to repetition.

- ([#36097](https://medsensehealth.ca)) Fixes `contact.conflictingFields` being updated with nullish values.

- ([#36085](https://medsensehealth.ca)) Fixes CORS headers not being sent for GET requests

- ([#35828](https://medsensehealth.ca)) Fixes an error where the user was not moving from Pending to Deactivated tab in the admin UI.

- ([#35962](https://medsensehealth.ca)) Fixes prune messages contextual bar not resetting form after submitting.

- ([#35600](https://medsensehealth.ca)) Blocks edit message action for Off-The-Record (OTR) messages.

- ([#35885](https://medsensehealth.ca)) Fixes registration pages a11y color contrast and apply browser theme configuration

- ([#35815](https://medsensehealth.ca)) Fixes some warnings on delete user modal when deleting a user who has owner role in some rooms.

- ([#36068](https://medsensehealth.ca)) Fixes an issue that caused Hono to not process payloads with content type `x-www-form-urlencoded` correctly

- ([#36006](https://medsensehealth.ca)) Recovers from errors in `RoomHistoryMaanger.getMore`, avoiding a "permaloading" state after user logout.

- ([#36027](https://medsensehealth.ca)) Fixes an issue with the `audit.settings` endpoint filtering by actor returning empty results.

- ([#36009](https://medsensehealth.ca)) Fixes buffer as response from API
...
- ([#35889](https://medsensehealth.ca)) Restores initial focus on description input in file upload modal for improved usability

- ([#35946](https://medsensehealth.ca)) Fixes an issue where searching messages by attachment description or title failed for non-ASCII characters, thereby enabling search across multiple languages.

- ([#35839](https://medsensehealth.ca)) Fixes an issue where the upload of malformed images failed silently with Message_Attachments_Strip_Exif disabled

- ([#35927](https://medsensehealth.ca)) Fixes incorrect character escaping in system messages by removing unnecessary escaping logic to ensure accurate rendering of special characters.

- ([#35903](https://medsensehealth.ca)) fixes a typo in "High scalability" word

- ([#32917](https://medsensehealth.ca)) Fixed links on settings redirecting to the outside of the chat, now they will open on a new tab

- ([#35594](https://medsensehealth.ca) by [@Gustrb](https://github.com/Gustrb)) Fixes an issue where removing the permissions of an app would make it impossible to enable it after update

- ([#36070](https://medsensehealth.ca)) Fixes an issue with REST API metrics sent to Prometheus not grouping endpoints by route

- ([#36007](https://medsensehealth.ca)) Fixes the Apps-Engine package installation, removing the dependency on an internal package

- ([#35959](https://medsensehealth.ca)) Fixes an issue with airgapped restrictions ignoring the warning period.

- ([#35797](https://medsensehealth.ca)) Fixes the room history pruning behavior when filesOnly is true to ensure only file-type attachments are removed, preserving quotes and non-file attachments.

- ([#36016](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#36019](https://medsensehealth.ca)) Adds new endpoint to handle contact's conflicting data

- ([#35905](https://medsensehealth.ca)) Fixes file deletion being called twice and ignoring errors with FileSystem storage type.
...
- Bump @rocket.chat/meteor version.
- ([#36101](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue that caused Hono to not process payloads with content type `x-www-form-urlencoded` correctly
...
- ([#36060](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes buffer as response from API

  Currently, the only known case affected is Apps Engine icons. This patch ensures the API returns a buffer as the response, resolving issues with clients expecting a binary payload.

- ([#36033](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes the Apps-Engine package installation, removing the dependency on an internal package
...
- ([#35717](https://medsensehealth.ca)) Adds new settings to allow configuring custom variables with string manipulation functions on the LDAP data mapper

- ([#35280](https://medsensehealth.ca)) Allows apps to react to department status changes.

- ([#35644](https://medsensehealth.ca)) Adds the ability to dynamically add and remove options from select/multi-select settings in the Apps Engine to support more flexible configuration scenarios by exposing two new methods on the settings API.

- ([#34954](https://medsensehealth.ca) by [@tapiarafael](https://github.com/tapiarafael)) Allows search omnichannel rooms by the exact visitor name using double quotes to have a faster response

- ([#35613](https://medsensehealth.ca)) Replaces the parent room tag in room header in favor of a button to back to the parent room
  > This change is being tested under `Enhanced navigation experience` feature preview, in order to check it you need to enabled it
- ([#35615](https://medsensehealth.ca)) Removes the avatar in the room header
  > This change is being tested under `Enhanced navigation experience` feature preview, in order to check it you need to enabled it
- ([#35218](https://medsensehealth.ca)) Adds a new admin page to audit settings changes in a server

- ([#35721](https://medsensehealth.ca)) Enhances the `/api/apps/installed` and `/api/apps/:id/status` endpoints so they get apps' status across the cluster in High-Availability and Microservices deployments

- ([#35631](https://medsensehealth.ca)) Places the room topic next to the room title
  > This change is being tested under `Enhanced navigation experience` feature preview, in order to check it you need to enabled it
- ([#34494](https://medsensehealth.ca)) Implements auditing events for `/v1/users.update` API endpoint

- ([#35672](https://medsensehealth.ca)) Restores the previous room announcement style
  > This change is being tested under `Enhanced navigation experience` feature preview, in order to check it you need to enabled it
- ([#35464](https://medsensehealth.ca)) Allows deleting federated remote users in case they are not present in the homeserver.

- ([#35718](https://medsensehealth.ca)) Adds a new setting to allow syncing federated users data through LDAP

- ([#35807](https://medsensehealth.ca)) Moves the room search functionality from the sidebar to the navbar and reorganize their relative actions
  > This change is being tested under `Enhanced navigation experience` feature preview, in order to check it you need to enabled it
- ([#35700](https://medsensehealth.ca)) Separates voice call and video call room actions on room header.

- ([#35703](https://medsensehealth.ca)) Adds close action to contact unknown callout displayed within Livechat rooms

### Patch Changes

- ([#35790](https://medsensehealth.ca)) Fixes an issue in `Admin > Settings` page where sometimes settings guarded by a license module would not be editable despite having the required modules.

- ([#35795](https://medsensehealth.ca)) Fixes incorrect message sender for incoming webhooks when "Post As" field is updated by ensuring both username and userId are synced to reflect the selected user.

- ([#35299](https://medsensehealth.ca)) Fixes user email verification update not working on admin > users
...
- ([#35762](https://medsensehealth.ca)) Fixes an issue with apps loading in microservices in some cases by ensuring all services have started before trying to load apps

- ([#35816](https://medsensehealth.ca)) Fixes an issue where the outlook calendar items list is not rendering properly

- ([#35244](https://medsensehealth.ca) by [@tapiarafael](https://github.com/tapiarafael)) Ensures seat limit validation in LDAP sync, preventing activations beyond license restrictions.

- ([#35497](https://medsensehealth.ca)) Fixes an issue where the app's logs index was not being created by default sometimes, also set to be always 30 days

- ([#35736](https://medsensehealth.ca)) Fixes an error causing the server to throw an "unhandled promise rejection" when removing an agent from a department without a business hour when using `Multiple` business hours

- ([#35722](https://medsensehealth.ca)) Fixes the behavior of "Maximum number of simultaneous chats" settings, making them more predictable. Previously, we applied a single limit per operation, being the order: `Department > Agent > Global`. This caused the department limit to take prescedence over agent's specific limit, causing some unwanted side effects.
...
- ([#35394](https://medsensehealth.ca) by [@sushen123](https://github.com/sushen123)) Fixes the save button loading state in app settings, ensuring it resets properly after saving.

- ([#35779](https://medsensehealth.ca)) Fixes a fetch infinite loop when adding agents to a department

- ([#35679](https://medsensehealth.ca)) Fixes a GUI crash when editing a canned response with tags via room contextual bar.

- ([#35568](https://medsensehealth.ca)) Fixes an issue with the leave room confirmation modal not displaying the room's name.

- ([#35782](https://medsensehealth.ca)) Fixes an issue where the incoming webhooks integration allowed messages to be sent to public channels under private teams by users who were not members of the team.

- ([#35618](https://medsensehealth.ca)) Fixes an issue when sending a message on an omnichannel room would cause the web client to fetch the room data again.

- ([#35273](https://medsensehealth.ca)) Fixes an issue where the code input type in settings renders duplicate code text boxes.

- ([#35765](https://medsensehealth.ca)) Fixes an issue causing VoIP calls to no longer reach the client after a temporary disconnection

- ([#35397](https://medsensehealth.ca)) Fixes a race condition while federating that caused direct messages to not work

- ([#35832](https://medsensehealth.ca)) Fixes an issue where Voice Calls were unable to gather Ice Servers

- ([#35757](https://medsensehealth.ca)) Fixes an issue where the bypass to call methods over microservices always returns to `{}`

- ([#35831](https://medsensehealth.ca)) Fixes an issue where enterprise routing algorithms could get stuck on selecting the same agent due to chat limits being applied after agent selection, but before agent assignment

- ([#35698](https://medsensehealth.ca)) Fixes the "Enabled" toggle always being displayed as active when editing a business hour.

- ([#35715](https://medsensehealth.ca)) Fixes an issue with dynamic API routes requiring a server restart to be operable.

- ([#35864](https://medsensehealth.ca)) Fixes an issue where OAuth login buttons were not showing up on the login page

- ([#35614](https://medsensehealth.ca)) Fixes an issue where header toolbox items are missing the proper margin in e2ee setup room header

- ([#35716](https://medsensehealth.ca)) Fixes an issue where a federated user's display name would be overwritten by its username

- ([#35766](https://medsensehealth.ca)) Fixes an issue where quick reactions (Feature Preview) would be absent on first access and after page refreshes

- ([#35709](https://medsensehealth.ca)) Improves UX for users with mandatory 2FA roles by clarifying required actions

- ([#35852](https://medsensehealth.ca)) Fixes an issue where rocket.chat would not properly log OAuth errors nor remove the credential with the error from the internal list

- ([#35616](https://medsensehealth.ca)) Fixes Omnichannel Contact Center's chats filter not working when "From" and "To" fields have the same date

- ([#35879](https://medsensehealth.ca)) Fixes contact's conflict resolution not working due to invalid parameters

- ([#35019](https://medsensehealth.ca)) Fixes issue with some charts on engagement dashboard not showing local time and missing data visualizers

- ([#35733](https://medsensehealth.ca) by [@Gustrb](https://github.com/Gustrb)) Fixes a typo in the app update success toast

- ([#35810](https://medsensehealth.ca)) Fixes an issue on the LDAP Background Sync where the process would stop syncing users if any of the LDAP users couldn't be properly mapped to a Rocket.Chat User (for example, by not having an email address)

- ([#35580](https://medsensehealth.ca)) Fixes contact custom fields not being updated when updating a visitor's custom field
...
- ([#35733](https://medsensehealth.ca)) Fixes a typo in the app update success toast
...
- ([#35732](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue with dynamic API routes requiring a server restart to be operable.

- ([#35728](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes Omnichannel Contact Center's chats filter not working when "From" and "To" fields have the same date
...
- ([#35361](https://medsensehealth.ca)) Fixes Some EE features not available after changing plans

- ([#35370](https://medsensehealth.ca)) Adds a new "Unit" field to the create/edit department page, allowing users to specify a business unit when creating or editing a department.

- ([#34983](https://medsensehealth.ca)) feat: Adds OpenAPI support

- ([#35474](https://medsensehealth.ca)) Adds automatic presence sync based on calendar events, updating the user’s status to “busy” when a meeting starts and reverting it afterward.

- ([#33882](https://medsensehealth.ca) by [@noobbbbb](https://github.com/noobbbbb)) Adds unicode character support for default avatars

- ([#35486](https://medsensehealth.ca)) Adds the executeLivechatRoomCreatePrevent hook to the Rocket.Chat Apps-Engine to prevent the creation of live chat rooms.

- ([#35258](https://medsensehealth.ca)) Introduces `/v1/audit.settings` endpoint for querying changed settings audit events

- ([#33796](https://medsensehealth.ca)) Replaces rc-scrollbars in favor of overlayscrollbars fixing some visual and a11y issues when using RTL languages

### Patch Changes

- ([#35330](https://medsensehealth.ca)) fixes an issue where videoconf calls could sometimes fail to ring an user after a temporary disconnection
...
- ([#35205](https://medsensehealth.ca)) Fixes livechat routing algorithm to ensure conversations are correctly assigned to the contact manager when triggers and/or automatic agent routing are enabled.

- ([#35546](https://medsensehealth.ca)) Restores `roomName` property in the `GET /groups.messages` endpoint to fix unintended removal.

- ([#35340](https://medsensehealth.ca)) Fixes an issue where account security page was flashing sometimes for users with mandatory two factor configured.

- ([#35545](https://medsensehealth.ca)) Fixes an issue where video conference popup not displaying properly when trying to call again in direct messages

- ([#35374](https://medsensehealth.ca)) Enforces app limitations on license downgrade by disabling premium marketplace apps, limiting marketplace apps to the oldest 5, and disabling private apps unless grandfathered based on historical statistics.

- ([#35328](https://medsensehealth.ca)) Fixes livechat inquiries not routing to the manager queue when manual routing is enabled.

- ([#35326](https://medsensehealth.ca)) Fixes an issue with embedded layout where the navigation bar was being shown when using new enhanced navigation.

- ([#35467](https://medsensehealth.ca)) Changes the default value for preference `Receive login detection emails` to true, to comply with existing behavior.

- ([#35298](https://medsensehealth.ca)) Fixes an issue where backup codes modal is not opening when regenerating codes

- ([#35257](https://medsensehealth.ca)) Fixes an issue where an unexpected error is thrown when webdav node doesn't have the mime parameter

- ([#35479](https://medsensehealth.ca)) Fixes deno dependency caching for the docker image builds

- ([#35414](https://medsensehealth.ca)) Fixes an issue that allowed agents without the `preview-c-room` permission to join a closed livechat conversation, creating a livechat room that could not be closed or removed from the sidebar.

- ([#35369](https://medsensehealth.ca)) Fixes an issue where recursively quoting messages multiple times (up to the configured chained quote limit) caused the inner attachment to appear empty.

- ([#35549](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#35311](https://medsensehealth.ca) by [@4shb0rne](https://github.com/4shb0rne)) Fixes an issue where menus inside sidebar keeps opened even if the sidebar is collapsed

- ([#35339](https://medsensehealth.ca)) Reduces web client memory comsumption due to memory leaks

- ([#35357](https://medsensehealth.ca) by [@Anxhul10](https://github.com/Anxhul10)) Fixes an issue where password inputs inside account security page is displaying a key icon instead of eye icon content toggler

- ([#35417](https://medsensehealth.ca)) Removes the room leader banner from the chat header in the new navigation.

- ([#35494](https://medsensehealth.ca)) Reduces web application memory comsumption related to duplicate Media Devices permissions events

- ([#35371](https://medsensehealth.ca)) Fixes an issue with `room-changed` event not being fired properly when switching between rooms that are available on cache.

- ([#35480](https://medsensehealth.ca)) Fixes Omnichannel's continuous sound notification not playing as expected

- ([#35540](https://medsensehealth.ca)) fixes an issue with OAuth not syncing user roles on their first login

- ([#35493](https://medsensehealth.ca) by [@sem22-dev](https://github.com/sem22-dev)) Ensures Translations Work in Offline Registration Setup Wizard

- ([#35667](https://medsensehealth.ca)) Fixes an issue with error handling where errors were not properly propagated to integrated apps.

- ([#35342](https://medsensehealth.ca)) Fixes an issue that prevents modifications from being persisted in the IPreRoomCreateModify

- ([#35532](https://medsensehealth.ca)) Fixes receiving webhook payloads encoded as x-www-form-urlencoded JSON.

- ([#35332](https://medsensehealth.ca)) Fixes error messages not showing in the UI when `Preview Public Channel` permission is not in the user role and an app is preventing the same user to join the room.

- ([#35555](https://medsensehealth.ca)) Fixes an issue causing Livechat to disconnect from the websocket when registerGuest is called multiple times with the same token

- ([#35469](https://medsensehealth.ca)) Fixes a memory leak in the web application's SDK

- ([#35559](https://medsensehealth.ca)) Fixes the enpoints `GET /api/apps/:id` and `GET /api/apps/:id/status` not returning app's data

- ([#35415](https://medsensehealth.ca)) Fixes an issue that prevents the Enable button from being displayed on some migrated apps

- ([#35386](https://medsensehealth.ca) by [@AyushKumar123456789](https://github.com/AyushKumar123456789)) Fixes an issue where the room menu inside the sidebar item remains visible when the sidebar is collapsed

- ([#35605](https://medsensehealth.ca)) Improves the performance of the Emoji Picker.

- ([#35603](https://medsensehealth.ca)) Fixes an issue where apps where not able to update messages using the BlockBuilder.

- ([#35269](https://medsensehealth.ca)) fixes a very rare issue where switching from a DM to a regular room would show an error page

- ([#35418](https://medsensehealth.ca)) Adds support for filtering bad words from messages for languages other than English
...
- ([#35422](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) fixes an issue where videoconf calls could sometimes fail to ring an user after a temporary disconnection
...
- ([#34208](https://medsensehealth.ca)) Adds a new endpoint `rooms.hide` to hide rooms of any type when provided with the room's ID

- ([#35147](https://medsensehealth.ca)) Allows users to filter by multiple departments & by livechat units on `livechat/rooms` endpoint.

- ([#34274](https://medsensehealth.ca)) Adds a new setting that if enabled, will not count bot messages in the average response time metrics

- ([#35177](https://medsensehealth.ca)) Adds a new IPostSystemMessageSent event, that is triggered whenever a new System Message is sent

- ([#34957](https://medsensehealth.ca)) Implements a modal to let users know about VoIP calls in direct messages and missing configurations.

- ([#34958](https://medsensehealth.ca)) Makes Omnichannel converstion start process transactional.

- ([#34926](https://medsensehealth.ca)) Enables control of video conference ringing and dialing sounds through the call ringer volume user preference, preventing video conf calls from always playing at maximum volume.

- ([#35260](https://medsensehealth.ca)) Enhances message sorting in the `im.messages` and `dm.messages` endpoints by enabling support for multi-parameter sorting.

- ([#34939](https://medsensehealth.ca)) Improves the search of permissions

- ([#35113](https://medsensehealth.ca)) Includes attachments metadata in JSON export if type is file when exporting messages

- ([#33885](https://medsensehealth.ca)) Fixes an issue where tokens generated while using the legacy notification provider would never be removed from the database.

- ([#33816](https://medsensehealth.ca) by [@matheusbsilva137](https://github.com/matheusbsilva137)) Replaces Livechat Visitors by Contacts on workspaces' MAC count.
...
- ([#35038](https://medsensehealth.ca)) Adds "DOMPurify" and "he" to sanitize ECDH and Livechat errors

- ([#35013](https://medsensehealth.ca)) Adds a filter option to include or exclude threads in the Apps Engine room read/unread messages bridge, enhancing flexibility in message retrieval.

- ([#35199](https://medsensehealth.ca)) Enables specifying hidden settings that are enabled to be accessed through the apps-engine in the permission list.

- ([#35089](https://medsensehealth.ca)) Adds wrapExceptions to handle an unhandled promise rejection when adding and/or updating OAuth apps

- ([#35208](https://medsensehealth.ca)) Adds the Leader group to rooms' members list for better role visibility and consistency.

### Patch Changes

- ([#35189](https://medsensehealth.ca)) fixes toast with empty error messages when a private app installation fails
...
- ([#34980](https://medsensehealth.ca)) Implements removal of OTR (off-the-record) conversation messsages just after OTR session ends.

- ([#35029](https://medsensehealth.ca)) Fixes a bug that caused routing algorithms to ignore the `Livechat_enabled_when_agent_idle` setting, effectively ignoring idle users from being assigned to inquiries.

- ([#35079](https://medsensehealth.ca)) Fixes unused `i18nTitle` provided by the app in message composer popup previewer

- ([#35044](https://medsensehealth.ca)) Fixes an issue that allowed departments to be removed via API even with setting `Omnichannel_enable_department_removal` disabled

- ([#35132](https://medsensehealth.ca)) Fixes broken link and improves messaging for invalid apps banner.

- ([#35120](https://medsensehealth.ca)) Fixes behavior of app updates that would save undesired field changes to documents

- ([#35188](https://medsensehealth.ca)) Fixes a UI issue where enabling/disabling TOTP two factor authentication didn't update in real-time.

- ([#35082](https://medsensehealth.ca)) Fixes a rerender on each sidebar item click

- ([#35233](https://medsensehealth.ca)) Improves color contrast in image gallery icon buttons to meet WCAG compliance.

- ([#35261](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#35100](https://medsensehealth.ca)) Fixes incorrect start date on omnichannel reports

- ([#35265](https://medsensehealth.ca)) Fixes app event `IPostLivechatAgentAssigned` receiving a room object previous to the assignment of agent, causing `room.servedBy` to be undefined on apps

- ([#35194](https://medsensehealth.ca)) fixes the possibility of see new messages without being subscribed to a public channel.

- ([#35121](https://medsensehealth.ca)) Fixes an issue which caused some weird scrolling in rooms when using secondary sidepanel navigation.

- ([#35170](https://medsensehealth.ca)) Fixes an issue that would cause marketplace apps to become invalid installations after an update

- ([#35140](https://medsensehealth.ca)) Fixes a UI issue where enabling/disabling email two factor authentication didn't update in real-time.

- ([#35181](https://medsensehealth.ca)) Bump meteor to 3.1.2 and Node version to 20.13.1

- ([#35107](https://medsensehealth.ca)) Fixes an issue where the room converter would throw if the room was an omnichannel room that had been closed by the visitor

- ([#35124](https://medsensehealth.ca)) Fixes an issue where non-Latin highlights were inconsistently applied, ensuring non-Latin characters are reliably detected and highlighted.

- ([#35208](https://medsensehealth.ca)) Removes the Leader role from the room header. Leaders are now displayed in their respective group within the room's members list.

- ([#34857](https://medsensehealth.ca) by [@AyushKumar123456789](https://github.com/AyushKumar123456789)) Fixes an issue where custom status' values in the custom status page table were not being properly translated

- ([#34987](https://medsensehealth.ca)) Fixes a behavior in Omnichannel that was causing bot agents to be waiting in the queue, when they should always skip it.

- ([#34988](https://medsensehealth.ca)) Fixes `channels.list` endpoint from rejecting pagination parameters

- ([#35204](https://medsensehealth.ca)) fixes an issue with embedded layout rooms displaying as if the user is not part of the room

- ([#35032](https://medsensehealth.ca)) Validates duplicated email and phone number when creating or editing omnichannel contacts.

- ([#35143](https://medsensehealth.ca)) Switches from GCM (unsupported) to FCM as the default push notification service for custom mobile apps

- ([#35167](https://medsensehealth.ca)) Fixes an issue where assign extension button were missing the proper margin

- ([#35028](https://medsensehealth.ca)) Fixes a missconception about `/v1/livechat/tags/:tagId` returning 404 if tag is not found

- ([#35269](https://medsensehealth.ca)) fixes a very rare issue where switching from a DM to a regular room would show an error page

- ([#34975](https://medsensehealth.ca)) Fixes issue where a invalid `Accounts_CustomFieldsToShowInUserInfo` value would break the ui

- ([#33141](https://medsensehealth.ca)) Fixes an issue where video conf message block wasn't considering display avatars preference

- ([#35046](https://medsensehealth.ca)) Fixes an issue which was preventing admins in enterprise workspaces from updating some premium layout settings.
...
- ([#35353](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes omnichannel transcript filename breaking download links
...
- ([#35212](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes incorrect start date on omnichannel reports

- ([#35222](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes `channels.list` endpoint from rejecting pagination parameters

- ([#35251](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) fixes an issue with embedded layout rooms displaying as if the user is not part of the room
...
- ([#35112](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes the queue processing of Omnichannel's waiting queue focusing on 3 main areas:
...
- ([#35096](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes a behavior in Omnichannel that was causing bot agents to be waiting in the queue, when they should always skip it.
...
- ([#33060](https://medsensehealth.ca)) Added `departmentsAllowedToForward` property to departments returned in the `livechat/config` endpoint

- ([#34153](https://medsensehealth.ca)) Groups members by their roles in the room's member list for improved clarity

- ([#34940](https://medsensehealth.ca)) Allows agents and managers to close Omnichannel rooms that for some reason ended up in a bad state. This "bad state" could be a room that appears open but it's closed. Now, the endpoint `livechat/room.closeByUser` will accept an optional `forceClose` parameter that will allow users to bypass most state checks we do on rooms and perform the room closing again so its state can be recovered.

- ([#34948](https://medsensehealth.ca)) Adds voice calls data to statistics

- ([#34922](https://medsensehealth.ca)) Fixes an issue where users without the "Preview public channel" permission would receive new messages sent to the channel

- ([#34191](https://medsensehealth.ca)) Allows granting the `mobile-upload-file` permission to guests

- ([#34153](https://medsensehealth.ca)) Adds `rooms.membersOrderedByRole` endpoint to retrieve members of groups and channels sorted according to their respective role in the room.

### Patch Changes

- ([#34904](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)
...
- ([#34846](https://medsensehealth.ca)) Fixes the send attachments option not working on iOS browsers by moving it from the composer dropdown menu to the composer primary actions.
...
- ([#34849](https://medsensehealth.ca)) Fixes an issue where losing connection could break app's action buttons

- ([#35010](https://medsensehealth.ca)) Prevent a bug that caused all sessions being marked as logged out if some required value was missing due to a race condition.

- ([#34864](https://medsensehealth.ca)) Allows users to fetch the `packageValue` of settings when calling `/settings` endpoint via `includeDefaults` query param.

- ([#34908](https://medsensehealth.ca)) Fixes an issue where room scroll position wasn't being restored when moving between rooms.

- ([#34887](https://medsensehealth.ca)) Fixes an issue where the system would throw the error: 'GUI Application error' while uninstalling an app(when on the requests tab).

- ([#34900](https://medsensehealth.ca)) Fixes an issue that prevented room history from loading under certain conditions.

- ([#35009](https://medsensehealth.ca)) Fix an issue with apps installations via Marketplace

- ([#34503](https://medsensehealth.ca)) Fixes an issue where changing user role in a room displays the wrong message

- ([#34873](https://medsensehealth.ca)) Fixes SAML login redirecting to wrong room when using an invite link.

- ([#34839](https://medsensehealth.ca)) Fixes apps actions showing in toolbar without an icon

- ([#34892](https://medsensehealth.ca)) Fixes an issue where Rocket cat would send '%s' as the mongo version when using a deprecated but supported version of MongoDB.

- ([#34860](https://medsensehealth.ca)) Fixes an issue where room's file list would incorrectly display incoming messages as files.

- ([#34210](https://medsensehealth.ca)) Fixes livechat conversations not being assigned to the contact manager even when the "Assign new conversations to the contact manager" setting is enabled

- ([#34113](https://medsensehealth.ca)) Fixes dates being incorrect when fetching omnichannel reports with the following time periods:
...
- ([#35055](https://medsensehealth.ca)) Fixes an issue where the container image could not be run as any random non-root user id.

- ([#34935](https://medsensehealth.ca)) Fixes an issue with the retention policy max age settings not maintaning it's previous value when upgrading from version < 6.10

- ([#34933](https://medsensehealth.ca)) Fixes SlackBridge service failing to connect to multiple slack workspaces

- ([#34851](https://medsensehealth.ca)) Fixes thumbnails not being deleted from storage on room deletion
...
- ([#34918](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes a behavior when running microservices that caused queue worker to process just the first 60 seconds of request.
...
- ([#34946](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes SlackBridge service failing to connect to multiple slack workspaces
...
- ([#34194](https://medsensehealth.ca)) Adds a new `contacts.checkExistence` endpoint, which allows identifying whether there's already a registered contact using a given email, phone, id or visitor to source association.

- ([#34004](https://medsensehealth.ca)) Allows Rocket.Chat to store call events.

- ([#33895](https://medsensehealth.ca)) Adds statistics related to the new **Contact Identification** feature:
...
- ([#34220](https://medsensehealth.ca)) Disables OTR messages selection when exporting messages

- ([#34121](https://medsensehealth.ca)) Organizes App Settings interface by introducing section-based accordion groups to improve navigation and readability for administrators.

- ([#34076](https://medsensehealth.ca)) Introduces a new option when exporting messages, allowing users to select and download a JSON file directly from client

- ([#34057](https://medsensehealth.ca)) Improves the workspace and subscription admin pages by updating font scaling, centralizing elements,
  enhancing responsiveness, and refactoring components to provide a better overall user experience.
- ([#33549](https://medsensehealth.ca)) Adds a new callout in the subscription page to inform users of subscription upgrade eligibility when applicable.
...
- ([#34137](https://medsensehealth.ca)) Fixes Unit's `numDepartments` property not being updated after a department is removed
...
- ([#34038](https://medsensehealth.ca)) Removes a validation that allowed only the room creator to propagate E2EE room keys. This was causing issues when the rooms were created via apps or some other integration, as the creator may not be online or able to create E2EE keys

- ([#34089](https://medsensehealth.ca)) Fixed an issue that caused clients to not properly receive certain server notifications right after login

- ([#34858](https://medsensehealth.ca)) Fixes an issue that prevented the apps-engine from reestablishing communications with subprocesses in some cases

- ([#34013](https://medsensehealth.ca)) Fixes issue that caused the livechat appearance form not be able to submit on CE servers

- ([#33873](https://medsensehealth.ca)) Fixes the incorrect registration status shown on admin users page for federated remote users.

- ([#34156](https://medsensehealth.ca)) Fixes "Average first response time" and "Best first response time" metrics being associated with the last agent who served the room (instead of the first one)

- ([#33280](https://medsensehealth.ca)) Fixes an issue preventing the creation of normal direct message rooms due to an invalid federation configuration, allowing proper room creation under standard settings.

- ([#34187](https://medsensehealth.ca)) Adds support for `Use Full Name Initials to Generate Default Avatar` setting for the generated avatar preview button when editing an User's avatar

- ([#34130](https://medsensehealth.ca)) Fixes an issue where room members menu doesn't display properly without enough space

- ([#34079](https://medsensehealth.ca)) Fixes missing images in Twitter article links to ensure proper display by relying on meta tags.

- ([#34205](https://medsensehealth.ca)) Fixes error propagation when trying to get the status of apps in some cases

- ([#33253](https://medsensehealth.ca)) Fixes an issue with Federation startup where the bridge would intermittently fail to start causing error being shown "Matrix Bridge isn't running yet".

- ([#34062](https://medsensehealth.ca)) Fixes condition causing Omnichannel queue to start more than once.

- ([#34197](https://medsensehealth.ca)) fixes "Change to language" button in login page not displaying the target language

- ([#34148](https://medsensehealth.ca)) Fixes messages not being processed for all slack servers

- ([#34060](https://medsensehealth.ca)) Fixes an issue where premium settings were using the wrong `Tag` variant in Omnichannel Appearance configuration

- ([#34215](https://medsensehealth.ca)) Fixes a UI issue that showed the incorrect migration number on the `Information` page. This was caused by a function calculating the stats before the server had migrated the database and updated the control.

- ([#34124](https://medsensehealth.ca)) Allows default avatars to be generated with more than one inital (limited to first 3) when setting `Use Full Name Initials to Generate Default Avatar` is true.

- ([#34165](https://medsensehealth.ca)) Fixes an issue where removing the only message of a thread would keep the unread thread messages badge

- ([#34109](https://medsensehealth.ca)) Fixes `im.counters` endpoint returning `null` on `unread` messages property for users that have never opened the queried DM

- ([#34205](https://medsensehealth.ca)) Fixes wrong data being reported to total failed apps metrics and statistics

- ([#34080](https://medsensehealth.ca)) Fixes sidepanel list not sorting by last message inside thread

- ([#34092](https://medsensehealth.ca)) fixes mail export form "To additional emails" field validation

- ([#34174](https://medsensehealth.ca)) Fixed an issue that added potencially infinite callbacks to the same event, degrading performance over time.

- ([#34127](https://medsensehealth.ca)) Fixes a behavior of the mentions parser that identified mentions inside markdown links text. Now, these components will be removed from the text before trying to parse mentions.

- ([#34217](https://medsensehealth.ca)) Fixes an issue where the update banner wasn't showing the new version number

- ([#34101](https://medsensehealth.ca)) Fixes an issue which caused action menu to be clipped in moderation page on smaller screens for better usability.

- ([#33906](https://medsensehealth.ca) by [@thepiyush-303](https://github.com/thepiyush-303)) Sorts the list of language options by name correctly

- ([#34068](https://medsensehealth.ca)) fixed an issue that caused the conference call ringer to fail to accept calls if the user logged out and in again

- ([#34000](https://medsensehealth.ca)) Fixes special characters not being escaped on sidepanel extended view

- ([#34209](https://medsensehealth.ca)) Fixed the data structure of the features preview

- ([#34139](https://medsensehealth.ca)) Fixes contact update failing in case a custom field is removed from the workspace

- ([#34202](https://medsensehealth.ca)) Fixes an issue where the notification sound was playing randomly
...
- ([#33897](https://medsensehealth.ca)) adds unread badge to sidebar collapser

- ([#32906](https://medsensehealth.ca)) Improves thread metrics featuring user avatars, better titles and repositioned elements.

- ([#33810](https://medsensehealth.ca)) Adds cursor pagination on chat.syncMessages endpoint

- ([#33214](https://medsensehealth.ca)) Adds a new route to allow fetching avatars by the user's id `/avatar/uid/<UserID>`

- ([#32727](https://medsensehealth.ca)) These changes aims to add:
...
- ([#33920](https://medsensehealth.ca)) Improves the customizability of the naming of automatic Persistent video calls discussions, allowing the date of the call to be in different parts of the name, using the `[date]` keyword.

- ([#33997](https://medsensehealth.ca)) Prevent apps' subprocesses from crashing on unhandled rejections or uncaught exceptions

- ([#33814](https://medsensehealth.ca)) Adds a confirmation modal to the cancel subscription action

- ([#33949](https://medsensehealth.ca)) Disables the possiblity to upload exempted apps
...
- ([#33776](https://medsensehealth.ca)) Fix user highlights not matching only whole words

- ([#33818](https://medsensehealth.ca)) Remove unused client side `setUserActiveStatus` meteor method.

- ([#33596](https://medsensehealth.ca)) Bump meteor to 3.0.4 and Node version to 20.18.0

- ([#33713](https://medsensehealth.ca)) Deprecated the `from` field in the apps email bridge and made it optional, using the server's settings when the field is omitted

- ([#32991](https://medsensehealth.ca)) Fixes an issue where updating custom emojis didn’t work as expected, ensuring that uploaded emojis now update correctly and display without any caching problems.

- ([#33985](https://medsensehealth.ca)) Fixes issue that could cause multiple discussions to be created when creating it from a message action

- ([#33904](https://medsensehealth.ca)) adds missing html attributes in sidebar item templates

- ([#33218](https://medsensehealth.ca)) Fixes message character limit not being applied to file upload descriptions

- ([#33908](https://medsensehealth.ca)) Fixes the issue where newly created teams are incorrectly displayed as channels on the sidebar when the DISABLE_DB_WATCHERS environment variable is enabled

- ([#33953](https://medsensehealth.ca)) Adds a divider on Create team modal - advanced settings

- ([#33786](https://medsensehealth.ca)) Fixed an issue that would grant network permission to app's processes in wrong cases

- ([#33986](https://medsensehealth.ca)) Fixes sidepanel not replicating sidebar sort preference

- ([#33689](https://medsensehealth.ca)) Fixes banner breaking the UI with specific payloads

- ([#33808](https://medsensehealth.ca)) Fixes client-side updates for recent emoji list when custom emojis are modified.

- ([#33902](https://medsensehealth.ca)) Adds "Master volume" and "Call ringer volume" to the user preferences sound section.

- ([#33311](https://medsensehealth.ca)) Sends server statistics only once a day despite multiple instance being started at different times.

- ([#33719](https://medsensehealth.ca)) stops calling an object through proxy calling getQueueWorker

- ([#33785](https://medsensehealth.ca)) Fixed an issue where the installed apps list would go stale without a refresh in some cases

- ([#33278](https://medsensehealth.ca)) Fixes display of emoji aliases in custom emoji list by adding commas between aliases

- ([#33772](https://medsensehealth.ca)) Fixes `waiting queue` feature. When `Livechat_waiting_queue` setting is enabled, incoming conversations should be sent to the queue instead of being assigned directly.

- ([#33963](https://medsensehealth.ca)) Fixes edge case of thread unread not being added to unread group

- ([#33994](https://medsensehealth.ca)) Adds login and permission validation for resetIrcConnection method

- ([#33880](https://medsensehealth.ca)) Updates VoIP field labels from 'Free Extension Numbers' to 'Available Extensions' to better describe the field's purpose and improve clarity.

- ([#33958](https://medsensehealth.ca)) Fixes an issue where resizable handler renders over the expanded thread view while using contextualbarResizable feature preview
...
- ([#33316](https://medsensehealth.ca)) Changes some displays to reflect new rules for private apps and adds a new modal before uploading a private app

- ([#32212](https://medsensehealth.ca)) Fixed broken translation in "Forgot Password" e-mail

- ([#33503](https://medsensehealth.ca)) Adds modal confirmation to enable and disable End-to-end encryption
...
- ([#33038](https://medsensehealth.ca)) Remove support of filtering by agent's username on the following endpoint:
...
- ([#33474](https://medsensehealth.ca)) Removes deprecated endpoint `pw.getPolicyReset`. Moving forward, use the `pw.getPolicy` endpoint.

- ([#32162](https://medsensehealth.ca)) As per MongoDB Lifecycle Schedules ([mongodb.com/legal/support-policy/lifecycles](https://www.mongodb.com/legal/support-policy/lifecycles)) we're removing official support to MongoDB version 4.4 that has reached end of life in February 2024.
...
- ([#33333](https://medsensehealth.ca)) Login services button was not respecting the button color and text color settings. Implemented a fix to respect these settings and change the button colors accordingly.
...
- ([#32162](https://medsensehealth.ca)) Added MongoDB 7.0 support

- ([#33449](https://medsensehealth.ca)) Removes deprecated method `livechat:webhookTest`. Moving forward use the endpoint `livechat/webhook.test`.

- ([#33373](https://medsensehealth.ca)) This adjustment removes the deprecated method `livechat:searchAgent`. Moving forward, use `livechat/users/agent/:_id` endpoint.

- ([#33442](https://medsensehealth.ca)) Removes deprecated `livechat:getAnalyticsOverviewData` method. Moving forward use the `livechat/analytics/overview` endpoint.

- ([#31889](https://medsensehealth.ca)) Removed upsert behavior on `users.update` endpoint (`joinDefaultChannels` param or empty `userId` are not allowed anymore)

- ([#33802](https://medsensehealth.ca)) Changes groups messages listing endpoint by moving query params from the 'query' attribute to standard query parameters.

- ([#32159](https://medsensehealth.ca)) Api login should not suggest which credential is wrong (password/username)

  Failed login attemps will always return `Unauthorized` instead of the internal fail reason

- ([#31117](https://medsensehealth.ca)) Adds a new set of permissions to provide a more granular control for the creation and deletion of rooms within teams
...
- ([#33391](https://medsensehealth.ca)) Removed deprecated method `livechat:loginByToken`. Moving forward, use the endpoint `livechat/visitor/:token`.

- ([#33473](https://medsensehealth.ca)) Removes deprecated method `getPasswordPolicy`. Moving forward, use the endpoint `pw.getPolicy`.

- ([#33210](https://medsensehealth.ca)) Removes private App installation via URL method following a deprecation warning.

- ([#33429](https://medsensehealth.ca)) Removed deprecated methods `livechat:saveTrigger` and `livechat:removeTrigger`. Moving forward use the endpoints `livechat/triggers (POST)` and `livechat/triggers/:_id (DELETE)` respectively.

- ([#33465](https://medsensehealth.ca)) Removes deprecated method `addOAuthApp`. Moving forward, use the endpoint `oauth-apps.create` instead.

- ([#33472](https://medsensehealth.ca)) Removes deprecated `deleteMessage` method. Moving forward, use the `chat.delete` endpoint.

- ([#33447](https://medsensehealth.ca)) Removes deprecated method `livechat:saveInfo`. Moving forward use the enpoint `livechat/room/save.info`.

- ([#31438](https://medsensehealth.ca)) Upgrades the version of the Meteor framework to 3.0
...
- ([#33426](https://medsensehealth.ca)) Removed deprecated method `livechat:getNextAgent`. Moving forward, use the `livechat/agent.next/:token` endpoint.

- ([#33526](https://medsensehealth.ca)) Removes deprecated method `livechat:setDepartmentForVisitor`. Moving forward, use the endpoint `livechat/visitor`.

- ([#33241](https://medsensehealth.ca)) Adds restrictions to air-gapped environments without a license

- ([#33042](https://medsensehealth.ca)) Removes `view-history` permission due to lack of usage

- ([#33448](https://medsensehealth.ca)) Removes the deprecated method `livechat:saveAppearance`. Moving forward use the endpoint `livechat/appearance`.

- ([#33445](https://medsensehealth.ca)) Removes deprecated `livechat:getAgentOverviewData` method. Moving forward use `livechat/analytics/agent-overview` endpoint.

- ([#33444](https://medsensehealth.ca)) Removes deprecated method `livechat:setCustomField`. The custom fields can be directly set via the `livechat/visitor` endpoint.

- ([#33471](https://medsensehealth.ca)) Removes deprecated endpoint `channels.images`. Moving forward, use `rooms.images` endpoint.

- ([#33650](https://medsensehealth.ca)) Changes custom emoji listing endpoint by moving query params from the 'query' attribute to standard query parameters.

- ([#32856](https://medsensehealth.ca)) Adds a new collection to store all the workspace cloud tokens to defer the race condition management to MongoDB instead of having to handle it within the settings cache.
...
- ([#33423](https://medsensehealth.ca)) Removed deprecated methods `livechat:removeAgent`, `livechat:removeManager` and `livechat:removeDepartment`. Moving forward, use `livechat/users/agent/:_id`, and `livechat/users/manager/:_id` and `livechat/department/:_id` respectively.`

- ([#31438](https://medsensehealth.ca)) Node.js 20.x support

- ([#33801](https://medsensehealth.ca)) Changes channels messages listing endpoint by moving query params from the 'query' attribute to standard query parameters.

- ([#32154](https://medsensehealth.ca)) Removed the ability to import data in the HipChat Enterprise format, as it was discontinued over five years ago.

- ([#33419](https://medsensehealth.ca)) Changes End-to-end encryption default setting to enable mention in encrypted messages

- ([#33451](https://medsensehealth.ca)) Removes deprecated method `livechat:saveIntegration`. Moving forward, use the endpoint `omnichannel/integrations (POST)`.

- ([#33443](https://medsensehealth.ca)) Removed deprecated method `livechat:saveSurveyFeedback`. Moving forward use the endpoint `livechat/room.survey`.

- ([#33787](https://medsensehealth.ca)) Changes settings public listing endpoint by moving query params from the 'query' attribute to standard query parameters.
...
- ([#33453](https://medsensehealth.ca)) Removes deprecated `livechat/inquiries.queued` endpoint. Moving forward use the `livechat/inquiries.queuedForUser` endpoint.

- ([#31438](https://medsensehealth.ca)) Remove linkedin oauth package, now linkedin oauth must to me configured as custom oauth

- ([#32285](https://medsensehealth.ca)) Fixed issue with LDAP sync triggering multiple cron jobs in case an invalid sync interval is provided

- ([#33328](https://medsensehealth.ca)) Allows authorized users to reset the encryption key for end-to-end encrypted rooms. This aims to prevent situations where all users of a room have lost the encryption key, and as such, the access to the room.

- ([#33539](https://medsensehealth.ca)) Removes deprecated method `livechat:registerAgent`. Moving forward, use the endpoint `livechat/visitor`.

- ([#33520](https://medsensehealth.ca)) Fixes a behavior of E2EE room creation that allowed any user on the room to define room keys before the room creator, causing race conditions.

- ([#33329](https://medsensehealth.ca)) Randomizes `e2eKeyId` generation instead of derive it from encoded key. Previously, we used the stringified & encoded version of the key to extract a keyID, however this generated the same keyID for all rooms. As we didn't use this keyID, and rooms didn't have the capability of having multiple keys, this was harmless.
...
- ([#33361](https://medsensehealth.ca)) Change the E2EE setting - "Access unencrypted content in encrypted rooms" default value, making the current behavior not allow to send unencrypted messages in end-to-end encrypted channels.

- ([#33470](https://medsensehealth.ca)) Removes deprecated endpoints `licenses.isEnterprise` and `licenses.get`. Moving forward use the endpoint `licenses.info.`

- ([#33371](https://medsensehealth.ca)) This adjustment removes deprecated `livechat:getCustomFields` method. Moving forward use the `livechat/custom-fields` endpoint.

- ([#33605](https://medsensehealth.ca)) Updates End-to-end settings translations and removes beta wording

- ([#33572](https://medsensehealth.ca)) Removes the ability of changing room's encryption status from the `key` icon placed on the room's header. Icon's purpose is now only informative, showing when a room uses E2EE. Use the kebab menu to enable/disable E2EE.

- ([#33432](https://medsensehealth.ca)) Removed deprecated methods `livechat:requestTranscript` and `livechat:discardTranscript`. Moving forward use `livechat/transcript/:rid` endpoint's POST and DELETE methods.

- ([#31383](https://medsensehealth.ca)) No longer shows archived rooms in `rooms.autocomplete.channelAndPrivate` endpoint

- ([#33807](https://medsensehealth.ca)) Changes ims and dms messages listing endpoint by moving query params from the 'query' attribute to standard query parameters.

- ([#32532](https://medsensehealth.ca)) Removed `meteor/check` from `chat` endpoints

- ([#33434](https://medsensehealth.ca)) Renames the settings group 'Voice Channel' to 'Omnichannel voice channel (VoIP)' to better reflect its responsibility.

- ([#33427](https://medsensehealth.ca)) Removed deprecated method `livechat:pageVisited`. Moving forward, use the `livechat/page.visited` endpoint.

- ([#33446](https://medsensehealth.ca)) Removed deprecated method `livechat:saveDepartmentAgents`. Moving forward, use the endpoint `livechat/department/:_id/agents`.

- ([#33630](https://medsensehealth.ca)) Changes the payload of the startImport endpoint to decrease the amount of data it requires

- ([#33238](https://medsensehealth.ca)) Adds new empty states for the marketplace view

- ([#33452](https://medsensehealth.ca)) Removes deprecated method `livechat:sendOfflineMessage`. Moving forward, use the endpoint `livechat/offline.message`.

- ([#33450](https://medsensehealth.ca)) Removes deprecated method `livechat:getAgentData`. Moving forward use the endpoint `livechat/agent.info/:rid/:token`.

- ([#33461](https://medsensehealth.ca)) Removes deprecated endpoint `livechat/room.visitor`.

### Minor Changes

- ([#32194](https://medsensehealth.ca)) Adds methods to the Apps-Engine to interact with unread messages to enhance message capabilities on Apps.

- ([#33377](https://medsensehealth.ca)) Send messages with encrypted attachments to mobile notification service

- ([#33489](https://medsensehealth.ca)) Adds `Recent` button on the new sidebar Search section to replicate the previous behavior of focusing the search bar - show recent chats.

- ([#33562](https://medsensehealth.ca)) Introduces a new featured action on the room header for action buttons using the non-default category to enhance user accessibility.

- ([#33592](https://medsensehealth.ca)) Adds ability to collapse/expand sidebar groups

- ([#33294](https://medsensehealth.ca)) Improves the accessibility of the report user modal by adding an appropriate label, description, and ARIA attributes.

- ([#33283](https://medsensehealth.ca)) Adds a warning to inform users they are about to send unencrypted messages in an E2E Encrypted room if they have the `Unencrypted messages in encrypted rooms` setting enabled.

- ([#33066](https://medsensehealth.ca)) Introduces new property `category` for Rocket.Chat Apps to register UI action buttons. This property is used to group buttons in the UI.

- ([#33386](https://medsensehealth.ca)) Fixes the departments filter on the omnichannel current chats page by ensuring that the selected department is fetched and
  ...
  UX issues.
- ([#33598](https://medsensehealth.ca)) Adds a new setting to allow mapping LDAP attributes to the user's extension

- ([#33496](https://medsensehealth.ca)) Applied category reorder from admin setting also on new sidebar - Enhanced navigation feature preview

- ([#33433](https://medsensehealth.ca)) Added support for interacting with add-ons issued in the license

- ([#33483](https://medsensehealth.ca)) Introduces new visual components into marketplace pages to inform an add-on necessity into the workspace.
...
- ([#32648](https://medsensehealth.ca) by [@AllanPazRibeiro](https://github.com/AllanPazRibeiro)) This adjustment removes the deprecated `eraseRoom` method. Moving forward, use the `room.delete` endpoint to delete rooms.

- ([#33381](https://medsensehealth.ca)) Fixes a race condition that causes livechat conversations to get stuck in the agent's sidebar panel after being forwarded.

- ([#33355](https://medsensehealth.ca)) This adjustment removes the deprecated `removeWebdavAccount` method. Moving forward, use the `webdav.removeWebdavAccount` endpoint to remove WebDAV accounts.

- ([#33527](https://medsensehealth.ca)) Fixes E2EE composer freezing when the room state changes

- ([#33332](https://medsensehealth.ca)) Fixes an issue where a thread message was not scrolled to view when it was opened using its link.

- ([#33028](https://medsensehealth.ca)) Fixes an issue where ignore threads parameter were not being affected by retention policy overriding in old channels

- ([#33346](https://medsensehealth.ca)) Implements integration with FreeSwitch to enable VoIP calls for team collaboration workspaces

- ([#33364](https://medsensehealth.ca)) Fixed Twilio request validation by accepting URLs with query strings

- ([#33330](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#33606](https://medsensehealth.ca)) Fixed the regex for domains in the isValidDomain function

- ([#33375](https://medsensehealth.ca)) Fixes title missing in thread list for threads starting with attachment in E2EE rooms

- ([#33435](https://medsensehealth.ca)) Fixes an issue causing server to not notify users via websocket of new E2EE keys suggested by other users to them when running in development environments.

- ([#33770](https://medsensehealth.ca)) Fixes page loading during reconnections

- ([#33412](https://medsensehealth.ca)) Fixes a logical error when updating the `responseBy` property of a room while `waitingResponse` property was still defined.

- ([#33631](https://medsensehealth.ca)) Fixes E2EE not rendering new navigation feature preview

- ([#33417](https://medsensehealth.ca)) Fixes issue with previously disabled private apps being auto enabled on update

- ([#33583](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#33424](https://medsensehealth.ca)) Fixes a problem with custom sounds causing files to not be playable when using filesystem storage.

- ([#28042](https://medsensehealth.ca)) Changes the default base Docker image to Alpine. Previously we were shipping Alpine as an alternative flavor under the tag rocketchat/rocket.chat:{release}.alpine , we have been testing this for a while now so we're migrating to use the official one to Alpine.

  We'll still ship a Debian variant under the tag rocketchat/rocket.chat:{release}.debian.

- ([#33390](https://medsensehealth.ca)) Removed deprecated method `livechat:loadHistory` method. Moving forward use the `ivechat/messages.history/:rid` endpoint

- ([#33358](https://medsensehealth.ca)) This adjustment removes the deprecated `Mailer.sendMail` and `Mailer:unsubscribe` methods. Moving forward, use the `mailer` and `mailer.unsubscribe` endpoints.

- ([#33405](https://medsensehealth.ca)) Fixes an issue where a user may receive an indefinite amount of "suggested keys" from all the users on a room even when a user already suggested a key for them, duplicating the work on server.
...
- ([#33652](https://medsensehealth.ca)) Fixes message toolbar showing in End-to-end encrypted room when messages are not decrypted

- ([#33372](https://medsensehealth.ca)) This adjustment removes deprecated `livechat:addAgent` and `livechat:addManager` method. Moving forward use `livechat/users/agent` and `livechat/users/manager` endpoints to add agent and manager respectively.

- ([#33507](https://medsensehealth.ca)) Fixed an issue where "Filter by room type" was selectable in the Rooms filter.

- ([#33378](https://medsensehealth.ca)) Fixes async E2EE key exchange in development environments where change streams are no longer used
...
- ([#33569](https://medsensehealth.ca)) Adds a `source` field to livechat visitors, which stores the channel (eg API, widget, SMS, email-inbox, app) that's been used by the visitor to send messages.
...
- ([#33156](https://medsensehealth.ca)) added `sidepanelNavigation` to feature preview list

- ([#32682](https://medsensehealth.ca)) Added support for specifying a unit on departments' creation and update

- ([#33139](https://medsensehealth.ca)) Added new setting `Allow visitors to finish conversations` that allows admins to decide if omnichannel visitors can close a conversation or not. This doesn't affect agent's capabilities of room closing, neither apps using the livechat bridge to close rooms.
...
- ([#32729](https://medsensehealth.ca)) Implemented "omnichannel/contacts.update" endpoint to update contacts

- ([#32510](https://medsensehealth.ca)) Added a new setting to enable mentions in end to end encrypted channels

- ([#32821](https://medsensehealth.ca)) Replaced new `SidebarV2` components under feature preview

- ([#33212](https://medsensehealth.ca)) Added new Admin Feature Preview management view, this will allow the workspace admins to both enable feature previewing in the workspace as well as define which feature previews are enabled by default for the users in the workspace.

- ([#33011](https://medsensehealth.ca)) Return `parent` and `team` information when calling `rooms.info` endpoint

- ([#32693](https://medsensehealth.ca)) Introduced "create contacts" endpoint to omnichannel

- ([#33177](https://medsensehealth.ca)) New `teams.listChildren` endpoint that allows users listing rooms & discussions from teams. Only the discussions from the team's main room are returned.

- ([#33114](https://medsensehealth.ca)) Wraps some room settings in an accordion advanced settings section in room edit contextual bar to improve organization

- ([#33160](https://medsensehealth.ca)) Implemented sending email via apps

- ([#32945](https://medsensehealth.ca)) Added a new setting which allows workspace admins to disable email two factor authentication for SSO (OAuth) users. If enabled, SSO users won't be asked for email two factor authentication.

- ([#33225](https://medsensehealth.ca)) Implemented new feature preview for Sidepanel

### Patch Changes

- ([#33339](https://medsensehealth.ca)) Fixes a problem that caused visitor creation to fail when GDPR setting was enabled and visitor was created via Apps Engine or the deprecated `livechat:registerGuest` method.
...
- ([#33317](https://medsensehealth.ca)) Fixed error during sendmessage client stub

- ([#33381](https://medsensehealth.ca)) Fixes a race condition that causes livechat conversations to get stuck in the agent's sidebar panel after being forwarded.

- ([#33211](https://medsensehealth.ca)) Allow to use the token from `room.v` when requesting transcript instead of visitor token. Visitors may change their tokens at any time, rendering old conversations impossible to access for them (or for APIs depending on token) as the visitor token won't match the `room.v` token.

- ([#33298](https://medsensehealth.ca)) Fixed a Federation callback not awaiting db call

- ([#32939](https://medsensehealth.ca)) Fixed issue where when you marked a room as unread and you were part of it, sometimes it would mark it as read right after

- ([#33197](https://medsensehealth.ca)) Fixes an issue where the retention policy warning keep displaying even if the retention is disabled inside the room

- ([#33321](https://medsensehealth.ca)) Changed the contextualbar behavior based on chat size instead the viewport

- ([#33246](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#32999](https://medsensehealth.ca)) Fixes multiple selection for MultiStaticSelectElement in UiKit

- ([#33155](https://medsensehealth.ca)) Fixed a code issue on NPS service. It was passing `startAt` as the expiration date when creating a banner.

- ([#33237](https://medsensehealth.ca)) fixed retention policy max age settings not being respected after upgrade

- ([#33216](https://medsensehealth.ca)) Prevented uiInteraction to subscribe multiple times

- ([#33295](https://medsensehealth.ca)) Resolves the issue where outgoing integrations failed to trigger after the version 6.12.0 upgrade by correcting the parameter order from the `afterSaveMessage` callback to listener functions. This ensures the correct room information is passed, restoring the functionality of outgoing webhooks, IRC bridge, Autotranslate, and Engagement Dashboard.

- ([#33193](https://medsensehealth.ca)) Fixed avatar blob image setting in setUserAvatar method by correcting service handling logic.

- ([#33209](https://medsensehealth.ca)) Fixed `LivechatSessionTaken` webhook event being called without the `agent` param, which represents the agent serving the room.

- ([#33296](https://medsensehealth.ca)) Fixed remaining direct references to external user avatar URLs
...
- ([#33157](https://medsensehealth.ca) by [@csuadev](https://github.com/csuadev)) Fixed inconsistency between the markdown parser from the composer and the rest of the application when using bold and italics in a text.

- ([#33181](https://medsensehealth.ca)) Fixed issue that caused an infinite loading state when uploading a private app to Rocket.Chat

- ([#33158](https://medsensehealth.ca)) Fixes an issue where multi-step modals were closing unexpectedly
...
- ([#33242](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Allow to use the token from `room.v` when requesting transcript instead of visitor token. Visitors may change their tokens at any time, rendering old conversations impossible to access for them (or for APIs depending on token) as the visitor token won't match the `room.v` token.

- ([#33268](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Security Hotfix (https://medsensehealth.ca)

- ([#33265](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) fixed retention policy max age settings not being respected after upgrade
...
- ([#32535](https://medsensehealth.ca)) Federation actions like sending message in a federated DM, reacting in a federated chat, etc, will no longer work if the configuration is invalid.

- ([#32916](https://medsensehealth.ca)) Added a new Audit endpoint `audit/rooms.members` that allows users with `view-members-list-all-rooms` to fetch a list of the members of any room even if the user is not part of it.

- ([#32032](https://medsensehealth.ca)) Added a new 'Deactivated' tab to the users page, this tab lists users who have logged in for the first time but have been deactivated for any reason. Also added the UI code for the Active tab;

- ([#33044](https://medsensehealth.ca)) Replaces an outdated banner with the Bubble component in order to display retention policy warning

- ([#32867](https://medsensehealth.ca)) Added an accordion for advanced settings on Create teams and channels

- ([#32709](https://medsensehealth.ca) by [@heet434](https://github.com/heet434)) Add "Created at" column to admin rooms table

- ([#32535](https://medsensehealth.ca)) New button added to validate Matrix Federation configuration. A new field inside admin settings will reflect the configuration status being either 'Valid' or 'Invalid'.

- ([#32969](https://medsensehealth.ca)) Upgrades fuselage-toastbar version in order to add pause on hover functionality

- ([#33003](https://medsensehealth.ca)) Added a new setting to enable/disable file encryption in an end to end encrypted room.

- ([#32868](https://medsensehealth.ca)) Added `sidepanel` field to `teams.create` and `rooms.saveRoomSettings` endpoints

- ([#33003](https://medsensehealth.ca)) Fixed a bug related to uploading end to end encrypted file.
...
- ([#33159](https://medsensehealth.ca)) Improves Omnichannel queue page performance

- ([#32986](https://medsensehealth.ca)) Fixed login with third-party apps not working without the "Manage OAuth Apps" permission

- ([#32852](https://medsensehealth.ca)) Federated users can no longer be deleted.

- ([#33033](https://medsensehealth.ca) by [@csuadev](https://github.com/csuadev)) Fixed an issue due to an endpoint pagination that was causing that when an agent have assigned more than 50 departments, the departments have a blank space instead of the name.

- ([#33058](https://medsensehealth.ca)) Prevent `processRoomAbandonment` callback from erroring out when a room was inactive during a day Business Hours was not configured for.

- ([#24889](https://medsensehealth.ca) by [@Shivansh2287](https://github.com/Shivansh2287)) Fixes an issue where the Announcement modal with long words was adding a horizontal scrollbar

- ([#32940](https://medsensehealth.ca)) Stopped non channel members from dragging and dropping files in a channel they do not belong

- ([#33001](https://medsensehealth.ca)) Allow apps to react/unreact to messages via bridge

- ([#32809](https://medsensehealth.ca)) Deactivating users who federated will now be permanent.

- ([#31525](https://medsensehealth.ca)) Fix: Show correct user info actions for non-members in channels.

- ([#33136](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#32931](https://medsensehealth.ca)) Fixed an issue that caused UI to show an error when the call to get the Business Hour type from settings returned `undefined`.

- ([#32743](https://medsensehealth.ca)) Fixes an issue where creating a new user with an invalid username (containing special characters) resulted in an error message, but the user was still created. The user creation process now properly aborts when an invalid username is provided.

- ([#33109](https://medsensehealth.ca)) Fixes the `expanded` prop being accidentally forwarded to `ContextualbarHeader`

- ([#32846](https://medsensehealth.ca)) Fixed issue with system messages being counted as agents' first responses in livechat rooms (which caused the "best first response time" and "average first response time" metrics to be unreliable for all agents)

- ([#32791](https://medsensehealth.ca)) Fixed a behavior when updating messages that prevented the `customFields` prop from being updated if there were no changes to the `msg` property. Now, `customFields` will be always updated on message update even if `msg` doesn't change

- ([#33169](https://medsensehealth.ca)) Fixed issue that prevented sending encrypted filed from the mobile app

- ([#33101](https://medsensehealth.ca)) Fixed an issue where teams were being created with no room associated with it.

- ([#33036](https://medsensehealth.ca)) Fixes multiple problems with the `processRoomAbandonment` hook. This hook is in charge of calculating the time a room has been abandoned (this means, the time that elapsed since a room was last answered by an agent until it was closed). However, when business hours were enabled and the user didn't open on one day, if an abandoned room happened to be abandoned _over_ the day there was no business hour configuration, then the process will error out.
...
- ([#33174](https://medsensehealth.ca)) Restored tooltips to the unit edit department field selected options

- ([#33054](https://medsensehealth.ca)) Fixed issue with livechat analytics in a given date range considering conversation data from the following day

- ([#32981](https://medsensehealth.ca)) fixed an issue with the "follow message" button not changing state after click

- ([#32928](https://medsensehealth.ca)) Fixed issue where `after-registration-triggers` would show up in a page when the user was not yet registered

- ([#33047](https://medsensehealth.ca)) Fixed: Custom fields in extraData now correctly added to extraRoomInfo by livechat.beforeRoom callback during livechat room creation.

- ([#33040](https://medsensehealth.ca)) Fixed an issue related to setting Accounts_ForgetUserSessionOnWindowClose, this setting was not working as expected.
...
- ([#33033](https://medsensehealth.ca)) Fixed an issue due to an endpoint pagination that was causing that when an agent have assigned more than 50 departments, the departments have a blank space instead of the name.
...
- ([#33084](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Prevent `processRoomAbandonment` callback from erroring out when a room was inactive during a day Business Hours was not configured for.

- ([#33153](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Security Hotfix (https://medsensehealth.ca)

- ([#33185](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Restored tooltips to the unit edit department field selected options

- ([#33129](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixed an issue related to setting Accounts_ForgetUserSessionOnWindowClose, this setting was not working as expected.

  The new meteor 2.16 release introduced a new option to configure the Accounts package and choose between the local storage or session storage. They also changed how Meteor.\_localstorage works internally. Due to these changes in Meteor, our setting to use session storage wasn't working as expected. This PR fixes this issue and configures the Accounts package according to the workspace settings.

- ([#33178](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue where multi-step modals were closing unexpectedly
...
- ([#33062](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)
...
- ([#32498](https://medsensehealth.ca)) Created a `transferChat` Livechat API endpoint for transferring chats programmatically, the endpoint has all the limitations & permissions required that transferring via UI has

- ([#32792](https://medsensehealth.ca)) Allows admins to customize the `Subject` field of Omnichannel email transcripts via setting. By passing a value to the setting `Custom email subject for transcript`, system will use it as the `Subject` field, unless a custom subject is passed when requesting a transcript. If there's no custom subject and setting value is empty, the current default value will be used

- ([#32739](https://medsensehealth.ca)) Fixed an issue where FCM actions did not respect environment's proxy settings

- ([#32706](https://medsensehealth.ca)) Added the possibility for apps to remove users from a room

- ([#32517](https://medsensehealth.ca)) Feature Preview: New Navigation - `Header` and `Contextualbar` size improvements consistent with the new global `NavBar`

- ([#32493](https://medsensehealth.ca)) Fixed Livechat rooms being displayed in the Engagement Dashboard's "Channels" tab

- ([#32742](https://medsensehealth.ca)) Fixed an issue where adding `OVERWRITE_SETTING_` for any setting wasn't immediately taking effect sometimes, and needed a server restart to reflect.

- ([#32752](https://medsensehealth.ca)) Added system messages support for Omnichannel PDF transcripts and email transcripts. Currently these transcripts don't render system messages and is shown as an empty message in PDF/email. This PR adds this support for all valid livechat system messages.

  Also added a new setting under transcripts, to toggle the inclusion of system messages in email and PDF transcripts.

- ([#32793](https://medsensehealth.ca)) New Feature: Video Conference Persistent Chat.
...
- ([#32176](https://medsensehealth.ca)) Added a method to the Apps-Engine that allows apps to read multiple messages from a room

- ([#32493](https://medsensehealth.ca)) Improved Engagement Dashboard's "Channels" tab performance by not returning rooms that had no activity in the analyzed period

- ([#32024](https://medsensehealth.ca)) Implemented a new tab to the users page called 'Active', this tab lists all users who have logged in for the first time and are active.

- ([#32744](https://medsensehealth.ca)) Added account setting `Accounts_Default_User_Preferences_sidebarSectionsOrder` to allow users to reorganize sidebar sections

- ([#32820](https://medsensehealth.ca)) Added a new setting `Livechat_transcript_send_always` that allows admins to decide if email transcript should be sent all the times when a conversation is closed. This setting bypasses agent's preferences. For this setting to work, `Livechat_enable_transcript` should be off, meaning that visitors will no longer receive the option to decide if they want a transcript or not.

- ([#32724](https://medsensehealth.ca)) Extended apps-engine events for users leaving a room to also fire when being removed by another user. Also added the triggering user's information to the event's context payload.

- ([#32777](https://medsensehealth.ca)) Added handling of attachments in Omnichannel email transcripts. Earlier attachments were being skipped and were being shown as empty space, now it should render the image attachments and should show relevant error message for unsupported attachments.

- ([#32800](https://medsensehealth.ca)) Added the ability to filter chats by `queued` on the Current Chats Omnichannel page
...
- ([#32679](https://medsensehealth.ca)) Fix validations from "UiKit" modal component

- ([#32730](https://medsensehealth.ca)) Fixed issue in Marketplace that caused a subscription app to show incorrect modals when subscribing

- ([#32628](https://medsensehealth.ca)) Fixed SAML users' full names being updated on login regardless of the "Overwrite user fullname (use idp attribute)" setting

- ([#32692](https://medsensehealth.ca)) Fixed an issue that caused the widget to set the wrong department when using the setDepartment Livechat api endpoint in conjunction with a Livechat Trigger

- ([#32527](https://medsensehealth.ca)) Fixed an inconsistent evaluation of the `Accounts_LoginExpiration` setting over the codebase. In some places, it was being used as milliseconds while in others as days. Invalid values produced different results. A helper function was created to centralize the setting validation and the proper value being returned to avoid edge cases.
...
- ([#32626](https://medsensehealth.ca)) livechat `setDepartment` livechat api fixes:
...
- ([#32810](https://medsensehealth.ca)) Fixed issue where bad word filtering was not working in the UI for messages

- ([#32707](https://medsensehealth.ca)) Fixed issue with livechat agents not being able to leave omnichannel rooms if joining after a room has been closed by the visitor (due to race conditions)

- ([#32837](https://medsensehealth.ca)) Fixed an issue where non-encrypted attachments were not being downloaded

- ([#32861](https://medsensehealth.ca)) fixed the contextual bar closing when editing thread messages instead of cancelling the message edit

- ([#32713](https://medsensehealth.ca)) Fixed the disappearance of some settings after navigation under network latency.

- ([#32592](https://medsensehealth.ca)) Fixes Missing line breaks on Omnichannel Room Info Panel

- ([#32807](https://medsensehealth.ca)) Fixed web client crashing on Firefox private window. Firefox disables access to service workers inside private windows. Rocket.Chat needs service workers to process E2EE encrypted files on rooms. These types of files won't be available inside private windows, but the rest of E2EE encrypted features should work normally

- ([#32864](https://medsensehealth.ca)) fixed an issue in the "Create discussion" form, that would have the "Create" action button disabled even though the form is prefilled when opening it from the message action

- ([#32691](https://medsensehealth.ca)) Removed 'Hide' option in the room menu for Omnichannel conversations.

- ([#32445](https://medsensehealth.ca)) Fixed LDAP rooms, teams and roles syncs not being triggered on login even when the "Update User Data on Login" setting is enabled

- ([#32328](https://medsensehealth.ca)) Allow customFields on livechat creation bridge

- ([#32803](https://medsensehealth.ca)) Fixed "Copy link" message action enabled in Starred and Pinned list for End to End Encrypted channels, this action is disabled now

- ([#32769](https://medsensehealth.ca)) Fixed issue that caused unintentional clicks when scrolling the channels sidebar on safari/chrome in iOS

- ([#32857](https://medsensehealth.ca)) Fixed some anomalies related to disabled E2EE rooms. Earlier there are some weird issues with disabled E2EE rooms, this PR fixes these anomalies.

- ([#32765](https://medsensehealth.ca)) Fixed an issue that prevented the option to start a discussion from being shown on the message actions

- ([#32671](https://medsensehealth.ca)) Fix show correct user roles after updating user roles on admin edit user panel.

- ([#32482](https://medsensehealth.ca)) Fixed an issue with blocked login when dismissed 2FA modal by clicking outside of it or pressing the escape key

- ([#32804](https://medsensehealth.ca)) Fixes an issue not displaying all groups in settings list

- ([#32815](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#32632](https://medsensehealth.ca)) Improving UX by change the position of room info actions buttons and menu order to avoid missclick in destructive actions.

- ([#32752](https://medsensehealth.ca)) Reduced time on generation of PDF transcripts. Earlier Rocket.Chat was fetching the required translations everytime a PDF transcript was requested, this process was async and was being unnecessarily being performed on every pdf transcript request. This PR improves this and now the translations are loaded at the start and kept in memory to process further pdf transcripts requests. This reduces the time of asynchronously fetching translations again and again.

- ([#32719](https://medsensehealth.ca)) Added the `user` param to apps-engine update method call, allowing apps' new `onUpdate` hook to know who triggered the update.
...
- ([#32570](https://medsensehealth.ca)) Login services button was not respecting the button color and text color settings. Implemented a fix to respect these settings and change the button colors accordingly.
...
- ([#32935](https://medsensehealth.ca)) Fixed an issue that prevented apps from being updated or uninstalled in some cases

- ([#32935](https://medsensehealth.ca)) Fixed an issue that prevented apps from handling errors during execution in some cases

- ([#32935](https://medsensehealth.ca)) Improved Apps-Engine installation to prevent start up errors on manual installation setups

- ([#32950](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixed a crash on web client due to service workers not being available, this can happen in multiple scenarios like on Firefox's private window or if the connection is not secure (non-HTTPS), [see more details](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts).
...
- ([#32935](https://medsensehealth.ca)) Fixed an issue that caused the video conference button on rooms to not recognize a video conference provider app in some cases
...
- ([#32819](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixed issue with livechat agents not being able to leave omnichannel rooms if joining after a room has been closed by the visitor (due to race conditions)

- ([#32894](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)

- ([#32829](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes an issue not displaying all groups in settings list

- ([#32836](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Security Hotfix (https://medsensehealth.ca)
...
- ([#32471](https://medsensehealth.ca)) Removed "Unknown media type" errors on the client side by using `application/octet-stream` as a fallback media type (MIME type) for all files

- ([#31859](https://medsensehealth.ca)) Introduced the use of the `API_User_Limit` setting to limit amount of members to simultaneously auto-join a room in a team

- ([#32551](https://medsensehealth.ca)) Implement E2EE warning callouts letting users know that encrypted messages can't be searched and auditted on search contextual bar and audit panel.

- ([#32446](https://medsensehealth.ca)) Added E2EE room setup header, with just limited functionality and room actions.

- ([#32552](https://medsensehealth.ca)) Fixed an issue that would not allow the user to dismiss the closeToSeatsLimit banner for old workspaces

- ([#31987](https://medsensehealth.ca)) Implemented a new "Pending Users" tab on the users page to list users who have not yet been activated and/or have not logged in for the first time.
...
- ([#32316](https://medsensehealth.ca)) Support encrypted files on end-to-end encrypted rooms.

- ([#32436](https://medsensehealth.ca)) Added a "LDAP group validation strategy" setting to LDAP channels and roles sync in order to enable faster syncs

- ([#32605](https://medsensehealth.ca)) Moves the quotes to be on top of the message for better readability

- ([#32197](https://medsensehealth.ca)) Async End-to-End Encrypted rooms key distribution process. Users now don't need to be online to get the keys of their subscribed encrypted rooms, the key distribution process is now async and users can recieve keys even when they are not online.

- ([#32559](https://medsensehealth.ca)) Disable "Reply in direct message", "Copy link" and "Forward message" message menu items for encrypted messages as they don't apply to encrypted messages and also disable apps menu items and show a warning.

- ([#32040](https://medsensehealth.ca)) Introduced a new setting which doesn't allow users to access encrypted rooms until E2EE is configured and also doesn't allow users to send un-encrypted messages in encrypted rooms.
...
- ([#31974](https://medsensehealth.ca)) Clicking on a message attachment link in the Desktop App will now initiate a direct download of the attachment only when the attachment is not a PDF file

- ([#31821](https://medsensehealth.ca)) New runtime for apps in the Apps-Engine based on the Deno platform

- ([#32425](https://medsensehealth.ca)) Added the possibility to choose the time unit (days, hours, minutes) to the global retention policy settings

### Patch Changes

- ([#32579](https://medsensehealth.ca)) Improved Retention Policy Warning messages

- ([#32152](https://medsensehealth.ca) by [@AllanPazRibeiro](https://github.com/AllanPazRibeiro)) Resolved an issue with the room type filter not being reset after navigating between admin sections.
...
- ([#32478](https://medsensehealth.ca)) Fixed "File Upload > Accepted Media Types" setting to allow all type of files uploads

- ([#32489](https://medsensehealth.ca)) Fixed streams being called when the user is not logged in

- ([#32610](https://medsensehealth.ca)) Fixes the supported versions problem, where in most cases the data chosen was the oldest

- ([#32696](https://medsensehealth.ca)) Added the allowDiskUse option to the users page queries so that if the mongodb memory threshold is exceeded it will use disk space instead of throwing an error.

- ([#32376](https://medsensehealth.ca)) Fixed an issue with how the UI checked for permissions when deciding if editing or deleting a message by moderators users

- ([#32459](https://medsensehealth.ca)) Prevent usage of OTR messages with End-to-end Encryption, both feature shouldn't and can't work together.

- ([#32563](https://medsensehealth.ca)) fixes not being able to reinstall app after installation failure

- ([#32499](https://medsensehealth.ca)) Fixed codeBlock styles in light mode

- ([#32380](https://medsensehealth.ca)) Decrypt pinned encrypted messages in the chat and pinned messages contextual bar.

- ([#32175](https://medsensehealth.ca)) Fixed "Take it" button behavior disabling it when agent status is set to offline

- ([#32472](https://medsensehealth.ca)) When using `DISABLE_DB_WATCHERS=true` this fixes message updates with URLs that were missing the link preview.

- ([#32587](https://medsensehealth.ca)) Fixes issues with loading license modules when loading the page while logged out

- ([#32452](https://medsensehealth.ca)) Fixed 2 issues with `QueueInactivityMonitor` callback. This callback was in charge of scheduling the job that would close the inquiry, but it was checking on a property that didn't exist. This caused the callback to early return without scheduling the job, making the feature to not to work.

- ([#32522](https://medsensehealth.ca) by [@AllanPazRibeiro](https://github.com/AllanPazRibeiro)) Changed streaming logic to prevent hidden system messages from being broadcasted through `stream-room-messages`.

- ([#32624](https://medsensehealth.ca)) Fixed an issue where private encrypted room creation was being forced even when E2EE feature was disabled.

- ([#32428](https://medsensehealth.ca)) Fixed an issue that allowed saveSettings method to save NaN values on numeric settings.

- ([#32640](https://medsensehealth.ca)) Fixes the issue where the modal backdrop is overlapping the options of the `Select` component

- ([#32636](https://medsensehealth.ca)) Fixed file name being incorrectly sent from the client when uploading assets

- ([#32431](https://medsensehealth.ca)) Fixed last message preview in Sidebar for E2E Ecrypted channels

- ([#32547](https://medsensehealth.ca)) Fixes the issue not allowing users without edit-room-retention-policy permission try to edit the room with the retention policy enabled

- ([#32653](https://medsensehealth.ca)) Prevent E2EE key reset on startup due to possible race conditions

- ([#32625](https://medsensehealth.ca)) Fixes an issue where settings code mirror is not being displayed correctly in full screen mode

- ([#32548](https://medsensehealth.ca)) Disable slash commands in encrypted rooms and show a disabled warning.

- ([#32566](https://medsensehealth.ca)) Fix the sorting by last chat in Contact Center table

- ([#32412](https://medsensehealth.ca)) Fixes an issue not rendering the proper error and empty state on users in role table

- ([#32485](https://medsensehealth.ca)) Adds the missing `ignoreThreads` param fixing the issue not allowing ignoring threads when overriding retention policy

- ([#31750](https://medsensehealth.ca)) Don't show Join default channels option on edit user form.

- ([#32612](https://medsensehealth.ca)) Fixes a cosmetic issue where emoji picker object and symbols category icon are swapped

- ([#32329](https://medsensehealth.ca)) Added a new setting `Restrict files access to users who can access room` that controls file visibility. This new setting allows users that "can access a room" to also download the files that are there. This is specially important for users with livechat manager or monitor roles, or agents that have special permissions to view closed rooms, since this allows them to download files on the conversation even after the conversation is closed.
...
- ([#32500](https://medsensehealth.ca)) Fix user not being set as online when setting "Use REST instead of websocket for Meteor calls" is disabled

- ([#32534](https://medsensehealth.ca)) Fixed an issue where apps installed via the Marketplace would not be shown in the installed list if the app is unpublished

- ([#32458](https://medsensehealth.ca)) Fixed `EditRoomInfo` encrypted field placement

- ([#32479](https://medsensehealth.ca)) Executing a logout and login action in the same "tab/instance", some streams were not being recreated, causing countless types of bugs.

  PS: as a workaround reloading after logout or login in also solves the problem.

- ([#32572](https://medsensehealth.ca)) Fixes issues causing nonstop sound notification when taking a chat from the `Current Chats` view

- ([#32381](https://medsensehealth.ca)) Fixed Encrypted thread main message reactivity issues. Earlier the encrypted thread main message was having some reactivity issues and flaky behavior.

- ([#32106](https://medsensehealth.ca)) Fixed inverted navigation direction in the image gallery

- ([#32507](https://medsensehealth.ca)) Forces the highlight code language registration, preventing it to not being available when trying to use on the UI

- ([#31363](https://medsensehealth.ca)) Remove password change reason when the `request password change` option is set to false

- ([#32690](https://medsensehealth.ca)) Security Hotfix (https://medsensehealth.ca)
...
- ([#32152](https://medsensehealth.ca)) Resolved an issue with the room type filter not being reset after navigating between admin sections.
...
- ([#32522](https://medsensehealth.ca)) Changed streaming logic to prevent hidden system messages from being broadcasted through `stream-room-messages`.
...
- ([#32683](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) livechat `setDepartment` livechat api fixes:
  - Changing department didn't reflect on the registration form in real time
  - Changing the department mid conversation didn't transfer the chat
  - Depending on the state of the department, it couldn't be set as default
- <details><summary>Updated dependencies []:</summary>
...
- ([#32621](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes the supported versions problem, where in most cases the data chosen was the oldest

- ([#32622](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes the issue not allowing users without edit-room-retention-policy permission try to edit the room with the retention policy enabled
...
- ([#32591](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes issues with loading license modules when loading the page while logged out

- ([#32588](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) Fixes issues causing nonstop sound notification when taking a chat from the `Current Chats` view
...
- ([#31917](https://medsensehealth.ca)) Introduced a tab layout to the users page and implemented a tab called "All" that lists all users.

- ([#32439](https://medsensehealth.ca)) Allow visitors & integrations to access downloaded files after a room has closed. This was a known limitation in our codebase, where visitors were only able to access uploaded files in a livechat conversation while the conversation was open.

- ([#32233](https://medsensehealth.ca)) Makes the triggers fired by the condition `after-guest-registration` persist on the livechat client, it will persist through reloads and pagination, only reseting when a conversation is closed (no changes were done on the agent side of the conversation)

- ([#32193](https://medsensehealth.ca)) Adds CheckOption to departments multi selects improving options visibility state

- ([#32317](https://medsensehealth.ca)) Replace the read receipt receipt indicator in order to improve the accessibility complience

- ([#32341](https://medsensehealth.ca)) Changes the scrollbar color in order to improve the contrast and accessibility compliance

- ([#32298](https://medsensehealth.ca)) Added "Rocket.Chat Cloud Workspace ID" to workspace statistics page

### Patch Changes

- ([#32393](https://medsensehealth.ca)) Fixed an issue causing monitors to dissapear from a saved unit every time a user saved the item. This was caused by the UI not sending the correct \_id of the monitors that were already saved, and this caused the Backend to ignore them and remove from the list.

- ([#31695](https://medsensehealth.ca)) Fix an issue where read receipts menu item wasn't considering the enabled setting to be displayed
...
- ([#32454](https://medsensehealth.ca)) Fixes an issue not allowing override retention policy in channels

- ([#32444](https://medsensehealth.ca)) Fixed an issue that prevented CAS users from being merged with existing user data on login

- ([#32289](https://medsensehealth.ca)) Fixed a problem in how server was processing errors that was sending 2 ephemeral error messages when @all or @here were used while they were disabled via permissions

- ([#32348](https://medsensehealth.ca)) Fixed an issue where translations would fallback to english some of the times.

- ([#32182](https://medsensehealth.ca)) Fixed an issue with object storage settings that was not allowing admins to decide if files generated via "Export conversation" feature were being proxied through server or not.

- ([#32311](https://medsensehealth.ca)) Fixed multiple issues with PDF generation logic when a quoted message was too big to fit in one single page. This was causing an internal infinite loop within the library (as it tried to make it fit, failing and then trying to fit on next page where the same happened thus causing a loop).
...
- ([#32364](https://medsensehealth.ca)) Fixed issue with "Export room as file" feature (`rooms.export` endpoint) generating an empty export when given an invalid date

- ([#32314](https://medsensehealth.ca)) Fixed an issue on Users converter that was not returning the `statusText` property from users even when the typing indicated property existed.

- ([#32500](https://medsensehealth.ca)) Fix user not being set as online when setting "Use REST instead of websocket for Meteor calls" is disabled

- ([#32391](https://medsensehealth.ca)) Fixes link image preview not opening in gallery mode

- ([#32318](https://medsensehealth.ca)) Fixed error handling for files bigger than NATS max allowed payload. This should prevent PDFs from erroring out when generating from rooms that contain heavy images.

- ([#32479](https://medsensehealth.ca)) Executing a logout and login action in the same "tab/instance", some streams were not being recreated, causing countless types of bugs.

  PS: as a workaround reloading after logout or login in also solves the problem.

- ([#32345](https://medsensehealth.ca)) Replaces the burger menu with an appropriate button fixing the semantics and mismatching color

- ([#32414](https://medsensehealth.ca)) Fixes the missing spacing on don`t ask again checkbox inside modals

- ([#32269](https://medsensehealth.ca)) Fixed a bad behavior with the interaction between OTR system messages & trash collection. We use trash collection as a temporary storage that holds recently deleted items from some collections. Messages is one of those. This was causing "User joined OTR" messages to be viewable when querying the trash collection.
...
- ([#32415](https://medsensehealth.ca)) This fuselage`s bump fixes:

  - The message toolbar visibility on hover (Firefox ESR)
  - `Bubble` missing font-family

  [more details](https://medsensehealth.ca)

- ([#32398](https://medsensehealth.ca)) Fixed issue with external users being able to reset their passwords even when the "Allow Password Change for OAuth Users" setting is disabled

- ([#32284](https://medsensehealth.ca)) fixed Engagement Dashboard and Device Management admin pages loading indefinitely

- ([#32342](https://medsensehealth.ca)) bump fuselage adding `AttachmentAuthorName` missing color token
...
- ([#31898](https://medsensehealth.ca)) Created a new endpoint to get a filtered and paginated list of users.

- ([#32224](https://medsensehealth.ca)) Allow Custom Fields in Messages. API-only feature. It can be enabled and configured in Workspace Settings.

- ([#32115](https://medsensehealth.ca)) Introduces sidebar navigability, allowing users to navigate on sidebar channels through keyboard

- ([#29461](https://medsensehealth.ca)) Introduces a resizable Contextualbar allowing users to change the width just by dragging it

- ([#31811](https://medsensehealth.ca)) Convert mute/unmute meteor methods to endpoints

- ([#32084](https://medsensehealth.ca)) Added a new setting to automatically disable users from LDAP that can no longer be found by the background sync

- ([#31965](https://medsensehealth.ca)) Added the ability to serve .well-known paths directly from Rocket.Chat, if using federation, removing the need for special reverse proxy configuration or another component layer for specific types of reverse proxies / loadbalancers.

- ([#31898](https://medsensehealth.ca)) Created a new endpoint to resend the welcome email to a given user

- ([#32208](https://medsensehealth.ca)) Added a new notification provider in light of the old FCM API deprecation, now you can choose to use the new provider or the old via the `Push_UseLegacy` setting

- ([#31976](https://medsensehealth.ca)) Added support for allowing agents to forward inquiries to departments that may not have any online agents given that `Allow department to receive forwarded inquiries even when there's no available agents` is set to `true` in the department configuration.
...
- ([#32173](https://medsensehealth.ca)) Added "Enable Users" option under "Sync User Active State" LDAP setting to allow only re-enabling users found on LDAP background sync

- ([#31865](https://medsensehealth.ca)) Redesign Save E2EE password modal

- ([#32272](https://medsensehealth.ca)) Support Message Custom Fields on upload API via field `customField` and JSON value

- ([#32055](https://medsensehealth.ca)) feat: `ConnectionStatusBar` redesign

- ([#32073](https://medsensehealth.ca)) Fixed an issue affecting the update modal/contextual bar by apps when it comes to error handling and regular surface update

### Patch Changes

- ([#31996](https://medsensehealth.ca)) Fixed Security tab visibility to allow password changes when 2FA/E2E is disabled.

- ([#32210](https://medsensehealth.ca)) Fixes error `audio.pause() is not a function` and makes the continuous new room notification (livechat) respect the volume set in user preferences.
...
- ([#32186](https://medsensehealth.ca)) Increased the timeout between calls for the three remaining Omnichannel Agenda Jobs. This should make them happen less often and reduce the load on MongoDB

- ([#32216](https://medsensehealth.ca)) Fixed an issue while creating tokens via the special `users.createToken` API was not respecting the maximum login tokens allowed for a user.
...
- ([#31958](https://medsensehealth.ca)) Force logout the clients which are actively online, whenever a user resets E2EE keys.

- ([#31989](https://medsensehealth.ca)) Fixed issue with login via SAML not redirecting to invite link

- ([#32187](https://medsensehealth.ca)) Fixes an issue that forces the focus on the last message when interacting by mouse on message list

- ([#31765](https://medsensehealth.ca)) Fixes the livechat client ignoring the `livechat_fileuploads_enabled` setting when uploading files

- ([#31811](https://medsensehealth.ca)) Deprecate muteUserInRoom and unmuteUserInRoom meteor methods

- ([#32287](https://medsensehealth.ca)) Fixed wrong `Business hours` validations between different weeks

- ([#32348](https://medsensehealth.ca)) Fixed an issue where translations would fallback to english some of the times.

- ([#31990](https://medsensehealth.ca)) Fixed open expanded view (galery mode) for image attachments sent by livechat widget

- ([#32248](https://medsensehealth.ca)) Fixes an issue where the last threads list item wasn't displaying properly

- ([#32112](https://medsensehealth.ca)) fixed an issue where mentioning a team would trigger the bot message warning that the team is not a part of the channel

- ([#32069](https://medsensehealth.ca)) Livechat: A registered user loses their messages if 'registerGuest' is called using the same token.

- ([#32063](https://medsensehealth.ca)) Fixed a UI issue that allowed a user to "mark" a room as favorite even when a room was not default. The Back-End was correctly ignoring the `favorite` property from being updated when the room was not default, but the UI still allowed users to try.
  As UI allowed but changes were not saved, this gave the impression that the function was not working.
- ([#32374](https://medsensehealth.ca)) Fixed an issue with some apps that didn't implement executeViewCloseHandler. This causes opened modals to be open forever on UI (unless Esc was clicked). This is because when the UI attempts to close it, it calls the aforementioned handler, and since it didn't exist, apps engine errored out.

  This returned an empty response to the UI, which ignored the response and continued to show the view.

- ([#32237](https://medsensehealth.ca)) **Fixed settings-related statistics not being updated according to the license.**
...
- ([#32170](https://medsensehealth.ca)) Fixed a language priority issue. It should now respect the following order: User Preference -> Browser Language -> Server Language

- ([#32202](https://medsensehealth.ca)) Fixed an issue where Rocket.Chat would ask admins to confirm fingerprint change (new workspace vs configuration update), even when `AUTO_ACCEPT_FINGERPRINT` environment variable set to `"true"`.

- ([#32220](https://medsensehealth.ca)) Fixed supported versions not being updated in airgapped environments

- ([#32141](https://medsensehealth.ca)) Deprecate `channels.images` in favor of `rooms.images`. `Rooms` endpoints are more broad and should interact with all types of rooms. `Channels` on the other hand are specific to public channels.
  This change is to keep the semantics and conventions of the endpoints
- ([#32157](https://medsensehealth.ca)) Fixed custom OAuth roles not synced on the first login (on user creation)

- ([#32136](https://medsensehealth.ca)) Fixes the missing space between name and user name on system messages

- ([#32007](https://medsensehealth.ca)) This PR have made enhancements to the select and multiselect inputs related to Omnichannel Departments, now the options properly display the complete department names, ensuring clarity for users and added text wrapping for long department names, enhancing readability and UX.

- ([#32221](https://medsensehealth.ca)) Fixed an issue where an endpoint was called before checking configuration that enables automatic translation when launching the application

- ([#32230](https://medsensehealth.ca)) Fixed a problem that caused OTR Session messages' to not being transmitted from one peer to another when running Rocket.Chat as microservices. This was caused by a legacy streamer that tried to use the websocket directly, which works on monolith but doesn't on microservices, cause these events are routed through DDP Streamer service.

- ([#32021](https://medsensehealth.ca)) Fixed duplicate API calls during livechat room forwarding by adding loading state for submit button

- ([#32123](https://medsensehealth.ca)) fixed search room not showing the new name room name changes

- ([#30309](https://medsensehealth.ca)) Fixed a problem that caused `afterCreateUser` callback to be called without new user's roles inside. This caused Omnichannel Business Hour manager to ignore these users from assigning open business hours until the manager restarted or the business hour restarted.

- ([#32172](https://medsensehealth.ca)) Fixes an issue where message reactions are vertically misaligned when zooming out

- ([#32062](https://medsensehealth.ca)) Fixed an issue where old exports would get overwritten by new ones if generated on the same day, when using external storage services (such as Amazon S3)
...
- ([#31840](https://medsensehealth.ca)) Encrypt file descriptions in E2EE rooms
...
- ([#32315](https://medsensehealth.ca) by [@dionisio-bot](https://github.com/dionisio-bot)) fixed Engagement Dashboard and Device Management admin pages loading indefinitely
...
- ([#32253](https://medsensehealth.ca)) Increased the timeout between calls for the three remaining Omnichannel Agenda Jobs. This should make them happen less often and reduce the load on MongoDB

- ([#32252](https://medsensehealth.ca)) Fixes an issue that forces the focus on the last message when interacting by mouse on message list

- ([#32256](https://medsensehealth.ca)) Fixed open expanded view (galery mode) for image attachments sent by livechat widget

- ([#32254](https://medsensehealth.ca)) Fixed an issue where Rocket.Chat would ask admins to confirm fingerprint change (new workspace vs configuration update), even when `AUTO_ACCEPT_FINGERPRINT` environment variable set to `"true"`.

- ([#32265](https://medsensehealth.ca)) Fixed supported versions not being updated in airgapped environments

- ([#32251](https://medsensehealth.ca)) Fixes an issue where message reactions are vertically misaligned when zooming out
...
- ([#31820](https://medsensehealth.ca)) **Added the ability for premium workspaces to hide Rocket.Chat's watermark as well as change the Livechat widget's logo**
...
- ([#31884](https://medsensehealth.ca)) Deprecate `insertOrUpdateUser` Meteor method

- ([#31751](https://medsensehealth.ca)) Added Livechat setting `Hide system messages` & API method `setHiddenSystemMessages`, to customize system message visibility within the widget.

- ([#31626](https://medsensehealth.ca)) Freezes the permission table's first column allowing the user to visualize the permission name when scrolling horizontally

- ([#31772](https://medsensehealth.ca)) Improved Livechat's theming capabilities
...
- ([#32043](https://medsensehealth.ca)) **Added Livechat's new theming settings to Appearance page (available for Premium workspaces)**
...
- ([#31679](https://medsensehealth.ca)) Added a new formatter shortcut to add hyperlinks to a message

- ([#31473](https://medsensehealth.ca)) feat: add a11y doc links

- ([#31572](https://medsensehealth.ca)) feat: show date on message's scroll

- ([#31268](https://medsensehealth.ca)) Added new Livechat trigger action "Send message (external service)"

- ([#31549](https://medsensehealth.ca)) Introduces message navigability, allowing users to navigate on messages through keyboard

- ([#31538](https://medsensehealth.ca)) Introduced new methods for Rocket.Chat Apps to interact with livechat: `findOpenRoomsByAgentId` and `countOpenRoomsByAgentId`

### Patch Changes

- ([#31663](https://medsensehealth.ca)) Fixes issue causing the setDepartment Livechat API overriding some triggers conditions

- ([#31745](https://medsensehealth.ca) by [@Pritam-sd-dev](https://github.com/Pritam-sd-dev)) Fix room name updation on admin edit room flow.

- ([#31948](https://medsensehealth.ca) by [@hardikbhatia777](https://github.com/hardikbhatia777)) Fixes an issue not allowing edit webhooks properly

- ([#31909](https://medsensehealth.ca)) fix: Trigger `IPostLivechatRoomStarted` app event after inquiry is created. Previously, this event was fired after a room was created. This allowed to do some actions on rooms, but more elevated actions like transfering a room were not possible as at this point, an inquiry didn't exist.

- ([#31881](https://medsensehealth.ca)) Fixed a problem that caused Business Hours feature (Multiple) to make bot agents not available when turning on the feature, and not making them available after that. Now, Business Hours will ignore Bot users, allowing admins to decide manually if a bot should be or not be active during a period of time
...
- ([#31720](https://medsensehealth.ca)) Now we are considering channels with auto-join inside teams on user creation

- ([#31970](https://medsensehealth.ca)) **Fixed enterprise settings value not being updated when license is removed or invalid**
...
- ([#31973](https://medsensehealth.ca) by [@VipinDevelops](https://github.com/VipinDevelops)) Fixed message composer command preview for text type

- ([#31713](https://medsensehealth.ca)) Fixes an issue not allowing admin users to edit the room name

- ([#31790](https://medsensehealth.ca) by [@tiran133](https://github.com/tiran133)) fix: Fixes SAML Role mapping of custom roles
  fixed: here https://medsensehealth.ca
- ([#31589](https://medsensehealth.ca)) fixed an issue with the composer losing its edit state and highlighted after resizing the window.

- ([#31700](https://medsensehealth.ca)) Fixed matrix homeserver domain setting not being visible in admin panel

- ([#31788](https://medsensehealth.ca)) fixed some apps-engine bridges receiving data in a wrong format

- ([#31660](https://medsensehealth.ca)) Fixed issue with quote attachments still being displayed within the message even after removing link

- ([#31759](https://medsensehealth.ca)) Fix notifications specially for DMs when preference is set to mentions.

- ([#31583](https://medsensehealth.ca)) Fixed login email verification flow when a user tries to join with username

- ([#31803](https://medsensehealth.ca)) Fixed room owner specified on room import not being inserted as a room member or owner.

- ([#31662](https://medsensehealth.ca) by [@SySagar](https://github.com/SySagar)) fix: Jump to message only works once

- ([#31475](https://medsensehealth.ca)) This fix allows links such as ones starting with "notes://" and other specific apps to be rendered in the User panel as they are in the messages

- ([#32085](https://medsensehealth.ca)) Fixed an internal server error when using the backwards compatibility flag to connect to the real time API for listening to all server message notifications.

- ([#31571](https://medsensehealth.ca)) Fixed Mail dryrun sending email to all users

- ([#31883](https://medsensehealth.ca)) fix: Corrected SVG image preview by setting correct format during thumbnail generation

- ([#31890](https://medsensehealth.ca)) Changed logic that process custom fields from visitors when updating its data, making the process more reliable and faster.

- ([#31860](https://medsensehealth.ca)) Fixed mentions not working when mentioned user changes username.

- ([#31723](https://medsensehealth.ca)) fixed an issue with the user presence not updating automatically for other users.

- ([#31894](https://medsensehealth.ca)) fixed a small issue that was causing the room layout to shift when loading apps messages

- ([#32019](https://medsensehealth.ca)) Allowed upload of `lst` files

- ([#31753](https://medsensehealth.ca)) Fixed an issue where the login button for Custom OAuth services would not work if any non-custom login service was also available

- ([#31983](https://medsensehealth.ca)) Fix error on changing a discussion name

- ([#31666](https://medsensehealth.ca)) Fixes an issue allowing only numbers, if trigger's condition is 'visitor time on site'

- ([#32012](https://medsensehealth.ca)) Don't use the registration.yaml file to configure Matrix Federation anymore.

- ([#31651](https://medsensehealth.ca)) Fixed auto-availability of reactivated livechat agents; they now stay 'Not Available' until manually set to 'Available'

- ([#32069](https://medsensehealth.ca)) Livechat: A registered user loses their messages if 'registerGuest' is called using the same token.

- ([#32031](https://medsensehealth.ca)) Fixes issue where the livechat offline form would render even when disabled

- ([#31703](https://medsensehealth.ca)) Added reactions tooltip loader

- ([#31701](https://medsensehealth.ca)) Fixed discussion names displaying as IDs in sidebar search results

- ([#31554](https://medsensehealth.ca) by [@shivang-16](https://github.com/shivang-16)) Fixed a bug on the rooms page's "Favorite" setting, which previously failed to designate selected rooms as favorites by default.

- ([#31844](https://medsensehealth.ca)) Fixed Federation not working with Microservice deployments

- ([#31927](https://medsensehealth.ca)) `stopped` lifecycle method was unexpectedly synchronous when using microservices, causing our code to create race conditions.

- ([#31880](https://medsensehealth.ca)) fix: CRM integration mismatch on callback type vs code validation. Previously, callback was attempting to register a `LivechatStarted` event, however, our internal code was expecting a `LivechatStart` event, causing the hook to receive incomplete data

- ([#31600](https://medsensehealth.ca)) Looking at the user's permission before rendering the 'Start Call' button on the UserInfo panel, so if the user does not have the permissions, the button does not show

- ([#31904](https://medsensehealth.ca)) fix: Show always all rooms when requesting chat history, even unserved ones. A faulty condition caused an issue where chat history was only able to present either served or unserved chats at once, without a proper way to get both. Now, the Chat history feature will showcase all closed rooms for the requested visitor.

- ([#31896](https://medsensehealth.ca)) fix: Validate rooms are not taken before processing by queue. This will prevent an issue that caused a room, that's on an invalid state, to be re-processed by the queue worker, assigning it again to another user despite being already assigned to one. This happens when a room's inquiry gets to an state where it desyncs from the room object. Room is taken & served while inquiry is still queued. This fix will also reconciliate both when something like this happens: whenever the queue picks a chat that's already taken, it will update it's inquiry object to reflect that and avoid processing again.

- ([#31823](https://medsensehealth.ca)) Revert unintentional changes real time presence data payload

- ([#31604](https://medsensehealth.ca)) Fixed an issue where the sync ldap avatars background process would never run

- ([#31621](https://medsensehealth.ca)) Improved the layout of the 2FA modals and changed the email 2FA resend email anchor to a button.

- ([#32038](https://medsensehealth.ca)) **Added tag to premium settings in CE workspaces**
...
- ([#31998](https://medsensehealth.ca)) Introduced a new step to the queue worker: when an inquiry that's on an improper status is selected for processing, queue worker will first check its status and will attempt to fix it.
...
- ([#32056](https://medsensehealth.ca)) Fix error during migration 304. Throwing `Cannot read property 'finally' of undefined` error.

- ([#31658](https://medsensehealth.ca)) Fixes an issue where messages are not updating properly after pruning the room

- ([#31895](https://medsensehealth.ca)) Fix users presence stuck as online after connecting using mobile apps

- ([#31833](https://medsensehealth.ca)) Fix web UI not showing users presence updating to offline
...
- ([#32064](https://medsensehealth.ca)) Fix an issue affecting Rocket.Chat Apps utilizing the OAuth 2 library from Apps Engine, ensuring that apps like Google Drive and Google Calendar are operational once more.
...
- ([#31184](https://medsensehealth.ca)) Add the possibility to hide some elements through postMessage events.

- ([#31516](https://medsensehealth.ca)) Room header keyboard navigability

  ![Kapture 2024-01-22 at 11 33 14](https://medsensehealth.ca)

- ([#30868](https://medsensehealth.ca)) Added `push.info` endpoint to enable users to retrieve info about the workspace's push gateway

- ([#31510](https://medsensehealth.ca)) Composer keyboard navigability

  ![Kapture 2024-01-22 at 11 33 14](https://medsensehealth.ca)

- ([#30464](https://medsensehealth.ca)) Mentioning users that are not in the channel now dispatches a warning message with actions

- ([#31369](https://medsensehealth.ca)) feat: add `ImageGallery` zoom controls

- ([#31393](https://medsensehealth.ca) by [@hardikbhatia777](https://github.com/hardikbhatia777)) Fixes an issue where avatars are not being disabled based on preference on quote attachments

- ([#30680](https://medsensehealth.ca)) feat: Skip to main content shortcut and useDocumentTitle

- ([#31299](https://medsensehealth.ca)) fix: Loading state for `Marketplace` related lists

- ([#31347](https://medsensehealth.ca) by [@Sayan4444](https://github.com/Sayan4444)) New feature to support cancel message editing message and hints for shortcuts.

- ([#30554](https://medsensehealth.ca)) **Added ‘Reported Users’ Tab to Moderation Console:** Enhances user monitoring by displaying reported users.

- ([#31417](https://medsensehealth.ca)) Added feature to sync the user's language preference with the autotranslate setting.

- ([#31478](https://medsensehealth.ca)) feat: `Bubble` on new messages indicators
  <img width="825" alt="image" src="https://medsensehealth.ca">
- ([#31348](https://medsensehealth.ca) by [@Sayan4444](https://github.com/Sayan4444)) Added a modal to confirm the intention to pin a message, preventing users from doing it by mistake

### Patch Changes

- ([#31318](https://medsensehealth.ca) by [@hardikbhatia777](https://github.com/hardikbhatia777)) Fixed Attachments not respecting collapse property when using incoming webhook

- ([#31138](https://medsensehealth.ca)) feat(uikit): Move `@rocket.chat/ui-kit` package to the main monorepo

- ([#31380](https://medsensehealth.ca)) Fix user being logged out after using 2FA

- ([#31281](https://medsensehealth.ca)) Improved support for higlighted words in threads (rooms are now marked as unread and notifications are sent)

- ([#31104](https://medsensehealth.ca)) Clear message box related items from local storage on logout

- ([#31556](https://medsensehealth.ca)) Bump @rocket.chat/meteor version.
...
- ([#31349](https://medsensehealth.ca) by [@Subhojit-Dey1234](https://github.com/Subhojit-Dey1234)) feat: Implemented InlineCode handling in Bold, Italic and Strike

- ([#31181](https://medsensehealth.ca)) Fixed issue with notifications for thread messages still being sent after thread has been read

- ([#30750](https://medsensehealth.ca)) fix: OAuth login by redirect failing on firefox

- ([#31312](https://medsensehealth.ca) by [@Sayan4444](https://github.com/Sayan4444)) Fixed an issue displaying the language selection preference empty when it should display 'Default' on the initial value

- ([#31204](https://medsensehealth.ca)) Fixed an issue that caused Omnichannel Business Units to be saved even when the "monitors" list passed on the endpoint included users without monitor role

- ([#31332](https://medsensehealth.ca) by [@Sayan4444](https://github.com/Sayan4444)) Fixed image dropping from another browser window creates two upload dialogs in some OS and browsers

- ([#31487](https://medsensehealth.ca)) Fixed a bug where some sessions were being saved without a sessionId

- ([#31070](https://medsensehealth.ca)) Fixed issue with read receipts for older messages not being created on the first time a user reads a DM

- ([#29367](https://medsensehealth.ca) by [@anefzaoui](https://github.com/anefzaoui)) Fixes an issue where texts are not being displayed in the correct direction on messages

- ([#31296](https://medsensehealth.ca)) Fixed a problem with the Fallback Forward Department functionality when transferring rooms, caused by a missing return. This provoked the system to transfer to fallback department, as expected, but then continue the process and transfer to the department with no agents anyways. Also, a duplicated "user joined" message was removed from "Forward to department" functionality.

- ([#31546](https://medsensehealth.ca)) fixed UI crashing for users reading a room when it's deleted.

- ([#31113](https://medsensehealth.ca)) fix: Discussion messages deleted despite the "Do not delete discussion messages" retention policy enabled

- ([#31269](https://medsensehealth.ca) by [@ChaudharyRaman](https://github.com/ChaudharyRaman)) fix: Resolved Search List Issue when pressing ENTER

- ([#31507](https://medsensehealth.ca) by [@Spiral-Memory](https://github.com/Spiral-Memory)) Fixed an issue not allowing users to remove the password to join the room on room edit

- ([#31413](https://medsensehealth.ca)) fix: multiple indexes creation error during 304 migration

- ([#31433](https://medsensehealth.ca)) Fixed values discrepancy with downloaded report from Active users at Engagement Dashboard

- ([#31049](https://medsensehealth.ca)) Fixed an `UnhandledPromiseRejection` error on `PUT livechat/departments/:_id` endpoint when `agents` array failed validation

- ([#31019](https://medsensehealth.ca)) fix: Off the record feature was calling a deprecated and useless method.

- ([#31225](https://medsensehealth.ca)) notification emails should now show emojis properly

- ([#31288](https://medsensehealth.ca)) Fixed toolbox sub-menu not being displayed when in smaller resolutions

- ([#30645](https://medsensehealth.ca)) Apply plural translations at a few places.

- ([#31514](https://medsensehealth.ca)) Show marketplace apps installed as private in the right place (private tab)

- ([#31289](https://medsensehealth.ca)) Added `push.test` POST endpoint for sending test push notification to user (requires `test-push-notifications` permission)

- ([#31346](https://medsensehealth.ca) by [@Sayan4444](https://github.com/Sayan4444)) Fixed error message when uploading a file that is not allowed

- ([#31371](https://medsensehealth.ca)) Fixed an issue that caused login buttons to not be reactively removed from the login page when the related authentication service was disabled by an admin.

- ([#30933](https://medsensehealth.ca) by [@ldebowczyk](https://github.com/ldebowczyk)) fix: Visitor message not being sent to webhook due to wrong validation of settings

- ([#31205](https://medsensehealth.ca)) Fixed a problem that caused the wrong system message to be sent when a chat was resumed from on hold status.
...
- ([#30478](https://medsensehealth.ca)) Added `chat.getURLPreview` endpoint to enable users to retrieve previews for URL (ready to be provided in message send/update)

- ([#31432](https://medsensehealth.ca)) Fixed SHIFT+ESCAPE inconsistency for clearing unread messages across browsers.

- ([#31537](https://medsensehealth.ca)) Fixed an issue where the webclient didn't properly clear the message caches from memory when a room is deleted. When this happened to basic DMs and the user started a new DM with the same target user, the client would show the old messages in the room history even though they no longer existed in the server.

- ([#31387](https://medsensehealth.ca)) fixed an issue when editing a channel's type or name sometimes showing "Room not found" error.

- ([#31128](https://medsensehealth.ca)) Fix: Mentioning discussions are appearing as ID

- ([#31594](https://medsensehealth.ca)) fix: missing slashcommand permissions for archive and unarchive

- ([#31134](https://medsensehealth.ca)) Fixed issue searching connected users on spotlight

- ([#31248](https://medsensehealth.ca)) Fixed Engagement Dashboard timezone selector freezing UI

- ([#31336](https://medsensehealth.ca)) Fixed issue with OEmbed cache not being cleared daily

- ([#30069](https://medsensehealth.ca)) Fixed using real names on messages reactions

- ([#31099](https://medsensehealth.ca)) fix: mention channel redirecting to own DM

- ([#31415](https://medsensehealth.ca)) fix: quote image gallery

- ([#31209](https://medsensehealth.ca)) Fixes the `overview` endpoint to show busiest time of the day in users timezone instead of UTC.

- ([#31164](https://medsensehealth.ca)) Improved the experience of receiving conference calls on the mobile app by disabling the push notification for the "new call" message if a push is already being sent to trigger the phone's ringing tone.

- ([#31540](https://medsensehealth.ca)) Fix multi-instance data formats being lost

- ([#31270](https://medsensehealth.ca)) Fixed an issue where room access and creation were hindered due to join codes not being fetched correctly in the API.

- ([#31368](https://medsensehealth.ca)) Added missing labels to "Users by time of the day" card at Engagement Dashboard page

- ([#31277](https://medsensehealth.ca)) Fixed the problem of not being possible to add a join code to a public room

- ([#31292](https://medsensehealth.ca)) Fixed the problem of displaying the wrong composer for archived room

- ([#31287](https://medsensehealth.ca)) Removed an old behavior that allowed visitors to be created with an empty token on `livechat/visitor` endpoint.

- ([#31267](https://medsensehealth.ca)) Fixed conversations in queue being limited to 50 items

- ([#31328](https://medsensehealth.ca)) Fixed an issue caused by the `Fallback Forward Department` feature. Feature could be configured by admins in a way that mimis a loop, causing a chat to be forwarded "infinitely" between those departments. System will now prevent Self & 1-level deep circular references from being saved, and a new setting is added to control the maximum number of hops that the system will do between fallback departments before considering a transfer failure.

- ([#31467](https://medsensehealth.ca) by [@apurb-coder](https://github.com/apurb-coder)) Fix an issue that breaks the avatar if you hit the button adding an invalid link

- ([#31228](https://medsensehealth.ca)) Fixed the filter for file type in the list of room files

- ([#31377](https://medsensehealth.ca)) Fixed LDAP "Group filter" malfunction, which prevented LDAP users from logging in.

- ([#31461](https://medsensehealth.ca)) Fixed issue with user presence displayed as offline on SAML login

- ([#31391](https://medsensehealth.ca)) Fixed Atlassian Crowd integration with Rocket.Chat not working

- ([#30910](https://medsensehealth.ca)) fix: change the push sound sent when the push is from video conference
...
- [#29596](https://medsensehealth.ca) [`74983f5b46`](https://medsensehealth.ca) Thanks [@rocketchat-github-ci](https://medsensehealth.ca)! - fix: Wrong IP usage on monolith TCP transporter configuration

- [#29596](https://medsensehealth.ca) [`929f457161`](https://medsensehealth.ca) Thanks [@rocketchat-github-ci](https://medsensehealth.ca)! - Fixed ENOTFOUND error in k8s deployments
...
- [#29545](https://medsensehealth.ca) [`8ade880306`](https://medsensehealth.ca) Thanks [@github-actions](https://github.com/apps/github-actions)! - fix: Frontend crash if IndexedDB is not available, i.e. in Firefox private mode
