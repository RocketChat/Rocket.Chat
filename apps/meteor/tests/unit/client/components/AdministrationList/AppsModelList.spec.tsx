...
describe('components/AdministrationList/AppsModelList', () => {
	...
	context('when clicked', () => {
		it('should go to end-user marketplace', async () => {
			const pushRoute = spy();
			const closeList = spy();
		...
		it('should go to end-user installed', async () => {
			const pushRoute = spy();
			const closeList = spy();
			const AppsModelList = proxyquire.load(COMPONENT_PATH, {
				'@rocket.chat/ui-contexts': {
			...
			userEvent.click(button);
			await waitFor(() => expect(pushRoute).to.have.been.called.with('marketplace', { context: 'installed' }));
			await waitFor(() => expect(closeList).to.have.been.called());
		});

		it('should go to admin installed', async () => {
			const pushRoute = spy();
			const closeList = spy();
			const AppsModelList = proxyquire.load(COMPONENT_PATH, {
				'@rocket.chat/ui-contexts': {
