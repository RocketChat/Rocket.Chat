...
const hasPermissionAsyncStub = sinon.stub();
...
const { generateAccessToken } = p.noCallThru().load('../../../../../../app/lib/server/methods/createToken.ts', {
	'@rocket.chat/core-services': { MeteorError: MeteorErrorMock, User: UserMock },
	'meteor/accounts-base': { Accounts: AccountsMock },
	'../../../authorization/server/functions/hasPermission': { hasPermissionAsync: hasPermissionAsyncStub },
});

describe('generateAccessToken', () => {
	beforeEach(() => {
		hasPermissionAsyncStub.reset();
	...
	it('should throw if secret does not match', async () => {
		await expect(generateAccessToken('targetId', 'wrong-secret', 'callerId')).to.be.rejectedWith('Not authorized');
	});

	it('should throw if caller targets another user and lacks user-generate-access-token permission', async () => {
		hasPermissionAsyncStub.resolves(false);

		await expect(generateAccessToken('targetId', 'V@9#mK2$pL8!nQ5^rT1&wX6*jY3%uZ7', 'callerId')).to.be.rejectedWith('Not authorized');
		sinon.assert.calledOnceWithExactly(hasPermissionAsyncStub, 'callerId', 'user-generate-access-token');
	});

	it('should succeed if caller targets another user and has user-generate-access-token permission', async () => {
		hasPermissionAsyncStub.resolves(true);
		AccountsMock._generateStampedLoginToken.returns({ token: 'abc123' });
		UserMock.ensureLoginTokensLimit.resolves();

		const result = await generateAccessToken('targetId', 'V@9#mK2$pL8!nQ5^rT1&wX6*jY3%uZ7', 'callerId');
	...
	it('should succeed without permission check when caller generates token for themselves', async () => {
		...
		const result = await generateAccessToken('userId', 'V@9#mK2$pL8!nQ5^rT1&wX6*jY3%uZ7', 'userId');

		expect(result).to.deep.equal({ userId: 'userId', authToken: 'abc123' });
		sinon.assert.notCalled(hasPermissionAsyncStub);
	});
});
