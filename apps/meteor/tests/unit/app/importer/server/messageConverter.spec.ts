...
describe('Message Converter', () => {
	...
	describe('[insertMessage]', () => {
		it('function should be called by the converter', async () => {
			...
			(converter as any)._cache.addRoom('general', 'general');
		...
		it('should call insertMessage lib function to save the message', async () => {
			const converter = new MessageConverter({ workInMemory: true });
			(converter as any)._cache.addRoom('general', 'main');

			await (converter as any).insertMessage(messageToImport as unknown as IImportMessage);
	...
	describe('[buildMessageObject]', () => {
		it('should have the basic info', async () => {
			...
			const converted = await (converter as any).buildMessageObject(messageToImport as unknown as IImportMessage, 'general', {
				_id: 'rocket.cat',
				username: 'rocket.cat',
			});
