...
/**
 * The call an app refused, as `CallDirector.recordPreventedCall` writes it: the server ends
 * it on the spot, so it never rings, never negotiates a feature and carries no hangup reason.
 */
const preventedCall = {
	_id: 'call-3',
	service: 'webrtc',
	...
	preventedBy: {
		appId: 'call-policy',
		appName: 'Call Policy',
		text: 'the callee is on a do-not-disturb list',
		i18n: { key: 'callee_is_dnd', ns: 'app-call-policy', args: { callee: 'jane' } },
	},
} as unknown as IMediaCall;
...
describe('AppCallHistoryConverter', () => {
	...
	describe('convertItem', () => {
		...
		// stored union without being added to the published one would surface here.
		it('reports a prevented row with the state it was stored with', () => {
			const result = converter.convertItem(preventedItem);

			expect(result.state).to.equal('prevented');
			expect(result.durationSeconds).to.equal(0);
		});
	...
	describe('convertCallDetails', () => {
		...
		it('copies the record of the app that refused the call', () => {
			const details = converter.convertCallDetails(preventedCall);

			expect(details.preventedBy).to.deep.equal({
				appId: 'call-policy',
				appName: 'Call Policy',
				text: 'the callee is on a do-not-disturb list',
				i18n: { key: 'callee_is_dnd', ns: 'app-call-policy', args: { callee: 'jane' } },
			});
			expect(details.state).to.equal('hangup');
			expect(details.features).to.deep.equal([]);
		});

		it('keeps the words of a prevention that named no translation key', () => {
			const worded = {
				...preventedCall,
				preventedBy: { appId: 'call-policy', appName: 'Call Policy', text: 'the callee is on a do-not-disturb list' },
			} as unknown as IMediaCall;

			expect(converter.convertCallDetails(worded).preventedBy).to.deep.equal({
				appId: 'call-policy',
				appName: 'Call Policy',
				text: 'the callee is on a do-not-disturb list',
			});
		...
		it('omits the prevention key for a call that was allowed to ring', () => {
			expect(converter.convertCallDetails(call)).to.not.have.property('preventedBy');
		});
