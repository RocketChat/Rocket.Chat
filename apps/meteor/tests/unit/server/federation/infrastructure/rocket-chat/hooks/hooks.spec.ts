...
describe('Federation - Infrastructure - RocketChat - Hooks', () => {
	...
	describe('#afterUserLeaveRoom()', () => {
		it('should NOT execute the callback if no room was provided', async () => {
			verifyFederationReady.returns(true);
	...
	describe('#onUserRemovedFromRoom()', () => {
		...
		it('should NOT execute the callback if federation module was disabled', async () => {
			...
			// @ts-expect-error
			expect(afterRemoveFromRoomCallback.run({ removedUser: 'removedUser', userWhoRemoved: 'userWhoRemoved' }, { federated: true })).to.have.rejectedWith(error);
			expect(stub.called).to.be.false;
		});
	...
	describe('#canAddFederatedUserToFederatedRoom()', () => {
		...
		it('should NOT execute the callback if federation module was disabled', () => {
			...
			const stub = sinon.stub();
			FederationHooks.canAddFederatedUserToFederatedRoom(stub);
			expect(hooks['federation-v2-can-add-federated-user-to-federated-room']({ user: 'user', inviter: 'inviter' }, { federated: true })).to.have.rejectedWith(error);
			expect(stub.called).to.be.false;
		});
	...
	describe('#afterMessageReacted()', () => {
		...
		it('should NOT execute the callback if federation module was disabled', () => {
			...
			const stub = sinon.stub();
			FederationHooks.afterMessageReacted(stub);
			expect(hooks['federation-v2-after-message-reacted']({ federation: { eventId: 'eventId' } }, { user: 'user', reaction: 'reaction' })).to.have.rejectedWith(error);
			expect(stub.called).to.be.false;
		});
	...
	describe('#afterMessageunReacted()', () => {
		...
		it('should NOT execute the callback if federation module was disabled', () => {
			...
			const stub = sinon.stub();
			FederationHooks.afterMessageunReacted(stub);
			expect(hooks['federation-v2-after-message-unreacted'](
	...
	describe('#afterMessageDeleted()', () => {
		...
		it('should NOT execute the callback if federation module was disabled', () => {
			...
			const stub = sinon.stub();
			FederationHooks.afterMessageDeleted(stub);
			expect(hooks['federation-v2-after-room-message-deleted']({ federation: { eventId: 'eventId' } }, { federated: true })).to.have.rejectedWith(error);
			expect(stub.called).to.be.false;
		});
	...
	describe('#afterMessageUpdated()', () => {
		...
		it('should NOT execute the callback if federation module was disabled', () => {
			...
			const stub = sinon.stub();
			FederationHooks.afterMessageUpdated(stub);
			expect(hooks['federation-v2-after-room-message-updated']({ federation: { eventId: 'eventId' } }, { federated: true })).to.have.rejectedWith(error);
			expect(stub.called).to.be.false;
		});
