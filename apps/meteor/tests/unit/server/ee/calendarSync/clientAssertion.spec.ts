...
import { buildClientAssertion, certificateThumbprint } from '../../../../../ee/server/lib/calendarSync/providers/graph/clientAssertion';

// Self-signed test certificate (no secret material); SHA-1 fingerprint:
// FD:0D:4C:E7:12:6D:0A:86:53:49:A8:A9:C0:A5:8D:A0:DF:75:70:F4
...
describe('calendarSync/graph/clientAssertion', () => {
	describe('certificateThumbprint', () => {
		it('should compute the base64url-encoded SHA-1 thumbprint of the DER certificate', () => {
			const expected = Buffer.from('FD0D4CE7126D0A865349A8A9C0A58DA0DF7570F4', 'hex')
				.toString('base64')
				.replace(/=+$/, '')
				.replace(/\+/g, '-')
				.replace(/\//g, '_');
			expect(certificateThumbprint(TEST_CERT)).to.equal(expected);
		});

		it('should reject values that are not PEM certificates', () => {
			expect(() => certificateThumbprint('not a cert'))
				.to.throw()
	...
	describe('buildClientAssertion', () => {
		...
		it('should produce an RS256 JWT with x5t header and Entra-compatible claims, verifiable with the public key', () => {
			...
			expect(header).to.deep.equal({ alg: 'RS256', typ: 'JWT', x5t: certificateThumbprint(TEST_CERT) });
