...
const emojiPickerVisible = await page.locator('.emoji-picker, [data-qa-id="emoji-picker"]').isVisible().catch(() => false);
...
const emojiClicked = await page.locator('.emoji-picker .emoji-item, .rcx-emoji-picker .emoji, [data-emoji]').first().click().then(() => true).catch(() => false);
