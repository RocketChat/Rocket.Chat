...
test.describe('Omnichannel close inquiry', () => {
	...
	test('Receiving a message from visitor', async ({ page }) => {
		...
		await test.step('Expect to have 1 omnichannel assigned to agent 1', async () => {
			await agent.poHomeOmnichannel.sidebar.getQueuedChat(newVisitor.name).click();
		...
		await test.step('Expect to inquiry be closed when navigate back', async () => {
			await agent.poHomeOmnichannel.navbar.openWorkspaceSettingsMenu();
			await agent.poHomeOmnichannel.navbar.omnichannelMenuItem.click();
