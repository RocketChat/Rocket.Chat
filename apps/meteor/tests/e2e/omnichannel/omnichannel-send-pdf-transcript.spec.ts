...
test.describe('omnichannel- export chat transcript as PDF', () => {
	...
	test('Export PDF transcript', async ({ page }) => {
		...
		await test.step('Expect to have exported PDF in rocket.cat', async () => {
			...
			await expect(agent.poHomeChannel.content.lastUserMessage).toHaveText('PDF Transcript successfully generated');
			await expect(agent.poHomeChannel.content.lastUserMessage.getByRole('link', { name: 'Transcript_Rocket.Chat' })).toBeVisible();
		});
