...
export class FlexTab extends BasePage {
	...
	async setUserOwner(user: string): Promise<void> {
		...
		await this.btnSetOwner.waitFor();
		await this.btnSetOwner.click();
	}

	async setUserModerator(user: string): Promise<void> {
		await this.enterUserView(user);
		await this.btnUserMoreActions.click();
		await this.btnSetModerator.waitFor();
		await this.btnSetModerator.click();
	}

	async muteUser(user: string): Promise<void> {
		await this.enterUserView(user);
		await this.btnUserMoreActions.click();
		await this.btnMuteUser.waitFor();
		await this.btnMuteUser.click();
		await this.modalConfirm.click();
		await this.btnSideBarBack.click();
	}
	...
	async archiveChannel(): Promise<void> {
		await this.btnArchive.waitFor();
		await this.btnArchive.click();
		await this.radioArchive.waitFor();
		await this.radioArchive.click();
		await this.btnArchiveSave.click();
	}

	async addPeopleToChannel(user: string): Promise<void> {
		await this.btnAddUser.click();
		await this.inputSearchUser.type(user);
		await this.page.waitForTimeout(3000);
		await this.btnChooseUserOptions.click();
		await this.btnAddUserAfterChoose.click();
	}

	async operateFlexTab(desiredTab: string, desiredState: boolean): Promise<void> {
	...
	async doAddRole(role: string): Promise<void> {
		await this.inputUserRoleList.click();
