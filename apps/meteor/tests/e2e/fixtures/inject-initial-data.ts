...
export default async function injectInitialData() {
	...
	const usersFixtures = [
		createUserFixture('user1'),
		createUserFixture('user2'),
		createUserFixture('user3'),
	];

	await Promise.all(usersFixtures.map(user => connection.db().collection('users').updateOne({ username: user.username }, { $set: user }, { upsert: true })))
