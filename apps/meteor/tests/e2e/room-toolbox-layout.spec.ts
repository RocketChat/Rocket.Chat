...
test.describe.serial('room toolbox layout', () => {
	...
	test.describe('Custom Layout Ordering and Pinning', () => {
		...
		test('actions beyond maxVisibleNormal (Files) are in the kebab menu', async () => {
			...
			await expect(poHomeChannel.roomToolbar.menuItemFiles).toBeVisible();
		});
	});
	...
	test.describe('Mobile Viewport', () => {
		...
		test('featured action (Threads) remains visible in the header on narrow viewport', async () => {
			await expect(poHomeChannel.roomToolbar.btnThreads).toBeVisible();
		});

		test('normal actions (Members, Discussions) collapse into Options dropdown on narrow viewport', async () => {
