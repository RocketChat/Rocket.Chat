	...
	test.describe('Admin View', () => {
		...
		test.describe('Rooms', () => {
			...
			test.describe('Render', () => {
				test('expect rom page is rendered is rendered', async () => {
					await admin.doVerifyCheckBoxIsVisible(checkBoxesSelectors);
					await expect(admin.inputSearchRooms).toBeVisible();
				});
			});

			test.describe('Filter search input', () => {
				test.beforeAll(async () => {
					await admin.inputSearchRooms.click();
				});

				test.afterAll(async () => {
					await admin.inputSearchRooms.click({ clickCount: 3 });
					await admin.keyPress('Backspace');
				});

				test('expect show the general channel', async () => {
					await admin.inputSearchRooms.type('general');
					await expect(admin.textRoomsChannelGeneral).toBeVisible();
				});

				test('expect dont show rooms when room dont exist', async () => {
					await admin.inputSearchRooms.type('any_room');
					await expect(admin.textNotFoundChannelOrUser).toBeVisible();
				});
			});
			test.describe('Filter checkbox', () => {
				...
				test('expect not show the general channel with direct', async () => {
					...
					await admin.textRoomsChannelGeneral.waitFor({ state: 'detached' });
					await expect(admin.textRoomsChannelGeneral).not.toBeVisible();
					await admin.adminCheckBox(checkBoxesSelectors[Checkbox.Direct]).click();
				});
				...
				test('expect show the general channel with public ', async () => {
					...
					await admin.textRoomsChannelGeneral.waitFor({ state: 'visible' });
					await expect(admin.textRoomsChannelGeneral).toBeVisible();
					await admin.adminCheckBox(checkBoxesSelectors[Checkbox.Public]).click();
				});
			...
			test.describe('Users', () => {
				...
				test.describe('Filter text', async () => {
					test.beforeEach(async () => {
						await admin.inputSearchUsers.click();
					});

					test.afterAll(async () => {
						await admin.inputSearchUsers.click();
						await admin.inputSearchUsers.type('');
					});

					test('expect should show rocket.cat', async () => {
						await admin.inputSearchUsers.type(ROCKET_CAT);
						await page.waitForSelector(ROCKET_CAT_SELECTOR);
					});
					test('expect dont user when write wrong name', async () => {
						await admin.inputSearchUsers.type('any_user_wrong');
						await expect(admin.textNotFoundChannelOrUser).toBeVisible();
					});
				});
		...
		test.describe('General Settings', () => {
			test.beforeAll(async () => {
				...
				await admin.inputSearchSettings.type('general');
				await admin.btnGeneralSettings.click();
			});
		...
		test.describe('Accounts', () => {
			test.beforeAll(async () => {
				await admin.btnGroupSettingsBack.click();
				await admin.inputSearchSettings.type('accounts');
				await admin.btnAccountSettings.click();
			});
