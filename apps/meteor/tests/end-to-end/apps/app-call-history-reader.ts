...
(IS_EE ? describe : describe.skip)('Apps - CallHistoryRead accessor', () => {
	describe('[with media-call.history permission]', () => {
		...
		// `prevented` is the newest member of the state union, and it travels from the app through
		// msgpack into a Mongo filter. The fixtures seed no prevented call, so the page is empty --
		// what this asserts is that the value is accepted and filtered on, not rejected or ignored.
		it('should accept the prevented state as a filter', async () => {
			await find({ states: 'prevented' })
				.expect(200)
				.expect((res: Response) => {
					expect(res.body.entries).to.be.an('array');

					res.body.entries.forEach((entry: any) => {
						expect(entry.item).to.have.property('state', 'prevented');
					});
				});
		});
