...
(IS_EE ? describe : describe.skip)('Apps - resolveVisitor API', () => {
	...
	const callResolveVisitor = async (
		externalId: { entityId: string; username?: string },
	...
	describe('externalId lookup', () => {
		...
		it('should find visitor directly by externalId', async () => {
			...
			const externalId = { entityId: `id-${Date.now()}`, username: '@user' };
		...
		it('should not update externalId when found by lookup', async () => {
			...
			const externalId = { entityId: `id-${Date.now()}`, username: '@original' };
			await callResolveVisitor(externalId, { phone });
			...
			const response = await callResolveVisitor({ entityId: externalId.entityId, username: '@changed' });
	...
	describe('phone fallback', () => {
		...
		it('should overwrite externalId when called with different entityId', async () => {
			...
			await callResolveVisitor({ entityId: `first-${Date.now()}`, username: '@first' }, { phone });

			const newExternalId = { entityId: `second-${Date.now()}`, username: '@second' };
	...
	describe('email fallback', () => {
		it('should find visitor by email and save externalId', async () => {
			...
			const externalId = { entityId: `id-email-${Date.now()}`, username: '@emailuser' };
		...
		it('should overwrite externalId when called with different entityId via email', async () => {
			...
			await callResolveVisitor({ entityId: `first-email-${Date.now()}`, username: '@first' }, { email });

			const newExternalId = { entityId: `second-email-${Date.now()}`, username: '@second' };
