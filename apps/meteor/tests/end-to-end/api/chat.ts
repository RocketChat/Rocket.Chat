...
describe('[Chat]', () => {
	...
	describe('[/chat.history]', () => {
		...
		before(async () => {
			testChannel = (await createRoom({ type: 'c', name: `channel.test.history.${Date.now()}` })).body.channel;
			testGroup = (await createRoom({ type: 'p', name: `group.test.history.${Date.now()}` })).body.group;
		});

		after(() => Promise.all([deleteRoom({ type: 'c', roomId: testChannel._id }), deleteRoom({ type: 'p', roomId: testGroup._id })]));
		...
		it('should return forbidden when the user cannot access the room', (done) => {
			void request
				.get(api('chat.history'))
				.set(credentials)
				.query({ roomId: 'invalid-room' })
		...
		it('should return the message history of a channel by roomId', async () => {
			const message = await sendSimpleMessage({ roomId: testChannel._id, text: 'channel history message' });

			const response = await request.get(api('chat.history')).set(credentials).query({ roomId: testChannel._id });

			expect(response.statusCode).to.equal(200);
			expect(response.body).to.have.property('success', true);
			expect(response.body).to.have.property('messages').and.to.be.an('array');
			expect(response.body.messages.map((msg: IMessage) => msg._id)).to.include(message.body.message._id);
		});

		it('should return the message history of a private group by the same endpoint (room type agnostic)', async () => {
			const message = await sendSimpleMessage({ roomId: testGroup._id, text: 'group history message' });

			const response = await request.get(api('chat.history')).set(credentials).query({ roomId: testGroup._id });
		...
		it('should only return messages created after the "oldest" timestamp', async () => {
			const { channel } = (await createRoom({ type: 'c', name: `channel.test.history.oldest.${Date.now()}` })).body;

			const oldMessage = await sendSimpleMessage({ roomId: channel._id, text: 'old message' });
			const oldest = new Date().toISOString();
			const newMessage = await sendSimpleMessage({ roomId: channel._id, text: 'new message' });

			const response = await request.get(api('chat.history')).set(credentials).query({ roomId: channel._id, oldest, inclusive: 'false' });

			expect(response.statusCode).to.equal(200);
			const ids = response.body.messages.map((msg: IMessage) => msg._id);
			expect(ids).to.include(newMessage.body.message._id);
			expect(ids).to.not.include(oldMessage.body.message._id);

			await deleteRoom({ type: 'c', roomId: channel._id });
		});

		it('should respect the "count" parameter', async () => {
			const { channel } = (await createRoom({ type: 'c', name: `channel.test.history.count.${Date.now()}` })).body;

			await sendSimpleMessage({ roomId: channel._id, text: 'message 1' });
			await sendSimpleMessage({ roomId: channel._id, text: 'message 2' });
			await sendSimpleMessage({ roomId: channel._id, text: 'message 3' });

			const response = await request.get(api('chat.history')).set(credentials).query({ roomId: channel._id, count: 2 });
