import { Uploads, Messages, SmarshHistory, Users, Rooms } from '@rocket.chat/models';
import { Meteor } from 'meteor/meteor';
import { TAPi18n } from 'meteor/rocketchat:tap-i18n';
import moment from 'moment';
import type { MessageAttachment, IMessage } from '@rocket.chat/core-typings';
...
type Smarsh = {
	generateEml(): void;
	sendEmail(data: { files: string[]; subject: string; body: string }): Promise<void>;
};
...
function _getLink(attachment?: MessageAttachment) {
...
export const smarsh: Smarsh = {
	sendEmail: async (data: { files: string[]; subject: string; body: string }) => {
		const attachments = [];
		...
		await Mailer.sendNoWrap({
			...
			html: data.body,
			// @ts-expect-error - Should attachments be part of mailer types?
			attachments,
		});
	},
	generateEml: () => {
		setImmediate(async () => {
			...
			for await (const room of Rooms.find()) {
				...
				const query: Filter<IMessage> = { rid: room._id };
				...
				for await (const message of Messages.find(query)) {
					rows.push(opentr);
