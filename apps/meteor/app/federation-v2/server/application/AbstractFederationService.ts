...
export abstract class FederationService {
    ...
    protected async createFederatedUser(externalUserId: string, username: string, existsOnlyOnProxyServer = false, providedName?: string): Promise<void> {
    ...
    protected isAnInternalIdentifier(externalIdentifier: string): boolean {
        return FederatedUser.isAnInternalUser(
			this.bridge.extractHomeserverOrigin(externalIdentifier),
