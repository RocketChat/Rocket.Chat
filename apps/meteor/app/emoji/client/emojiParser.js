...
const emojiParser = (message) => {
	...
	const emojis = Array.from(checkEmojiOnly.querySelectorAll('.emoji:not(:empty), .emoji-toolkit:not(:empty)'));

	let hasText = false;

	if (!isIE11) {
		const filter = (node) => {
			if (node.nodeType === Node.ELEMENT_NODE && (node.classList.contains('emoji-toolkit') || node.classList.contains('emoji'))) {
