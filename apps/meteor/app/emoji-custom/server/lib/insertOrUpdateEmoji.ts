...
export async function insertOrUpdateEmoji(userId: string | null, emojiData: EmojiData) {
	...
	if (emojiData.aliases) {
		...
		aliases = _.without(emojiData.aliases.split(/[\s,]/).filter(Boolean), emojiData.name).map((alias) =>
			limax(alias, { replacement: '_' }),
		);
	}
