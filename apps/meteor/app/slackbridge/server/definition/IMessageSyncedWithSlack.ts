...
export type IMessageSyncedWithSlack = IMessage & { slackTs?: SlackTS };

export type IMessageImportedFromSlack = IMessage & { _id: `slack-${string}-${string}-${string}` };

export const isMessageSyncedWithSlack = (message: IMessage): message is IMessageSyncedWithSlack => 'slackTs' in message;
