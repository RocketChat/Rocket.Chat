...
const beforeGetTeamMentions = async (mentionIds: string[], teamMentions?: MessageMention[]): Promise<string[]> => {
	...
	const members = await Team.getMembersByTeamIds(teamsIds, { projection: { userId: 1 } } as { projection: { userId: 1 } });
	return [...new Set([...mentionIds, ...members.map((data) => data.userId)])];
};
