...
export const CallHistoryCallDetailsCodec = z.codec(z.custom<IMediaCall>(), z.custom<IAppsCallHistoryCallDetails>(), {
	decode: (call): IAppsCallHistoryCallDetails => ({
		...
		// Named field by field, like everything else here. The record is written from app input,
		// so it is also the one place a field could arrive that no one reviewed. The key is
		// omitted rather than set to `undefined`, because an allowed call has no prevention at
		// all and the value crosses a msgpack boundary.
		...(call.preventedBy && {
			preventedBy: {
				appId: call.preventedBy.appId,
				appName: call.preventedBy.appName,
				text: call.preventedBy.text,
				...(call.preventedBy.i18n && {
					i18n: {
						key: call.preventedBy.i18n.key,
						ns: call.preventedBy.i18n.ns,
						...(call.preventedBy.i18n.args && { args: call.preventedBy.i18n.args }),
					},
				}),
			},
		}),
