...
export async function getFullUserDataByIdOrUsernameOrImportId(
	...
	const [canViewOtherUserEmails, canViewOtherUserPhone, canViewOtherUserRoles] = await Promise.all([
		hasPermissionAsync(userId, 'view-other-user-emails'),
		hasPermissionAsync(userId, 'view-other-user-phone'),
		hasPermissionAsync(userId, 'view-other-user-roles'),
	]);

	const options = {
		projection: {
			...fields,
			...(canViewOtherUserEmails && { emails: 1 }),
			...(canViewOtherUserPhone && { phone: 1 }),
			...(canViewOtherUserRoles && { roles: 1 }),
