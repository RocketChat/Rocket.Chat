...
MessageTypes.registerType({
	...
	data(message: IOmnichannelSystemMessage) {
		...
		const transferTypes = {
			agent: (): string =>
				translationHelper.t(`Livechat_transfer_to_agent${commentLabel}`, {
					from,
					to: message?.transferData?.transferredTo?.name || message?.transferData?.transferredTo?.username || '',
					...(comment && { comment }),
				}),
			department: (): string =>
				translationHelper.t(`Livechat_transfer_to_department${commentLabel}`, {
					from,
					to: message?.transferData?.nextDepartment?.name || '',
					...(comment && { comment }),
				}),
			queue: (): string =>
				translationHelper.t(`Livechat_transfer_return_to_the_queue${commentLabel}`, {
					from,
					...(comment && { comment }),
				}),
			autoTransferUnansweredChatsToAgent: (): string =>
				translationHelper.t(`Livechat_transfer_to_agent_auto_transfer_unanswered_chat`, {
					from,
					to: message?.transferData?.transferredTo?.name || message?.transferData?.transferredTo?.username || '',
					duration: comment,
				}),
			autoTransferUnansweredChatsToQueue: (): string =>
				translationHelper.t(`Livechat_transfer_return_to_the_queue_auto_transfer_unanswered_chat`, {
					from,
					duration: comment,
				}),
		};
...
MessageTypes.registerType({
	...
	data(message: IOmnichannelSystemMessage) {
		...
		const requestTypes = {
			visitor: (): string =>
				translationHelper.t('Livechat_visitor_transcript_request', {
					guest: visitor?.name || visitor?.username || '',
				}),
			user: (): string =>
				translationHelper.t('Livechat_user_sent_chat_transcript_to_visitor', {
...
MessageTypes.registerType({
	...
	render(message) {
		if (message.msg === 'ended' && message.webRtcCallEndTs && message.ts) {
			return translationHelper.t('WebRTC_call_ended_message', {
		...
		if (message.msg === 'declined' && message.webRtcCallEndTs) {
			return translationHelper.t('WebRTC_call_declined_message');
		}
