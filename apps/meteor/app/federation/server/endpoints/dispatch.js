...
const eventHandlers = {
			...
			case contextDefinitions.ROOM.type:
				...
				if (eventResult.success) {
					...
					if (persistedRoom) {
						...
						// Notify watch.rooms listener
						void notifyListenerOnRoomChanges(insertedRoom.insertedId);
					}
				}
				return eventResult;
		}
	...
	async [eventTypes.ROOM_DELETE](event) {
		...
		if (persistedRoom) {
			...
			// Notify watch.rooms listener
			void notifyListenerOnRoomChanges(roomId, 'removed', persistedRoom);
		}
	...
	async [eventTypes.ROOM_ADD_USER](event) {
		...
		if (eventResult.success) {
			...
			if (federationAltered) {
				...
				// Notify watch.rooms listener
				void notifyListenerOnRoomChanges(roomId);
			}
		}
