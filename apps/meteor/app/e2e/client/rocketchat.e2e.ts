...
class E2E extends Emitter {
	...
	async startClient(): Promise<void> {
		...
		if (!private_key && this.db_private_key) {
			try {
				...
				this.openAlert({
					title: translationHelper.t("Wasn't possible to decode your encryption key to be imported."),
		...
		if (randomPassword) {
			const passwordRevealText = translationHelper.t('E2E_password_reveal_text', {
				postProcess: 'sprintf',
				sprintf: [randomPassword],
			});

			this.openAlert({
				title: translationHelper.t('Save_Your_Encryption_Password'),
				html: translationHelper.t('Click_here_to_view_and_copy_your_password'),
				modifiers: ['large'],
				closable: false,
				icon: 'key',
				action: () => {
	...
	async requestPassword(): Promise<string> {
		return new Promise((resolve) => {
			...
			const showAlert = () => {
				this.openAlert({
					title: translationHelper.t('Enter_your_E2E_password'),
					html: translationHelper.t('Click_here_to_enter_your_encryption_password'),
					modifiers: ['large'],
					closable: false,
					icon: 'key',
					action() {
