	...
	{ authRequired: true },
	{
		async post() {
			...
			try {
				...
				await uploadEmojiCustomWithBuffer(this.userId, fileBuffer, file.mimetype, emojiData);
			} catch (e) {
	...
	{ authRequired: true },
	{
		async post() {
			...
			if (isNewFile) {
				...
				await uploadEmojiCustomWithBuffer(this.userId, fileBuffer, file.mimetype, updatedEmojiData);
			} else {
