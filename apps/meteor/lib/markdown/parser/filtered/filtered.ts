import { getFilteredLinkRegexes } from '../original/linkRegexes';
...
export const filtered = (
	...
	message = message.replace(inlineCodeRegex, (match: string) => match.substr(1, match.length - 2));

	// Filter [text](url), ![alt_text](image_url)
	message = message.replace(linkRegexes.link, (_match: string, title: string) => title);

	// Filter <http://link|Text>
	message = message.replace(linkRegexes.pipedLink, (_match: string, title: string) => title);
...
/** @param {string} message */
export const filterMarkdown = (message: any) => filtered(message);
