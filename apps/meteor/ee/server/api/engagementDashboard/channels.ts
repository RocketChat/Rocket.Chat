			...
			if (hideRoomsWithNoActivity) {
				apiDeprecationLogger.deprecatedParameterUsage(
					...
					({ parameter, endpoint, version }) =>
						`Setting the \`${parameter}\` param is deprecated in ${endpoint} and will be removed on version ${version}. This endpoint will behave as when \`${parameter}\` is set as \`true\` by default starting on ${version}`,
				);
			} else {
				apiDeprecationLogger.deprecatedParameterUsage(
					this.request.route,
					...
					this.response,
					({ parameter, endpoint, version }) =>
						`Returning rooms that had no activity in ${endpoint} is deprecated and will be removed on version ${version}. Set \`${parameter}\` as \`true\` to check how the endpoint will behave starting on ${version}`,
				);
			}
