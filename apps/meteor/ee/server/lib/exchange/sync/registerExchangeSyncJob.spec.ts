...
describe('configureExchangeSyncJob', () => {
	const settingsOf = (over: Record<string, unknown> = {}) => {
		const values: Record<string, unknown> = {
			...
			Outlook_Calendar_Server_Sync_Interval: 15,
			...over,
		};
	...
	it.each([
		...
		settingsOf({ Outlook_Calendar_Server_Sync_Interval: minutes });
...
describe('registerExchangeSyncJob', () => {
	...
	beforeEach(() => {
		...
		get.mockImplementation((key: string) => ({ Outlook_Calendar_Enabled: true, Outlook_Calendar_Mode: 'server' })[key]);
	});
	...
	it('watches the settings that decide whether the job exists at all', () => {
		...
		expect(watchMultiple).toHaveBeenCalledWith(
			['Outlook_Calendar_Enabled', 'Outlook_Calendar_Mode', 'Outlook_Calendar_Server_Sync_Interval'],
			expect.any(Function),
		);
	});
