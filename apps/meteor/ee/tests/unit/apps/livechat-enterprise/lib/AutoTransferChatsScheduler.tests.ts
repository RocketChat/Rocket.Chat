...
describe('AutoTransferChats', () => {
	...
	describe('executeJob', () => {
		it('should execute job', async () => {
			...
			(scheduler as any).transferRoom = transferRoomStub;

			await (scheduler as any).executeJob({ attrs: { data: { roomId: 'roomId' } } });
		...
		it('shouldnt fail even if job fails', async () => {
			...
			(scheduler as any).transferRoom = sinon.stub().throws('dummy error');

			const r = await (scheduler as any).executeJob({ attrs: { data: { roomId: 'roomId' } } });
	...
	describe('transferRoom', () => {
		...
		it('should not transfer undefined rooms', async () => {
			...
			await expect((scheduler as any).transferRoom('roomId')).to.be.rejectedWith('Room is not open or is not being served by an agent');
		});
		...
		it('should return a room as inquiry when the routing method is not automatic', async () => {
			...
			await (scheduler as any).transferRoom('roomId');
		...
		it('should do nothing if routing manager returns no agents for automatic algo', async () => {
			...
			const r = await (scheduler as any).transferRoom('roomId');
		...
		it('should do nothing if cannot find rocket.cat', async () => {
			...
			await expect((scheduler as any).transferRoom('roomId')).to.be.rejectedWith('error-no-cat');
