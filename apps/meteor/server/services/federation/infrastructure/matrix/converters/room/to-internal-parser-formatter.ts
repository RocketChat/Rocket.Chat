...
import type { MentionPill as MentionPillType } from 'matrix-bot-sdk';
...
const replaceMentionsFromLocalExternalUsersForExternalFormat = async (message: string): Promise<string> => {
	const { MentionPill } = await import('matrix-bot-sdk');
...
export const toExternalQuoteMessageFormat = async ({
	...
	const { RichReply } = await import('matrix-bot-sdk');
