...
import { isFederationReady, isFederationEnabled, FederationMatrixInvalidConfigurationError, getFederationVersion } from '../services/federation/utils';
...
export const addRoomOwner = async (fromUserId: IUser['_id'], rid: IRoom['_id'], userId: IUser['_id'], { skipMatrix = false }: { skipMatrix?: boolean } = {}): Promise<boolean> => {
	check(rid, String);
	check(userId, String);
	...
	if (isFederated && getFederationVersion() !== 'native' &&
	 (!isFederationEnabled() || !isFederationReady())) {
	...
	if (isFederated && getFederationVersion() === 'native' && !skipMatrix) {
		await FederationMatrix.setUserModerator(fromUserId, userId, rid, 'owner');
	}
