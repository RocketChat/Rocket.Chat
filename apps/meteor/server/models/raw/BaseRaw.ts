...
> implements IBaseModel<T, C, TDeleted>
{
	...
	// Moving these from Base.js

	// eslint-disable-next-line @typescript-eslint/no-empty-function
	roleBaseQuery(_userId: string, _scope?: string): Filter<T> | void {}

	findRolesByUserId(userId: string): FindCursor<T> {
		const query = this.roleBaseQuery(userId);
		if (!query) {
			throw new Error('roleBaseQuery is not implemented');
		}

		return this.find(query, { projection: { roles: 1 } });
	}

	async isUserInRole(userId: string, roleId: string, scope?: string): Promise<boolean> {
		...
		// @ts-expect-error - roles is not defined in T
		query.roles = roleId;
		return (await this.findOne(query, { projection: { roles: 1 } })) !== undefined;
	}

	async isUserInRoleScope(uid: string, scope?: string): Promise<boolean> {
	...
	addRolesByUserId(userId: string, roles: string[], scope?: string): Promise<UpdateResult> {
	...
	removeRolesByUserId(userId: string, roles: string[], scope?: string): Promise<UpdateResult> {
		roles = ([] as string[]).concat(roles);
		const query = this.roleBaseQuery(userId, scope);
		if (!query) {
			throw new Error('roleBaseQuery is not implemented');
