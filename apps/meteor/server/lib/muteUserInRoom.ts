...
export async function muteUserInRoom(
	roomId: IRoom['_id'],
	...
	const mutedUser = (await UsersRaw.findOneById(userIdOrUsername)) || Users.findOneByUsernameIgnoringCase(userIdOrUsername);
	if (!mutedUser) {
		throw new Meteor.Error('error-invalid-user', 'Invalid executor user id', {
			method: 'muteUserInRoom',
		});
	...
	Rooms.muteUsernameByRoomId(roomId, mutedUser.username);

	Messages.createUserMutedWithRoomIdAndUser(roomId, mutedUser, {
		u: {
			_id: fromUser._id,
