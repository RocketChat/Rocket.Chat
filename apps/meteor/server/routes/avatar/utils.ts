...
export const serveAvatar = (avatar: string, format: string, res: http.ServerResponse) => {
...
async function isUserAuthenticated({ headers, query }: { headers: http.IncomingHttpHeaders; query?: Record<string, string | undefined> }) {
	if (!query) {
		return false;
	}
...
export async function userCanAccessAvatar({ headers, query }: http.IncomingMessage & { query?: Record<string, string | undefined> }) {
...
const getCacheTime = (cacheTime?: string) => cacheTime || settings.get<string>('Accounts_AvatarCacheTime');

export function setCacheAndDispositionHeaders(req: http.IncomingMessage & { query?: Record<string, string> }, res: http.ServerResponse) {
