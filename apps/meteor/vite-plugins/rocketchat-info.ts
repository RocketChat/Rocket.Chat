	...
	return {
		...
		async load(id) {
			if (id === resolvedVirtualId) {
				...
				return `
					export const Info = ${JSON.stringify(info.api, null, 4)};
...
async function getInfo() {
	...
	const appsEngineVersion = await getAppsEngineVersion();
...
async function getAppsEngineVersion() {
	try {
		// Try to find it in node_modules
		const appsEnginePkgPath = path.resolve(process.cwd(), 'node_modules/@rocket.chat/apps-engine/package.json');
		...
		const localPath = path.resolve(process.cwd(), '../../packages/apps-engine/package.json');
