	...
	return (
		<Box width='100%' height='100%'>
			...
			>
				...
				/>
				<Box position='relative' width='100%' height='calc(100% - 40px)' flexDirection='column'>
					<Box
						position='absolute'
					...
					>
						{Object.values(tabsItem).map(({ Container: ContainerComponent }, index) => (
							<Fragment key={index}>{index === editorTabsToggle ? <ContainerComponent /> : <Box width='100%' h='100%' />}</Fragment>
						))}
					</Box>
				</Box>
			</Box>
