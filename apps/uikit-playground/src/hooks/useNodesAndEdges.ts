...
export function useNodesAndEdges() {
	...
	useEffect(() => {
		...
		setNodes(prevNodes);
	}, [activeProject, projects, screens, JSON.stringify(screens), projects[activeProject].screens, setNodes]);
	useEffect(() => {
		const _edges = projects[activeProject].flowEdges;
		_setEdges(_edges);
	}, [activeProject, projects, screens, projects[activeProject].flowEdges, _setEdges]);
