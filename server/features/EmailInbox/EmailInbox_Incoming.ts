...
function getGuestByEmail(
	...
	logger.debug(
		`Attempt to register a guest for ${ email } on department: ${ department }`,
	);
...
export async function onEmailReceived(
	...
	const references =		typeof email.references === 'string'
		? [email.references]
		: email.references;
	...
	if (email.html) {
		...
		msg = stripHtml(
			email.html.replace(/<div name="messageSignatureSection.+/s, ''),
		).result;
	}
	...
	Livechat.sendMessage({
		...
		.then(async () => {
			...
			for await (const attachment of email.attachments) {
				...
				try {
					attachments.push(
						await uploadAttachment(attachment, rid, guest.token),
					);
				} catch (e) {
					Livechat.logger.error(
						'Error uploading attachment from email',
						e,
					);
				}
			}
