Meteor.publish 'spotlight', (selector, options, collName) ->
	...
	subHandlePrivateGroups = if username?
			RocketChat.models.Rooms.findByNameContainingTypesWithUsername(selector.name.$regex, [{type: 'p', username: username}], { limit: 10, fields: { t: 1, name: 1 }, sort: {name: 1}}).observeChanges
				added: (id, fields) ->
					data = { type: 'r', rid: id, name: fields.name, t: fields.t }
					self.added("autocompleteRecords", id, data)
				removed: (id) ->
