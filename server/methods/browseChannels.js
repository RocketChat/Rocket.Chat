...
const getChannelsAndGroups = (
	...
	if (
		(!user && !canViewAnon)
		|| (user && !hasPermission(user._id, 'view-c-room'))
	) {
		return;
	}
	...
	const userTeamsIds =		Promise.await(
		Team.listTeamsBySubscriberUserId(user._id, {
			projection: { teamId: 1 },
		}),
	)?.map(({ teamId }) => teamId) || [];
	...
	const teamsMains = Promise.await(
		Team.listByIds([...new Set(teamIds)], {
	...
	const results = result.map((room) => {
		if (room.teamId) {
			const team = teamsMains.find(
				(mainRoom) => mainRoom._id === room.teamId,
			);
			if (team) {
...
const getChannelsCountForTeam = mem(
	(teamId) =>
		Promise.await(
			RoomsRaw.findByTeamId(teamId, { projection: { _id: 1 } }).count(),
		),
	{
		maxAge: 2000,
	},
...
const getTeams = (user, searchTerm, sort, pagination) => {
	...
	const result = Rooms.findContainingNameOrFNameInIdsAsTeamMain(
		searchTerm,
		ids,
		{
			...pagination,
...
const getUsers = async (user, text, workspace, sort, pagination) => {
	...
	let result;
	if (workspace === 'all') {
		result = Users.findByActiveUsersExcept(
			text,
			[],
			options,
			forcedSearchFields,
...
Meteor.methods({
	async browseChannels({
		...
		const roomParams = ['channels', 'teams'].includes(type)
		...
		switch (type) {
			...
			case 'users':
				return getUsers(
					...
					text,
					workspace,
					sortUsers(sortBy, sortDirection),
					pagination,
				);
			default:
		}
