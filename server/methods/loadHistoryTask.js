...
import { loadTaskHistory } from '../../app/lib/server';

Meteor.methods({
	loadHistoryTask(rid, end, limit = 20, ls, showThreadMessages = true) {
		check(rid, String);
		...
		return loadTaskHistory({ userId: fromId, rid, end, limit, ls, showThreadMessages });
	},
});
