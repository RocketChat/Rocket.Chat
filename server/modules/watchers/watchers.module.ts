...
type Watcher = <T extends IBaseData>(
	model: IBaseRaw<T>,
	fn: (event: IChange<T>) => void
) => void;
...
export function initWatchers(
	...
	const getSettingCached = mem(
		async (setting: string): Promise<SettingValue> =>
			Settings.getValueById(setting),
		{ maxAge: 10000 },
	);

	const getUserNameCached = mem(
		async (userId: string): Promise<string | undefined> => {
			const user = await Users.findOne<Pick<IUser, 'name'>>(userId, {
				projection: { name: 1 },
			});
			return user?.name;
		},
	...
	watch<ISubscription>(
		Subscriptions,
		async ({ clientAction, id, data, diff }) => {
			switch (clientAction) {
				case 'inserted':
				case 'updated': {
				...
				case 'removed': {
					const trash = await Subscriptions.trashFindOneById<
					Pick<ISubscription, 'u' | 'rid'>
					>(id, { projection: { u: 1, rid: 1 } });
	...
	if (isPresenceMonitorEnabled()) {
		watch<IUserSession>(
			UsersSessions,
			async ({ clientAction, id, data: eventData }) => {
				switch (clientAction) {
					case 'inserted':
					case 'updated':
	...
	watch<ILivechatInquiryRecord>(
		...
		async ({ clientAction, id, data, diff }) => {
			switch (clientAction) {
				...
				case 'updated':
					data =						data
						?? (await LivechatInquiry.findOneById(id))
						?? undefined;
					break;
	...
	watch<ILivechatDepartmentAgents>(
		LivechatDepartmentAgents,
		async ({ clientAction, id, diff }) => {
	...
	watch<IPermission>(
		Permissions,
		async ({ clientAction, id, data: eventData, diff }) => {
			if (diff && Object.keys(diff).length === 1 && diff._updatedAt) {
				// avoid useless changes
				return;
			}
			...
			if (data.level === 'settings' && data.settingId) {
				...
				const setting = await Settings.findOneNotHiddenById(
					data.settingId,
				);
				if (!setting) {
					return;
	...
	watch<ILoginServiceConfiguration>(
		LoginServiceConfiguration,
		async ({ clientAction, id }) => {
	...
	watch<IInstanceStatus>(
		InstanceStatus,
		({ clientAction, id, data, diff }) => {
			broadcast('watch.instanceStatus', { clientAction, data, diff, id });
		},
	);

	watch<IIntegrationHistory>(
		IntegrationHistory,
		async ({ clientAction, id, data, diff }) => {
			switch (clientAction) {
				case 'updated': {
