...
import {
	emit,
	StreamPresence,
} from '../../../app/notifications/server/lib/Presence';
...
export class NotificationsModule {
	...
	constructor(private Streamer: IStreamerConstructor) {
		...
		this.streamPresence = StreamPresence.getInstance(
			Streamer,
			'user-presence',
		);
	...
	async configure({
		...
		this.streamRoomMessage.allowEmit(
			...
			async function(_eventName, { rid }) {
				...
				try {
					...
					const roomParticipant =						await Subscriptions.countByRoomIdAndUserId(
						room._id,
						this.userId,
					);
		...
		this.streamRoom.allowRead(async function(
			eventName,
			extraData,
		): Promise<boolean> {
			...
			if (extraData?.token) {
				...
				Pick<IOmnichannelRoom, 't' | 'v'>
				>(rid, { projection: { t: 1, 'v.token': 1 } });
				return (
		...
		async function canType({
			...
			try {
				...
				const key = await Settings.getValueById('UI_Use_Real_Name')
					? 'name'
					: 'username';

				const user = await Users.findOneById<
				Pick<IUser, 'name' | 'username'>
				>(userId, {
					projection: {
						[key]: 1,
					},
		...
		this.streamRoom.allowWrite(async function(
			...
			if (
				...
				streamRoom.emit(
					`${ rid }/typing`,
					username,
					_activity.includes('user-typing'),
				);
			} else if (e === 'typing') {
				streamRoom.emit(
					`${ rid }/user-activity`,
		...
		this.streamRoomUsers.allowWrite(async function(eventName, ...args) {
			...
			if (!this.userId) {
				...
				const livechatSubscriptions: ISubscription[] =					await Subscriptions.findByLivechatRoomIdAndNotUserId(
					roomId,
					this.userId,
				...
				if (livechatSubscriptions && e === 'webrtc') {
					livechatSubscriptions.forEach(
						(subscription) =>
							subscription.v
							&& notifyUser(subscription.v._id, e, ...args),
					);
					return false;
				}
		...
		this.streamLivechatRoom.allowRead(async function(roomId, extraData) {
			const room = await Rooms.findOneById<
			Pick<IOmnichannelRoom, 't' | 'v'>
			>(roomId, { projection: { _id: 0, t: 1, v: 1 } });
