...
const migrateCollection = async (): Promise<void> => {
	const LivechatOfficeHour = new Mongo.Collection<IBusinessHourWorkHour>(
		...
		workHours: officeHours.map(
			(officeHour): IBusinessHourWorkHour => ({
				...
				start: {
					...
					utc: {
						dayOfWeek: moment(
							`${ officeHour.day }:${ officeHour.start }`,
							'dddd:HH:mm',
						)
							.utc()
							.format('dddd'),
				...
				finish: {
					...
					utc: {
						dayOfWeek: moment(
							`${ officeHour.day }:${ officeHour.finish }`,
							'dddd:HH:mm',
						)
							.utc()
							.format('dddd'),
	...
	if (
		await LivechatBusinessHours.find({
			type: LivechatBusinessHourTypes.DEFAULT,
		}).count() === 0
	) {
...
addMigration({
	...
	async up() {
		...
		await Settings.removeById(
			'Livechat_allow_online_agents_outside_office_hours',
		);
		const permission = await Permissions.findOneById(
			'view-livechat-officeHours',
		);
		if (permission) {
			await Permissions.update(
				{ _id: 'view-livechat-business-hours' },
				{ $set: { roles: permission.roles } },
				{ upsert: true },
			);
